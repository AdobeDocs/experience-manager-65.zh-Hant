...
在權利內，的預設產品設定檔 [!DNL Adobe Stock] 存在於 [!DNL Admin Console]. 可建立多個設定檔，這些設定檔決定誰可以授權Stock資產。 直接存取產品設定檔的使用者可以存取 [https://stock.adobe.com/](https://stock.adobe.com/) 並授權Stock資產。 雖然有其他方法可使用開發人員存取權來建立整合(API)，以驗證以下專案之間的通訊： [!DNL Experience Manager] 和 [!DNL Adobe Stock].
...
一次an [!DNL Adobe Stock] 資產授權於 [!DNL Experience Manager]，其使用和管理方式與一般資產相同。 在 [!DNL Experience Manager]，使用者可以搜尋及預覽資產、複製及發佈資產、共用資產 [!DNL Brand Portal]；透過以下方式存取及使用資產 [!DNL Experience Manager] 案頭應用程式等。
...
* **與整合的問題 [!DNL Experience Manager] Service Pack 6.5.7.0和更高版本**：在與整合期間識別出非預期的問題 [!DNL Experience Manager] 6.5.7.0和更高版本。 此問題正在測試中，預計可在中取得 [!DNL Experience Manager] 6.5.11.0.連絡人 [!DNL Customer Support] 取得立即的hotfix。
