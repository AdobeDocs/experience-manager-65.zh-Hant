...
若 [!DNL Assets] 要從網站擷取影像的使用狀況統計資料，您必須在網站程式碼中包含影像的內嵌程式碼。

若要讓「資產前瞻分析」顯示資產的使用統計資料，請先設定功能，從Adobe Analytics擷取報告資料。 如需詳細資訊，請參 [閱設定資產分析](/help/assets/configure-asset-insights.md)。
...
1. 從使 [!DNL Assets] 用者介面(UI)中選取影像，然後從工具列按一 **[!UICONTROL 下]** 「屬性」。
...
1. 若要取得您包含在網站中的資產的內嵌代碼以取得效能資料，請按一下資產縮圖 **[!UICONTROL 下方的「取得內嵌代碼]** 」。 如需如何將內嵌程式碼加入協力廠商網頁的詳細資訊，請參閱「使 [用頁面追蹤器」和網頁內嵌程式碼](/help/assets/use-page-tracker.md)。
...
1. 在使用 [!DNL Assets] 者介面中，導覽至包含您要檢視其見解之資產的檔案夾。
