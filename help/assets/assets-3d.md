...
![3d鞋子](/help/assets/assets-dm/3d-dimensional-viewer-quickpublish-url-embed2.png) *三維鞋的詳細資訊頁面。*
...
下列逐步工作流程說明可協助您在Dynamic Media - Scene7模式中快速上手並執行3D資產。

>[!IMPORTANT]
>
>Dynamic Media — 混合模式不支援3D資產。

在Dynamic Media中使用3D資產之前，請確定Experience Manager管理員已在Dynamic Media - Scene7模式下啟用並設定Dynamic MediaCloud Service。
...
1. **管理3D資產**
   ...
   * 檢視三維資產
      ...
      * [檢視和互動3D資產](#viewing-three-d-assets).
...
>[!IMPORTANT]
>
>您必須先將3D媒體元件新增至網頁，並指派3D資產給該元件，才能完成此工作。 另請參閱 [新增3D媒體元件至網頁](#adding-the-three-d-media-component-to-a-web-page) 和 [為3D媒體元件指派3D資產](#assigning-a-three-d-asset-to-the-component).
...
Dynamic Media內含Dynamic Media 3D Media元件，您可以在Adobe Experience Manager Sites中使用，透過互動方式在網頁上檢視3D模型。
...
1. 選取「 」中的空白區域 **[!UICONTROL 配置容器]** 區域，讓您選取它並開啟其相關的工具列。
...
1. 選取 **[!UICONTROL 編輯]** （鉛筆）圖示，讓您可以在頁面編輯器中開啟頁面。 請確定 **[!UICONTROL 編輯]** 模式即會在頁面右上角附近選取。
...
1. 在Experience Manager Sites頁面編輯器中，選取 **[!UICONTROL 3D媒體檢視器]** 您先前新增至頁面的元件。
...
>[!NOTE]
>
>當網頁位於Experience Manager Sites中時 **[!UICONTROL 編輯]** 模式，3D Media元件會顯示3D資產，但無法與資產互動。 若要讓資產成為互動式，您可以使用 **[!UICONTROL 預覽]** 功能可讓您在頁面編輯器中檢視網頁，並可完整存取3D媒體元件的功能。

## 發佈靜態Dynamic Media 3D資產 {#publishing-three-d-assets}

Dynamic Media接受以下支援的各種3D檔案格式 *靜態內容* 在Dynamic Media中。 靜態內容表示您可以上傳和發佈3D資產，但不支援 *變數影像* 或是與3D資產相關聯的影像重新調整。 這是因為Dynamic Media Imaging Server無法辨識3D格式。 因此，在Dynamic Media中發佈3D資產後，您就可以立即複製URL。 3D資產的URL會遵循一般的Dynamic Media URL結構。 不過，與Dynamic Media中的傳統影像資產不同，您無法編輯資產URL中的任何引數。
...
如果您要發佈Dynamic Media 3D資產，請使用下列兩種方法 *非* 使用Experience Manager做為WCM。
