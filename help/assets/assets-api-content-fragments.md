...
description: 瞭解AEM HTTP API支援內容片段，這是Assets的Headless傳送功能的重要一環。
...
此API可讓您藉由向Adobe Experience Manager前端應用程式提供內容服務，以將JavaScript當作Headless CMS （內容管理系統）來運作。 或者任何其他可以執行 HTTP 要求並處理 JSON 回應的應用程式。
...
Assets REST API適用於最新版AEM的每一次現成安裝。
...
如果是在沒有特定驗證需求的環境中使用Assets REST API，則需要正確設定AEM的CORS篩選器。
