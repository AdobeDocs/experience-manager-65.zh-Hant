...
# 關 [!DNL Adobe Experience Manager Assets] 於DAM解決方案 {#administering-assets}

[!DNL Assets] 是數位資產管理(DAM)工具，是平台的一個不可或缺的部 [!DNL Experience Manager] 分，可讓您的企業管理和分發數位資產。 組織內的使用者可以管理、儲存和存取多種類型的數位資產，例如影像、視訊、檔案、音訊剪輯、3D檔案和多媒體，以便用於網路、平面印刷和數位散發。
...
[!DNL Assets] 完全整合， [!DNL Sites] 並順暢地運作於所有使用案例。 例如，在製作網頁時，作者可 [!DNL Sites] 以透過Content Finder尋找和使用數位資產。 其使用者介 [!DNL Assets] 面與之相同 [!DNL Sites]。 如需 [完整詳細資訊](/help/sites-authoring/page-authoring.md) ，請參閱網站總覽。

基本用戶介面與基本用戶介面相同 [!DNL Sites]。 如需 [完整詳細資訊，請參閱](/help/sites-authoring/page-authoring.md) 「網站概觀」。
...
* 例如，如果影像具有單獨的生命週期，則使用兩次或在WCM外使用影像，則使用 [!DNL Assets]。
...
您可以對資產或系列執行動作。 動作可以建立或修改資產、系列和轉譯。 您對資產執行的許多基本動作——上傳、刪除、更新、儲存子資產——會觸發預先設定的工作流程。 這些功能會在中自動開啟， [!DNL Assets] 並會在媒體處理常式中詳細 [!DNL Assets] 說明。
