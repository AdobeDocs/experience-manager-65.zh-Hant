...
1. **添加欄位**
   ...
   * 將欄位添加到模型後，右面板將顯示 **屬性** 可為該特定資料類型定義的。 在此，您可以定義該欄位的必需內容。
      ...
      * 鍵入 **欄位標籤** 將自動完成 **屬性名稱**   — 如果為空，則可以隨後手動更新。
         ...
         >[!CAUTION]
         >
         >手動更新屬性時 **屬性名稱** 對於資料類型，請注意，名稱必須僅包含A-Z、a-z、0-9和下划線「_」作為特殊字元。
         >
...
* **屬性名稱**
   ...
   手動更新資料類型的此屬性時，請注意名稱 **必須** 含 *僅* A-Z、a-z、0-9和下划線「_」作為特殊字元。
...
* **[片段引用](#fragment-reference-nested-fragments)** （嵌套片段）
   ...
   * 允許您包括/檢索結構化資料。
      ...
      >[!NOTE]
      >
      >該方法與 [使用內容片段與GraphQL進行無頭內容傳遞](/help/assets/content-fragments/content-fragments-graphql.md)。
...
>[!NOTE]
>
>這與 [使用內容片段與GraphQL進行無頭內容傳遞](/help/assets/content-fragments/content-fragments-graphql.md)。
...
>
>在GraphQL，還對片段引用提供重複保護。 如果跨兩個相互引用的內容片段建立深度查詢，則它將返回空值。
...
* 模型不再可用作建立 *新* 內容片段。
* 但是：
   * GraphQL架構仍在生成且仍可查詢（以避免影響JSON API）。
...
>[!NOTE]
>
>該機制類似於 [允許頁面模板](/help/sites-authoring/templates.md#allowing-a-template-author) 的子級。
