...
description: 建立包含非銳化蒙版和智慧裁剪或智慧色板設定的「影像配置檔案」，然後將該配置檔案應用於影像資產的資料夾。
...
>[!IMPORTANT]
>
>· Smart Crop僅在Dynamic Media-Scene7模式下提供。
...
智慧裁剪坐標與縱橫比相關。 對於影像配置檔案中的各種智慧裁剪設定，如果影像配置檔案中添加的尺寸的長寬比相同，則向Dynamic Media發送相同的長寬比。 Adobe建議您使用相同的裁剪區域。 這樣可確保對「影像配置檔案」中使用的不同尺寸沒有影響。

您建立的每個智慧裁剪生成都需要額外的處理。 例如，添加5個以上的Smart Crop縱橫比可導致資產吞吐率降低。 它還會增加系統的負載。 因為您可以在資料夾級別應用Smart Crop，所以Adobe建議您在資料夾上使用它 *僅* 需要它的地方。

**在影像配置檔案中定義智慧裁剪的指南**
為了控制Smart Crop的使用，並優化農作物的加工時間和儲存，Adobe建議使用以下指導和提示：

* 避免建立具有相同寬度和高度值的重複智慧裁剪配置檔案。
* 根據裁剪維度而不是最終使用情況命名智慧作物。 這樣做有助於優化多個頁面上使用單個維的重複項。
* 為特定資料夾和子資料夾建立按頁/資產類型的映像配置檔案，而不是應用於所有資料夾或所有資產的通用智慧裁剪配置檔案。
* 應用於子資料夾的影像配置檔案會覆蓋應用於該資料夾的影像配置檔案。
* 為特定資料夾和子資料夾建立按頁/按資產類型的映像配置檔案，而不是應用於所有資料夾或所有資產的通用智慧裁剪配置檔案。
* 應用於子資料夾的影像配置檔案會覆蓋應用於該資料夾的影像配置檔案。
* 理想情況下，每張影像擁有10到15個智慧作物，以優化螢幕比例和處理時間。
...
您有兩個影像裁剪選項可供選擇：像素裁剪或智慧裁剪。 還可以選擇自動建立顏色和影像色板。

>[!IMPORTANT]
·Adobe建議您檢查所有生成的作物和色板，以確保它們與您的品牌和價值相適合且相關。
·智慧裁剪不支援CMYK影像格式。
...
1. 選取&#x200B;**[!UICONTROL 儲存]**。新建立的配置檔案將出現在可用配置檔案清單中。
...
將「影像配置檔案」分配給資料夾時，任何子資料夾都會自動從其父資料夾繼承配置檔案。 此工作流意味著您只能將一個「影像配置檔案」分配給資料夾。 因此，請仔細考慮上載、儲存、使用和存檔資產所在的資料夾結構。
...
您可以重新處理資料夾中的資產，該資料夾中已經有您稍後更改的現有映像配置檔案。 請參閱 [編輯資料夾的處理配置檔案後，重新處理資料夾中的資產](processing-profiles.md#reprocessing-assets)。

### 將Dynamic Media映像配置檔案應用於特定資料夾 {#applying-image-profiles-to-specific-folders}

您可以將「影像配置檔案」應用到 **[!UICONTROL 工具]** 或 **[!UICONTROL 屬性]**。 本節介紹如何將映像配置檔案雙向應用於資料夾。
...
1. 選擇Experience League徽標並導航至 **[!UICONTROL 資產]**。 然後導航到要應用映像配置檔案的資料夾的父資料夾。
...
>[!IMPORTANT]
·智慧作物僅在Dynamic Media-Scene7模式下提供。
...
您可以從 **[!UICONTROL 工具]** 或 **[!UICONTROL 屬性]**。 本節介紹如何從資料夾中同時刪除映像配置檔案。
