...
影像集由字詞`IMAGESET`的橫幅指定。 此外，如果已發佈影像集，則會顯示以 **[!UICONTROL World]** 圖示表示的發佈日期與上次修改日期(以 **** Pencil圖示表示)在橫幅上。
...
1. [建立影像集](#creating-image-sets)。
   ...
   若要在資產中建立影像集，請前往&#x200B;**[!UICONTROL Create]** > **[!UICONTROL Image Sets]**。 然後，添加影像並選擇&#x200B;**[!UICONTROL Save]**。
...
1. 視需要新增[影像集檢視器預設集](/help/assets/managing-viewer-presets.md)。
   ...
   管理員可以建立或修改影像集檢視器預設集。 若要檢視您的影像集與檢視器預設集，請選取「影像集」，然後在左側導軌下拉式選單中，選取「**[!UICONTROL 檢視器]**」。

   如果要建立或編輯檢視器預設集，請導覽至&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 資產]** > **[!UICONTROL 檢視器預設集]**。

1. （可選）使用批集預設集建立的[查看影像集](/help/assets/image-sets.md#viewing-image-sets)。
1. [預覽影像集](/help/assets/previewing-assets.md)。

   選取「影像集」，即可預覽。 選取縮圖圖示，以便在選取的檢視器中檢查您的影像集。 您可以從&#x200B;**[!UICONTROL 檢視器]**&#x200B;選單中選擇不同的檢視器，該選單可從左側導軌下拉式選單中取得。
...
1. 在「Experience Manager」中，選擇Experience Manager徽標以訪問全局導航控制台，然後轉至&#x200B;**[!UICONTROL Navigation]** > **[!UICONTROL Assets]**。 導覽至您要建立影像集的位置，然後前往&#x200B;**[!UICONTROL Create]** > **[!UICONTROL Image Set]**&#x200B;以開啟「影像集編輯器」頁面。
...
1. 執行下列任一操作：
   ...
   * 在「影像集編輯器」頁面的中間，選取「**[!UICONTROL 點選」以開啟「資產選取器」]**。
   選取您要納入影像集的資產。 選取的資產上面有核取標籤圖示。完成後，在頁面右上角附近，選擇&#x200B;**[!UICONTROL Select]**。
...
1. 將資產新增至資產集時，資產會以英數字元順序自動新增。 新增資產後，您可以手動重新排序或排序資產。
   ...
   如果要更改縮圖或色票，請選擇影像旁的&#x200B;**+** **縮圖**&#x200B;表徵圖，並導航到所需的縮圖或色票。 選擇完所有影像後，選擇&#x200B;**[!UICONTROL Save]**。
...
1. （選用）執行下列任一操作：
   ...
   * 若要套用預設集，在頁面右上角附近，選取「**[!UICONTROL Preset]**」，然後選取要一次套用至所有資產的預設集。
   >[!NOTE]
   >
   >建立「影像集」時，您可以變更「影像集」縮圖，或允許Experience Manager根據「影像集」中的資產自動選取縮圖。 若要選取縮圖，請選取「影像集編輯器」頁面上「標題」欄位上方的「變更縮圖&#x200B;]**」，然後選取任何影像（您也可以導覽至其他資料夾以尋找影像）。**[!UICONTROL &#x200B;如果您已選取縮圖，然後決定要Experience Manager從影像集產生縮圖，請選取「**[!UICONTROL 切換至]** > **[!UICONTROL 自動縮圖]**」。

1. 選擇&#x200B;**[!UICONTROL 保存]**。 新建立的「影像集」(Image Set)會顯示在您建立它的資料夾中。
...
>[!NOTE]
>
>您可以通過用戶介面查看集，如[編輯影像集](#editing-image-sets)中所述。
...
1. 執行下列任一操作：
   ...
   * 暫留在「影像集」資產上，選取「**[!UICONTROL 選取]**」（勾選圖示），然後選取工具列上的「**[!UICONTROL 編輯]**」。
   * 在「影像集」資產上選取，然後選取工具列上的「編輯&#x200B;****（鉛筆圖示）」 。

1. 要編輯「影像集」中的影像，請執行以下操作之一：

   * 若要重新排序資產，請拖曳影像至新位置（選取重新排序圖示以移動項目）。
   * 若要依遞增或遞減順序排序項目，請選取欄標題。
   * 若要新增資產或更新現有資產，請選取「**[!UICONTROL 新增資產]**」。 導覽至資產，選取資產，然後選取頁面右上角的&#x200B;**[!UICONTROL 選取]**。
   ...
   * 若要刪除整個影像集，請導覽至影像集，選取它，然後選取&#x200B;**[!UICONTROL Delete]**。
   ...
   >[!NOTE]
   >
   >您可以導覽至影像集，選取左側導軌中的&#x200B;**[!UICONTROL 設定成員]**，然後選取個別資產上的鉛筆圖示以開啟編輯視窗，以編輯影像集中的影像。
