...
1. [建立影像集。](#creating-image-sets)
   ...
   您也可以透過批次集預設集自動 [建立影像集](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。
   >[!IMPORTANT]
   >
...
1. 視需 [要新增影像集檢視器預設](/help/assets/managing-viewer-presets.md)。
   ...
   管理員可以建立或修改影像集檢視器預設集。 若要查看您的影像集及檢視器預設集，請選取影像集，然後在左側導軌下拉式選單中選取「檢 **[!UICONTROL 視器]**」。
...
>[!NOTE]
>
>您也可以透過批次集預設集自動 [建立影像集](/help/assets/config-dms7.md#creating%20batch%20set%20presets%20to%20auto-generate%20image%20sets%20and%20spin%20sets)。
...
1. 執行下列任一項作業：
   ...
   * 在「影像集編輯器」頁面的中間，點選「點選」 **[!UICONTROL 以開啟「資產選擇器」]**。
   點選以選取您要納入影像集的資產。 選取的資產上面有核取標籤圖示。 完成後，在頁面右上角附近點選「選 **[!UICONTROL 取」]**。
...
您可以在使用者介面中建立影像集，或使用批次集預 [設集自動建立](/help/assets/config-dms7.md#creating%20batch%20set%20presets%20to%20auto-generate%20image%20sets%20and%20spin%20sets)。
...
1. 若要編輯影像集中的影像，請執行下列任一項作業：
   ...
   * 若要套用預設，請在頁面右上角附近點選「預設 **[!UICONTROL 」]**，然後選取檢視器預設。
