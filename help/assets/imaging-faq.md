---
title: 智慧型影像
description: 智慧映像應用每個用戶的獨特觀看特性，自動提供針對其體驗而優化的正確影像，從而獲得更好的效能和參與。
...
>[!TIP]
>
>使用Dynamic Media，嘗試並瞭解Dynamic Media映像修改器和智慧映像的優勢 [_快照_](https://snapshot.scene7.com/)。
>
...
另請參閱 [基於新一代影像格式（WebP和AVIF）的影像優化](https://blog.developer.adobe.com/image-optimisation-with-next-gen-image-formats-webp-and-avif-248c75afacc4)
...
通過附加開啟瀏覽器格式轉換 `bfc=on` 到影像URL時，會自動將JPEG和PNG轉換為有損AVIF、有損WebP、有損JPEGXR、有損JPEG2000，用於不同的瀏覽器。 對於不支援這些格式的瀏覽器，Smart Imaging繼續為JPEG或PNG服務。 與格式一起，Smart Imaging會重新計算新格式的質量。
...
啟用設備像素比率優化將以螢幕的本機解析度呈現影像，使影像變得清晰。
...
另請參閱 [使用影像時](/help/assets/adding-dynamic-media-assets-to-pages.md#when-working-with-images) 和 [使用Smart Crop時](/help/assets/adding-dynamic-media-assets-to-pages.md#when-working-with-smart-crop)。
...
對於JPEG影像檔案格式，新格式的質量由Smart Imaging重新計算。
...
例如，假設影像預設是使用JPEG格式定義的，大小為500 x 500，質量為85，非銳化蒙版=0.1,1,5。 當Smart Imaging檢測到用戶在Chrome瀏覽器上時，影像將轉換為WebP格式，大小為500 x 500。 並且，非銳化掩模=0.1,1,5的WebP質量與盡可能接近的85的JPEG質量相匹配。 將該WebP轉換的佔用空間與JPEG進行比較，並返回兩者中較小的一個。
...
1. 為最大限度地提高智慧映像的效能，Adobe建議將「生存時間」(TTL)設定為24小時或更長。 TTL定義CDN快取資產的時間。 要更改此設定：
   ...
   1. 如果你用Dynamic Media，跟上 [這些說明](/help/assets/dm-publish-settings.md#common-thumbnail-attributes-tab)。 設定 **[!UICONTROL 到期]** 值24小時或更長。
...
![智慧映像頭](/help/assets/assets-dm/smart-imaging-header2.png)
...
如果要在預設情況下切換回為WebP服務，請為此建立支援案例。 與往常一樣，可以通過添加參數來關閉智慧映像 `bfc=off` 的子菜單。 但是，您不能在Smart Imaging的URL修飾符中選擇WebP或AVIF。 此能力在您公司的客戶級別保持。
...
## 我在Chrome Web瀏覽器上有一個帶fmt=tif的URL。 但我的請求失敗，出現ImageServer錯誤。 原因為何？ {#fmt-tif}
