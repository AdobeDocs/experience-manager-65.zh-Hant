...
>* 要向用戶發送電子郵件，請在「第一天CQ郵件服務」中配 [置SMTP伺服器詳細資訊](#configmailservice)。
...
1. 在工具列中，按一下「共 **[!UICONTROL 用連結]**![共用資產」圖示](assets/do-not-localize/assets_share.png)。
   ...
   按一下「共用」後將建立的連 [!UICONTROL 結] ，會先顯示在「共用連 [!UICONTROL 結」欄位中] 。 連結的預設有效期為一天。
   ...
   *圖：將資產共用為連結的對話方塊。*
   ...
   >[!NOTE]
   >
   >如果您想要將您的「作者」部署的連結共用給外部實體，請確定您只針對請求公開下列URL（用於連結共用） [!DNL Experience Manager]`GET` 。 出於安全原因，封鎖其他URL。
   >
...
1. Open the **[!UICONTROL Day CQ Link Externalizer]** configuration and modify the following properties in the **[!UICONTROL Domains]** field with the values mentioned against `local`, `author`, and `publish`. 對於和 `local` 屬性 `author` ，請分別提供本機實例和作者實例的URL。 如果 `local` 您執 `author` 行單一作者例項，兩個屬性的值都 [!DNL Experience Manager] 相同。 對於「發佈」例項，請提供發佈例項 [!DNL Experience Manager] 的URL。
...
1. 在「主 **[!UICONTROL 體]** 」欄位中，輸入主體行。

1. 在「消 **[!UICONTROL 息]** 」欄位中，輸入可選消息。

1. 在「過 **[!UICONTROL 期]** 」欄位中，指定連結停止運作的到期日期和時間。 依預設，有效期是從您共用連結之日起的一週設定。

   ![設定共用連結的到期日](assets/Set-shared-link-expiration.png)

1. 若要讓使用者下載原始資產以及轉譯，請選取「允 **[!UICONTROL 許下載原始檔案」]**。 依預設，使用者只能下載您以連結形式共用之資產的轉譯。
...
1. 若要檢視您已共用為連結的資產，請前往使用者介 [!DNL Assets] 面並按一下標 [!DNL Experience Manager] 志。 選擇 **[!UICONTROL 導覽]**。 In the Navigation pane, choose **[!UICONTROL Shared Links]** to display a list of shared assets.
