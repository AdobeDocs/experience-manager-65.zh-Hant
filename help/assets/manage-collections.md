...
和檔案夾不同，收藏集可包含來自不同位置的資產。您可以與獲指派不同許可權層級的不同使用者共用集合，包括檢視、編輯等。
...
1. 若要將新中繼資料附加至現有中繼資料，請選取「 」 **[!UICONTROL 附加模式]**. 如果您未選取此選項，新的中繼資料會取代欄位中現有的中繼資料。按一下 **[!UICONTROL 提交]**.
...
在 [!DNL Assets] 使用者介面，您可以根據特定規則、搜尋准則或自訂搜尋Facet來搜尋或篩選資產。 如果您將這些項目儲存為「 **[!UICONTROL 已儲存的搜尋]**」，您稍後可從「篩選」面板的「已儲存的搜尋 **** 」清單中存取。建立儲存的搜尋也會建立智慧型系列。

![saved_searches_list](assets/saved_searches_list.png)

儲存的搜尋會在您建立智慧型系列時建立。智慧型系列會自動新增至「已儲 **[!UICONTROL 存的搜尋]** 」清單。此 [!UICONTROL 已儲存的搜尋] 集合的查詢會儲存在 `dam:query` CRXDE中相對位置的屬性 `/content/dam/collections/`. 您可以儲存的搜尋以及清單中顯示的已儲存搜尋沒有限制。
