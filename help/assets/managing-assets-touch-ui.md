...
1. 選取要檢視或刪除轉譯的轉譯。
   ...
   **刪除轉譯**
   ...
   從「轉譯」面板中選取轉 **[!UICONTROL 譯]** ，然後按一下「刪除轉譯選項」，從工具列中刪除轉譯選項 ****![](assets/do-not-localize/deleteoutline.png) 。 資產處理完成後，無法大量刪除轉譯。 對於個別資產，您可以從使用者介面手動移除轉譯。 對於多個資產，您可以自訂Experience Manager以刪除特定轉譯或刪除資產，然後重新上傳已刪除的資產。
...
若要解析或移除其他頁面的傳入參照，請先更新相關參照，再刪除資產。 若要禁止使用者刪除參照的資產並離開中斷的連結，請停用使用覆蓋的強制刪除選項。

1. 瀏覽至您要刪除的資產所在的位置。

1. 選取資產，然後按一下工 **[!UICONTROL 具列]** 的 ![「刪除](assets/do-not-localize/deleteoutline.png) 」選項。
