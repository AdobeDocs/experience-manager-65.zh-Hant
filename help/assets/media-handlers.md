...
# 使用媒體處理常式和工作流程{#processing-assets-using-media-handlers-and-workflows}處理資產

[!DNL Adobe Experience Manager Assets] 隨附一組預設工作流程和媒體處理常式來處理資產。工作流程會定義要在資產上執行的工作，然後將特定工作委派給媒體處理常式，例如縮圖產生或中繼資料擷取。

工作流程可設定為在上傳特定MIME類型的資產時自動執行。 這些處理步驟是根據一系列[!DNL Assets]介質處理程式定義的。 [!DNL Experience Manager] 提供一些 [內置處理程式，](#default-media-handlers) 其它處理程式可以是自 [定義](#creating-a-new-media-handler) 開發或通過將進程委託給命令行工 [具來定義](#command-line-based-media-handler)。

媒體處理常式是[!DNL Assets]中對資產執行特定動作的服務。 例如，當MP3音訊檔案上傳至[!DNL Experience Manager]時，工作流程會觸發MP3處理常式，以擷取中繼資料並產生縮圖。 媒體處理常常與工作流程搭配使用。 [!DNL Experience Manager]中支援最常見的MIME類型。 特定工作可透過擴充／建立工作流程、擴充／建立媒體處理常式或停用／啟用媒體處理常式，對資產執行。

>[!NOTE]
>
>有關[!DNL Assets]支援的所有格式以及每種格式支援的功能的說明，請參閱[ Assets supported formats](assets-formats.md)頁面。

## 預設媒體處理程式{#default-media-handlers}

[!DNL Assets]中提供下列媒體處理常式，並可處理最常見的MIME類型：
...
[!DNL Experience Manager] 有一些預設的工作流程來處理資產。要查看它們，請開啟「工作流控制台」，然後按一下&#x200B;**[!UICONTROL Models]**&#x200B;頁籤：以[!DNL Assets]開頭的工作流程標題是資產特定標題。
...
請參閱[開發工具](../sites-developing/dev-tools.md)以安裝並設定Eclipse及[!DNL Maven]外掛程式，並設定[!DNL Maven]專案所需的相依性。

執行以下過程後，將TXT檔案上載到[!DNL Experience Manager]時，將提取檔案的元資料並生成兩個帶有水印的縮略圖。

1. 在Eclipse中，建立`myBundle` [!DNL Maven]專案：

   1. 在菜單欄中，按一下&#x200B;**[!UICONTROL 檔案]** > **[!UICONTROL 新建]** > **[!UICONTROL 其他]**。
   1. 在對話框中，展開[!DNL Maven]資料夾，選擇[!DNL Maven]項目，然後按一下&#x200B;**[!UICONTROL Next]**。
   1. 選中「建立簡單項目」框和「使用預設工作區位置」框，然後按一下「下一步」。****
   1. 定義[!DNL Maven]專案：
...
1. 將[!DNL Java]編譯器設定為1.5版：
   ...
   1. 選擇[!UICONTROL Java Compiler]並將以下屬性設定為1.5:
...
1. 建立包`com.day.cq5.myhandler`，該包包含`myBundle/src/main/java`下的[!DNL Java]類：
...
1. 建立[!DNL Java]類`MyHandler` :
   ...
   1. 在對話框窗口中，為[!DNL Java]類`MyHandler`命名，然後按一下[!UICONTROL 完成]。 [!DNL Eclipse] 建立並開啟檔案 `MyHandler.java`。
...
1. 編譯[!DNL Java]類並建立包：
   ...
   1. 按一下右鍵`myBundle`項目，選擇&#x200B;**[!UICONTROL 以]**&#x200B;運行，然後選擇&#x200B;**[!UICONTROL Maven Install]**。
   1. 在`myBundle/target`下建立包`myBundle-0.0.1-SNAPSHOT.jar`（包含編譯的類）。

1. 在CRX瀏覽器中，在`/apps/myApp`下建立一個新節點。 名稱= `install`，類型= `nt:folder`。
1. 複製包`myBundle-0.0.1-SNAPSHOT.jar`並將其儲存在`/apps/myApp/install`下（例如，與WebDAV一起）。 新文字處理常式現在在[!DNL Experience Manager]中處於作用中。
1. 在您的瀏覽器中，開啟[!UICONTROL Apache Felix Web Management Console]。 選擇[!UICONTROL Components]標籤並禁用預設文本處理程式`com.day.cq.dam.core.impl.handler.TextHandler`。

## 基於命令行的媒體處理程式{#command-line-based-media-handler}

[!DNL Experience Manager] 可讓您在工作流程中執行任何命令列工具，以轉換資產( [!DNL ImageMagick]例如)，並將新的轉譯新增至資產。您只需在[!DNL Experience Manager]伺服器所在的磁碟上安裝命令行工具，並在工作流中添加和配置進程步驟。 調用的進程（稱為`CommandLineProcess`）還可以根據特定MIME類型進行篩選，並根據新的轉譯建立多個縮略圖。
...
* 在[!DNL Experience Manager]伺服器所在的磁碟上建立臨時目錄。
* 將原始檔案流到臨時目錄。
* 執行由步驟的參數定義的命令。 該命令在運行[!DNL Experience Manager]的用戶的權限下在臨時目錄中執行。
...
以下示例說明如何設定命令行處理步驟，以便每次將具有miMIME e-type GIF或TIFF的資產添加到[!DNL Experience Manager]伺服器上的`/content/dam`時，將建立原始影像的翻轉影像以及另外三個縮圖（140x100、48x48和10x25）0)。

若要這麼做，請使用[!DNL ImageMagick]。 [!DNL ImageMagick] 是免費的命令列軟體，用來建立、編輯和合成點陣圖影像。

在[!DNL Experience Manager]伺服器所在的磁碟上安裝[!DNL ImageMagick] :
...
1. 要查看工具是否已正確安裝，請在命令行上運行以下命令`convert -h`。
   ...
   >[!NOTE]
   >
   >在某些Windows版本中，轉換命令可能無法運行，因為它與[!DNL Windows]安裝中的本機轉換實用程式衝突。 在此例中，請提及用於將影像檔案轉換為縮圖的[!DNL ImageMagick]軟體的完整路徑。 例如，`"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`。

1. 要查看工具是否正常運行，請將JPG影像添加到工作目錄並運行命令行convert `<image-name>.jpg -flip <image-name>-flipped.jpg`。 翻轉的影像會新增至目錄。 然後，將命令行處理步驟添加到&#x200B;**[!UICONTROL DAM更新資產]**&#x200B;工作流。
1. 轉至&#x200B;**[!UICONTROL Workflow]**&#x200B;控制台。
1. 在&#x200B;**[!UICONTROL Models]**&#x200B;標籤中，編輯&#x200B;**[!UICONTROL DAM Update Asset]**&#x200B;模型。
1. 將&#x200B;**[!UICONTROL 啟用Web的轉譯]**&#x200B;步驟的[!UICONTROL 參數]更改為：`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`。
1. 儲存工作流程。
...
使用逗號分隔[!UICONTROL 處理參數]的值，不要以空格開頭。
...
例如，如果[!DNL ImageMagick]安裝在[!DNL Experience Manager]伺服器所在的磁碟上，並且使用[!UICONTROL CommandLineProcess]作為實施，並使用以下值作為[!UICONTROL Process Arguments]建立進程步驟：
...
使用[!UICONTROL 處理參數]使用[!DNL ImageMagick]建立三個標準縮圖：

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=319x319 -thumbnail "319x319>" -background transparent -gravity center -extent 319x319 -write png:cq5dam.thumbnail.319.319.png -thumbnail "140x100>" -background transparent -gravity center -extent 140x100 -write cq5dam.thumbnail.140.100.png -thumbnail "48x48>" -background transparent -gravity center -extent 48x48 cq5dam.thumbnail.48.48.png`

使用[!UICONTROL 處理參數]使用[!DNL ImageMagick]建立啟用Web的轉譯：
