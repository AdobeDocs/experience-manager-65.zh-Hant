...
| 日期 | 與資產相關聯的日期或期間。 |
| 說明 | 資產的詳細資訊。 |
| 格式 | 資產的檔案格式、實體媒體或維度。 [!DNL Experience Manager] 使 `dc:format` 用來表示資產的MIME類型。 |
| 識別碼 | 資產的唯一參考。 |
| 語言 | 資產的語言（例如，英文為`en`）。 |
| 發佈者 | 負責使資產可用的人員或公司。 |
| 關係 | 相關資產。 |
...
XMP（可擴充中繼資料平台）是[!DNL Adobe Experience Manager Assets]用於所有中繼資料管理的中繼資料標準。 XMP提供標準格式，供多種應用程式建立、處理和交換中繼資料。
...
[!DNL Adobe Experience Manager Assets] 支援IPTC元資料標準，該標準被廣泛用於描述資產。這樣，[!DNL Assets]增強了各方對其影像的接受，包括攝影師、創意機構、圖書館、博物館等。
...
1. 從工具欄按一下「**[!UICONTROL 屬性]**」。
1. 按一下&#x200B;**[!UICONTROL IPTC]**&#x200B;標籤以查看資產的IPTC元資料。
1. 視需要編輯IPTC元資料屬性。

   ![iptc_tab](assets/keywords-in-iptc-tab.png)

1. 按一下&#x200B;**[!UICONTROL IPTC擴充功能]**&#x200B;標籤以查看資產的IPTC擴充功能元資料。
1. 視需要編輯IPTC擴展元資料屬性。
