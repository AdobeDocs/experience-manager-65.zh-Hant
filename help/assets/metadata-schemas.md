...
1. 在[!DNL Experience Manager]介面中，轉至&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 資產]** > **[!UICONTROL 中繼資料結構]**。
...
1. （選用）在中繼資料結構編輯器中，建立新欄位以條件化。 在&#x200B;**[!UICONTROL Settings]**&#x200B;標籤中指定名稱和屬性路徑。
   ...
   若要建立新索引標籤，請按一下`+`以新增索引標籤，然後新增中繼資料欄位。
...
1. 機碼值組是提供給表單使用者的選項。 您可以手動或從JSON檔案提供索引鍵值配對。
   ...
   * 要手動指定值，請選擇&#x200B;**[!UICONTROL 手動添加]**，然後按一下&#x200B;**[!UICONTROL 添加選擇]**&#x200B;並指定選項文本和值。 例如，指定視訊、PDF、Word和影像資產類型。
...
1. 按一下「**[!UICONTROL 新增規則]**」 ，然後選擇「**[!UICONTROL 資產類型]**」欄位以建立相依性。 也選擇要在其上建立相關性的欄位值。在這種情況下，請選擇「 **[!UICONTROL 視訊」]**。按一下&#x200B;**[!UICONTROL Done]**&#x200B;以儲存變更。
...
1. 按一下工具列中的「**[!UICONTROL 套用至資料夾]** 」，將自訂中繼資料套用至資料夾。

1. 選擇要在其上應用修改的架構的資料夾，然後按一下&#x200B;**[!UICONTROL Apply]**。
...
1. 按一下「**[!UICONTROL 儲存]**」。修改後的表單列在&#x200B;**[!UICONTROL 中繼資料結構Forms]**&#x200B;頁面中。 選取表單，然後按一下工具列中的「**[!UICONTROL 套用至資料夾]** 」，將自訂中繼資料套用至資料夾。
...
1. （可選）訪問`https://[aem_server]:[port]/system/console/components/`。 配置並啟用預設禁用的`com.day.cq.dam.core.impl.MissingMetadataNotificationJob`元件。 設定[!DNL Experience Manager]檢查資產上中繼資料是否有效的頻率。 此設定會將屬性`hasValidMetadata`新增至資產的`jcr:content`。 [!DNL Experience Manager] 使用此屬性來篩選搜尋結果中的無效資產。如果您在檢查後新增資產，直到下次排程檢查前，資產不會標籤為`hasValidMetadata`。 因此，直到下次排程檢查後，資產才會出現在無效中繼資料的搜尋篩選器中。
