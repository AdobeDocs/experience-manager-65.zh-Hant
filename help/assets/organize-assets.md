...
[!DNL Experience Manager Assets] 支援多種組織內容的方式。 您可以使用資料夾以階層方式組織資料夾，也可以使用標籤（例如）以無序、臨機方式組織資料夾。 使用者可以在DAM Asset Editor中編輯顯示子資產、轉譯和中繼資料的標籤。
...
[!DNL Adobe Experience Manager Assets] 會使用自我學習演演算法來建立高度說明性的標籤，讓您只需按幾下滑鼠即可找到正確的資產。 智慧標籤使用Adobe的人工智慧和機器學習架構Adobe Sensei，這項架構可訓練您識別標準標籤和業務特定標籤，並將其套用至影像。 智慧標籤也可以識別內容、個別字詞或片語，並自動將描述性標籤套用至資產
