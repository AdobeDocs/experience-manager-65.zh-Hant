...
中的多站點管理員(MSM)功能 [!DNL Adobe Experience Manager] 可讓使用者重複使用一次撰寫並在多個網站位置重複使用的內容。 MSM在數位資產中也提供相同功能 [!DNL Assets] 功能。 將MSM用於 [!DNL Assets]，您可以：
...
若要瞭解在多個網站位置重複使用相同內容（文字和資產）的使用案例，請參閱 [可能的MSM案例](/help/sites-administering/msm.md). [!DNL Experience Manager] 維護原始資產與其連結副本之間的連結，稱為即時副本(LC)。 維持的連結可將集中變更推送至多個即時副本。 這樣可以在消除管理重複副本的限制的同時實現更快的更新。 變更的傳播不會發生錯誤，而且是集中化的。 功能提供空間，讓更新僅限於選取的即時副本。 使用者可以分離連結（亦即中斷繼承），並進行本機編輯，這些編輯在下次更新主要副本並轉出變更時不會遭到覆寫。 分離功能適用於幾個選取的中繼資料欄位或整個資產。 它可讓您在本機靈活地更新原來從主要副本繼承的資產。
...
|---|---|---|
| [!UICONTROL 引用] 邊欄 | 當您已選取來源時，請快速同步處理。 | 另請參閱 [來源的「參考」邊欄中的快速動作](#refrailsource) |
...
* MSM不支援為設定頁面屬性的MSM鎖定（觸控式UI） [!DNL Assets].
