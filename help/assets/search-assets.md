...
| [了解搜尋UI](#searchui) | [視覺或相似性搜尋](#configvisualsearch) | [檢查資產的屬性和中繼資料](#checkinfo) |
...
### 使用和不使用增強的智慧標籤產生的結果 {#withsmarttags}

預設情況下， [!DNL Experience Manager]搜索將搜索詞與AND子句組合。 例如，請考慮搜尋關鍵字「女性」跑步。 依預設，中繼資料中只有同時包含女性和執行中關鍵字的資產會顯示在搜尋結果中。 與關鍵字搭配使用特殊字元（句號、底線或破折號）時，會保留相同的行為。 下列搜尋查詢會傳回相同的結果：

* `woman running`
* `woman.running`
* `woman-running`

不過，查詢`woman -running`會傳回元資料中沒有`running`的資產。
使用智慧標籤會新增額外的`OR`子句，以尋找任何搜尋詞作為套用的智慧標籤。 使用智慧標籤標示為`woman`或`running`的資產也會出現在這類搜尋查詢中。 搜索結果是，

* 中繼資料中具有`woman`和`running`關鍵字的資產（預設行為）。

* 使用任一關鍵字（智慧標籤行為）標示的資產智慧。

### 在您輸入時搜尋建議 {#searchsuggestions}

開始鍵入關鍵字時，[!DNL Experience Manager]建議可能的搜索關鍵字或片語。 建議是以現有資產的中繼資料為基礎。 [!DNL Experience Manager] 索引所有元資料欄位，以協助搜尋。為了提供搜尋建議，系統會使用下列幾個中繼資料欄位的值。 若要提供搜尋建議，請考慮使用適當的關鍵字填入下列欄位：
...
若要接收多個搜尋關鍵字的建議，請繼續輸入所有關鍵字，而不要為單一關鍵字選取任何建議。
...
## 從其他[!DNL Experience Manager]產品或介面{#search-assets-other-surfaces}中搜尋資產
...
### 編寫網頁時在「內容尋找器」中搜尋資產{#content-finder}
...
* **標籤**:標籤可協助您對可以更有效率地瀏覽和搜尋的資產進行分類。標籤有助於將適當的分類法傳播給其他使用者和工作流程。 [!DNL Experience Manager] 提供使用Adobe Sensei的人工智慧服務來自動標籤資產的方法，讓您更能透過使用方式和訓練來標籤資產。當您搜尋資產時，如果帳戶已啟用功能，系統會加入智慧標籤。 可與內建的搜尋功能搭配使用。 請參閱[搜尋行為](#searchbehavior)。 若要最佳化搜尋結果的顯示順序，您可以[提升一些所選資產的搜尋排名](#searchrank)。
...
資產探索仰賴DAM內容的索引，包括中繼資料。 更快、更準確的資產發現依賴於優化的索引和適當的配置。 請參閱[搜尋索引](/help/assets/performance-tuning-guidelines.md#search-indexes)、[oak查詢和索引](/help/sites-deploying/queries-and-indexing.md)及[最佳實務](/help/sites-deploying/best-practices-for-queries-and-indexing.md)。

若要從搜尋結果中排除特定資產，請在Lucene索引中使用`excludedPath`屬性。

### 視覺或相似性搜尋 {#configvisualsearch}

視覺搜尋使用智慧標籤。 設定智慧標籤功能後，請遵循下列步驟。

1. 在[!DNL Experience Manager] CRXDE的`/oak:index/lucene`節點中，新增下列屬性和值並儲存變更。

   * `costPerEntry` 屬性( `Double` 包含值) `10`。
   * `costPerExecution` 屬性( `Double` 包含值) `2`。
   * `refresh` 屬性( `Boolean` 包含值) `true`。

   此配置允許從相應索引中進行搜索。

1. 要建立Lucene索引，請在CRXDE的`/oak:index/damAssetLucene/indexRules/dam:Asset/properties`處，建立名為`imageFeatures`類型`nt-unstructured`的節點。 在`imageFeatures`節點中，

   * 使用值`jcr:content/metadata/imageFeatures/haystack0`添加`String`類型的`name`屬性。
   * 新增`Boolean`類型的`nodeScopeIndex`屬性，其值為`true`。
   * 新增`Boolean`類型的`propertyIndex`屬性，其值為`true`。
   * 使用值`true`添加`Boolean`類型的`useInSimilarity`屬性。

   儲存變更。

1. 訪問`/oak:index/damAssetLucene/indexRules/dam:Asset/properties/predictedTags`並添加`Boolean`類型的`similarityTags`屬性，其值為`true`。
1. 將智慧標籤套用至[!DNL Experience Manager]存放庫中的資產。 請參閱[如何設定智慧標籤](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/configuring/tagging.html?lang=en#configuring)。
1. 在CRXDE的`/oak-index/damAssetLucene`節點中，將`reindex`屬性設定為`true`。 儲存變更。
1. （可選）如果已自定義搜索表單，則將`/libs/settings/dam/search/facets/assets/jcr%3Acontent/items/similaritysearch`節點複製到`/conf/global/settings/dam/search/facets/assets/jcr:content/items`。 儲存變更。

如需相關資訊，請參閱[了解Experience Manager](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/metadata/image-smart-tags.html)中的智慧標籤，以及[如何管理智慧標籤](/help/assets/enhanced-smart-tags.md)。

>[!CAUTION]
>
>如果在[!DNL Adobe Experience Manager]之外完成Lucene索引，則基於智慧標籤的搜尋將無法如預期運作。

### 必填中繼資料 {#mandatorymetadata}

業務使用者、管理員或DAM圖書館員可將某些中繼資料定義為必要中繼資料，這是業務流程運作所必須的。 由於各種原因，某些資產可能會遺失此中繼資料，例如大量移轉的舊版資產或資產。 系統會根據索引中繼資料屬性，偵測並報告遺失或無效的中繼資料資產。 要配置它，請參閱[強制元資料](/help/assets/metadata-schemas.md#define-mandatory-metadata)。

### 修改搜尋刻面 {#searchfacets}

為了改善發現速度，[!DNL Experience Manager Assets]提供可用來篩選搜索結果的搜索Facet。 依預設，「篩選器」面板包含一些標準Facet。 管理員可以自訂「篩選」面板，使用內建的述詞來修改預設Facet。 [!DNL Experience Manager] 提供內建述詞的完整集合，以及可自訂Facet的編輯器。請參閱[搜尋Facet](/help/assets/search-facets.md)。

### 上傳資產時擷取文字 {#extracttextupload}

您可以設定[!DNL Experience Manager]在使用者上傳PSD或PDF檔案等資產時，從資產中擷取文字。 [!DNL Experience Manager] 索引擷取的文字，並協助使用者根據擷取的文字來搜尋這些資產。請參閱[上傳資產](/help/assets/manage-assets.md#uploading-assets)。

如果對於您的部署而言，文本提取變得過於耗用資源，請考慮[禁用文本提取](https://helpx.adobe.com/experience-manager/kb/Disable-binary-text-extraction-to-optimize-Lucene-indexing-AEM.html)。

### 篩選搜尋結果的自訂述詞 {#custompredicates}

述詞可用來建立Facet。 管理員可以使用預先設定的述詞來自訂「篩選器」面板中的搜尋刻面。 這些述詞可使用覆蓋來自訂。 請參閱[建立自訂述詞](/help/assets/searchx.md)。
...
### 大量更新元資料屬性 {#metadataupdates}

您可以大量更新多個資產的通用中繼資料欄位。 從搜尋結果中選取一或多個資產。 從工具列按一下「**[!UICONTROL 屬性]**」，然後視需要更新中繼資料。 完成後，按一下「**[!UICONTROL 儲存並關閉]**」。 更新欄位中先前存在的中繼資料會遭到覆寫。

對於單一資料夾或集合中可用的資產，若不使用搜尋功能，[大量更新中繼資料會較為容易。 ](/help/assets/metadata.md)對於可跨資料夾使用或符合通用條件的資產，透過搜尋大量更新中繼資料會更快。
