...
1. 在「同 **[!UICONTROL 步模式]** 」下拉式清單中，選取下列其中一項：
   ...
   | 同步模式 | 說明 |
   | --- | --- |
   | **[!UICONTROL 已繼承]** | 資料夾上沒有明確的同步值；而是會從其上級資料夾或動態媒體配置中設定的預設模式繼承同步 **[!UICONTROL 值]**。 「繼承」的詳 **[!UICONTROL 細狀態]** ，通過工具提示顯示。 |
...
1. 從「快速發佈」功能表清單中，選取 **[!UICONTROL 下列選項之一]** 。
   ...
   | 發佈至 Dynamic Media | 立即將選取的資產發佈至動態媒體。<br>資產必須已同步至動態媒體。 如有必要，請確定 **[!UICONTROL 資料夾屬性中的「同步模式]** 」已設定為「將此資料夾子樹中的所有內容同步到動態媒體」 ****。 |
