...
以下檔案說明Dynamic Media執行中的疑難排解 **dynamicmedia_scene7** 執行模式。

## 設定與組態 {#setup-and-configuration}

執行下列操作，確定Dynamic Media已正確設定：

* 啟動指令包含 `-r dynamicmedia_scene7` 執行模式引數。
...
* 針對影像和影片，確認 `<object_node>/jcr:content/metadata/dam:scene7ID` 值存在於執行移動、複製或刪除作業之前。
* 針對影像和檢視器預設集，確認 `https://<server>/crx/de/index.jsp#/etc/dam/presets/viewer/testpreset/jcr%3Acontent/metadata` 值存在於執行移動、複製或刪除作業之前。
  ...
  <tr>
   ...
   <td><p>若要判斷視訊編碼是否仍在進行中，或已進入失敗狀態：</p>
    <ul>
     ...
     <li>從工作流程主控台監視視訊 <code>https://localhost:4502/libs/cq/workflow/content/console.html</code> &gt;執行個體，封存，失敗索引標籤。</li>
    </ul> </td>
   <td> </td>
  </tr>
  <tr>
