...
![Dynamic Media中的註解與音訊曲目標籤，以及顯示已上傳視訊的.VTT註解檔案與已上傳的.MP3音訊曲目檔案的表格。](assets-dm/msma-subtitle-audiotracks-tab.png)
...
1. 在視訊的「屬性」頁面上，選取 **[!UICONTROL 註解與音訊曲目]** 標籤。
   ...
   ![屬性頁面上的註解和音訊曲目索引標籤。](assets-dm/msma-audiotracks.png)*視訊屬性頁面上的註解和音訊曲目索引標籤。*
...
1. （可選）若要新增一或多個註解檔案至視訊，請執行下列動作：
   ...
   * 若要在媒體播放器上顯示註解，請 *必須* 新增必要的詳細資訊（中繼資料）關於 *每個* 您上傳的註解檔案。 選取註解檔案名稱右側的鉛筆圖示。 在 **編輯標題** 對話方塊中，輸入檔案的下列必要詳細資訊，然後選取 **[!UICONTROL 儲存]**. 對您上傳的每個註解檔案重複此程式：
     ...
     | 標籤 | 在中針對註解名稱顯示的文字 **[!UICONTROL 選取音訊或標題]** 媒體播放器中的快顯清單。 客戶看到的標籤與標題追蹤相對應。 例如，`English (CC)`。 |
...
![註解和音軌欄位中反白顯示的狀態列。](assets-dm/msma-lifecycle-status.png)*每個上傳的標題和音訊曲目檔案的生命週期狀態。*
