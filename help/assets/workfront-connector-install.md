...
在中具有管理員存取許可權的使用者 [!DNL Adobe Experience Manager] 安裝增強型聯結器。 安裝之前，請先檢閱平台支援及其他 [聯結器的先決條件](https://one.workfront.com/s/csh?context=2467&amp;pubname=the-new-workfront-experience).
...
1. 在Dispatcher上，允許名為的HTTP標頭 `authorization`， `username`、和 `apikey`. 允許 `GET`， `POST`、和 `PUT` 要求給 `/bin/workfront-tools`.
1. 確認下列路徑不存在於 [!DNL Experience Manager] 存放庫：
...
1. 新增自訂屬性至的開箱即用索引定義 **`ntFolderDamLucene(/oak:index/ntFolderDamLucene)`**. 執行以下步驟：
   * 新增 **`nt:unstructured`** 已命名的屬性 **`wfReferenceNumber`** 至：
     `/oak:index/ntFolderDamLucene/indexRules/nt:folder/properties/wfReferenceNumber`。
   * 重新索引 `index /oak:index/ntFolderDamLucene` 將重新索引標幟翻轉到 `true`.

系統使用者 `workfront-tools` 會自動建立，並自動管理所需的許可權。 所有使用者來自 [!DNL Workfront] 使用該聯結器的使用者會自動新增為此群組的一部分。
...
Experience Manager Assets可讓您更新 [!DNL Workfront for Experience Manager enhanced connector] 從舊版更新至最新版本。
