...
產品欄位可讓您透過不同檢視導覽至使用者想要選取的產品。 依預設，product欄位會傳回產品的ID，但可使用 `selectionId` 屬性。
...
此外，標準對話方塊欄位屬性，例如 `name`， `fieldLabel`，或 `fieldDescription`也支援。
...
此外，標準對話方塊欄位屬性，例如 `name`， `fieldLabel`，或 `fieldDescription` 亦受支援。
...
>[!CAUTION]
>
>從CIF Core Components 2.0.0版開始，支援 `id` 已移除並取代為 `uid`. Adobe建議使用 `uid` 或 `urlPath` 作為類別識別碼。 Adobe持續支援 `id` 和 `idAndUrlPath` 僅適用於使用CIF核心元件1.x版的專案。
