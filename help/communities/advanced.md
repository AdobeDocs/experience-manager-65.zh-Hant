...
進階評分可授予徽章，將成員識別為專家。 進階評分會根據數量指定點數 *和* 成員建立的內容品質，而基本評分會根據建立的內容數量指派分數。
...
因此，進階評分引擎需要足夠的資料，才能讓分析有意義。 當演演算法持續調整以符合所建立內容的數量和品質時，系統會持續重新評估成為專家的成就門檻。 還有一個概念 *衰減* 成員的較舊貼文。 如果專家成員在某個預先確定的時間停止參與他們獲得專家地位的主題內容(請參閱 [評分引擎設定](#configurable-scoring-engine))可能失去專家身分。
...
* [啟用元件的預算](/help/communities/implementing-scoring.md#enable-badges-for-component) 是通用的。
...
* **評分權重**
  ...
  對於主題，請指定計算分數時，應給予最高優先順序的動詞。 可輸入一或多個主題，但限於 **每個主題一個動詞**. 另請參閱 [主題和動詞](/help/communities/implementing-scoring.md#topics-and-verbs).
...
此發行版本包含兩個進階徽章規則，分別對應至 [進階論壇和評論評分規則](#included-scoring-rules-and-sub-rules).
