...
您不需要Sidekick，但可將其最小化，以免妨礙存取報表套裝設定。
...
對於包含Communities元件但不使用[網站建立精靈](/help/communities/sites-console.md)建立的&#x200B;**一般AEM網站**，必須新增元素。 href的值應為網站的路徑。 例如，如果網站路徑為`/content/my/company/en`，則使用：
...
* UGC *的* SRP路徑： `/content/usergenerated/asi/.../forum/jmtz-topic3`
...
為社群網站啟用Analytics後，可從框架設定中看到Analytics eVar和事件與AEM變數的對應。
...
如果Communities eVar和事件覆寫報表套裝中預先存在的對應，則為避免資料遺失，請將用於Communities功能的AEM變數指派給其他Analytics eVar或事件，並還原原始對應。
...
>* **`event1`**&#x200B;透過&#x200B;**`event7`**
>
>**在發佈社群網站之前，**&#x200B;請先還原現有的對應。 將自動對應至其他Analytics變數的社群AEM變數（社群網站啟用Analytics時）移至這些變數。 所有Communities元件之間均應一致進行此重新對應。
>
>若未這麼做，可能會導致無法復原的資料遺失。
...
Adobe Analytics憑證已加密。 為了便於作者與發佈者之間複製或傳輸加密的分析憑證，所有AEM執行個體都必須共用相同的主要加密金鑰。
