...
部落格功能由兩個主要元件組成，可透過新增[Blog函式](/help/communities/functions.md#blog-function)或在作者編輯模式中將元件新增至頁面來使用。
...
包含[部落格函式](/help/communities/functions.md#blog-function)的社群網站結構將配置`Blog`和`Blog Sidebar`元件。 Blog函式支援標識[特權成員用戶組](/help/communities/users.md#privileged-members-group)。
...
自AEM 6.1社群起，UGC使用[公用商店](/help/communities/working-with-srp.md)包括程式化存取UGC，而不論選擇的儲存選項（例如ASRP、MSRP或JSRP）。
...
如需詳細資訊，請參閱[主要發佈者](/help/communities/deploy-communities.md#primary-publisher) 。
...
選取`Embed Media from External Sites`圖示，即可將多媒體內嵌在部落格文章中：
