...
修改訊息清單的組態 **收件匣**， **已傳送專案**、和 **垃圾桶** 傳訊功能頁面，請在中開啟網站 [作者編輯模式](/help/communities/sites-console.md#authoring-site-content).

1. 在 `Preview` 模式，選取 **訊息** 開啟主要傳訊頁面的連結。 然後選取 **收件匣**， **已傳送專案** 或 **垃圾桶** 為該訊息清單設定元件。

1. 在 `Edit` 模式，在頁面上選取元件。
1. 若要存取設定對話方塊，請選取 `link` 圖示。
取消繼承後，可以選取設定圖示以開啟設定對話方塊。

1. 組態完成後，必須選取 `broken link` 圖示。
...
* **服務選擇器**
  ...
  (*必填*)將此設定為屬性的值 **`serviceSelector.name`** 從 [AEM Communities訊息操作服務](/help/communities/messaging.md#messaging-operations-service).
...
* **資料夾路徑**
  ...
  (*必填*)參考為設定的值 **inbox.path.name** 和 **sentitems.path.name** 在 [AEM Communities訊息操作服務](/help/communities/messaging.md#messaging-operations-service). 為設定時 `Inbox`，使用下列的值新增一個專案： **inbox.path.name**. 為設定時 `Outbox`，使用下列的值新增一個專案： **sentitems.path.name**. 設定時 `Trash`，新增兩個同時含有值的專案。
...
* **標示讀取按鈕**
  ...
  如果勾選，會顯示 `Read`按鈕允許將訊息標示為已讀取。

* **標示為未讀取按鈕**

  如果勾選，會顯示 `Mark Unread` 按鈕允許將訊息標示為已讀取。

* **刪除按鈕**

  如果勾選，會顯示 `Delete` 按鈕允許將訊息標示為已讀取。 複製刪除功能，如果 **`Message Options`** 也會勾選。
...
* **顯示使用者**
  ...
  選擇 **`Sender`** 或 **`Recipients`** 以便您判斷要顯示寄件者還是收件者。

### 設定撰寫訊息 {#configure-compose-message}

若要修改撰寫訊息頁面的設定，請開啟以下位置的網站： [作者編輯模式](/help/communities/sites-console.md#authoring-site-content).

* 在 `Preview` 模式，選取 **訊息** 開啟主要傳訊頁面的連結。 接著，選取「新增訊息」按鈕，讓您開啟 `Compose Message` 頁面。

* 在 `Edit` 模式，在包含訊息內文的頁面上選取主要元件。
* 若要存取設定對話方塊，請選取 `link` 圖示。
取消繼承後，可以選取設定圖示以開啟設定對話方塊。

* 組態完成後，必須選取 `broken link` 圖示。
...
* **顯示主旨欄位**
  ...
  如果勾選，顯示 `Subject` 欄位並啟用新增主旨至訊息。 未勾選預設值。

* **主旨標籤**

  輸入您要在旁邊顯示的文字 `Subject` 欄位。 預設為 `Subject`.

* **顯示附加檔案欄位**

  如果勾選，顯示 `Attachment` 欄位並啟用新增檔案附件至訊息。 未勾選預設值。

* **附加檔案標籤**

  輸入您要在旁邊顯示的文字 `Attachment` 欄位。 預設為 **`Attach File`**.

* **顯示內容欄位**

  如果勾選，顯示 `Content` 欄位並啟用新增訊息內文。 未勾選預設值。
