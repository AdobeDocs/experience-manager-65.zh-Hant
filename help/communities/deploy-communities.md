...
1. 提取mysql連接器 — java-&lt;version>-bin.jar（捆綁包）
1. 使用Web控制台安裝並啟動包：
...
有關安裝捆綁包的詳細資訊，請參閱 [Web控制台](/help/sites-deploying/web-console.md) 的子菜單。
...
因此，有必要 **編輯所有輔助發佈實例的配置** 取消選中 **`Primary Publisher`** 複選框。

![主發佈者](assets/primary-publisher.png)
...
* 在作者上訪問複製控制台
   ...
   * 從全局導航，導航到 **[!UICONTROL 工具]** > **[!UICONTROL 部署]** > **[!UICONTROL 複製]** > **[!UICONTROL 作者代理]**
...
![預設代理發佈](assets/default-agent-publish.png)
...
從AEM6.3開始，密鑰材料將儲存在檔案系統中，而不再儲存在儲存庫中。

為了將關鍵材料從作者複製到所有其他實例，必須：

* 訪問AEM包含要複製的關鍵材料的實例（通常為作者實例）

   * 查找 `com.adobe.granite.crypto.file` 例如，
...
* 對於每個目標實AEM例
   ...
   * 貼上以前複製的2個檔案
   * 有必要 [刷新花崗岩加密捆綁包](#refresh-the-granite-crypto-bundle) 目標實AEM例當前正在運行
...
* [刷新花崗岩加密捆綁包](#refresh-the-granite-crypto-bundle)
   ...
   ![複製儲存庫](assets/replicare-repository.png)
...
* 選擇 **刷新**
   ...
   ![花崗岩密碼](assets/granite-crypto.png)
