...
**用於[AEM平台](/help/sites-deploying/deploy.md#what-is-aem)**
...
如需AEM 6.5的最新更新，請參閱[Adobe Experience Manager 6.4 Cumulative Fix Pack和Service Pack](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)。
...
1. 從封存中擷取mysql-connector-java-&lt;version>-bin.jar （套件）
1. 使用Web主控台安裝並啟動套件組合：
...
1. 如果在設定JDBC之後於現有部署上進行安裝，則從Web主控台重新儲存JDBC設定，以將JDBC重新繫結至新聯結器：
   ...
   * 尋找`Day Commons JDBC Connections Pool`組態
   * 選取以開啟
   * 選取`Save`

1. 在所有製作和發佈執行個體上重複步驟3和4

在[網頁主控台](/help/sites-deploying/web-console.md)頁面上找到有關安裝套裝的詳細資訊。

#### 範例：已安裝MySQL聯結器套件組合 {#example-installed-mysql-connector-bundle}

![聯結器套件](assets/connector-bundle.png)
...
在本機AEM執行個體上，使用封裝管理員（例如[https://localhost:4502/crx/packmgr/](https://localhost:4502/crx/packmgr/)）來上傳至本機AEM的封裝存放庫。

或者，使用「封裝共用」從本機AEM執行個體存取封裝（例如，[https://localhost:4502/crx/packageshare/](https://localhost:4502/crx/packageshare/)），「`Download`」按鈕會下載到本機AEM執行個體的封裝存放庫。
...
通用存放區支援在發佈環境中稽核UGC以及對UGC進行分析，同時消除對UGC [復寫](/help/communities/sync.md)的需求。
...
* 存取作者上的復寫主控台
   ...
   * 從全域導覽，瀏覽至&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 部署]** > **[!UICONTROL 復寫]** > **[!UICONTROL 作者代理程式]**
...
AEM Communities提供兩項功能，要求所有AEM伺服器執行個體使用相同的加密金鑰。 這些是[Analytics](/help/communities/analytics.md)和[ASRP](/help/communities/asrp.md)。
...
例如，
   ...
   * 貼上先前複製的兩個檔案
   * 如果目標AEM執行個體正在執行，則必須[重新整理Granite Crypto套件](#refresh-the-granite-crypto-bundle)
...
將關鍵資料儲存在存放庫中（AEM 6.2和更早版本的情況一樣）可以保留。 在每個AEM執行個體（建立初始存放庫）第一次啟動時指定系統屬性`-Dcom.adobe.granite.crypto.file.disable=true`。
...
### Apache HTTP 伺服器 {#apache-http-server}
...
* [安裝Dispatcher](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-dispatcher/using/getting-started/dispatcher-install)
