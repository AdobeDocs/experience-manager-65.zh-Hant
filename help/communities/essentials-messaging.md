...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>請參閱<a href="/help/communities/configure-messaging.md" target="_blank">設定傳訊</a></td>
  </tr>
  <tr>
...
>[!CAUTION]
>
>下列MessageBuilder方法的String引數必須&#x200B;*不*&#x200B;包含結尾斜線「/」：
>
>* `setInboxPath`()
>* `setSentItemsPath`()
>
...
使用精靈建立的社群網站結構包含訊息功能（選取時）。 檢視[社群網站主控台](/help/communities/sites-console.md#user-management)的`User Management`設定。
...
下列範例是事件處理常式，它會接聽`message sent`事件，並使用`Day CQ Mail Service`傳送電子郵件給所有郵件收件者。

若要嘗試伺服器端範例指令碼，您需要開發環境和建置OSGi套件的功能：

1. 以系統管理員身分登入` [CRXDE|Lite](https://localhost:4502/crx/de)`。
1. 在`/apps/engage/install`中建立具有任意名稱的`bundle node`，例如：
...
1. 導覽至`/apps/engage/install/com.engage.media.social.messaging.MessagingNotification/src/main/java/com/engage/media/social/messaging/notification`，然後：
   ...
   1. 刪除自動建立的`Activator.java`類別。
   1. 建立類別`MessageEventHandler.java`。
   1. 將下列程式碼複製並貼到`MessageEventHandler.java`中。

1. 按一下&#x200B;**「儲存全部」**。
1. 瀏覽至`/apps/engage/install/com.engage.media.social.messaging.MessagingNotification/com.engage.media.social.messaging.MessagingNotification.bnd`，並新增所有匯入陳述式，如`MessageEventHandler.java`程式碼中所寫入。
1. 建立套件組合。
1. 請確定已設定`Day CQ Mail Service`OSGi服務。
