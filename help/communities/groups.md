...
* 在作者上，以管理員權限登入。
* 從全域導覽：社 **[!UICONTROL 群]** > **[!UICONTROL 網站]**。
* 選擇現有的社區站點資料夾以將其開啟。
* 在資料夾中選擇社區站點的實例。

   * 社群網站的結構必須包含群組功能。
   * 這些螢幕擷取自「開始使用」教學課程，當您在發佈 [時建立群組後](/help/communities/published-site.md)。
...
* **社群群組名稱**
   ...
   * 重複勾選名稱，因為在建立群組後不易變更。
   * 基本URL將顯示在下方 `Community Group Name`。
   * 若為有效的URL，請附加&quot;。html&quot;
...
* **必要的成員資格**
   ...
   如果選中此選項，則社群群組為開放群組。 社群網站成員可以檢視群組的內容，但需要加入群組才能張貼內容。 成員通過選擇發佈環 `Join` 境中的按鈕加入。 未選擇預設值。

* **限制的成員資格**

   如果選中此選項，則社群群組為機密群組。 必須明確邀請社群成員。 已邀請的成員將輸入到搜索框中。 您稍後可以使用作者環境的「 [成員」和「群組」控制台](/help/communities/members.md) ，來新增成員。 未選擇預設值。
