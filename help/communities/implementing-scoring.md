...
* `rules`和`sub-rules`節點都屬於cq:Page型別。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><em>（必要）</em>格式為'number|path'的多值字串
    ...
    </ul> 這些字串必須經過排序，數值才會增加，而且數字與路徑之間不應出現空格。<br /> 範例專案：<br /> <code>80|/libs/settings/community/badging/images/gold-badge/jcr:content/gold.png</code></td>
  </tr>
  <tr>
   <td>badgingtype</td>
   <td>字串</td>
...
cURL -i -X POST -H *標頭* -u *登入* -F *作業* -F *徽章* *member-profile-url*
