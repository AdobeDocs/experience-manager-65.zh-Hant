...
* 計分和標籤規則必 [須套用](#applytopage) 至放置元件的頁面（或上階）。
...
* 對於每個計分規則累積的點數，請指 [派標章](#badges) ([標籤規則](#badging-rules))。
...
* 節 `rules` 點和 `sub-rules` 節點都屬於cq:Page類型。
...
標籤 `scoringRules` 規則上的屬性只會限制哪些評分規則可與該特定標籤規則配對。
...
cURL -i -X POST *H標頭* -u *signin* -F *operation* -F ***badgeChember-profile-url*
...
元件的SocialEvent例項會將事件記 `actions` 錄為某個事件 `topic`。 SocialEvent包含傳回與動作相 `verb` 關聯的方法。 和之 *間有* n- `actions` 1關係 `verbs`。

對於傳遞的社群元件，下表說明為每個 `verbs` 可用於計 `topic` 分子規則 [的元件定義](#scoring-sub-rules)。
...
對於自訂元件，會執行個體化SocialEvent，以記錄元件的 `actions` 事件為 `topic`。

若要支援計分，SocialEvent必須覆寫方法， `getVerb()` 以針對每個方 `verb` 法傳回適當的計分 `action`。 針 `verb` 對動作傳回的可能是常用(例如 `POST`)或專用於元件(例如 `ADD RATING`)。 和之 *間有* n- `actions` 1關係 `verbs`。
...
請參閱標 `scoringRules` 記規則 [的屬性](#badging-rules)。
