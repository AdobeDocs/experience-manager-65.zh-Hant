...
* `Admin`
   ...
   屬於社群管理員群 [組的使用者](users.md) 。

* `Moderator`

   社群協調者 [群組的成員](users.md#publishenvironmentusersandgroups) (具 [有協調者權限](in-context.md#moderatorpermissions))。
...
當UGC預先協調時，貼文在協調動作核准後，才會顯示在發佈的網站上。 在建立社群網 [站時](/help/communities/sites-console.md)，勾選「內容已預 [先協調](sites-console.md#moderation) 」方塊將啟用整個網站的預先協調。 將元件放在頁面上後，就可以設定支援協調的元件，以使用其編輯對話方塊中的設定進行預先協調：

* [在「使用](comments.md) 者 [協](reviews.md)調 **[!UICONTROL >預]** 先協調 **[!UICONTROL 」中]**&#x200B;加上意見和審核。

* [論壇](/help/communities/forum.md)[理念](/help/communities/ideation-feature.md)、QnA、 [Settings](/help/communities/working-with-qna.md)、Settings [>Chreaded](/help/communities/calendar.md)********（設定）> Chreaded（協調）。
...
1. 開 [啟Web Console](https://localhost:4502/system/console/configMgr)，請前往 `/system/console/configMgr`。

1. 找到 **AEM Communities Auto Moderation** （AEM社群自動協調）設定並加以編輯。
1. 添加 **[!UICONTROL SpamProcess項]** 。
...
AEM社群提供 *Watchword分析器* ，做為評估情緒的程式 [步驟](#sentiment)。 關注字詞對情緒值的貢獻是因為比較已張貼內容中使用的負面和正面關注字詞，以及禁止字詞。

#### 設定情緒和關注字詞 {#configure-sentiment-and-watchwords}

可自訂正面和負面監看字詞清單，因為可以是情緒規則。

預設的監視字詞清單可以作為節點的屬性在儲存庫中輸入，類似於預設值，或者通過使用字詞清單配置OSGi服務來覆蓋 `sentimentprocess.name` 預設值。
...
* **監視字詞節點的顯式路徑**
   ...
   包含指定預設監看字的預設和屬 `positive` 性的 `negative` 節點的儲存庫位置。 預設為 `/libs/settings/community/watchwords/default`。

* **情緒規則**

   根據正面和負面關注字詞計算情緒的規則存放庫位置。 預設值 `/libs/cq/workflow/components/workflow/social/sentiments/rules` 為（不過，不再涉及任何工作流程）。
