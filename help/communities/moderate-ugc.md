...
社群內容(也稱為使用者產生的內容(UGC))是在成員（登入網站訪客）透過與以下社群元件之一的互動從已發佈的社群網站張貼內容時建立的：
...
版主可以看到這類貼文，並將其標籤為未出現在網站上。 這些貼文上的稽核動作可在內容中或透過大量稽核UI執行。
...
情緒是根據貼文(UGC)中存在的正負關鍵字（[關注字詞](#configuringwatchwords)）數目計算的。
...
從[大量仲裁主控台](/help/communities/moderation.md)，可以根據情緒是負值、中性或正值來篩選和檢視UGC。

#### 關注字詞 {#watchwords}

AEM Communities提供&#x200B;*關注字分析器*&#x200B;作為評估[情緒](#sentiment)的程式中的步驟。 關注字詞對情緒值的貢獻來自於對張貼內容中使用的負面和正面關注字詞與禁止字詞的比較。

#### 設定情緒和標語 {#configure-sentiment-and-watchwords}

正面和負面標語清單可以自訂，就像情緒規則一樣。
...
* **正面標語**
  ...
  以逗號分隔的單字清單，構成可覆寫預設值的正情緒。 預設為空白清單。

* **負面關注字詞**

  以逗號分隔的單字清單，構成覆寫預設值的負情緒。 預設為空白清單。
...
* **情緒規則**
  ...
  根據正面和負面標語計算情緒的規則存放庫位置。 預設值為`/libs/cq/workflow/components/workflow/social/sentiments/rules` （不過，不再涉及任何工作流程）。
