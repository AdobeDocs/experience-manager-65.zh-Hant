...
如需進一步資訊，請參閱Adobe隱私權中心的[GDPR頁面。](https://www.adobe.com/privacy/general-data-protection-regulation.html)

>[!NOTE]
>
>若您已在AEM Communities[&#128279;](/help/communities/analytics.md)網站中設定Adobe Analytics，則擷取的使用者資料會傳送至Adobe Analytics伺服器。 Adobe Analytics提供的API可讓您存取、匯出和刪除使用者資料，以及遵守GDPR。 如需詳細資訊，請參閱[提交存取及刪除要求](https://experienceleague.adobe.com/docs/analytics/admin/data-governance/gdpr-submit-access-delete.html?lang=zh-Hant)。
...
* **使用者**：使用者的可授權識別碼。
* **outputStream**：結果會以輸出資料流傳回，此資料流為zip檔案，包含使用者產生的內容（json檔案）和附件（包含使用者上傳的影像或視訊）。

例如，若要匯出名為Weston McCall且使用weston.mccall@dodgit.com作為可授權ID登入Communities網站之使用者的UGC，您可以傳送類似下列的http GET要求：
...
例如，若要透過http-POST請求刪除其可授權識別碼為`weston.mccall@dodgit.com`之使用者的UGC，請使用下列引數：
...
適用於Adobe Analytics的![AEM communities變數對應](assets/analytics-communities-mapping.png)
...
例如，若要透過http-POST要求刪除其可授權識別碼為`weston.mccall@dodgit.com`的使用者設定檔，請使用下列引數：
...
>[!NOTE]
>
>`deleteUserAccount()` API只會停用系統中的使用者設定檔，並移除UGC。 但是，若要從系統刪除使用者設定檔，請瀏覽至&#x200B;**位於`https://<server>:<port>/crx/de`的CRXDE Lite**，找出該使用者節點，然後將其刪除。
