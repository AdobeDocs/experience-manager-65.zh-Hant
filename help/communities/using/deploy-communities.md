...
1. 如果在設定JDBC之後於現有部署上進行安裝，則從Web主控台重新儲存JDBC設定，以將JDBC重新繫結至新聯結器：
   ...
   * 尋找 `Day Commons JDBC Connections Pool` 設定，並選取「 」以開啟設定。
   * 選取 `Save`.

1. 在所有製作和發佈執行個體上重複步驟3和4。

如需有關安裝套裝的詳細資訊，請參閱 [網頁主控台](/help/sites-deploying/web-console.md#bundles) 頁面。

#### 範例：已安裝MySQL聯結器套件組合 {#example-installed-mysql-connector-bundle}

![Adobe Experience Manager Web主控台MySQL聯結器套件組合](../assets/mysql-connector.png)
...
* 請對這兩個代理程式遵循相同的程式：
   ...
   * **反向復寫代理程式（反向發佈）**
      ...
      1. 選取 **編輯**.
      1. 選取 **傳輸** 標籤
      1. 如果不是連線埠 `4503`，編輯 **URI** 以指定正確的連線埠。
...
![AEM Communities發佈通道服務會顯示「啟用」核取方塊（已選取或已勾選）。](../assets/tunnel-service.png)
...
* 針對每個目標AEM例項
   ...
   * 貼上先前複製的兩個檔案
   * 需要 [重新整理Granite加密套件組合](#refresh-the-granite-crypto-bundle) 目標AEM執行個體是否正在執行。
...
![CRXDE Lite在左面板上顯示/etc/key路徑，並在右下面板中選取Replication標籤。](../assets/replicare-repository.png)
...
![正在重新整理AdobeGranite Crypto支援套件。](../assets/refresh-granite-bundle.png)
