...
若要新增 `QnA` 元件至作者模式下的頁面，請使用元件瀏覽器來尋找 `Communities / QnA` 並將其拖曳至QnA論壇應出現的頁面上。
...
選取已放置的 `QnA` 元件供您存取及選取 `Configure` 圖示可開啟編輯對話方塊。
...
* **允許關注**
  ...
  如果勾選，則為論壇貼文納入以下功能，可讓成員成為 [已通知](/help/communities/notifications.md) 新貼文的。 預設為取消選取。
...
* **允許電子郵件訂閱**
  ...
  如果勾選，允許成員透過電子郵件接收新貼文的通知([訂閱](/help/communities/subscriptions.md))。 需要勾選「允許關注」和 [電子郵件已設定](/help/communities/email.md). 預設為取消選取。
...
* **允許的檔案類型**
  ...
  只有在 `Allow File Uploads` 已勾選。 以「點」分隔符號的副檔名清單（以逗號分隔）。 例如： .jpg、.jpeg、.png、.doc、.docx、.pdf。 如果指定了任何檔案型別，則無法上傳未指定的檔案型別。 預設為未指定，使得 **全部** 允許檔案型別。
...
* **顯示徽章**
  ...
  如果勾選，則顯示已習得和已指派 [徽章](/help/communities/implementing-scoring.md) 使用成員的部落格專案。 預設為取消選取。

* **允許主要內容**

  如果勾選，該創意可識別為 [主要內容](/help/communities/featured.md). 預設為取消選取。
...
在 **使用者稽核** 標籤，指定如何管理張貼的主題（問題）和答案（使用者產生的內容）。 如需詳細資訊，請參閱 [稽核使用者產生的內容](/help/communities/moderate-ugc.md).
...
在 **標籤欄位** 標籤，則標籤會套用（如果底下允許） **設定** 索引標籤中，會根據選取的名稱空間而有所限制。
...
一個答案可以使用以下標籤為正確或有用的答案 `Select Answer` 按鈕。 一旦問題被標示為「已回答」，除非使用 `Unmark Chosen Answer` 按鈕。

選取為可行答案後，可使用 `Unmark Chosen Answer` 按鈕。

一旦選取一個答案作為可行答案，表示問題已經 `Answered` 會顯示在主要QnA頁面上的問題主題旁。
...
如需詳細資訊，請參閱 [QnA要點](/help/communities/qna-essentials.md) 開發人員頁面。
