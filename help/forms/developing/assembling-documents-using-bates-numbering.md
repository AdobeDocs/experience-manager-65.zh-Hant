...
建立Assembler服務使用者端後，請參考包含的DDX檔案 `BatesNumber` 元素資訊、參考輸入PDF檔案以及設定執行階段選項，您可以叫用 `invokeDDX` 作業會導致Assembler服務組裝包含唯一頁面識別碼的PDF檔案。
...
>[!NOTE]
>
>如果您叫用 `invokeDDX` 作業。 將兩個或多個輸入PDF檔案傳遞至Assembler服務時，會使用此作業。 不過，如果您只將一個輸入PDF檔案傳遞至組合器服務，則應叫用 `invokeOneDocument` 作業。 如需有關使用此作業的資訊，請參閱 [組合加密的PDF檔案](/help/forms/developing/assembling-encrypted-pdf-documents.md).
...
1. 參考輸入PDF檔案。
   ...
   * 針對每個輸入PDF檔案，建立 `java.io.FileInputStream` 物件，使用它的建構函式並傳遞輸入PDF檔案的位置。 在此情況下，請傳送不安全PDF檔案的位置。
...
1. 擷取結果。
   ...
   * 逐一檢視 `java.util.Map` 物件，直到您找到 `com.adobe.idp.Document` 物件。
...
1. 設定初始Bates數字值。
   ...
   * 建立 `AssemblerOptionSpec` 使用建構函式來儲存執行階段選項的物件。
   * 將數值指定給，以設定初始Bates編號 `firstBatesNumber` 屬於的資料成員 `AssemblerOptionSpec` 物件。
