...
您可以使用組合器Java和網頁服務API組合PDFPortfolio。 產品組合可以組合多種型別的檔案，包括Word檔案、影像檔案（例如jpeg檔案）和PDF檔案。 產品組合的版面配置可以設定為不同的樣式，例如 *有預覽的格線*，則 *在影像上* 版面配置或平均 *旋轉*.

下圖是產品組合的熒幕擷圖 *在影像上* 樣式配置。
...
DXX檔案必須包含 `Portfolio` 具有巢狀結構的標籤 `Navigator` 標籤之間。 記下標籤 `<Resource name="navigator/image.xxx" source="myImage.png"/>` 只有在 `myNavigator` 會指派為onImage配置導覽器： `AdobeOnImage.nav`. 此標籤可讓組合器服務選取要做為產品組合背景的影像。 包含 `PackageFiles` 和 `File` 標籤定義封裝檔案的檔案名稱和MIME型別。
...
要組裝PDFPortfolio，參照代表要組裝檔案的所有檔案。 例如，將DDX檔案中指定的所有影像檔案傳遞至組合器服務。 請注意，這些檔案參照於本節所指定的DDX檔案： *myImage.png* 和 *saint_bernard.jpg*.
...
若要組裝PDFPortfolio，請呼叫 `invokeDDX` 作業。 組合器服務會傳回集合物件中的PDFPortfolio。
...
1. 參考必要的檔案。
   ...
   * 建立 `java.io.FileInputStream` 物件（使用其建構函式）。 傳遞所需NAV檔案的位置（針對建立投資組合所需的每個檔案重複此工作）。
...
1. 儲存組合的產品組合。
   ...
   * 叫用 `com.adobe.idp.Document` 物件的 `copyToFile` 用於擷取PDFPortfolio的方法。
