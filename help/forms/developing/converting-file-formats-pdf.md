...
此指令碼包含導覽原生應用程式視窗元素的指示，並提供這些元素的特定回應。 包含此資訊的檔案是 `appmon.`[appname]&quot; `.script.`[地區]`.xml`。 例如appmon.notepad.script.en_US.xml。
...
>[!NOTE]
>
>在此內容中，「附加」一詞系指 `appmon.[applicationname].addition.[locale]`.xml檔案的內容。 此類檔案指定對話XML檔案的覆蓋和添加。
