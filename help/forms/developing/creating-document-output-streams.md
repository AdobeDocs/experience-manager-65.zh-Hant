...
您可以使用Output服務來建立PDF/A檔案。 由於PDF/A是用於長期儲存檔案內容的封存格式，因此所有字型都會嵌入，且檔案會解壓縮。 因此，PDF/A 文件通常比標準 PDF 文件大。此外，PDF/檔案不包含音訊和視訊內容。 如同其他Output服務工作，您提供表單設計和資料以與表單設計合併，以建立PDF/A檔案。
