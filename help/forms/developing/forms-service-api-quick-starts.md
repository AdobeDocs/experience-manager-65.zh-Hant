...
description: 瞭解如何使用Java&trade； API呈現互動式PDF、HTML Forms和片段。
contentOwner: admin
...
[快速入門（SOAP模式）：使用Java™ API處理以XML格式提交的PDF forms](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api)
...
[快速入門（SOAP模式）：使用Java™ API以可流動的配置預先填入Forms](forms-service-api-quick-starts.md#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api)
...
使用Forms服務API的應用程式邏輯已實作為Java™ Servlet。 AEM Forms作業可使用AEM Forms強型別API執行，且連線模式應設為SOAP。
...
下列程式碼範例會將名為&#x200B;*Loan.xdp*&#x200B;的互動式PDF表單轉譯給使用者端網頁瀏覽器。 檔案會附加至表單。 請注意，表單設計是應用程式的一部分，且使用內容根URI值`repository:///`參考。 （請參閱[呈現互動式PDF forms](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms)。）
...
下列程式碼範例使用HTML服務Java™ API轉譯Forms表單。 工具列會新增至HTML表單和兩個檔案附件。 此外，從`HttpServletRequest`物件取得使用者代理程式值。 （請參閱[將Forms轉譯為HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)。）
...
## 快速入門（SOAP模式）：透過Java™ API呈現使用CSS檔案的HTML表單 {#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api}

下列程式碼範例使用HTML服務使用者端API轉譯Forms表單。 參考的自訂CSS檔案名稱為&#x200B;*custom.css*。 （請參閱[使用自訂CSS檔案呈現HTML Forms](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files)。）
...
下列程式碼範例會轉譯HTML表單，其工具列顯示為法文。 fscmenu.xml的位置為C:\Adobe （此資料夾必須位於託管AEM Forms的伺服器上）。 請注意，地區設定值為`fr_FR`。 討論如何使用自訂工具列轉譯HTML表單的區段會顯示此快速入門中所使用的fscmenu.xml檔案語法。 （請參閱[使用自訂工具列呈現HTML Forms](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars)。）
...
以下程式碼範例會處理以PDF資料提交的表單。 傳遞至`processFormSubmission`方法的內容型別值為`CONTENT_TYPE=application/pdf`。 提交的表單會儲存為名為&#x200B;*tempPDF.pdf*&#x200B;的PDF檔案。 此外，由於表單是以PDF提交，因此可擷取檔案附件。 任何檔案附件都會儲存為JPEG檔案。 （請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。）
...
以下Java™程式碼範例會處理以XML格式提交的表單資料。 表單資料是使用Forms API從表單提交中擷取的，並傳送至輸出服務。 表單資料和表單設計可用來建立非互動式PDF檔案。 非互動式PDF檔案儲存在名為`/Company Home/Test Directory`的內容服務（已棄用）節點中。 表單名稱會以動態方式建立。 也就是說，系統會使用使用者的名字和姓氏來命名PDF檔案。 新內容的資源識別碼會寫入使用者端Web瀏覽器。 （請參閱[使用已提交的XML資料建立PDF檔案](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data)。）
...
下列Java™快速入門會根據名為&#x200B;*Loan.xdp*&#x200B;的表單設計值，呈現互動式PDF表單。 請注意，表單設計是用來填入名為&#x200B;*inputXDP*&#x200B;的`com.adobe.idp.Document`物件。 （請參閱[依據值](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)呈現Forms。）
