...
description: 瞭解如何使用Java&amp；trade； API來呈現互動式PDF、HTMLForms、片段。
contentOwner: admin
...
[快速入門(SOAP模式)：使用Java呈現互動式PDF表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api)

[快速入門(SOAP模式)：使用Java在使用者端轉譯表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-a-form-at-the-client-using-the-java-api)

[快速入門(SOAP模式)：使用Java根據片段轉譯表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-a-form-based-on-fragments-using-the-java-api)

[快速入門(SOAP模式)：使用Java轉譯已啟用許可權的表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-a-rights-enabled-form-using-the-java-api)

[快速入門(SOAP模式)：使用Java轉譯HTML表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-an-html-form-using-the-java-api)

[快速入門(SOAP模式)：使用Java使用自訂工具列轉譯HTML表單](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-an-html-form-with-a-custom-toolbar-using-the-java-api)

[快速入門(SOAP模式)：使用Java處理以XML格式提交的PDF forms](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api)

[快速入門(SOAP模式)：使用Java處理以PDF形式提交的PDF forms](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api)

[快速入門(SOAP模式)：使用Java處理以XML格式提交的HTML表單](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-html-forms-submitted-as-xml-using-the-java-api)

[快速入門(SOAP模式)：使用Java以提交的XML資料建立PDF檔案](forms-service-api-quick-starts.md#quick-start-soap-mode-creating-pdf-documents-with-submitted-xml-data-using-the-java-api)

[快速入門(SOAP模式)：使用Java以可流動的版面配置預先填入Forms](forms-service-api-quick-starts.md#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api)

[快速入門(SOAP模式)：使用Java處理包含計算指令碼的表單](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-a-form-containing-a-calculation-script-using-the-java-api)

[快速入門(SOAP模式)：使用Java最佳化效能](forms-service-api-quick-starts.md#quick-start-soap-mode-optimizing-performance-using-the-java-api)

[快速入門(SOAP模式)：使用Java依值演算](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-by-value-using-the-java-api)

[快速入門(SOAP模式)：使用Java將檔案傳遞至Forms服務](forms-service-api-quick-starts.md#quick-start-soap-mode-passing-documents-to-the-forms-service-using-the-java-api)

使用Forms服務API的應用程式邏輯已實作為Java™ Servlet。 AEM Forms作業可使用AEM Forms強型別API執行，且連線模式應設定為SOAP。

>[!NOTE]
>
>使用v進行程式設計的快速入門是根據您使用其他作業系統(例如UNIX®)的Forms伺服器，以適用作業系統支援的路徑取代Windows特定路徑。 同樣地，如果您使用其他J2EE應用程式伺服器，請確定您指定有效的連線屬性。 請參閱[設定連線內容](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)。

**秘訣**： Adobe Developer網站包含下列文章，討論如何建立叫用Forms服務及轉譯表單的ASP.NET應用程式。

## 快速入門(SOAP模式)：使用Java™ API呈現互動式PDF表單 {#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api}

下列程式碼範例會將名為&#x200B;*Loan.xdp*&#x200B;的互動PDF表單轉譯給使用者端網頁瀏覽器。 檔案會附加至表單。 請注意，表單設計是應用程式的一部分，且使用內容根URI值`repository:///`參考。 (請參閱[呈現互動式PDF forms](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms)。)
...
下列程式碼範例使用Forms服務Java™ API在使用者端轉譯名為&#x200B;*Loan.xdp*&#x200B;的表單。 請注意，表單設計是應用程式的一部分，且使用內容根URI值`repository:///`參考。 (請參閱[在使用者端轉譯Forms](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)。)
...
下列程式碼範例會將名為&#x200B;*TLALifeClaim.xdp*&#x200B;的指南（已過時）轉譯給使用者端網頁瀏覽器。
...
下列程式碼範例會根據片段轉譯表單。 表單設計的名稱是&#x200B;*PurchaseOrderDynamic.xdp*，它位於AEM Forms存放庫中（XDP檔案儲存在存放庫中名為`FormsFolder`的資料夾中）。 此外，POFragment表單參考的片段也必須在存放庫中。 (請參閱[根據片段呈現Forms](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments)。)
...
下列程式碼範例使用Forms服務Java™ API轉譯HTML表單。 工具列會新增至HTML表單和兩個檔案附件。 此外，從`HttpServletRequest`物件取得使用者代理程式值。 (請參閱[將Forms轉譯為HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)。)
...
## 快速入門(SOAP模式)：呈現使用CSS檔案的HTML表單(使用Java™ API) {#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api}

下列程式碼範例使用Forms服務使用者端API轉譯HTML表單。 參考的自訂CSS檔案名稱為&#x200B;*custom.css*。 (請參閱[使用自訂CSS檔案呈現HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files)。)
...
下列程式碼範例會轉譯HTML表單，其工具列以法文顯示。 fscmenu.xml的位置為C:\Adobe (此資料夾必須位於託管AEM Forms的伺服器上)。 請注意，地區設定值為`fr_FR`。 討論如何使用自訂工具列轉譯HTML表單的區段會顯示此快速入門中使用的fscmenu.xml檔案語法。 (請參閱[使用自訂工具列呈現HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars)。)
...
## 快速入門(SOAP模式)：使用Java™ API處理以XML格式提交的PDF forms {#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api}

下列程式碼範例會處理以XML格式提交的表單。 傳遞至`processFormSubmission`方法的內容型別值為`CONTENT_TYPE=text/xml`。 顯示對應至名為`mortgageAmount`、`lastName`和`firstName`之欄位的值。 此快速入門中使用名為`getNodeText`的使用者定義方法。 它接受`org.w3c.dom.Document`執行個體和指定節點名稱的字串值。 此方法會傳回代表節點值的字串值。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
>[!NOTE]
>
>在同一應用程式中使用`com.adobe.idp.Document`物件和`org.w3c.dom.Document`時，完全符合`org.w3c.dom.Document`的資格。

## 快速入門(SOAP模式)：使用Java™ API處理以PDF形式提交的PDF forms {#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api}

以下程式碼範例會處理以PDF資料提交的表單。 傳遞至`processFormSubmission`方法的內容型別值為`CONTENT_TYPE=application/pdf`。 提交的表單會儲存為名為&#x200B;*tempPDF.pdf*&#x200B;的PDF檔案。 此外，由於表單是以PDF方式提交，因此可擷取檔案附件。 任何檔案附件都會儲存為JPEG檔案。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
下列程式碼範例會處理以XML資料提交的HTML表單。 傳遞至`processFormSubmission`方法的內容型別值為`CONTENT_TYPE=application/x-www-form-urlencoded`。 顯示對應至名為`mortgageAmount`、`lastName`和`firstName`之欄位的值。 此快速入門中使用名為`getNodeText`的使用者定義方法。 它接受`org.w3c.dom.Document`執行個體和指定節點名稱的字串值。 此方法會傳回代表節點值的字串值。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms)。)
...
以下Java™程式碼範例會處理以XML格式提交的表單資料。 表單資料是使用Forms API從表單提交中擷取的，並傳送至輸出服務。 表單資料和表單設計可用來建立非互動式PDF檔案。 非互動式PDF檔案儲存在名為`/Company Home/Test Directory`的Content Services （已過時）節點中。 表單名稱會以動態方式建立。 也就是說，系統會使用使用者的名字和姓氏來命名PDF檔案。 新內容的資源識別碼會寫入使用者端Web瀏覽器。 (請參閱[使用已提交的XML資料建立PDF檔案](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data)。)
...
## 快速入門(SOAP模式)：使用Java™ API以可流動的配置預先填入Forms {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}

下列程式碼範例會將動態資料來源預先填入表單中。 也就是說，資料來源是在執行階段建立的，不會包含在XML檔案中，或是在設計階段建立的。 此程式碼範例包含三種使用者定義方法：

* `createDataSource`：建立代表用來預先填入表單之資料來源的`org.w3c.dom.Document`物件。 這個使用者定義的方法傳回`org.w3c.dom.Document`物件。
* `convertDataSource`：將`org.w3c.dom.Document`物件轉換為`com.adobe.idp.Document`物件。 此方法接受`org.w3c.dom.Document`物件作為輸入引數，並傳回`com.adobe.idp.Document`物件。
* `renderPOForm`：使用Forms服務Java™ API來轉譯動態採購單表單。 `convertDataSource`方法傳回的`com.adobe.idp.Document`物件是用來預先填入表單。

  所有這些方法都是從Java™ servlet的`doPost`方法內叫用的。 (請參閱[使用可流動配置預先填入Forms](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts)。)
...
下列程式碼範例會藉由設定快取、獨立和線性化選項來最佳化效能。 線性化的檔案已最佳化，以利在網路上傳送。 (請參閱[最佳化Forms服務的效能](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service)。)
...
下列Java™快速入門會根據名為&#x200B;*Loan.xdp*&#x200B;的表單設計，以值呈現互動式PDF表單。 請注意，表單設計是用來填入名為&#x200B;*inputXDP*&#x200B;的`com.adobe.idp.Document`物件。 (請參閱[依據值](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)呈現Forms。)
...
以下Java™快速入門會從內容服務（已棄用）擷取檔案Loan.xdp 。 此XDP檔案位於空間`/Company Home/Form Designs`。 已在`com.adobe.idp.Document`執行個體中傳回XDP檔案。 `com.adobe.idp.Document`執行個體傳遞至Forms服務。 互動式表單會寫入使用者端Web瀏覽器。 (請參閱[將檔案傳遞至Forms服務](/help/forms/developing/passing-documents-forms-service.md)。)
