...
seo-description: 使用Forms服務API的快速入門。
...
[快速入門（SOAP模式）:使用Java API處理以XML形式提交的PDF forms](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api)

[快速入門（SOAP模式）:使用Java API處理以PDF格式提交的PDF forms](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api)
...
[快速入門（SOAP模式）:使用Java API將可排程的版面預填入Forms](forms-service-api-quick-starts.md#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api)
...
[快速入門（SOAP模式）:使用Java API將檔案傳送至Forms服務](forms-service-api-quick-starts.md#quick-start-soap-mode-passing-documents-to-the-forms-service-using-the-java-api)

使用Forms服務API的應用程式邏輯會建置為Java servlet。 AEM Forms操作可以使用AEM Forms強類型API執行，連接模式應設定為SOAP。

>[!NOTE]
>
>「使用v進行寫程式」中的快速啟動基於您正在使用其他作業系統（如Unix）的表單伺服器，使用適用作業系統支援的路徑替換特定於Windows的路徑。 同樣地，如果您使用其他J2EE應用程式伺服器，請確定您指定有效的連線屬性。 請參閱[設定連接屬性](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api}轉換互動式PDF表單

下列程式碼範例會將名為&#x200B;*Loan.xdp*&#x200B;的互動式PDF表單轉譯至用戶端網頁瀏覽器。 一個檔案附加到表單。 請注意，表單設計是應用程式的一部分，使用內容根URI值`repository:///`來參考。 (請參閱[演算互動式PDF forms](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms)。)
...
以下代碼示例在使用Forms服務Java API的客戶端上呈現名為&#x200B;*Loan.xdp*&#x200B;的表單。 請注意，表單設計是應用程式的一部分，使用內容根URI值`repository:///`來參考。 (請參閱[在客戶端上呈現Forms。)](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-a-guide-deprecated-using-the-java-api}轉譯指南（已過時）
...
下列程式碼範例會轉譯以片段為基礎的表格。 表單設計的名稱為&#x200B;*PurchaseOrderDynamic.xdp*，它位於AEM Forms儲存庫中（XDP檔案儲存在儲存庫中名為FormsFolder的資料夾中）。 此外，POFragment表單引用的片段也必須位於儲存庫中。 (請參閱[根據片段呈現Forms。)](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments)
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-a-rights-enabled-form-using-the-java-api}轉譯啟用權限的表單

下列程式碼範例會將啟用權限的表單轉譯至用戶端網頁瀏覽器。 此程式碼範例中設定的使用權限，可讓使用者在表單中新增註解並儲存表單資料。 (請參閱[啟用版權的Forms](/help/forms/developing/rendering-forms.md#rendering-rights-enabled-forms)。)
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-an-html-form-using-the-java-api}轉換HTML表格

下列程式碼範例會使用Forms服務Java API轉譯HTML表格。 HTML表格和兩個檔案附件會新增一個工具列。 此外，用戶代理值是從`HttpServletRequest`對象中獲得的。 (請參閱[將Forms轉換為HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)。)
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api}轉換使用CSS檔案的HTML表單

下列程式碼範例使用Forms服務用戶端API轉譯HTML表格。 參考的自訂CSS檔案名稱為&#x200B;*custom.css*。 (請參閱[使用自訂CSS檔案轉換HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-an-html-form-with-a-custom-toolbar-using-the-java-api}轉換具有自訂工具列的HTML表單

下列程式碼範例會轉譯HTML表格，其工具列以法文顯示。 fscmenu.xml的位置為C:\Adobe (此資料夾必須位於代管AEM Forms的伺服器上)。 請注意，地區設定值為`fr_FR`。 討論如何使用自訂工具列來轉換HTML表格的章節，顯示此快速入門中使用的fscmenu.xml檔案語法。 (請參閱[使用自訂工具列轉換HTMLForms](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api}處理以XML形式提交的PDF forms

下列程式碼範例會處理以XML形式提交的表單。 傳遞給`processFormSubmission`方法的內容類型值為`CONTENT_TYPE=text/xml`。 將顯示與名為`mortgageAmount`、`lastName`和`firstName`的欄位相對應的值。 此快速啟動中使用了名為`getNodeText`的用戶定義方法。 它接受一個`org.w3c.dom.Document`實例和一個指定節點名稱的字串值。 此方法返回表示節點值的字串值。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms))。
...
>[!NOTE]
>
>在相同應用程式中使用`com.adobe.idp.Document`物件和`org.w3c.dom.Document`時，請完全符合`org.w3c.dom.Document`的資格。

## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api}處理以PDF格式提交的PDF forms

下列程式碼範例會處理以PDF資料提交的表單。 傳遞給`processFormSubmission`方法的內容類型值為`CONTENT_TYPE=application/pdf`。 提交的表單會儲存為名為&#x200B;*tempPDF.pdf*&#x200B;的PDF檔案。 此外，由於表單是以PDF格式提交，因此可以擷取檔案附件。 任何檔案附件都會儲存為JPEG檔案。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-handling-html-forms-submitted-as-xml-using-the-java-api}處理以XML形式提交的HTML表單

下列程式碼範例會處理以XML資料提交的HTML表單。 傳遞給`processFormSubmission`方法的內容類型值為`CONTENT_TYPE=application/x-www-form-urlencoded`。將顯示與名為`mortgageAmount`、`lastName`和`firstName`的欄位相對應的值。 此快速啟動中使用了名為`getNodeText`的用戶定義方法。 它接受一個`org.w3c.dom.Document`實例和一個指定節點名稱的字串值。 此方法返回表示節點值的字串值。 (請參閱[處理已提交的Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-creating-pdf-documents-with-submitted-xml-data-using-the-java-api}建立包含已提交XML資料的PDF檔案

下列Java程式碼範例會處理以XML形式提交的表單資料。 表單資料是使用FormsAPI從表單提交中擷取，並傳送至輸出服務。 表單資料和表單設計可用來建立非互動式PDF檔案。 非互動式PDF檔案會儲存在名為`/Company Home/Test Directory`的「內容服務（已過時）」節點中。 表格名稱會動態建立。 亦即，使用者的名字和姓氏會用來命名PDF檔案。 將新內容的資源標識符寫出到客戶端Web瀏覽器。 （請參閱[使用已提交的XML資料建立PDF檔案](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data)）。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}預先填入可排程的版面配置Forms

下列程式碼範例會預先填入動態資料來源的表單。 也就是說，資料來源是在執行時期建立，不包含在XML檔案中，或是在設計時期建立。 此程式碼範例包含三種使用者定義的方法：

* `createDataSource`:建立 `org.w3c.dom.Document` 一個對象，該對象表示用於預填充表單的資料源。此用戶定義的方法返回`org.w3c.dom.Document`對象。
* `convertDataSource`:將對象 `org.w3c.dom.Document` 轉換為對 `com.adobe.idp.Document` 像。此方法接受`org.w3c.dom.Document`物件作為輸入參數，並傳回`com.adobe.idp.Document`物件。
* `renderPOForm`:使用Forms服務Java API來轉換動態採購單表單。`convertDataSource`方法傳回的`com.adobe.idp.Document`物件會用來預先填入表單。

   所有這些方法都是從Java servlet的`doPost`方法中呼叫。 (請參閱[使用可流式版面預填Forms](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-optimizing-performance-using-the-java-api}最佳化效能

下列程式碼範例會設定快取、獨立和線性化選項，以最佳化效能。 線性化檔案被優化以在Web上發送。 (請參閱[優化Forms服務的效能](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service)。)
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-rendering-by-value-using-the-java-api}依值呈現

以下Java快速入門會轉譯互動式PDF表單，其基於名為&#x200B;*Loan.xdp*&#x200B;的表單設計，依值區分。 請注意，表單設計是用來填入名為&#x200B;*inputXDP*&#x200B;的`com.adobe.idp.Document`物件。 (請參閱[RenderingFormsBy Value](/help/forms/developing/rendering-forms.md#rendering-forms-by-value))。
...
## 快速入門（SOAP模式）:使用Java API {#quick-start-soap-mode-passing-documents-to-the-forms-service-using-the-java-api}將檔案傳送至Forms服務

以下Java快速入門會從Content Services擷取檔案Loan.xdp（已過時）。 此XDP檔案位於空間`/Company Home/Form Designs`中。 在`com.adobe.idp.Document`實例中返回XDP檔案。 `com.adobe.idp.Document`實例將傳遞給Forms服務。 互動式表單會寫入用戶端網頁瀏覽器。 (請參閱[將檔案傳遞至Forms服務](/help/forms/developing/passing-documents-forms-service.md)。)
