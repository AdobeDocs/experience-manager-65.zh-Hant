...
參考PDF表單和有效的XML資料來源後，即可將資料匯入PDF表單。

**將PDF表單儲存為PDF檔案**

將資料匯入表單後，可將表單儲存為PDF檔案。 儲存為PDF檔案後，使用者就可以在Adobe Reader或Acrobat中開啟表單，並使用匯入的資料檢視表單。
...
參照包含表單資料的PDF表單後，即可從表單匯出資料。 資料會匯出至以表單為基礎的XML結構描述中。
