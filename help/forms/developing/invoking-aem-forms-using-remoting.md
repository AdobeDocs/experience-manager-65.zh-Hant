...
1. 取得不安全的PDF檔案，該檔案會作為輸入值傳遞。 此動作是根據 `SetValue` 作業。 輸入引數的名稱為 `inDoc` 且其資料型別為 `document`. (此 `document` data type是Workbench中的可用資料型別。)
1. 使用密碼加密PDF檔案。 此動作是根據 `PasswordEncryptPDF` 作業。 此處理序的輸出值名稱為 `outDoc` 和代表以密碼加密的PDF檔案。 outDoc的資料型別為 `document`.
1. 將密碼加密的PDF檔案儲存為PDF檔案至本機檔案系統。 此動作是根據 `WriteDocument` 作業。

>[!NOTE]
>
>此 `MyApplication/EncryptDocument` 程式並非以現有AEM Forms程式為基礎。 若要與程式碼範例一起追蹤，請建立名為的程式 `MyApplication/EncryptDocument` 使用Workbench。

>[!NOTE]
>
>如需有關使用遠端功能來叫用長期處理程式的資訊，請參閱 [叫用以人為中心的長期流程](/help/forms/developing/invoking-human-centric-long-lived.md#invoking-human-centric-long-lived-processes).
...
>[!NOTE]
>
>*請務必將crossdomain.xml檔案放在適當的位置。 例如，假設您在JBoss上部署了AEM Forms，請將此檔案放在以下位置： &lt;install_directory>\Adobe_Experience_Manager_forms\jboss\server\lc_turnkey\deploy\jboss-web.deployer\ROOT.war.*
...
以Flex建置的使用者端應用程式無法直接將檔案傳遞至AEM Forms程式。 相反地，使用者端應用程式使用 `mx.rpc.livecycle.DocumentReference` 將輸入引數傳遞給預期作業的ActionScript類別 `com.adobe.idp.Document` 執行個體。 Flex使用者端應用程式有數個選項可設定 `DocumentReference` 物件：
...
* 當檔案不在伺服器上時，請使用遠端上傳servlet將檔案上傳至AEM Forms。 AEM Forms的新功能是上傳安全檔案。 上傳安全檔案時，您必須使用擁有 *檔案上傳應用程式使用者* 角色。 若沒有此角色，使用者無法上傳安全檔案。 建議您使用單一登入上傳安全檔案。 (請參閱 [使用遠端功能傳遞安全檔案以叫用程式](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)

>[!NOTE]
>
如果將AEM Forms設定為允許上傳不安全的檔案，您可以使用沒有檔案上傳應用程式使用者角色的使用者來上傳檔案。 使用者也可以擁有檔案上傳許可權。 但是，如果AEM Forms設定為僅允許安全檔案，則請確保使用者具有檔案上傳應用程式使用者角色或檔案上傳許可權。 (請參閱 [將AEM Forms設定為接受安全及不安全的檔案](invoking-aem-forms-using-remoting.md#configuring-aem-forms-to-accept-secure-and-unsecure-documents).

您可對指定的上傳URL使用標準Flash上傳功能： `https://SERVER:PORT/remoting/lcfileupload`. 然後，您可以使用 `DocumentReference` 物件，只要輸入引數為型別 `Document` 應為
` private function startUpload():void  {  fileRef.addEventListener(Event.SELECT, selectHandler);  fileRef.addEventListener("uploadCompleteData", completeHandler);  try  {   var success:Boolean = fileRef.browse();  }    catch (error:Error)  {   trace("Unable to browse for files.");  }  }      private function selectHandler(event:Event):void {  var request:URLRequest = new  URLRequest("https://SERVER:PORT/remoting/lcfileupload")  try   {   fileRef.upload(request);   }    catch (error:Error)   {   trace("Unable to upload file.");   }  }    private function completeHandler(event:DataEvent):void  {   var params:Object = new Object();   var docRef:DocumentReference = new DocumentReference();   docRef.url = event.data as String;   docRef.referenceType = DocumentReference.REF_TYPE_URL;  }`遠端快速入門使用遠端上傳servlet將PDF檔案傳遞至 `MyApplication/EncryptDocument`程式。 (請參閱 [使用(不建議用於AEM表單) AEM Forms Remoting傳遞不安全的檔案，以叫用短暫的程式](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting).)
...
遠端快速入門使用遠端上傳servlet將PDF檔案傳遞至 `MyApplication/EncryptDocument`程式。 (請參閱 [使用(不建議用於AEM表單) AEM Forms Remoting傳遞不安全的檔案，以叫用短暫的程式](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting).)

### 將檔案傳回使用者端應用程式 {#passing-a-document-back-to-a-client-application}

使用者端應用程式接收型別物件 `mx.rpc.livecycle.DocumentReference` 針對傳回 `com.adobe.idp.Document` 執行個體作為輸出引數。 由於使用者端應用程式處理ActionScript物件而非Java，因此您無法將Java型Document物件傳回Flex使用者端。 相反地，伺服器會為檔案產生URL，並將URL傳回使用者端。 此 `DocumentReference` 物件的 `referenceType` 屬性會指定內容是否位於 `DocumentReference` 物件或必須從以下位置的URL擷取： `DocumentReference.url` 屬性。 此 `DocumentReference.contentType` 屬性指定檔案的型別。
...
>[!NOTE]
>
本節探討如何叫用AEM Forms程式，以及在AEM Forms設定為上傳不安全的檔案時上傳檔案。 如需如何叫用AEM Forms程式並上傳安全檔案，以及如何設定AEM Forms以接受安全和不安全檔案的詳細資訊，請參閱 [使用遠端功能傳遞安全檔案以叫用程式](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).
...
在 `mx:RemoteObject` 標籤，指定 `<mx:method>` 標籤，指定處理序的呼叫方法名稱。 通常Forms叫用方法的名稱為 `invoke`.

下列程式碼範例會建立 `mx:RemoteObject` 叫用的執行個體 `MyApplication/EncryptDocument` 程式。
...
使用者端應用程式可透過在MXML或ActionScript中指定通道來叫用AEM Forms，如以下ActionScript範例所示。 頻道必須為 `AMFChannel`， `SecureAMFChannel`， `HTTPChannel`，或 `SecureHTTPChannel`.
...
指派 `ChannelSet` 執行個體至 `mx:RemoteObject` 執行個體的 `channelSet` 欄位（如先前的程式碼範例所示）。 一般而言，您會在匯入陳述式中匯入channel類別，而不是在叫用時指定完整名稱 `ChannelSet.addChannel` 方法。

**傳遞輸入值**

在Workbench中建立的處理作業可採用零個或多個輸入引數，並傳回輸出值。 使用者端應用程式會將輸入引數傳入 `ActionScript` 物件，其欄位對應至屬於AEM Forms流程的引數。 短期程式，已命名 `MyApplication/EncryptDocument`，需要一個名為的輸入引數 `inDoc`. 處理公開的作業名稱是 `invoke` （短期流程的預設名稱）。 (請參閱 [使用(AEM表單已棄用) AEM Forms遠端功能叫用AEM Forms](invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).)
...
在此程式碼範例中， `pdfDocument` 是 `DocumentReference` 包含不安全PDF檔案的執行個體。 關於的資訊 `DocumentReference`，請參閱 [使用(不用於AEM表單) AEM Forms遠端處理檔案](invoking-aem-forms-using-remoting.md#handling-documents-with-remoting).

**叫用特定版本的服務**

您可以使用叫用特定版本的Forms服務 `_version` 引動引數對應中的引數。 例如，若要叫用 `MyApplication/EncryptDocument` 服務：
...
此 `version` 引數必須是包含單一句號的字串。 句點左側（主要版本）與右側（次要版本）的值必須是整數。 如果未指定此引數，則會叫用Head作用中版本。

**處理傳回值**

AEM Forms處理輸出引數會還原序列化為ActionScript物件，使用者端應用程式會依名稱從中擷取特定引數，如下列範例所示。 (的輸出值 `MyApplication/EncryptDocument` 處理程式已命名 `outDoc`.)
...
1. 呼叫ChannelSet `login` 方法或服務的 `setCredentials` 指定使用者識別碼值和密碼的方法。 (請參閱 [使用單一登入](invoking-aem-forms-using-remoting.md#using-single-sign-on).)
...
1. PDF透過呼叫 `mx:RemoteObject` 執行個體的 `invoke` 方法。 傳遞 `Object` 包含輸入引數(這是不安全的PDF檔案)。 請參閱傳遞輸入值。
...
您可以在管理主控台中啟用自訂驗證，方法是將遠端端點上的驗證方法從基本變更為自訂。 如果您使用自訂驗證，使用者端應用程式會呼叫 `ChannelSet.login` 登入方法和 `ChannelSet.logout` 登出的方法。

>[!NOTE]
>
在舊版AEM Forms中，您透過呼叫 `RemoteObject.setCredentials` 方法。 此 `setCredentials` 在元件第一次嘗試連線到伺服器之前，方法實際上並未將認證傳遞給伺服器。 因此，如果元件發出錯誤事件，您就無法確定錯誤是否因驗證錯誤或其他原因而發生。 此 `ChannelSet.login` 方法會在您呼叫伺服器時連線至伺服器，以便您立即處理驗證問題。 雖然您可以繼續使用 `setCredentials` 方法，建議您使用 `ChannelSet.login` 方法。

由於多個目的地可以使用相同的管道和對應的ChannelSet物件，因此登入一個目的地會將使用者登入使用相同管道或管道的任何其他目的地。 如果兩個元件將不同的認證套用至相同的ChannelSet物件，則會使用最後套用的認證。 若有多個元件使用相同的已驗證ChannelSet物件，則呼叫 `logout` 方法會將所有元件登出目的地。

以下範例使用 `ChannelSet.login` 和 `ChannelSet.logout` 方法與RemoteObject控制。 此應用程式會執行下列動作：

* 建立 `ChannelSet` 中的物件 `creationComplete` 代表所使用管道的處理常式， `RemoteObject` 元件
...
AEM Forms開發人員撰寫使用者端應用程式，以擴充表單指南（已棄用）的功能及自訂工作區。 例如，Workspace應用程式可以啟動程式。 使用者端應用程式接著會使用遠端端點，從Forms服務擷取資料。
...
由於此機制不使用AEM Forms單一登入機制，因此不會將驗證Cookie寫入使用者端。 登入憑證會儲存在 `ChannelSet` 物件。 因此，任何 `RemoteObject` 您透過相同方式進行的呼叫 `ChannelSet` 都會在這些認證的內容中進行。
...
AEM Forms使用者端應用程式透過遠端端點使用 `RemoteObject` 元件，如下列範例所示。
...
因此，呼叫 `RemoteObject.logout` 使用者端應用程式中的方法會在使用者端上產生一則錯誤訊息，指定未登出工作階段。 使用者可以使用集中式登入服務來登出並刪除驗證Cookie。
...
當叫用需要一個或多個檔案的程式時，您可以將安全檔案傳遞到AEM Forms。 藉由傳遞安全檔案，您就能保護商業資訊與機密檔案。 在這種情況下，檔案可以指PDF檔案、XML檔案、Word檔案等等。 將AEM Forms設定為允許安全檔案時，需要從在Flex中編寫的使用者端應用程式傳遞安全檔案至AEM Forms。 (請參閱 [將AEM Forms設定為接受安全及不安全的檔案](invoking-aem-forms-using-remoting.md#configuring-aem-forms-to-accept-secure-and-unsecure-documents).)

AEM傳遞安全檔案時，請使用單一登入，並指定擁有 *檔案上傳應用程式使用者* 角色。 若沒有此角色，使用者無法上傳安全檔案。 您可以程式化方式將角色指派給使用者。 (請參閱 [管理角色和許可權](/help/forms/developing/users.md#managing-roles-and-permissions).)
...
如果將AEM Forms設定為只允許上傳安全檔案，而使用者沒有 *檔案上傳應用程式使用者* 角色，則會擲回例外狀況。 如果使用者擁有此角色，則會上傳檔案並叫用程式。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>txtCustId</p></td>
   <td><p>指定新帳戶所屬的客戶識別碼值。 此文字方塊是由客戶服務的 <code>createCustomer</code> 作業。 </p></td>
  </tr>
 </tbody>
</table>
...
有些AEM Forms作業會以複雜的資料型別作為輸入值。 這些複雜資料型別會定義作業所使用的執行階段值。 例如，客戶服務的 `createCustomer` 作業需要 `Customer` 包含服務所需的執行階段值的執行個體。 若沒有複雜型別，客戶服務會擲回例外狀況，且不會執行作業。
...
下列程式碼範例會叫用Customer service並建立客戶。 執行此程式碼範例時，請確定您填寫所有文字方塊。 此外，請確定您建立的Customer.as檔案將對應至 `com.adobe.livecycle.sample.customer.Customer`.
