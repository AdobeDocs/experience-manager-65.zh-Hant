...
在進入備份模式之前，您應確定一個唯一的標籤，確定要分配多少時間來執行備份，並決定是否希望表單伺服器保持備份模式。 這些注意事項對於與您組織建立的備份程式整合非常重要。 （請參閱[管理幫助](https://www.adobe.com/go/learn_aemforms_admin_63)。）
...
1. 建立BackupService客戶端API對象
   ...
   您可以一起使用`ServiceClientFactory`對象和BackupService客戶端API對象。
   ...
   * 使用其建構子並傳遞`ServiceClientFactory`對象，建立`BackupService`對象。
...
1. 進入備份模式
   ...
   * 一個`String`值，它指定標識備份模式會話的唯一人類可讀標籤。 建議您不要使用無法編碼為XML格式的空格或字元。
   * 一個`int`值，它指定在備份模式下保持的分鐘數。 您可以指定從`1`到`10080`的值（一週中的分鐘數）。 使用連續備份模式時，將忽略此值。
   * 一個`Boolean`值，它指定是否處於連續備份模式。 值`True`指定為處於連續備份模式。 在連續備份模式下，您為保持備份模式所指定的分鐘數指定的值將被忽略。

      連續備份模式意味著在完成當前備份模式會話後將啟動新的備份模式會話。 值`False`表示不使用連續備份模式，在離開備份模式後，恢復從GDS中清除檔案。

1. 檢索伺服器上備份模式會話的資訊

   使用調用`enterBackupMode`方法後返回的`BackupModeEntryResult`對象檢索資訊。 進入備份模式後可檢索的資訊對於與備份過程整合非常有用。 例如，標籤、備份ID和開始時間可作為備份過程檔案名的輸入。
...
1. 建立BackupService客戶端API對象
   ...
   使用Microsoft .NET客戶端元件，通過調用其預設建構子建立`BackupServiceService`對象，並使用`Credentials`方法指定憑據。
...
1. 進入備份模式
   ...
   * 一個`String`值，它指定標識備份模式會話的唯一人類可讀標籤。 建議您不要使用無法編碼為XML格式的空格或字元。
   * 一個`Uint32`值，它指定在備份模式下保持的分鐘數。 您可以指定從`1`到`10080`的值（一週中的分鐘數）。 使用連續備份模式時，將忽略此值。
   * 一個`Boolean`值，它指定是否處於連續備份模式。 值`True`指定為處於連續備份模式。 在連續備份模式下，您為保持備份模式所指定的分鐘數指定的值將被忽略。 連續備份模式意味著在完成當前備份模式會話後將啟動新的備份模式會話。
...
1. 建立BackupService客戶端API對象
   ...
   * 建立包含連接屬性的`ServiceClientFactory`對象。 （請參閱[設定連接屬性](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)。）
   * 使用其建構子並將`ServiceClientFactory`對象作為參數傳遞，建立`BackupService`對象。

1. 進入備份模式

   通過調用`leaveBackupMode`方法，使備份模式保持不變。

1. 檢索伺服器上備份模式會話的資訊

   使用返回的`BackupModeResult`對象檢索有關操作的資訊。 進入備份模式後可檢索的資訊對於與備份過程整合非常有用。 例如，標籤、備份ID和開始時間可作為備份過程檔案名的輸入。

### 使用web service API {#leave-backup-mode-using-the-web-service-api}離開備份模式
...
1. 建立BackupService客戶端API對象
   ...
   使用Microsoft .NET客戶端元件，通過調用其預設建構子建立`BackupServiceService`對象。

1. 進入備份模式

   通過調用`leaveBackupMode` Web服務操作，使備份模式保持不變。
