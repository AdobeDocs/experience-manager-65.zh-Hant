...
若要拆解PDF檔案，請確定 `PDFsFromBookmarks` 元素位於DDX檔案中。 此 `PDFsFromBookmarks` 元素是結果元素，而且只能是 `DDX` 元素。 它沒有 `result` 屬性，因為它可能會導致產生多個檔案。

此 `PDFsFromBookmarks` 元素會針對來原始檔中的每個第1層書籤產生單一檔案。
...
>[!NOTE]
>
>閱讀本節之前，建議您先熟悉使用Assembler服務來組裝PDF檔案。 (請參閱 [以程式設計方式組合PDF檔案](/help/forms/developing/programmatically-assembling-pdf-documents.md).)

>[!NOTE]
>
>將單一PDF檔案傳遞至組合器服務並取回單一檔案時，您可以叫用 `invokeOneDocument` 作業。 然而，若要拆解PDF檔案，請使用 `invokeDDX` 作業的原因是，雖然有一個輸入PDF檔案傳遞到Assembler服務，但Assembler服務會傳回包含一或多個檔案的集合物件。
...
建立Assembler服務使用者端、參照DDX檔案、參照PDFPDF檔案以拆解並設定執行階段選項之後，您可以透過叫用 `invokeDDX` 方法。 只要DDX檔案包含解組PDF檔案的指示，組裝程式服務就會傳回集合物件中已解組的PDF檔案。
...
1. 參考現有的DDX檔案。
   ...
   * 建立 `System.IO.FileStream` 物件，透過叫用它的建構函式。 傳遞代表DDX檔案檔案位置以及開啟檔案的模式的字串值。
...
1. 參照要拆解的PDF檔案。
   ...
   * 建立位元組陣列，儲存 `System.IO.FileStream` 物件。 您可以取得 `System.IO.FileStream` 物件的 `Length` 屬性。
   * 透過叫用 `System.IO.FileStream` 物件的 `Read` 方法，並傳遞位元組陣列、起始位置以及要讀取的資料流長度。
   * 填入 `BLOB` 物件，透過指派其 `MTOM` 欄位位位位元組陣列的內容。
...
1. 設定執行階段選項。
   ...
   * 建立 `AssemblerOptionSpec` 使用建構函式來儲存執行階段選項的物件。
   * 將值指派給屬於下列專案的資料成員，以設定執行階段選項以符合您的業務需求 `AssemblerOptionSpec` 物件。 例如，若要指示Assembler服務在發生錯誤時繼續處理工作，請指派 `false` 至 `AssemblerOptionSpec` 物件的 `failOnError` 欄位。
...
1. 拆解PDF檔案。
   ...
   * 此 `MyMapOf_xsd_string_To_xsd_anyType` 包含PDF檔案的物件進行拆解
