...
使用片段可簡化並加速大量表單的建立與維護。 建立新表單時，您會插入所需片段的參考，而片段會顯示在表單中。 片段參考包含指向實體XDP檔案的子表單。 如需有關根據片段建立表單設計的資訊，請參閱 [Forms設計工具](https://www.adobe.com/go/learn_aemforms_designer_63_tw)
