...
伺服器端指令碼執行導致的表單變更，不會反映在使用者端轉譯的表單中，除非root子表單包含 `restoreState` 屬性已設定為 `auto`. 如需有關此屬性的詳細資訊，請參閱 [Forms Designer。](https://www.adobe.com/go/learn_aemforms_designer_63_tw)
