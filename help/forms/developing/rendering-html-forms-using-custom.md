...
description: 使用Forms服務來參照自訂CSS檔案，以便針對Web瀏覽器的HTTP請求轉譯HTML表單。 您可以使用Java API和Web服務API呈現使用CSS檔案的HTML表單。
contentOwner: admin
...
* **類別層級選取器樣式配對**：如果存在於自訂CSS檔案中，則會使用在HTML表單中作為類別樣式使用的選取器。 未使用的類別樣式會被忽略。
* **識別項層級選擇器樣式配對**：如果所有識別項樣式都用於HTML表單中，則會使用這些樣式。
* **元素層級選取器樣式配對**：所有元素樣式都會在HTML表單中使用。
* **樣式優先順序**：樣式優先順序（如重要）受到支援，並且可用於自訂CSS檔案中。
* **媒體型別**：一或多個選擇器樣式配對可以包裝在@media樣式中，以定義媒體型別。 Forms服務不會檢查指定的媒體型別是否受支援。 自訂CSS檔案中指定的媒體型別會在HTML表單中合併。

您可以使用FormsIVS應用程式擷取範例CSS檔案。 上傳表單，在「測試表單設計」頁面中選取表單，然後按一下「產生CSS」。 按一下按鈕之前，您不需要設定HTML轉換型別。 接著選取「儲存」。 您可以編輯此CSS檔案以符合您的業務需求。

>[!NOTE]
>
>在轉譯使用自訂CSS檔案的HTML表單之前，請務必充分了解轉譯HTML表單。 (請參閱[將Forms轉譯為HTML](/help/forms/developing/rendering-forms-html.md)。)
...
[呈現使用Java API之CSS檔案的HTML表單](#render-an-html-form-that-uses-a-css-file-using-the-java-api)
...
[快速入門(SOAP模式)：呈現使用CSS檔案的HTML表單（使用Java API）](/help/forms/developing/forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api)

[包含AEM Forms Java程式庫檔案](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)

[設定連線屬性](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)

## 呈現使用網頁服務API的CSS檔案的HTML表單 {#render-an-html-form-that-uses-a-css-file-using-the-web-service-api}

使用Forms API （網頁服務）呈現使用自訂CSS檔案的HTML表單：
