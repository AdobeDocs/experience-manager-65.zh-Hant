...
* **類別層級選擇器樣式配對**：如果存在於自訂CSS檔案中，則會使用在HTML表單中作為類別樣式使用的選取器。 未使用的類別樣式會被忽略。
* **識別碼層級選擇器樣式配對**：所有識別碼樣式都會在HTML表單中使用。
* **元素層級選取器樣式配對**：所有元素樣式都會在HTML表單中使用。
* **樣式優先順序**：支援樣式優先順序（如重要），並且可用於自訂CSS檔案。
* **媒體型別**：一或多個選擇器樣式配對可以包在@media樣式中來定義媒體型別。 Forms服務不會檢查指定的媒體型別是否受支援。 自訂CSS檔案中指定的媒體型別會在HTML表單中合併。
