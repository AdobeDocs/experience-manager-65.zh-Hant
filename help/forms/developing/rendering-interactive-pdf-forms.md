...
>[!NOTE]
>
>使用Forms服務轉譯表單前，請先建立表單設計。 通常，表單設計會在Designer中建立並儲存為XDP檔案。 如需關於建立表單設計的資訊，請參閱 [Forms Designer](https://www.adobe.com/go/learn_aemforms_designer_63_tw).
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>3</p></td>
   <td><p>使用者填寫貸款表單並按一下提交按鈕後，資料將提交至 <code>HandleData</code> Java Servlet。 (請參閱 <i>「貸款表格」</i>.)</p></td>
  </tr>
  <tr>
   <td><p>4</p></td>
   <td><p>此 <code>HandleData</code> Java Servlet使用Forms服務使用者端API來處理表單提交與擷取表單資料。 然後，資料會儲存在企業資料庫中。 (請參閱 <a href="/help/forms/developing/handling-submitted-forms.md#handling-submitted-forms">處理已提交的Forms</a>.)</p></td>
  </tr>
  <tr>
   <td><p>5</p></td>
   <td><p>確認表單會轉譯回網頁瀏覽器。 如使用者的名字和姓氏等資料會在呈現之前與表單合併。 (請參閱 <a href="/help/forms/developing/prepopulating-forms-flowable-layouts.md">使用可流程配置預先填入Forms</a>.)</p></td>
  </tr>
 </tbody>
</table>
...
此互動式貸款表單由範例貸款申請的 `GetLoanForm` Java Servlet。
...
一般而言，您不會將Forms服務使用者端API程式碼放在Java Servlet的 `doGet` 或 `doPost` 方法。 程式設計實務建議將此程式碼放在另一個類別中，從內將類別例項化 `doPost` 方法(或 `doGet` 方法)，並呼叫適當的方法。 不過，為了程式碼簡潔，本節中的程式碼範例儘量小，程式碼範例放置在 `doPost` 方法。
...
您可以指定Forms服務轉譯表單所需的URI值。 若表單設計儲存為Forms應用程式的一部分，則可使用內容根URI值加以參照 `repository:///`. 例如，考慮以下名為的表單設計 *Loan.xdp* 位在名為的Forms應用程式中 *表單應用程式*：
...
>[!NOTE]
>
>您也可以傳遞URL值來參照XDP檔案，而不是指定 `com.adobe.idp.Document` 執行個體重新命名為Forms服務。 此 `com.adobe.idp.Document` 例項包含表單設計。 (請參閱 [將檔案傳遞至Forms服務](/help/forms/developing/passing-documents-forms-service.md).)
...
此 `renderPDFForm` 使用 `URLSpec` 物件。 xdp檔案的內容根目錄會使用傳遞至Forms服務 `URLSpec` 物件的 `setContentRootURI` 方法。 表單設計名稱( `formQuery`)以個別引數值傳遞。 這兩個值會串連在一起，以取得表單設計的絕對參照。
...
1. 將檔案附加至表單
   ...
   * 建立 `java.util.HashMap` 物件，使用它的建構函式來儲存檔案附件。
   ...
   * A `com.adobe.idp.Document` 包含檔案附件的物件。
   ...
   >[!NOTE]
   >
   >對每個要附加到表單的檔案重複此步驟。 此步驟為選填，您可以通過 `null` 如果您不想傳送檔案附件。
