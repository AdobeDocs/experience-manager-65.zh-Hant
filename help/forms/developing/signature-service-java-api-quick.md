...
下列Java程式碼範例修改名為SignatureField1的簽章欄位，方法是在簽章套用至簽章欄位時，鎖定表單中的所有欄位，並確保不允許任何變更。 簽名服務傳回包含已修改簽名欄位的PDF檔案後，PDF檔案會儲存為名為LoanSig.pdf的PDF檔案。 （此範例會覆寫傳遞至簽名服務的PDF檔案。） （請參閱[修改簽章欄位](/help/forms/developing/digitally-signing-certifying-documents.md#modifying-signature-fields)。）
