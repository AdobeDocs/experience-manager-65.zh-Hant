...
AEM Forms附加元件套件提供立即可用的Chart元件。 您可以在調適型表單和檔案中使用，以視覺化呈現中的二維資料 **可重複面板** 和 **表格**. 「圖表」元件可讓您新增及設定下列型別的圖表：
...
* **資料來源的重複列名稱或面板名稱**：指定資料來源的表格列或可重複面板的元素名稱。 在範例中，statementDetails是「陳述式詳細資訊」表格中可重複資料列的元素名稱。
...
此範例中使用的信用卡陳述式是調適型表單檔案，陳述式詳細資料區段是表格，在撰寫模式中看起來如下所示。
...
除了在圖表中使用預設函式之外，您還可以撰寫 [自訂函式](/help/forms/using/rule-editor.md#custom-functions-in-rule-editor-custom-functions) 在JavaScript中，並在「圖表」元件的函式清單中提供。
