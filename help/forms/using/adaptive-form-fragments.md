...
1. 按一下以打開「 **表單模型** 」標籤，然後從「 **從中選擇** 」下拉清單中，為片段選擇以下模型之一：
   ...
   * **表單範本** ：指定使用上傳到AEM Forms的 XDP 範本創建片段。 選取適當的XDP範本作為片段的表單模型。
...
在編輯模式中，您可以拖放任何最適化表單元件從AEM sidekick至片段。 有關自我調整表單元件的資訊，請參閱 [ 創作自我調整表單 ](../../forms/using/introduction-forms-authoring.md) 簡介。
...
1. 在「片段建立」對話方塊中，指定下列資訊：
   ...
   * **目標路徑**：儲存片段的存放庫路徑。 如果未指定路徑，則會在包含自我調整表單的節點旁邊創建與片段同名的節點。 片段會儲存在此節點。

   * **表單模型：根據自我調整表單的表單模型** ，此欄位顯示 **XML 架構** 、 **表單範本** 或 **無** 。 這是一個不可編輯的欄位。

   * **片段模型根** ：僅出現在基於 XSD 的最適化表單中。 它指定片段模型的根。 您可以選擇 **/** 或下拉式清單中的XSD複雜型別。 請注意，僅當您選擇複雜類型作為片段模型根時，才能在另一個自我調整表單中重複使用片段。如果您選擇 **/** 作為片段模型根，根的完整XSD樹狀結構會顯示在最適化表單資料模型標籤中。 對於複雜型別片段模型根，在調適型表單資料模型標籤中只會顯示所選複雜型別的子系。
...
1. 按一下 **資產** ![資產 — 瀏覽器](assets/assets-browser.png) 在側邊欄中。 在資產瀏覽器中，選取 **最適化表單片段** 從下拉式清單。
   ...
   您還可以選擇顯示所有自我調整表單片段或基於其表單模型（表單範本、XML 架構或基本）進行篩選。

1. 拖放最適化表單片段至最適化表單。

   >[!NOTE]
   >
   >最適化表單片段未啟用，供您從最適化表單內創作。 此外，您不能在基於 JSON 的自我調整表單中使用基於 XSD 的片段，反之亦然。
...
當您使用 XFA 表單 範本 或 XSD 複雜類型創建自我調整表單片段並將該片段拖放到自我調整表單時，XFA 片段或 XSD 複雜類型將自動替換為相應的自我調整表單片段，其片段模型根映射到 XFA 片段或 XSD 複雜類型。
...
您可以使用此AEM Forms UI對最適化表單片段執行數個操作。
...
* 在XDP型最適化表單中，如果您將面板儲存為包含其他XDP片段的片段，則產生的片段將自動與子XDP片段繫結。 如果存在XSD型最適化表單，則產生的片段將繫結至結構描述根。
