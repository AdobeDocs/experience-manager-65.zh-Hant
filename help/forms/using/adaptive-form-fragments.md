...
1. 按一下以開啟 **表單模型** 標籤，並從 **選取自** 下拉式選單，為片段選取以下其中一個模型：
   ...
   * **表單範本** ：指定使用上傳到AEM Forms的 XDP 範本創建片段。 選擇適當的 XDP 範本作為片段的表單模型。

   ![以表單範本作為模型建立自我調整表單](assets/form-template-model.png)
...
您建立的最適化表單片段會顯示在 AEM 內容 Finder 的「最適化表單片段」標籤中。 要在最適化表單中插入最適化表單片段：
...
自我調整表單片段通過引用插入自我調整表單中，並與獨立自我調整表單片段同步。 這意味著當您更新自我調整表單片段時，更改會反映在使用片段的所有自我調整表單中。
...
* 您無法從最適化表單內編輯通過引用插入的最適化表單片段。 若要編輯，請編輯獨立的自適應表單片段，或將片段嵌入自適應表單中。
* 發佈最適化表單時，您需要發佈在最適化表單中透過參考插入的獨立最適化表單片段。
* 當您重新發佈更新的自適應表單片段時，變更會反映在使用片段的自適應表單的已發佈例項中。
* 包含驗證元件的自我調整表單不支援匿名使用者。 此外，不建議在自適應表單片段中使用驗證元件。
