...
# 在建立對應UI {#add-custom-action-button-in-create-correspondence-ui}中新增自訂動作／按鈕
...
### 將按鈕添加到建立對應用戶介面{#add-the-button-to-the-create-correspondence-user-interface}

1. 前往`https://'[server]:[port]'/[ContextPath]/crx/de`，以管理員身分登入。
1. 在應用程式檔案夾中，建立名為`defaultApp`的檔案夾，其路徑／結構類似於defaultApp檔案夾（位於config檔案夾中）。 使用下列步驟建立資料夾：

   1. 在以下路徑中按一下右鍵&#x200B;**defaultApp**&#x200B;資料夾，然後選擇&#x200B;**覆蓋節點**:
...
1. 複製acmExtensionsConfig.xml檔案（存在於/libs分支下），位於/apps分支下。
   ...
   1. 在acmExtensionsConfig.xml檔案上按一下滑鼠右鍵，然後選取&#x200B;**Copy**。

      ![複製acmExtensionsConfig.xml](assets/3_acmextensionsconfig_xml_copy.png)

   1. 在「/apps/fd/cm/config/defaultApp/」的&#x200B;**defaultApp**&#x200B;資料夾上按一下滑鼠右鍵，然後選取「貼上」。****
   1. 按一下&#x200B;**保存全部**。
...
1. 若要以電子郵件寄送信件，您可以使用LiveCycleForms Workflow。 在acmExtensionsConfig.xml的modelExtension標籤下方新增customAction標籤，如下所示：
   ...
   | permissionName | 僅當用戶具有permissionName指定的權限時，才顯示相應的操作。 當您將permissionName指定為`forms-users`時，所有使用者都可存取此選項。 |
...
#### 在/apps分支{#create-a-locale-folder-with-properties-file-in-the-apps-branch}中使用屬性檔案建立區域設定資料夾

ACMExtensionsMessages.properties檔案包含「建立對應」使用者介面中各欄位的標籤和工具提示訊息。 若要使自訂動作／按鈕正常運作，請在/apps分支中複製此檔案。

1. 在以下路徑中按一下右鍵&#x200B;**locale**&#x200B;資料夾，然後選擇&#x200B;**覆蓋節點**:
...
1. 按一下右鍵以下路徑上的&#x200B;**locale**&#x200B;資料夾，然後選擇&#x200B;**貼上**:

   `/apps/fd/cm/config/defaultApp/locale/`

   ACMExtensionsMessages.properties檔案被複製到地區設定資料夾中。

1. 要本地化新添加的自定義操作／按鈕的標籤，請為`/apps/fd/cm/config/defaultApp/locale/`中的相關地區設定建立ACMExtensionsMessages.properties檔案。
...
1. 在apps資料夾中，在CRX的/apps分支中建立名為`js`的資料夾，其結構類似於下列資料夾：
...
DSCSample.jar檔案使用renderLetter API，從C作為輸入提供的XML資料，將letter轉譯為PDF位元組。 如需renderLetter和其他API的詳細資訊，請參閱[Letter Render Service](https://helpx.adobe.com/aem-forms/6-2/javadocs/com/adobe/icc/ddg/api/LetterRenderService.html)。
...
1. 通過檔案瀏覽器選擇&#x200B;**DSCSample.jar**&#x200B;檔案，然後按一下&#x200B;**開啟**。
1. 按一下右鍵&#x200B;**RenderWrapper**&#x200B;並選擇&#x200B;**啟動元件**。 如果元件啟動，則元件名稱旁會出現綠色箭頭。

## 傳送信函以進行審查{#send-letter-for-review}

在您設定傳送信函以供審核的動作和按鈕後：

1. 清除瀏覽器快取。

1. 在「建立對應」UI中，按一下&#x200B;**Letter Review**&#x200B;並指定審核者的電子郵件ID。
