---
title: 新增自訂屬性至通信管理資產
seo-title: 新增自訂屬性至通信管理資產
description: 了解如何將自訂屬性新增至通信管理資產。
seo-description: 了解如何將自訂屬性新增至通信管理資產。
...
# 將自訂屬性新增至通信管理資產{#add-custom-properties-to-correspondence-management-assets}

## 概覽 {#overview}

您可以自訂通信管理使用者介面，並向使用者提供量身打造的屬性和索引標籤集。 此自訂功能包括將自訂欄位/屬性和索引標籤新增至特定資產類型/信函，或所有資產類型和信函。

## 將自訂屬性新增至通信管理資產{#adding-custom-properties-to-correspondence-management-assets}
...
1. 在應用程式資料夾中，使用下列步驟建立名為css的資料夾，其路徑/結構類似於css資料夾（位於ccrui資料夾中）:
   ...
   1. 確定「覆蓋節點」對話方塊有下列值：
      ...
      **路徑：** /libs/fd/cm/ma/gui/content/cmmetadatapropertys/commonproperties/col1/items
...
1. 在新建立的項目資料夾下，為所有資產中的自訂屬性新增節點(範例：地理位置)，使用下列步驟：
   ...
   1. 按一下右鍵項資料夾，然後選擇&#x200B;**Create** > **Create Node**。
...
1. 若要檢視自訂，請將滑鼠指標暫留在資產（文字、清單、條件或版面片段）或信函上，按一下「檢視屬性&#x200B;**」，然後按一下「**&#x200B;編輯&#x200B;**」。**&#x200B;新欄位（收件者的位置）會顯示在資產/信函屬性的「基本」索引標籤中。
   ...
   >[!NOTE]
   >
   >您新增之所有資產的通用屬性會顯示在資產屬性的基本索引標籤中。 依預設，為所有資產新增的通用屬性會出現在屬性頁面和資產建立頁面上。 要隱藏公共屬性，需要<!--link to show / hide properties]-->。

### 案例：將自訂下拉式清單和值新增至自訂屬性/欄位{#scenario-add-custom-drop-down-and-values-to-a-custom-property-field}
...
為需要為其建立類型nt:unstructured的下拉清單（此處為`geographicallocation`）的每個屬性（欄位）建立一個節點。
1. 將下列屬性新增至節點（此處為地理配置），然後按一下「保存全部」****:
...
1. 在屬性節點下（此處為地理位置分配），添加名稱為`items`的新節點。 在「項目」節點下，為下拉式清單中的值新增每個節點。 將第一個節點新增為空白，作為下拉式清單的預設值，以及讓使用者為欄位指定任何值的選項，會是一個正確的作法。 若要新增多個選項/下拉式值，請重複下列步驟：

   1. 按一下右鍵屬性節點（此處為地理分配），然後選擇&#x200B;**Create** > **Create Node**。
   1. 將欄位名稱輸入為`item1,`，將類型保留為nt:unstructured，然後按一下&#x200B;**OK**。
   1. 將以下屬性添加到新建立的節點（此處為項目1），然後按一下&#x200B;**Save All**:
      ...
      <table>
         ...
         <tr>
          <td>文字</td>
          <td>字串</td>
          <td>這是使用者可看見的下拉式選項值。 保留空白（預設）值，或輸入值，如<strong>International</strong>或<strong>Within US</strong>。<br /> </td>
         </tr>
         <tr>
          <td>值</td>
          <td>字串</td>
...
### 案例：所有資產類型{#scenario-common-tab-for-all-asset-types}的通用標籤
...
1. 在cmmetadataproperties資料夾下，新增節點，以建立所有資產的自訂索引標籤(範例：公用標籤)，請執行下列步驟：
   ...
   1. 按一下右鍵cmmetadataproperties資料夾，然後選擇&#x200B;**Create** > **Create Node**。

      ![建立節點](assets/cmmetadatapropertiescreatenode.png)

   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:

      **名稱：** 公用標籤（或您要為此屬性提供的名稱）
...
1. 對於在最後一個步驟中建立的頁簽節點（此處為公用頁簽），請使用以下步驟建立名為item的節點：
   ...
   1. 按一下右鍵相關節點（此處為公用頁簽），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 在上一步中建立的項節點（公用頁簽下）中，使用以下步驟（要添加更多列，請重複此步驟），在自定義頁簽（公用頁簽）中添加用於建立列的節點（此處為Column1）:
   ...
   1. 按一下右鍵項節點，然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:

      **名稱：** Column1（或您要為節點提供的名稱 — 此名稱不會出現在使用者介面中。）

      **類型：** nt:unstructured

   1. 將以下屬性添加到節點（此處為Column1），然後按一下&#x200B;**Save All**:
...
1. 在您在上一步驟中建立的節點（此處為Column1）中，使用以下步驟添加名為items的節點：
   ...
   1. 按一下右鍵節點（此處為Column1），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 若要在自訂索引標籤中建立欄位（此處為收件者），請新增節點（此處為GeorlactionLocation）。 此屬性與您建立的列相對應。 使用下列步驟建立欄位（若要建立更多欄位/節點，請重複這些步驟）。:
   ...
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
      ...
      **名稱：** GeorgalLocation（或欄位屬性的其他名稱）

      **類型：** nt:unstructured

   1. 將下列屬性添加到欄位節點（此處為GeorcalLocation），然後按一下&#x200B;**Save All**。
...
1. 若要為「信函」新增此索引標籤，請在下列路徑建立具有類似下列項目資料夾之路徑/結構的覆蓋資料夾：
   ...
   若要為字母或不同資產建立覆蓋，請使用下列路徑，將[assettype]取代為文字、條件、清單、資料字典或片段：
...
1. 若要檢視自訂，請將滑鼠指標暫留在相關資產上（此處顯示信函），按一下「檢視屬性」，然後按一下「**編輯**」。 新的索引標籤（收件者）和欄位（收件者的位置）會顯示在使用者介面中。
...
1. 在您在上一步驟中建立的節點（此處為「自訂」索引標籤）中，使用下列步驟新增名為「項目」的節點：
   ...
   1. 按一下右鍵節點（此處自定義頁簽），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 對於您建立的每列（如前一步中指定的 — 此處為Column1），請使用以下步驟建立名為item的節點：
   ...
   1. 按一下右鍵相關列節點（此處為Column1），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 對於已建立的每列，在項節點下建立一個節點，以在用戶介面的新頁簽中建立欄位。 重複此步驟，在欄中建立更多欄位：
   ...
   1. 按一下右鍵相關節點（此處是列1下的項），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
### 在資產建立頁面{#display-custom-properties-on-the-asset-creation-page}上顯示自訂屬性
...
1. 對於字母，請確定「覆蓋節點」對話方塊具有下列值。 對於其他資產類型，下表提供路徑：
   ...
   **路徑：** /libs/fd/cm/ma/gui/content/createasset/createletter/jcr:content/body/items/form/items/letterWizard/items/properties/items/letter/properties/items/letterproperties/items
...
1. 在您建立的覆蓋項目節點下，建立名稱col4的節點（或任何其他名稱），然後按一下「保存全部」****。

   例如，以下是為字母建立的覆蓋節點。

   `/apps/fd/cm/ma/gui/content/createasset/createletter/jcr:content/body/items/form/items/letterWizard/items/properties/items/properties/items/letterproperties/items`

1. 將以下屬性添加到新建立的節點（此處為col4），然後按一下&#x200B;**Save All**:
...
1. 對於已建立的每個屬性，在列節點下建立一個節點，以在用戶介面中建立列。 重複此步驟，在UI中建立更多欄：
   ...
   1. 按一下右鍵相關節點（列），然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:

      **名稱：** 您選取的名稱（此處為GeorlactionLocation）
...
1. 開啟childlistpage.jsp檔案(/apps/fd/cm/ma/gui/components/admin/childpagerenderer/childlistpage/childlistpage.jsp)，然後進行以下更改：
   ...
   1. 在&lt;tr>標籤(&lt;tr &lt;%= attrs.build()%>>)開始之前新增下列內容：
   ...
   1. 若要在UI中顯示新屬性，請在結尾tr(&lt;/tr>)標籤前新增TD標籤，如下所示：
...
當您新增sling:orderBefore屬性以指定欄位時，您也需要更新此程式步驟6.4中指定之對應&lt;td>標籤的順序。 例如，在此情況下，您需要確保「地理位置」的&lt;td>標籤放在「版本」欄的&lt;td>標籤之前：
...
## 啟用自定義屬性搜索{#enable-search-for-custom-properties}
...
1. 前往`https://'[server]:[port]'/[ContextPath]/crx/de`並以管理員身分登入。
1. 前往`/oak:index/cmLucene`，並在其下新增名為&#x200B;**aggregates**&#x200B;的節點。

   1. 按一下右鍵cmLucene資料夾，然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 在新建立的匯總資料夾下，新增節點cm:resource。 在cm:resource下，添加名為include0的節點。
   ...
   1. 按一下右鍵聚合資料夾，然後選擇&#x200B;**Create** > **Create Node**。 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:

      **名稱：** cm:resource

      **類型：** nt:unstructured

   1. 按一下右鍵cm:resource資料夾，然後選擇&#x200B;**Create** > **Create Node**。 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
...
1. 轉至以下位置的屬性，並在其下添加節點位置：`/oak:index/cmLucene/indexRules/cm:resource/properties`
   ...
   1. 按一下右鍵屬性資料夾，然後選擇&#x200B;**Create** > **Create Node**。
   1. 確保「建立節點」對話框具有以下值，然後按一下&#x200B;**OK**:
   ...
   1. 將下列屬性新增至節點（此處位置）:
      ...
      |---|---|---|
      | 分析 | 字串 | true |
      | 名稱 | 字串 | extendedProperties/location（或要添加到搜索的屬性的名稱） |
      | propertyIndex | 布林值 (Boolean) | true |
      | useInSougkent | 布林值 (Boolean) | true |
...
1. 在新建立的節點中，列出，添加以下屬性，然後按一下&#x200B;**Save All**:
...
## 在「資產建立」和「屬性」頁面{#show-hide-custom-properties-on-asset-creation-and-properties-pages}上顯示/隱藏自訂屬性

若要顯示或隱藏自訂屬性，請完成下列步驟：

1. 在自訂屬性節點（例如地理分配）下，建立一個名為「granite:rendercondition」的「nt:unstructured」類型的新節點。
1. 將下列屬性新增至節點，然後按一下&#x200B;**Save All**:
...
1. 若要在資產建立頁面上隱藏此屬性，請新增下列屬性至該屬性，然後按一下「**儲存全部**」：
...
1. 若要隱藏資產屬性頁面上的自訂屬性，請新增下列屬性，然後按一下&#x200B;**儲存全部**:
   ...
   若要再次顯示值，請將屬性值重設為`false`或刪除屬性項目。
