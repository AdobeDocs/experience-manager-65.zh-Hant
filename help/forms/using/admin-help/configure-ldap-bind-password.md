...
description: 瞭解如何在將配置檔案導入到其他系統之前配置綁定密碼欄位。
...
為避免安全風險，未配置導出的配置檔案(config.xml)中的綁定密碼欄位。 在將配置檔案導入到其他系統之前，請確保配置此密碼。 此密碼將覆蓋儲存在資料庫中的現有密碼。 空密碼不覆蓋現有的非空密碼值。

1. 在管理控制台中，按一下「設定」>「用戶管理」>「配置」>「導入和導出配置檔案」。
1. 要將當前配置設定導出到檔案，請按一下「導出」，然後將配置檔案保存到另一個位置。
1. 在檔案中，找到 `Domains` > *[您的域名]* > `DirectoryConfigs` > `LDAPGroupConfig` 的下界。 下面是一個示例：
   ...
   鍵入值 `bindpassword` 並保存更改。

1. 在檔案中，找到 `Domains` > *[您的域名]* > `DirectoryConfigs` > `LDAPGroupConfig` > `LDAPUserConfig` 的下界。 下面是一個示例：
