...
Workbench提供在執行階段執行處理序執行個體時加以記錄，然後播放它們以觀察處理序行為的功能。 (請參閱 [Workbench說明](https://www.adobe.com/go/learn_aemforms_workbench_63).) 為了節省表單伺服器檔案系統的空間，您可以限制儲存的處理程式記錄資料量。 您可以設定「稽核工作流程服務」的下列屬性( `AuditWorkflowService`)：
...
**HTML至PDF轉換的遞補字型系列：** AEM表單伺服器無法使用原始HTML中使用的字型時，用於PDF檔案的字型系列名稱。 如果要轉換使用無法使用字型的HTML頁面，請指定字型系列。 例如，以地區語言撰寫的頁面可能會使用無法使用的字型。
...
當您建立指南時，某些資源（例如指南定義）會內嵌在指南中。 資源也可作為儲存在本機或AEM表單伺服器上的應用程式資產的參考而存在。 指南不包含資料，且提交位置和輸入的值不適合所有外部環境。
...
**資料來源名稱：** 字串值，代表用來連線至資料庫伺服器的資料來源的JNDI名稱。 資料來源必須在裝載表單伺服器的應用程式伺服器上定義。 預設值是AEM表單資料庫的資料來源的JNDI名稱。
...
**保留資料夾：** 保留資料夾的預設值。 此資料夾用於將來源檔案複製到中，以備成功處理輸入時使用。 此值可以是具有檔案模式的空白、相對或絕對路徑，如「結果資料夾」設定所述。
...
**金鑰存放區型別：** 金鑰庫的型別。 不提供使用針對執行表單伺服器的JVM所設定的預設金鑰存放區型別的值。 否則，請提供下列其中一個值：
...
**信任存放區型別：** 信任庫的型別。 不提供使用針對執行表單伺服器的JVM所設定的預設金鑰存放區型別的值。 否則，請提供下列其中一個值：
...
**工廠名稱：** 用於執行XSLT轉換的Java類別的完整名稱。 如果未指定值，則會使用執行Forms伺服器的Java虛擬機器器中設定的預設處理站。

## 修改服務的安全性設定 {#modifying-security-settings-for-a-service}

forms伺服器可讓您為每個服務設定安全性設定，讓您依服務層級設定微調存取控制。
