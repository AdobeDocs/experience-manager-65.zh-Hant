...
1. 按一下清單中的現有網域。 如果您為新網域新增驗證，請參閱 [新增企業網域](/help/forms/using/admin-help/adding-domains.md#add-an-enterprise-domain) 或 [新增混合式網域](/help/forms/using/admin-help/adding-domains.md#add-a-hybrid-domain).
...
即時提供會在使用者透過驗證提供者成功驗證後，自動在「使用者管理」資料庫中建立使用者。 相關的角色和群組也會動態指派給新使用者。 您可以啟用企業網域和混合網域的即時布建。
...
1. 新增驗證提供者。 新增驗證提供者時，請在[新增驗證]畫面上，選取已註冊的[識別建立者]和[指派提供者]。 (請參閱 [設定驗證提供者](configuring-authentication-providers.md#configuring-authentication-providers).)
1. 儲存網域。
