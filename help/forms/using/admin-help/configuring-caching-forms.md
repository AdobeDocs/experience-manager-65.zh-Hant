...
**範本組態快取大小：** 要保留在記憶體中的範本組態物件數目上限。 預設值為 100。建議將此值設定為大於或等於「範本快取大小」值。 此設定不會影響磁碟快取。

**範本快取大小：** 要保留在記憶體中的範本內容物件數上限。 預設值為 100。此設定不會影響磁碟快取。
...
**快取大小：** 指定記憶體快取中可容納的已轉譯表單數上限。 預設值為 100。此設定不會影響磁碟快取。
