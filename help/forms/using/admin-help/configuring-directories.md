...
**匿名：** 不需要用戶名或密碼。 匿名用戶只能獲取有限數量的資料。 這個選項可能對初始測試非常有用。
...
>[!NOTE]
>
>要啟用VLV，請配置Sun One。 請參 [閱配置用戶管理以使用虛擬清單視圖(VLV)](configuring-directories.md#configure-user-management-to-use-virtual-list-view-vlv)。
...
1. 配置目錄時，在「用戶設定」頁和「組設定」頁上選擇「啟用虛擬清單視圖(VLV)控制」。 選中該複選框時，還必須在「排序欄位」框中指定排序名稱。 預設值為uid。 (請參 [閱添加目錄或自定義SPI](configuring-directories.md#adding-directories-or-custom-spis)[或編輯目錄](configuring-directories.md#edit-a-directory)。)
