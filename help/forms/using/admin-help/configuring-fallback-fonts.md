...
1. 視需要修改一般字型和備援字型位置和名稱，並儲存檔案。
   ...
   FontManagerResources.properties檔案中的字型條目與&#x200B;*`[aem-forms root]`*/fonts目錄相關。 如果指定的字型不是預設的AEM表單字型，則必須在此目錄結構內（在現有目錄內或新建立的目錄中）安裝這些字型。
...
1. 重新封裝adobe-livecycle-*`[appserver]`*.ear檔案，然後手動重新部署，或執行Configuration Manager。
