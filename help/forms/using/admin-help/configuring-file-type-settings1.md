...
1. 按一下「新增」或按一下設定的名稱。
1. 在「副檔名」方塊中，輸入此應用程式接受的檔案型別的副檔名（以逗號分隔）。 請勿包含擴充功能之前的句點或空格。 預設為 `bmp,gif,jpeg,jpg,tif,tiff,png`。
1. （可選）若要在圖形或影像中使用文字的光學程式碼辨識(OCR)，請選取「使用OCR」並設定下列選項：

**主要語言：** 用於識別字元的OCR引擎語言。 預設值為英文（美國）。
...
Adobe PDF匯出設定區段中的「匯出檔案型別」設定可用來將PDF檔案轉換為其他格式。 預設為具有階層式樣式表(CSS) 1.0的HTML4.01(&amp;ast；.htm，&amp;ast；.html)。
...
1. 如果您只想嵌入使用中的嵌入字型子集，請選取 **將所有內嵌字型設為子集**.
   ...
   >[!NOTE]
   >
   >*如果您要搭配使用此選項&#x200B;**取消嵌入某些字型**，中的字型&#x200B;**A**要取消內嵌清單的dd字型仍完全取消內嵌。*
...
**副檔名**：指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `xls,xlsx`。
請勿在擴充功能前加上句點或空格。
...
1. **[!UICONTROL 副檔名：]** 指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `mpp`。
請勿在擴充功能前加上句點或空格。
...
**[!UICONTROL 副檔名]**：指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `doc,docx,rtf,txt`。
請勿在擴充功能前加上句點或空格。
...
**[!UICONTROL 副檔名]**：指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `pub`。
請勿在擴充功能前加上句點或空格。
...
**[!UICONTROL 副檔名]**：指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `dwg`。
請勿在擴充功能前加上句點或空格。
...
**副檔名**：指定此應用程式接受的檔案型別的副檔名（以逗號分隔）。 預設為 `odt,odp,ods,odg,odf,sxw,sxi,sxd`。
請勿在擴充功能前加上句點或空格。
