...
Adobe PDF匯出設定區段中的「匯出檔案型別」設定可用來將PDF檔案轉換為另一種格式。 預設為具有階層式樣式表(CSS) 1.0的HTML4.01 (&amp;amp；ast；.htm，&amp;amp；ast；.html)。
...
1. 如果您只想內嵌使用中的內嵌字型子集，請選取&#x200B;**所有內嵌字型子集**。
   ...
   >[!NOTE]
   >
   >*如果您使用此選項搭配&#x200B;**取消內嵌某些字型**，要取消內嵌清單的&#x200B;**A**dd字型中的字型仍會完全取消內嵌。*
...
**[!UICONTROL 建立符合PDF/E-1的檔案]**：建立符合PDF/E-1的檔案。 PDF/E是用於交換工程和技術檔案的ISO標準。 如需PDF/E-1的詳細資訊，請參閱[Adobe和產業標準](https://www.adobe.com/enterprise/standards/index.html)。
