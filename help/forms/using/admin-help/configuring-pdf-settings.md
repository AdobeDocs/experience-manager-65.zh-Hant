...
**PDF/X-3 2002：檢查傳入的** 工作是否符合PDF/X-3規範，並僅在符合規範時建立PDF檔案。與PDF/X-1a一樣，PDF/X-3是圖形內容交換的ISO標準。 主要的不同點在於，PDF/X-3支援不受裝置影響的色彩。

**印刷品質：** 建立PDF檔案以進行高品質的印刷製作（例如，在製版機或製版機上）。在這種情況下，不考慮檔案大小。 其目的在於維護商業印表機或印前服務供應商在PDF檔案中正確列印檔案所需的所有資訊。 這組選項：
...
**系結：** 指定要顯示具有左側或右側系結的PDF檔案。此設定會影響「對頁——連續」版面中的頁面顯示，以及縮圖的並排顯示。

**解析度：** 為輸入檔案設定打印機解析度的模擬，這些輸入檔案根據要打印的打印機的解析度調整其行為。對於大部分的輸入檔案，較高的解析度設定會產生較大但品質較高的PDF檔案，而較低的設定則會產生較小但品質較低的PDF檔案。 最常見的是，解析度會決定漸層或混合中的步驟數。 您可以輸入一個介於72到4000之間的值。 除非您打算將PDF檔案列印至特定印表機，而且想要模擬原始輸入檔案中定義的解析度，否則請將此設定保留為預設值。

>[!NOTE]
>
>增加解析度設定會增加檔案大小，並可能會稍微增加處理某些檔案所需的時間。

**所有頁面或頁面自：指** 定要轉換的頁面。將「至」方塊留空，以建立從您在「自」方塊中輸入的頁碼到檔案結尾的範圍。

**針對快速Web檢視進行最** 佳化：重新架構檔案，以便從Web伺服器進行逐頁下載（位元組伺服）。無論您在「影像」索引標籤上選取何種壓縮設定，此選項都會壓縮文字和線條圖。 從網路或網路下載檔案時，壓縮可加快存取和檢視的速度。 依預設，此選項不會啟用。
...
「影像」選項會指定影像的壓縮和重新取樣。 您可以嘗試這些選項，以在檔案大小和影像品質之間取得適當的平衡。 如需有關存取「影像」設定的指示，請參閱[新增或編輯PDF設定](configuring-pdf-settings.md#add-or-edit-pdf-settings)。
...
**平均縮減取樣至：** 平均取樣區域中的像素，並以指定解析度的平均像素顏色取代整個區域。

**雙立方縮減取樣至：** 使用加權平均來判斷像素色彩，而且通常產生的結果比簡單的縮減取樣平均法好。雙立方體是最慢但最精確的方法，會產生最順暢的色調漸層。
...
**消除鋸齒至灰色：讓單色影** 像的鋸齒邊緣更平滑。選取2位元、4位元或8位元，以指定4、16或256個灰階。 （消除鋸齒可能會模糊小型或細線。）

>[!NOTE]
>
>文字和線條圖的壓縮永遠開啟。

**影像原則：** 設定彩色、灰階和單色影像的原則。如果影像解析度低於指定的解析度，您仍可以選擇繼續（忽略）、提供警告訊息或取消工作。
...
**使用的字元百分比小於時的子集內嵌字型：** 如果您選取此選項，請指定臨界值百分比以僅內嵌字型的子集。例如，如果使用的臨界值是35且少於35%的字元，PDF產生器只會嵌入這些字元。 只有具有適當權限位的字型才會嵌入。

**內嵌失敗時：指** 定當PDF產生器在處理檔案時找不到要內嵌的字型時，其回應方式。您可以讓PDF產生器忽略請求並取代字型、警告您並取代字型，或取消目前工作的處理。
...
**檔案演算方式：** 在色域間映射顏色的方法。任何特定方法的結果取決於色域的輪廓。 例如，某些描述檔會以不同的方法產生相同的結果。 這些選項可用：

>[!NOTE]
>
>在所有情況下，在建立PDF檔案後所發生的色彩管理作業可能會忽略或覆寫意圖。

**保留：** 表示輸出裝置中已指定意圖，而非PDF檔案中。在許多輸出設備中，「相對比色」是預設方式。

**感應：** 當原始像素映射至目標色域時，維持其間的相對色彩值。雖然顏色值本身可能會改變，但此方法仍會保留顏色之間的視覺關係。

**飽和度：** 維持原始像素的相對飽和度值。這種方法適用於商業圖形，在商業圖形中，色彩之間的精確關係並不像有亮飽和色彩那麼重要。

**相對比色：** 將源空間的白點重新映射到目標空間的白點。

**絕對比色：** 在轉換顏色時停用白點與黑點的比色。除非您必須保留簽名顏色，例如商標或標誌中使用的顏色，否則不建議使用此方法。

### 工作空間{#working-spaces}

對於「色彩管理策略」（「保持色彩不變」除外）下清單中的所有值，請從「使用中色域」(Working Space)區域的清單中選取，以指定用於定義和校準蒸餾PDF檔案中灰階、RGB和CMYK色域的ICC描述檔。 這些選項可用：

**灰階：定** 義檔案中所有灰階影像的色域。只有在您選擇「標籤色彩管理的所有項目」或「僅標籤色彩管理的影像」時，此選項才可用。 灰色影像的預設ICC設定檔是Gray Gamma 2.2。您也可以選取「無」，以防止灰階影像被轉換。

**RGB：定** 義檔案中所有RGB影像的色域。預設的sRGB IEC61966-2.1是一個很好的選擇，因為它正在成為行業標準，許多輸出設備都能識別它。 您也可以選取「無」來防止轉換RGB影像。

**CMYK：定** 義檔案中所有CMYK影像的色域。預設值為US Web Coopted(SWOP)v2。 您也可以選取「無」來防止CMYK影像轉換。
...
**找到傳遞函式時：確** 定找到傳遞函式時要執行的操作：

**保留：** 保留傳統上用來補償影像傳輸至影片時可能發生的點增益或點損失的傳遞函式。當構成列印影像的墨點比半色調網屏大（例如，因為在紙上鋪展）時，就會產生點增加；當點打印得更小時，就會出現點丟失。 使用此選項，傳輸函式將作為檔案的一部分保留，並在輸出檔案時應用於檔案。

**套用：** 不保留傳輸功能，但會將它套用至檔案，如此會變更檔案中的顏色。此選項對於在檔案中建立色彩效果非常有用。 根據預設，會針對新設定選取此選項。

**刪除：刪** 除所有應用的轉移函式。移除已套用的轉移功能，除非PDF檔案會輸出至原始PostScript檔案所用的相同裝置。
...
**保留第2級組排文字：** 使用在LanguageLevel 2 PostScript中定義的組排文字運算子，而非在LanguageLevel 3 PostScript中。如果您有PostScript檔案並選取此選項，則會有複製頁面運算子複製頁面。 如果未選取此選項，則會執行相當於showpage操作的操作，但圖形狀態未重新初始化。

**保留疊印設定：** 保留轉換為PDF之檔案中的任何疊印設定。疊印的顏色是兩個或兩個以上相互疊印的油墨。 例如，當青色油墨印在黃色油墨上時，產生的疊印是綠色。 若未疊印，則不會列印底色的黃色，產生青色。
...
**將可攜式工作票證儲存在PDF檔案中：** 將PostScript工作票證保留在PDF檔案中。作業票證包含有關PostScript檔案的資訊，例如頁面大小、解析度和補漏白資訊，而非有關內容的資訊。 這些資訊稍後可用於工作流程或列印PDF。

**使用Prolog.ps和Epilog.ps：將prolog和** exilog檔案與每個作業一起發送。這些檔案有許多用途。 例如，可以編輯prolog檔案以指定封面頁。 可編輯尾檔案以解析PostScript檔案中的一系列過程。 您可以上傳或下載檔案。 （請參閱上傳和下載prolog和結尾檔案。）

**處理DSC注釋：** 維護PostScript檔案的DSC資訊。這些子選項可用：

**記錄DSC警告：顯** 示處理過程中有問題的DSC注釋的警告消息，並將其添加到日誌檔案。

**保留來自DSC的EPS資訊：保** 留資訊，如EPS檔案的源應用程式和建立日期。如果取消選取此選項，頁面會根據頁面上左上角物件的左上角和右下角物件的右下角來調整大小並置中。
...
## 標準報告和合規性選項{#standards-reporting-and-compliance-options}
...
PDF/A相容檔案主要用於封存。 因為長期保存是目標，所以檔案必須只包含在檔案整個預定生命週期中開啟和檢視所需的內容。 例如，PDF/A相容檔案只能包含文字、點陣影像和向量物件；它們不能包含加密和指令碼。 此外，所有字型都必須內嵌，才能開啟檔案並視其建立而檢視。 換言之，PDF/A相容檔案比PDF/X相容檔案&#x200B;*更薄*，適用於高階製作。
...
**不相容時：指** 定如果PostScript檔案不符合PDF/X要求，是否要建立PDF檔案。當「標準報告與法規遵循」頁面上的Compliance Standard設為「無」以外的選項時，此選項可用。

**繼續：** 建立PDF檔案。

**取消工作：** 只有在PostScript檔案符合所選報表選項的PDF/X要求且其他有效時，才建立PDF檔案。如果同時選取PDF/X報表選項，而PostScript檔案僅符合一組PDF/X標準（例如PDF/X-3）,PDF產生器會建立相容檔案。

**如果未指定TrimBox或ArtBox：當「標準報告與合規性」頁面上的** Compliance Standard設為「無」以外的選項時，即可使用。

**報告為錯誤：** 如果選取其中一個報告選項且任何頁面中都遺失修剪方塊或圖稿方塊，則將PostScript檔案標示為不相容。

**將「修剪框設定為帶偏移的媒體框」(Set TrimBox To MediaBox With Offsets)：如果未指定修剪框或藝術框，則根據各頁的媒體框的偏移計算修剪框的點值。** 修剪框始終小於封閉介質框。

**如果未指定出血框：當「標** 準報告和合規性」頁上的Compliance Standard設定為「無」以外的選項時，可使用出血框。

**將出血框設定為MediaBox:** 如果未指定出血框，則使用出血框的媒體框值。

**將「出血框設定為帶偏移的修剪框：如果未指定出血框，則根據各頁的修剪框的偏移計算出血框的點值。** 出血框總是比封閉修剪框大。

**如果未在「文檔」中指定預設值：當「標準報告和合規性」頁上的Compliance Standard設定為「無」以外的選項時，此選項可用。** 

**輸出方式描述檔名** 稱：指出檔案所準備的特徵列印條件。如果文檔未指定OutputIntent名稱，PDF生成器將使用此菜單中的選定值。 您可以選擇提供的名稱之一，或在提供的空間中輸入名稱。 如果工作流要求文檔指定輸出方式，請選擇「無」。 任何不符合要求的檔案都無法進行符合性檢查。

**輸出條件標識** 符：指示輸出方式配置檔案名稱的註冊表所指定的引用名稱。

**輸出條件：** 說明預期的列印條件。此項目對於PDF檔案的預定接收方非常有用。

**註冊表名稱(URL)：指** 出網址，以取得註冊表的詳細資訊。系統會自動為ICC註冊表名稱輸入URL。

**陷印：** 表示文檔中的陷印狀態。PDF/X合規性要求值為True或False。 如果檔案未指定陷印狀態，則會使用此處提供的值。 如果您的工作流程要求檔案指定陷印狀態，請選取「保留未定義」。 任何不符合要求的檔案都無法進行符合性檢查。
...
**取消工作：** 只有在PostScript檔案符合PDF/A要求且其他有效時，才建立PDF檔案。

**輸出方式描述檔名** 稱：指出已準備檔案且符合PDF/A規範的特定列印條件。如果您的工作流要求文檔指定「輸出方式」資訊，請選擇「無」。 如果未提供此資訊，檔案將無法進行符合性檢查。
...
**顯示：依** 預設，會決定哪些窗格和標籤會顯示在應用程式視窗中。「書籤面板」和「頁面」會開啟檔案窗格並顯示「書籤」標籤。
...
**將視窗調整為初始頁面：** 根據您在「檔案選項」下選取的選項，調整檔案視窗以配合開啟頁面的周圍。

**畫面上的中央視** 窗：將視窗置於畫面區域的中央。

**在全螢幕模式中開啟：** 最大化檔案視窗並顯示檔案，而無選單列、工具列或視窗控制項。

**Show:** Filename在窗口的標題欄中顯示檔案名。文檔標題在窗口的標題欄中顯示文檔標題。
