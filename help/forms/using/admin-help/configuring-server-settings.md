...
* **啟用** 傳出電子郵件訊息的電子郵件設定，以及這些訊息使用的電子郵件伺服器設定。（請參閱[設定電子郵件設定](configuring-server-settings.md#configuring-email-settings)。）
...
* **管理員通** 知設定，可啟用、停用或修改管理工作的電子郵件通知中傳送的訊息。（請參閱[為管理員配置通知](configuring-server-settings.md#configuring-notifications-for-administrators)。）
...
若要讓AEM表單接收和處理使用者傳入的電子郵件訊息，請為「完成工作」服務建立電子郵件端點。 （請參閱[為完成任務服務建立電子郵件端點](/help/forms/using/admin-help/configuring-email-endpoints.md#create-an-email-endpoint-for-the-complete-task-service)）。
...
1. 在「電子郵件地址」方塊中，輸入要作為表單工作流程傳送之電子郵件回傳地址的電子郵件地址。
   ...
   >[!NOTE]
   >
   >如果您使用Microsoft Exchange Server且電子郵件地址是無效的電子郵件地址，則Microsoft Exchange伺服器無法向分發清單發送電子郵件。 要解決此問題，請為Microsoft Exchange伺服器上的每個分發清單分別選擇&#x200B;**啟用外部通信**&#x200B;選項。
...
依預設，AEM表單傳送的電子郵件包含（JEE上的AEM表單已過時）Flex工作區的連結。 您可以設定AEM表格，以傳送含有AEM表格工作區連結的電子郵件。 若要進一步瞭解AEM Forms Workspace的優點，而非（JEE上的AEM Forms已過時）Flex Workspace，請參閱[this](/help/forms/using/features-html-workspace-available-flex.md)文章。
...
### 為{#configure-reminders-for-users-or-groups}使用者或群組設定提醒
...
1. 在「主旨」方塊中，輸入電子郵件訊息主旨行的文字。 此欄位會預先填入預設文字。 如需自訂此欄位的詳細資訊，請參閱[自訂通知的內容](configuring-server-settings.md#customizing-the-content-of-notifications)。
1. 在「通知範本」方塊中，輸入電子郵件內文的文字。 此欄位會預先填入預設文字。 如需自訂此欄位的詳細資訊，請參閱[自訂通知的內容](configuring-server-settings.md#customizing-the-content-of-notifications)。
...
### 為{#configure-task-assignment-notifications-for-users-or-groups}用戶或組配置任務分配通知
...
### 設定使用者或群組{#configure-deadline-notifications-for-users-or-groups}的期限通知
...
### 隱藏所有電子郵件{#hide-the-do-not-delete-tag-for-all-emails}的DO NOT DELETE標籤

您可以在以人為中心的流程中傳送的所有電子郵件中，將電子郵件設定為隱藏至「不刪除」追蹤標籤。 如需詳細資訊，請參閱[如何使用CSS](https://blogs.adobe.com/LiveCycleHelp/2013/09/how-to-hide-the-do-not-delete-tag-with-css.html)隱藏&#39;DO-NOT-DELETE&#39;標籤

## 為管理員{#configuring-notifications-for-administrators}配置通知
...
## 自訂通知的內容{#customizing-the-content-of-notifications}
...
如果分支編號為4868，而伺服器名稱為`ServerXYZ`，則管理員會收到一封包含以下內容的電子郵件：
