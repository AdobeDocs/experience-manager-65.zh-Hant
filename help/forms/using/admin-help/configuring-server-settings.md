...
* **電子郵件設定** 啟用外寄電子郵件訊息，以及用於這些訊息的電子郵件伺服器設定。 (請參閱 [正在設定電子郵件設定](configuring-server-settings.md#configuring-email-settings).)
...
若要讓AEM表單接收並處理來自使用者的傳入電子郵件訊息，請為「完成任務」服務建立電子郵件端點。 (請參閱 [建立Complete Task服務的電子郵件端點](/help/forms/using/admin-help/configuring-email-endpoints.md#create-an-email-endpoint-for-the-complete-task-service))。
...
1. 在「電子郵件地址」方塊中，輸入電子郵件地址，用作Forms Workflow所傳送電子郵件訊息的回傳地址。
   ...
   >[!NOTE]
   >
   >如果您使用Microsoft Exchange Server，而且電子郵件地址為無效的電子郵件地址，則Microsoft Exchange Server無法傳送電子郵件至通訊群組清單。 若要解決問題，請選取 **啟用外部通訊** 選項分別適用於Microsoft Exchange伺服器上的每個通訊群組清單。
...
依預設，AEM表單傳送的電子郵件會包含指向(JEE不建議用於AEM表單) Flex工作區的連結。 您可以設定AEM表單，以傳送包含AEM Forms工作區連結的電子郵件。 若要進一步瞭解AEM Forms工作區相對於(JEE上的AEM表單已棄用) Flex工作區的優點，請參閱 [此](/help/forms/using/features-html-workspace-available-flex.md) 文章。
...
* OpenTask可用於「提醒」和「任務指派」通知型別。 此URL提供工作區中工作的連結，可讓使用者從電子郵件通知快速存取工作。 當您將OpenTask URL拖曳到「通知範本」方塊時，URL的格式如下：

  `https://@@notification-host@@:<PORT>/workpace/Main.html?taskId=@@taskid@@`

* ClaimTask可用於「群組 — 提醒」和「群組 — 任務指派」通知型別。 此URL提供工作區中任務詳細資訊頁面的連結，使用者可以在其中宣告或宣告並開啟工作專案。 當您將ClaimTask URL拖曳到「通知範本」方塊時，URL的格式如下：
...
如果您的解決方案部署在叢集環境中，請將 `@@notification-host@@` 使用叢集位址。

`<`*連線埠* `>` 是應用程式伺服器之HTTP接聽程式的連線埠號碼。 支援的應用程式伺服器的預設HTTP監聽器連線埠如下：
...
若要讓這些URL正常運作，請將 `<`*連線埠* `>` 搭配適合您環境的連線埠號碼。
...
「變數選取器」清單提供有用的變數，您可以將其拖放至「主旨」或「通知範本」方塊中。 將變數放入「主旨」或「通知範本」方塊時，它會變成實際表單工作流程變數名稱，其兩側會有兩個@符號，例如 `@@taskid@@`.

對於使用者和群組的提醒、任務指派和截止日期，您可以在「主旨」和「通知範本」方塊中使用下列變數：

**說明** 「摘要」屬性的內容，如在「維護作業」中處理之使用者步驟（起點、「指定作業」作業或「指定多重作業」作業）中所定義。

**指示** 「工作指示」屬性的內容，如Workbench中處理作業的使用者步驟中所定義。
