...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul>
     <li><p>自訂身分密鑰庫：<code>[</code><i>appserverdomain<code>]</code></i><code>/adobe/</code><i>[server name]</i><code>/ads-ssl.jks</code></p></li>
     <li><p>自訂信任金鑰庫：<code>[</code><i>appserverdomain<code>]</code></i><code>/adobe/</code><i>[server name]</i><code>/ads-ca.jks</code></p></li>
    </ul></td>
  </tr>
  <tr>
   <td><p>-file</p></td>
  ...
  <tr>
   <td><p>-validity</p></td>
   <td><p>認為證書有效的天數。</p></td>
   <td><p>三六五零年</p><p>您可以使用不同的值，視您公司的政策而定。</p></td>
  </tr>
  <tr>
   <td><p>-storepass</p></td>
  ...
  <tr>
   ...
   <td><p>請使用與<code>-storepass</code>選項相同的密碼。 密鑰密碼必須至少為6個字元。</p></td>
  </tr>
  <tr>
   <td><p>-dname</p></td>
...
## 建立自訂身分與信任關鍵存放區{#create-the-custom-identity-and-trust-keystores}
...
1. 輸入以下命令，從ads-credentials密鑰庫中提取證書：
   ...
   >[!NOTE]
   >
   >將`[JAVA_HOME]`取代為安裝JDK的目錄，並將&#x200B;`store`*_* `password`*取代為自訂身分密鑰庫的密碼。*
...
1. 在「環境」下的「域配置」中，選擇「**伺服器> &#39;server&#39; >配置>常規」。**
1. 在「General（常規）」下，在「Configuration（配置）」中，確保選擇了&#x200B;**Listen Port Enabled（啟用監聽埠**）和&#x200B;**SSL Listen Port Enabled（啟用SSL監聽埠**）。 如果未啟用，請執行下列動作：

   1. 在「變更中心」下，按一下「鎖定並編輯」以修改選取範圍和值。****
   1. 選中&#x200B;**啟用監聽埠**&#x200B;和&#x200B;**啟用SSL監聽埠**&#x200B;複選框。

1. 如果此伺服器是受控伺服器，請將「監聽埠」更改為未使用的埠值（如8001），將「SSL監聽埠」更改為未使用的埠值（如8002）。 在獨立伺服器上，預設的SSL埠為7002。
1. 按一下&#x200B;**Release Configuration**。
1. 在「環境」下的「域配置」中，按一下「伺服器」>「受管理伺服器」>「配置」>「常規」。**[**]**
1. 在「常規」下，在「配置」中，選擇&#x200B;**Keystores**。
1. 在「變更中心」下，按一下「鎖定並編輯」以修改選取範圍和值。****
1. 按一下「**變更**」，以將密鑰庫清單取為下拉式清單，並選取「自訂身分與自訂信任」**。**
1. 在「身分」下，指定下列值：
