...
1. 在 **金鑰存放區使用情況** 下拉式清單，確定 **SSL金鑰存放區** 已選取。 按一下 **新增**.
1. 輸入邏輯名稱和說明。
...
1. 在 **設定** 標籤，下 **通訊** 區段，展開 **連線埠**，然後按一下 **詳細資料**.
...
1. 找到 `sling.bootdelegation.ibm` 屬性和新增 `com.ibm.websphere.ssl.*`至其值欄位。 更新的欄位如下所示：
