...
**重複間隔：** 掃描watched資料夾以進行輸入的間隔（以秒為單位）。 除非啟用「節流閥」設定，否則「重複間隔」應比處理平均作業的時間長；否則，系統可能會過載。 預設值為 5。如需詳細資訊，請參閱「批次大小」的說明。
...
**等待時間：** 建立資料夾或檔案後，掃描資料夾或檔案前的等待時間（毫秒）。 例如，如果等待時間為3,600,000毫秒（一小時），且檔案是在一分鐘前建立的，則系統會在59分鐘或更長時間後擷取此檔案。 預設值為 0。
