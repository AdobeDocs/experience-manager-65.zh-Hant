---
title: 控制對受策略保護文檔的訪問
seo-title: 控制對受策略保護文檔的訪問
description: 了解如何查看、管理和控制對受策略保護文檔的訪問。
seo-description: 了解如何查看、管理和控制對受策略保護文檔的訪問。
...
# 控制對受策略保護文檔{#controlling-access-to-policy-protected-documents}的訪問
...
## 查看有關文檔{#view-information-about-a-document}的資訊

對於「文檔」頁上列出的每個文檔，您可以看到文檔名、發佈者名、策略名和文檔受保護的日期。 如果已刪除保護文檔的策略，則該策略ID將列在「策略名稱」下。
...
**文檔ID:** 將策略應用於文檔時，文檔安全性指定的唯一標識符。文檔安全性使用此編號來跟蹤文檔。
...
**相關小版本：** 如果文檔具有相關小版本，則此項目也會出現在清單中。按一下連結可查看文檔的相關小版本清單。
...
## 查看文檔{#view-related-iterations-for-a-document}的相關小版本
...
## 撤消和恢復對文檔{#revoking-and-reinstating-access-to-documents}的訪問

您可以撤消並恢復對受策略保護文檔的訪問：

**用戶：** 可以撤消或恢復對使用其個人策略或共用策略保護的文檔的訪問，其撤消功能已針對應用該策略的用戶啟用。無法撤消對文檔的訪問或切換策略的用戶需要與管理員聯繫。

**管理員：** 可以撤銷或恢復對任何受策略保護的文檔（包括受個人或共用策略保護的文檔）的訪問權限。如果管理員撤消對受共用策略保護的文檔的訪問，則只有管理員才能恢復該文檔的訪問權限。

**策略集協調者：** 可以撤消或恢復策略集所保護的文檔的訪問權限。
...
## 切換應用到文檔{#switch-a-policy-that-is-applied-to-a-document}的策略
...
**發佈者名稱：** 將策略附加到文檔的用戶名。您可以從所有網域或指定的網域中選取使用者。

**策略ID:** 附加到文檔的策略的ID號。

**策略名稱：** 附加到文檔的策略的名稱。

**所有檔案：** 受管理員和使用者保護的所有檔案。使用「所有文檔」選項進行搜索可返回一長串文檔。
...
1. 在「查找」清單中，選擇所需的搜索標準。
   ...
   如果指定發佈商名稱，請按一下「通訊簿」表徵圖並指定要搜索用戶的域，然後按一下「確定」返回「文檔」搜索頁。
...
## 對文檔清單{#sort-the-document-list}排序
