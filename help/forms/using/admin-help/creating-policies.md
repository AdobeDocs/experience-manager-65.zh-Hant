...
*policy*&#x200B;定義了一組機密設定和可訪問策略所應用文檔的用戶。 *策略集*&#x200B;用於對一組具有共同業務目的的策略進行分組。 然後，這些策略集將提供給系統中的用戶子集。 有關策略的詳細資訊，請參閱[策略和受策略保護的文檔](/help/forms/using/admin-help/document-security.md#policies-and-policy-protected-documents)。
...
1. 將使用者和群組新增至原則並設定適當的權限。 （請參閱[使用者和群組](creating-policies.md#users-and-groups)）。
...
1. 在「不可更改的高級設定」下，選擇相應的選項。 （請參閱[不可更改的高級設定](creating-policies.md#unchangeable-advanced-settings)。）
1. 按一下「儲存」。策略將出現在策略清單中。 新策略旁邊會出現一個帶有紅色圓圈的表徵圖，表示該表徵圖仍處於禁用狀態。

   若要讓原則可供使用者使用，請啟用它。 （請參閱[啟用或禁用共用策略](creating-policies.md#enable-or-disable-shared-policies)。）
