...
有關DB2資料庫備份和恢復的詳細資訊，請參閱[開發DB2](https://publib.boulder.ibm.com/infocenter/db2luw/v9/index.jsp?topic=/com.ibm.db2.udb.admin.doc/doc/c0005945.htm)的備份和恢復策略。
...
[Oracle備份和恢復： ](https://www.oracle.com/technetwork/database/features/availability/br-overview-097160.html) 更詳細地說明了備份和恢復的概念以及使用Recovery Manager(RMAN)進行備份、恢復和報告的最常見技術，並提供了有關如何規劃備份和恢復策略的詳細資訊。
...
有關詳細資訊，請參閱[備份和還原](https://msdn.microsoft.com/en-us/library/ms187048(v=SQL.90).aspx)。

### MySQL {#mysql}

使用MySQLAdmin或在Windows中修改INI檔案，以配置MySQL資料庫以二進位日誌模式運行。 （請參閱[MySQL二進位記錄](https://dev.mysql.com/doc/refman/5.1/en/binary-log.html)。） InnoBase軟體還提供了MySQL的熱備份工具。 （請參閱[Innobase熱備份](https://www.innodb.com/hot-backup/features.md)。）
...
您可以使用mysqldump實用程式獲取完整資料庫備份。 需要完整備份，但並不總是方便。 它們會生成大型備份檔案，並需要時間才能生成。 要執行增量備份，請確保使用 — `log-bin`選項啟動伺服器，如前一節所述。 每次MySQL伺服器重新啟動時，它都停止寫入當前的二進位日誌，建立新的日誌，然後，新的日誌就變成當前日誌。 可以使用`FLUSH LOGS SQL`命令手動強制交換機。 第一次完全備份後，使用mysqladmin實用程式和`flush-logs`命令完成後續增量備份，該命令將建立下一個日誌檔案。
...
**內容儲存根目錄：** 通常是可供叢集中所有節點讀/寫存取的共用網路目錄
...
索引根目錄在每個節點上的預設位置為`[aem-forms root]/lucene-indexes`。
...
>[!NOTE]
>
>預設情況下，隨AEM Forms安裝的Adobe字型位於`[aem-forms root]/fonts`目錄中。
