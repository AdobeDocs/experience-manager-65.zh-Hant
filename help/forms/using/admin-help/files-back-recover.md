...
GDS的位置是在AEM表單安裝程式期間或更新版本中，使用管理控制台來決定。 除了保留GDS的高可用性位置外，您還可以為文檔啟用資料庫儲存。 請參見[將資料庫用於文檔儲存時的備份選項](files-back-recover.md#backup-options-when-database-is-used-for-document-storage)。
...
如果原始位置不再可用，則在恢復期間可更改GDS的位置。 （請參閱[在恢復過程中更改GDS位置](/help/forms/using/admin-help/recovering-aem-forms-data.md#changing-the-gds-location-during-recovery)。）

### 資料庫用於文檔儲存{#backup-options-when-database-is-used-for-document-storage}時的備份選項
...
有關DB2資料庫備份和恢復的詳細資訊，請參見[開發DB2](https://publib.boulder.ibm.com/infocenter/db2luw/v9/index.jsp?topic=/com.ibm.db2.udb.admin.doc/doc/c0005945.htm)的備份和恢復策略。

### Oracle {#oracle}

使用快照備份或將Oracle資料庫配置為在歸檔日誌模式下運行。 (請參見[Oracle備份：簡介](https://www.databasedesign-resource.com/oracle-backup.md)。) 有關備份和恢復Oracle資料庫的詳細資訊，請轉至以下站點：

[Oracle備份和恢復：](https://www.oracle.com/technetwork/database/features/availability/br-overview-097160.html) 更詳細地解釋備份和恢復的概念以及使用Recovery Manager(RMAN)進行備份、恢復和報告的最常用技術，並提供有關如何規劃備份和恢復策略的詳細資訊。

[Oracle Database Backup and Recovery User&#39;s Guide:](https://download.oracle.com/docs/cd/E11882_01/backup.112/e10642.pdf)  Provides indutional information about RMAN體系結構、備份和恢復概念和機制、高級恢復技術（如時間點恢復和資料庫閃回功能），以及備份和恢復效能調整。它還包括使用主機作業系統功能（而非RMAN）的用戶管理備份和恢復。 此卷對於備份和恢復更複雜的資料庫部署以及高級恢複方案至關重要。

[Oracle Database Backup and Recovery Reference：提](https://download.oracle.com/docs/cd/E11882_01/backup.112/e10643.pdf) 供有關所有RMAN命令的語法和語義的完整資訊，並說明可用於報告備份和恢復活動的資料庫視圖。
...
有關詳細資訊，請參見[備份和還原](https://msdn.microsoft.com/en-us/library/ms187048(v=SQL.90).aspx)。

### MySQL {#mysql}

使用MySQLAdmin或修改Windows中的INI檔案，以配置MySQL資料庫以二進位日誌模式運行。 （請參見[MySQL二進位日誌](https://dev.mysql.com/doc/refman/5.1/en/binary-log.html)。） InnoBase軟體也提供了MySQL的熱備份工具。 （請參見[Innobase熱備份](https://www.innodb.com/hot-backup/features.md)。）
...
您可以使用mysqldump實用程式獲得完整的資料庫備份。 需要完整備份，但並不總是方便的。 它們會生成大型備份檔案，並需要時間生成。 要執行增量備份，請確保使用- `log-bin`選項啟動伺服器，如上節所述。 每次MySQL伺服器重新啟動時，它都停止寫入當前二進位日誌，建立新日誌，然後，從此開始，新日誌將變為當前日誌。 可以使用`FLUSH LOGS SQL`命令手動強制交換機。 在第一次完全備份後，使用mysqladmin實用程式和`flush-logs`命令完成後續增量備份，該命令將建立下一個日誌檔案。
...
**內容儲存根目錄：通** 常是可供群集中所有節點讀／寫訪問的共用網路目錄

**索引根目錄：** 在群集中每個節點上建立的目錄，始終具有相同的路徑和目錄名

內容儲存根目錄的預設位置為`[GDS root]/lccs_data`，其中`[GDS root]`是[GDS位置](files-back-recover.md#gds-location)中描述的位置。 備份位於「內容儲存根目錄」中的以下目錄：
...
每個節點上的索引根目錄的預設位置為`[aem-forms root]/lucene-indexes`。
...
>[!NOTE]
>
>依預設，隨AEM表單安裝的Adobe字型位於`[aem-forms root]/fonts`目錄中。
