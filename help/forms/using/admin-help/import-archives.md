...
# 導入和管理歸檔{#import-and-manage-archives}
...
1. 如果按一下「配置」，將顯示「配置端點」頁，您可以在其中進行任何需要的更改：
   ...
   * 要更名端點或編輯其說明，請按一下該端點。
   * 要添加任務管理器端點，請按一下添加任務管理器。 有關Task Manager設定的詳細資訊，請參閱[配置Task Manager端點](/help/forms/using/admin-help/configuring-task-manager-endpoints.md#configuring-task-manager-endpoints)。
   * 若要新增「監看資料夾」端點，請按一下「新增WatchedFolder」。 有關「監視資料夾」設定的詳細資訊，請參閱[「監視資料夾」端點設定](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#watched-folder-endpoint-settings)。
   * 若要新增電子郵件端點，請按一下「新增電子郵件」。 如需「電子郵件」設定的詳細資訊，請參閱「電子郵件端點設定」[。](/help/forms/using/admin-help/configuring-email-endpoints.md#email-endpoint-settings)
   * 要添加EJB端點，請按一下「添加EJB」並指定端點的名稱和說明。
   * 要添加SOAP端點，請按一下「添加SOAP」並指定端點的名稱和說明。
   * 要添加遠程端點，請按一下添加遠程。 有關「遠程」設定的詳細資訊，請參閱[遠程端點設定](/help/forms/using/admin-help/configuring-remoting-endpoints.md#remoting-endpoint-settings)。
...
1. 在「配置安全性配置檔案」頁上，進行任何需要的更改：
   ...
   * **要求呼叫者驗證：此** 設定指出是否可使用認證來呼叫服務。

      如果顯示&#x200B;*當前需要呼叫者來驗證*，則必須驗證服務呼叫者，並且必須授權該呼叫者的用戶主體來調用該服務；否則，將拒絕調用。 若要移除驗證的需要，請按一下「允許未驗證的呼叫者」。

      如果顯示&#x200B;*不需要呼叫者來驗證*，則無需驗證服務呼叫者。 由於沒有授權檢查，因此服務的調用始終會成功。 要要求驗證，請按一下要求呼叫者驗證。

   * **運行方式：** 指定調用服務後由服務使用的運行時標識。若要變更此選項，請按一下「變更」。 從下列選項中選擇：

      **未指定：** 使用預設行為。

      **調用者：** 使用與呼叫服務之使用者相同的身分。
   ...
   * 要將承擔者添加到安全配置檔案中，請按一下「添加承擔者」(Add Principal)，然後選擇要添加為承擔者的用戶或組。 按一下「下一步」(Next)，然後選擇要分配給此承擔者的權限：
      ...
      **START_STOP_PERM：啟** 動和停止服務

      **ADD_REMOVE_ENDIPTS_PERM：要** 添加、刪除和修改服務的端點

      **CREATE_VERSION_PERM:** 要建立服務的新版本

      **DELETE_VERSION_PERM：刪** 除服務的版本

      **MODIFY_VERSION_PERM:** 要修改服務的版本
...
## 設定屬於封存檔案{#configure-the-aem-forms-that-are-part-of-an-archive-file}一部分的AEM表單
...
## 使用設定精靈來設定屬於封存檔案{#use-the-configuration-wizard-to-configure-the-aem-forms-that-are-part-of-an-archive-file}一部分的AEM表單
...
1. 在「配置安全性配置檔案」頁上，您可以進行任何需要的更改：
   ...
   * **要求呼叫者驗證：此** 設定指出是否可使用認證來呼叫服務。
      ...
      如果顯示&#x200B;*不需要呼叫者來驗證*，則服務呼叫者可能被驗證，也可能不被驗證。 由於沒有授權檢查，因此服務的調用始終會成功。 要要求驗證，請按一下要求呼叫者驗證。
