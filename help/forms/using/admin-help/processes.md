...
1. 在管理主控台中，按一下「服務>表單工作流程>表單工作流程」。 「流程清單」顯示以下資訊：
   ...
   **應用程** 序：流程所屬的應用程式，如Workbench中定義。

   **狀態：** 活動表示流程是為流程版本激活的流程。非活動」表示流程是舊版，仍有流程實例。

   **建立日期：** 部署流程的日期和時間。

1. 按一下進程名稱，在「進程實例」頁上查看其進程實例。

## 使用進程實例{#working-with-process-instances}

如果從「流程清單」頁訪問「流程實例」頁，則會列出所選流程的所有流程實例。 如果在執行搜索後訪問「流程實例」頁，則只列出找到的流程實例。

對於每個流程實例，清單顯示以下資訊：

**流程ID:** 當流程實例化時（即當用戶或自動步驟啟動流程時），表單工作流所指派的標識符。您可以使用此標識符跟蹤流程實例的生命週期。

**流程名稱——版** 本：流程名稱，如Workbench中定義。

**狀態：指** 出流程實例是正常運行、更改狀態還是已停止。（請參閱關於流程實例狀態。）

**建立日期：** 建立流程實例的日期和時間。

**更新日期：** 流程實例狀態上次更改的日期和時間。
...
### 關於進程實例狀態{#about-process-instance-statuses}

流程實例（包括子流程）可具有以下狀態：

**COMPLETE:** 流程實例中的所有分支和操作都已完成。COMPLETE是流程實例的最終狀態。

**完成：** 流程實例的狀態即將更改為「完成」。

**INITIATED:** 流程實例已建立但尚未運行。INITIATED是進程實例的第一個狀態。

**運行：** 進程實例正常運行。自動步驟可能正在進行中，或者進程實例可能正在接收用戶輸入或等待用戶交互。

**SUSPENDED:** 進程實例已由管理員或進程中的步驟暫停。在狀態變更之前，不會再執行任何操作。

**暫停：** 狀態即將變更為暫停。如果某個操作被設計為忽略暫停請求且尚未完成，則該操作必須在進程實例暫停之前完成。

**TERMINATED:** 進程實例已由管理員終止。

**終止：** 狀態即將更改為TERMINATED。如果某個操作被設計為忽略終止請求，但尚未完成，則該操作必須在流程實例終止之前完成。

**取消掛** 起：狀態在掛起後即將變更為「運行」。
...
## 使用進程實例詳細資訊{#working-with-process-instance-details}
...
**狀態：指** 出操作是正常運行還是已停止。（請參閱關於流程實例狀態。）
...
**流程ID:** 當流程實例化時（即當用戶或自動步驟啟動流程時），表單工作流分配的正整數。您可以使用此標識符跟蹤流程實例的整個生命週期。

**流程名稱——版** 本：流程的名稱，如設計器中定義。

**狀態：指** 出流程實例是正常運行、更改狀態還是停止。（請參閱關於流程實例狀態。）

**建立日期：** 建立子流程的日期和時間。

**更新日期：** 上次更改子流程狀態的日期和時間。
...
**運行：** 操作正常運行。它可能接收用戶輸入或等待用戶交互，或者自動化步驟可能正在進行。

**STALTED:** 正在處理操作時發生問題。在「停止的操作」頁中檢查錯誤或異常。
