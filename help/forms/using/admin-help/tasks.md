...
**工作ID：** 當任務例項化（由使用者啟動）時，形成工作流程指派的正整數。 您可以使用此識別碼來追蹤整個生命週期中的任務。 按一下工作ID可檢視工作歷程記錄的詳細資訊，或將工作重新指派給其他使用者。

**狀態：** 已指派表示任務目前已指派給使用者。 未指派表示任務先前已指派給使用者。 狀態也可以是「已完成」。

**活動：** 顯示初始作業或產生工作的程式作業的表單和名稱。

**程式ID：** 當程式例項化時（即當使用者或自動步驟啟動程式時），表單工作流程指派的正整數。 您可以使用此識別碼來追蹤流程例證整個生命週期。
...
**提升：** 經過的預定時間（如Workbench中的「使用者」動作中所設定），沒有使用者互動且另一個使用者被指派此任務。

**查詢：** 工作擁有者已將此工作轉送給其他使用者，以便進行諮詢，這些使用者可以開啟表單、儲存資料、修改附件和附註，但無法完成此步驟。 使用者必須將任務傳回給與使用者諮詢的任務擁有者。
