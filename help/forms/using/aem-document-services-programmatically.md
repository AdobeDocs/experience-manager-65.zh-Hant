...
* [簽名服務Java API快速入門](https://experienceleague.adobe.com/docs/experience-manager-65/forms/developer-reference/programming-aem-forms-jee/java-api-quick-start-code-examples/signature-service-java-api-quick.html?lang=zh-Hant&#programming-aem-forms-jee)

* [加密服務Java API快速入門](https://experienceleague.adobe.com/docs/experience-manager-65/forms/developer-reference/programming-aem-forms-jee/java-api-quick-start-code-examples/encryption-service-java-api-quick.html?lang=zh-Hant&#developer-reference)

* [Acrobat Reader擴充功能服務Java API快速入門](https://experienceleague.adobe.com/docs/experience-manager-65/forms/developer-reference/programming-aem-forms-jee/java-api-quick-start-code-examples/acrobat-reader-dc-extensions-service.html?lang=zh-Hant&#developer-reference)

## 必備條件 {#prerequisite}

* 在使用DocAssurance服務API之前，[設定DocAssurance服務](/help/forms/using/install-configure-document-services.md)。

* 使用您的AEM Forms maven專案下載及設定[AEM使用者端SDK](https://helpx.adobe.com/tw/aem-forms/kb/aem-forms-releases.html)。 使用AEM檔案服務建置Maven專案所需的使用者端類別可在[AEM Forms使用者端SDK](https://helpx.adobe.com/tw/aem-forms/kb/aem-forms-releases.html)中使用
...
* [新增簽章欄位](/help/forms/using/aem-document-services-programmatically.md#p-adding-a-signature-field-nbsp-p)
* [套用文件時間戳記](/help/forms/using/aem-document-services-programmatically.md#apply-document-timestamp)

* [取得簽章](/help/forms/using/aem-document-services-programmatically.md#p-getting-signature-p)
* [取得簽章欄位清單](/help/forms/using/aem-document-services-programmatically.md#p-getting-signature-field-list-nbsp-p)
* [修改簽章欄位](/help/forms/using/aem-document-services-programmatically.md#p-modifying-signature-fields-nbsp-p)
...
數位簽章會出現在簽章欄位中，這些欄位是包含簽章圖形表示的表單欄位。 簽章欄位可以顯示或隱藏。 簽名者可以使用預先存在的簽名欄位，也可以以程式設計方式新增簽名欄位。 無論何種情況，簽章欄位都必須存在，PDF檔案才能簽署。 您可以使用簽名服務Java API或簽名網站服務API，以程式設計方式新增簽名欄位。 您可以將多個簽名欄位新增到PDF檔案中。 但是，每個簽名欄位名稱必須是唯一的。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><code>FieldMDPOptionSpec</code>物件，指定簽名欄位簽署後鎖定的PDF檔案欄位。 此引數為選用引數，可接受null值。</td>
  </tr>
  <tr>
   <td><code>seedValueOptionsSpec</code></td>
   <td>指定欄位各種種子值的<code>SeedValueOptions</code>物件。 T此引數為選用引數，可接受null值。<span class="acrolinxCursorMarker"></span></td>
  </tr>
  <tr>
...
以下是Java程式碼範例，會在PDF檔案中新增隱藏的簽名欄位。
...
您也可以使用[CAdES](https://en.wikipedia.org/wiki/CAdES_%28computing%29)規格來簽署檔案。 使用以下範常式式碼來設定[CAdES.](https://en.wikipedia.org/wiki/CAdES_%28computing%29)的簽署格式
...
您可以使用簽名服務Java API或簽名網站服務API，以程式設計方式新增簽名欄位。 您可以將多個簽名欄位新增至PDF檔案。 但是，每個簽名欄位名稱必須是唯一的。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>簽章欄位的名稱。 此引數為必要引數，無法接受null值。</td>
  </tr>
  <tr>
   <td><code>pageNumber</code></td>
   <td>新增簽名欄位的頁碼。 有效值為1到檔案中包含的頁數。 此引數為必要引數，無法接受null值。<br /> </td>
  </tr>
  <tr>
   <td><code>positionRectangle</code></td>
   <td>指定簽章欄位位置的<code>PositionRectangle object</code>。 此引數為必要引數，無法接受null值。 如果指定的矩形未至少部分位於指定頁面的裁切方塊上，則會擲回<code>InvalidArgumentException</code>。 此外，指定矩形的高度或寬度都不能為0或負數。 左下X或左下Y座標可以為0或更大，但不是負數，而且會與頁面的裁切方塊相關。</td>
  </tr>
  <tr>
   <td><code>fieldMDPOptionsSpec</code></td>
   <td><code>FieldMDPOptionSpec</code>物件，指定簽名欄位簽署後鎖定的PDF檔案欄位。 此為選用引數，可為Null。</td>
  </tr>
  <tr>
   <td><code>seedValueOptionsSpec</code></td>
   <td>指定欄位各種種子值的<code>SeedValueOptions</code>物件。 此為選用引數，可為Null。</td>
  </tr>
  <tr>
   <td><code>unlockOptions</code></td>
   <td>包含解除鎖定加密檔案所需的引數。 只有加密的檔案才需要此引數。</td>
...
以下是Java程式碼的範例，此程式碼會將簽名欄位新增至PDF檔案。
...
您可以依照[PAdES 4](https://en.wikipedia.org/wiki/PAdES)規格以程式設計方式為檔案加上時間戳記。 您也可以針對交易相關檔案使用[CAdES](https://en.wikipedia.org/wiki/CAdES_%28computing%29)規格。
...
下列程式碼範例會根據[PAdES 4](https://en.wikipedia.org/wiki/PAdES)將時間戳記新增至檔案。
...
您可以擷取您要簽署或認證之PDF檔案中所有簽名欄位的名稱。 如果您不確定PDF檔案中的簽名欄位名稱或驗證名稱，則以程式設計方式擷取名稱。 簽章服務傳回簽章欄位的完整名稱，例如`form1[0].grantApplication[0].page1[0].SignatureField1[0]`。
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>signatureFieldName</code></td>
   <td>包含簽章的簽章欄位名稱。 指定簽章欄位的完整名稱。 使用以XFA表單為基礎的PDF檔案時，可以使用簽名欄位的部分名稱。 例如，<code>form1[0].#subform[1].SignatureField3[3]</code>可指定為<code>SignatureField3[3]</code>。</td>
  </tr>
  <tr>
   <td><code>UnlockOptions</code></td>
   <td>包含解除鎖定加密檔案所需的引數。 只有在檔案已加密時，才需要此專案。</td>
...
以下Java程式碼範例會擷取PDF檔案中指定簽名欄位的簽名資訊。
...
您可以擷取您要簽署或認證之PDF檔案中所有簽名欄位的名稱。 如果您不確定PDF檔案中的簽名欄位名稱，可以程式設計方式擷取並驗證它們。 簽章服務傳回簽章欄位的完整名稱，例如`form1[0].grantApplication[0].page1[0].SignatureField1[0]`。
...
您可以修改PDF檔案中的簽名欄位。 修改簽章欄位涉及處理其簽章欄位鎖定字典值或種子值字典值。

欄位鎖定字典指定簽署簽章欄位時鎖定的欄位清單。 鎖定的欄位可防止使用者編輯該欄位。 種子值字典包含套用簽章時使用的限制資訊。 例如，您可以變更在不使簽名失效的情況下控制可能發生的動作的許可權。

透過修改現有簽名欄位，您可以編輯PDF檔案以反映不斷變化的業務需求。 例如，新業務需求要求在簽署檔案後鎖定所有檔案欄位。
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>signatureFieldName</code></td>
   <td>簽名欄位的名稱。 此引數為必要引數，無法接受null值。<br /> </td>
  </tr>
  <tr>
   <td><code>pdfSignatureFieldProperties</code></td>
   <td>指定有關簽章欄位<code>PDFSeedValueOptionSpec</code>和<code>FieldMDPOptionSpec</code>值的資訊的物件。</td>
  </tr>
  <tr>
   <td><code>unlockOptions</code></td>
   <td>包含解除鎖定加密檔案所需的引數。 只有在檔案已加密時，才需要此專案。</td>
...
* 這必須是套用至PDF檔案的第一個簽章。 換言之，當套用認證簽名時，檔案中的其他簽名欄位必須未簽署。 PDF檔案中只允許一個認證簽名。 若要簽署及認證PDF檔案，請在簽署前先進行認證。 在認證PDF檔案後，您可以數位簽署其他簽名欄位。
...
secureDocument可讓您以個別或任意組合，依特定順序加密、簽署/認證及讀取器擴充PDF檔案。 若要存取任何此功能，請傳遞對應的引數。 如果為null，則會假設不需要特定處理。

**使用密碼加密PDF檔案**

使用密碼加密PDF檔案時，使用者必須指定密碼，才能在Adobe Reader或Acrobat中開啟PDF檔案。 此外，在其他AEM Forms Document Services作業使用檔案之前，必須解除鎖定以密碼加密的PDF檔案。
...
>[!NOTE]
>
>您必須確定將憑證新增至PDF信任存放區，才能使用憑證加密AEM檔案。

**套用使用許可權至PDF檔案**

您可以使用PDF擴充功能Java使用者端API和Web服務，將使用許可權套用至Reader檔案。 使用許可權與Acrobat預設提供但不適用於Adobe Reader的功能相關，例如新增註解至表單或填寫表單欄位及儲存表單的功能。 已套用使用許可權的PDF檔案稱為許可權啟用檔案。 在Adobe Reader中開啟許可權啟用檔案的使用者，可以執行為該特定檔案啟用的操作。

在Reader使用憑證延伸PDF檔案之前，您必須確定您將憑證新增至AEM Keystore。

**數位簽署PDF檔案**

數位簽名可套用至PDF檔案，以提供安全等級。 數位簽章（例如手寫簽章）提供簽署者識別自己及就檔案發表宣告的方法。
...
>[!NOTE]
>
>您必須確定已在PDF金鑰存放區中新增認證，才能以數位方式簽署AEM檔案。 認證是用於簽署的私密金鑰。

>[!NOTE]
>
>AEM Forms也支援數位簽署PDF檔案的&#x200B;*[CAdES](https://en.wikipedia.org/wiki/CAdES_%28computing%29)*&#x200B;規格。

**認證PDF檔案**

您可以使用稱為認證簽名的特定簽名型別來認證PDF檔案，以保護其安全。 認證簽名與數位簽名的區別如下：

這必須是套用至PDF檔案的第一個簽章；也就是說，在套用已驗證簽章時，檔案中的任何其他簽章欄位都必須未簽署。

PDF檔案中只允許一個認證簽名。 如果您想要簽署和認證PDF檔案，則必須在簽署前進行認證。
...
**範例1**：此範例用於執行密碼加密、認證簽名欄位及Reader擴充PDF檔案。
...
**範例2**：此範例用於執行PKI加密、簽署簽章欄位及Reader擴充PDF檔案。
...
使用以下專案管理PDF檔案中JavaScripts定義的逾時間隔：
...
若要擷取特定`credentialAlias`所指定之認證的使用權資訊，請從`SecureDocument` API呼叫此API。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>指定認證的<code>credentialAlias</code>。<br /> </td>
  </tr>
  <tr>
   <td><code>credentialPassword</code><br /> </td>
   <td>若認證已加密，則需使用認證的密碼；若認證未加密，則需使用null。<br /> </td>
  </tr>
 </tbody>
</table>
...
若要擷取指定檔案的使用許可權資訊，請從`docAssuranceService`API中呼叫此API。
...
您可以從`docAssuranceService`API中呼叫`removeUsageRights`API來移除檔案的使用許可權。
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>inDocument</code><br /> </td>
   <td>要移除使用許可權的檔案。<br /> </td>
  </tr>
  <tr>
   <td><code>unlockOptions</code><br /> </td>
   <td>包含解除鎖定加密檔案所需的引數。 只有在檔案已加密時才需要此專案。<br /> </td>
  </tr>
 </tbody>
</table>
...
可以驗證數位簽章，以確保已簽署的PDF檔案未修改且數位簽章有效。 驗證數位簽章時，您可以檢查簽章的狀態和簽章的屬性，例如簽署者的身分。 在信任數位簽名之前，建議您先驗證它。 驗證數位簽名時，請參考包含數位簽名的PDF檔案。
 ...
 <tbody>
  ...
  <tr>
   <td><code class="code">signatureField
      Name</code><br /> </td>
   <td>要驗證的簽名欄位名稱。 可指定完整名稱或部分名稱<br /> </td>
  </tr>
  <tr>
   <td><code>revocationCheckStyle</code></td>
   <td>用來控管驗證期間所遇到憑證的撤銷檢查的選項</td>
  </tr>
  <tr>
   <td><code>verificationTime</code></td>
   <td>簽章應驗證的時間</td>
  ...
  <tr>
   ...
   <td>控制各種驗證設定的偏好設定。 若為加密檔案，請使用設定解鎖選項 <code>setUnlockOptions()</code></td>
  </tr>
  <tr>
   <td><code>resourceResolver</code></td>
   <td>Granite信任存放區的資源解析程式</td>
...
這個範常式式碼使用`DocAssuranceService`來驗證加密的PDF檔案中的簽章欄位。
...
AEM可讓您驗證PDF檔案中的數位簽名。 如果PDF檔案必須經過需要由多個簽名者簽名的業務流程，則它可以包含多個數位簽名。 例如，金融交易需要貸款專員和經理的簽名。 您可以使用簽名服務API來驗證PDF檔案中的所有簽名。 驗證多個數位簽名時，您可以檢查每個簽名的狀態和屬性。 在您信任數位簽名之前，Adobe建議您先驗證它。
...
您可以擷取您要簽署或認證之PDF檔案中所有簽名欄位的名稱。 如果您不確定PDF檔案中的簽名欄位名稱，或想要驗證這些名稱，可以程式設計方式擷取它們。 簽章服務傳回簽章欄位的完整名稱，例如`asform1[0].grantApplication[0].page1[0].SignatureField1[0]`。
...
移除PDF檔案中的密碼式加密，讓使用者不必指定密碼，即可在Adobe Reader或Acrobat中開啟PDF檔案。 從PDF檔案中移除密碼式加密後，檔案就不再安全。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>檔案開啟或許可權密碼，用來移除檔案的保全性。<br /> </td>
  </tr>
 </tbody>
</table>
...
您可以從PDF檔案中移除憑證式加密，讓使用者可以在Adobe Reader或Acrobat中開啟PDF檔案。 若要從已使用憑證加密的PDF檔案中移除加密，請參考私密金鑰。 從PDF檔案中移除加密後，其安全性即告降低。
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>inDoc</code><br /> </td>
   <td>代表憑證加密的PDF檔案的Document物件。<br /> </td>
  </tr>
  <tr>
   <td><code>alias</code><br /> </td>
   <td>別名對應至Granite信任存放區中用來從PDF檔案中移除憑證式加密的金鑰。<br /> </td>
  </tr>
  <tr>
   <td><code>ResourceResolver</code></td>
   <td>ResourceResolver，存取特定使用者的金鑰存放區，以擷取認證。</td>
  </tr>
 </tbody>
</table>
...
* **[generatePDFOutputBatch](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutputbatch-p)：**&#x200B;將表單設計與資料合併，以建立PDF檔案。 選擇性地為每個記錄產生中繼資料檔案，或將輸出儲存至PDF檔案。
* **[generatePrintedOutput](/help/forms/using/aem-document-services-programmatically.md#p-generateprintedoutput-p)：**&#x200B;從儲存在網路位置、本機檔案系統或HTTP位置的表單設計和資料檔產生PCL、PostScript或ZPL輸出做為常值。

* **[generatePrintedOutput](/help/forms/using/aem-document-services-programmatically.md#p-generateprintedoutput-p)：**&#x200B;從儲存在應用程式中的表單設計和資料檔產生PCL、PostScript和ZPL輸出。
...
<table>
 <tbody>
  ...
  <tr>
   <td>uriOrFileName</td>
   <td>指定輸入檔案的路徑和名稱。 檔案的型別可以是PDF或XDP。 如果只指定filename，則會讀取與options中指定的contentRoot相關的檔案。</td>
  </tr>
  <tr>
   <td>資料</td>
  ...
  <tr>
   <td>選項</td>
   <td>指定contentRoot、locale、AcrobatVersion、linearizedPDF和taggedPDF變數的值。 options引數接受PDFOutputOptions型別的物件。<br /> </td>
  </tr>
 </tbody>
</table>
...
generatePDFOutput API會將表單設計與資料合併，以產生PDF檔案。 或者為每筆記錄產生中繼資料檔案，或將輸出儲存至PDF檔案。 針對儲存在應用程式中的表單設計或資料使用generatePrintedOutput API。 如果表單設計和XML資料儲存在網路位置、本機或HTTP位置中作為常值，請使用[generatePDFOutput](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutput-p) API。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>指定輸入檔案的路徑和名稱。 檔案的型別可以是PDF或XDP。 如果只指定filename，則會讀取與options中指定的contentRoot相關的檔案。<br /> </td>
  </tr>
  <tr>
   <td>資料</td>
   <td>包含與PDF檔案合併之資料的XML檔案。<br /> </td>
...
將表單設計與資料合併以建立PDF檔案。 選擇性地為每個記錄產生中繼資料檔案，或將輸出儲存至PDF檔案。 使用generatePDFOutputBatch API進行表單設計或儲存在網路位置、本機檔案系統或HTTP位置上的資料做為常值值。
...
<table>
 <tbody>
  ...
  <tr>
   <td>資料</td>
   <td>指定索引鍵和資料檔案的地圖。 如果索引鍵不是Null，則會使用範本對應中指定的對應索引鍵來轉譯資料檔案。 </td>
  </tr>
  <tr>
   <td>選項</td>
   <td>指定contentRoot、locale、AcrobatVersion、linearizedPDF和taggedPDF變數的值。 options引數接受PDFOutputOptions型別的物件。</td>
  ...
  <tr>
   <td>batchOptions</td>
   <td>指定變數<code>generateManyFiles</code>的值。 設定generateManyFiles旗標以產生多個檔案。 options引數接受BatchOptions型別的物件。</td>
  </tr>
 </tbody>
</table>
...
從表單設計和資料檔案產生PCL、PostScript和ZPL輸出。 資料檔會與表單設計合併，並格式化以供列印。 您可以將輸出直接傳送至印表機或另存為檔案。 針對儲存在應用程式中的表單設計或資料使用generatePrintedOutput API。
...
下列Java程式碼範例會從表單設計和資料產生PCL、PostScript和ZPL輸出。 輸出型別取決於傳遞給`printConfig`引數的值。
...
將表單設計與資料合併，產生PS、PCL和ZPL格式的檔案。 或者為每筆記錄產生中繼資料檔案，或將輸出儲存至PDF檔案。 使用generatePrintedOutputBatch API進行表單設計或儲存在網路位置、本機檔案系統或HTTP位置上的資料做為常值值。
...
<table>
 <tbody>
  ...
  <tr>
   <td>資料</td>
   <td>指定索引鍵和資料檔案的地圖。 如果索引鍵不是null，則會使用範本對應中對應索引鍵的範本轉譯資料檔案。<br /> </td>
  </tr>
  <tr>
   <td>選項</td>
   <td>指定PrintedOutputOptions型別的物件。 此物件用來設定contentRoot、locale、printConfig、copies、paginationOverride的值。<br /> </td>
  </tr>
  <tr>
   <td>batchOptions</td>
...
下列Java程式碼範例會從多個表單設計範本和資料檔批次產生PCL、PostScript和ZPL輸出。 輸出型別取決於傳遞給`printConfig`引數的值。
...
Forms服務提供可匯入及匯出互動式PDF表單之資料的API。 互動式PDF表單是一份PDF檔案，其中包含一或多個用來顯示和收集使用者資訊的欄位。 此服務支援下列API：
...
<table>
 <tbody>
  ...
  <tr>
   <td>dataFormat<br /> </td>
   <td>指定資料匯出的格式。 它接受enum(XDP， XmlData， Auto)型別的變數。<br /> </td>
  </tr>
 </tbody>
</table>

下列Java程式碼範例會以XML和XDP格式從互動式PDF表單匯出表單資料。
...
PDF Generator服務提供可將原生檔案格式轉換為PDF的API。 它也會將PDF轉換為其他檔案格式，並最佳化PDF檔案的大小。
...
#### PDF Generator API可在Microsoft Windows和Linux上使用 {#pdf-generator-api-available-on-microsoft-windows-and-linux}
...
createPDF API會將支援的檔案型別轉換為PDF檔案。 它支援各種檔案格式，例如Microsoft Word、Microsoft PowerPoint、Microsoft Excel和Microsoft Project。 除了這些應用程式外，任何產生應用程式型別的協力廠商通用PDF也可以插入API中。
...
<table>
 <tbody>
  ...
  <tr>
   <td>xmpDoc </td>
   <td>此檔案包含套用至產生的PDF檔案的中繼資料資訊。 此引數是選用的。<br /> </td>
  </tr>
 </tbody>
</table>
...
<table>
 <tbody>
  ...
  <tr>
   <td>settingsDoc </td>
   <td>檔案包含產生輸出檔案時要套用的組態。 一般來說，這是XML檔案。</td>
  </tr>
 </tbody>
</table>

以下Java程式碼範例將PDF檔案轉換為指定的檔案型別。
...
OptimizePDF API透過縮小PDF檔案的大小來最佳化檔案。 此轉換的結果為PDF檔案可能小於其原始版本。 此操作也會將PDF檔案轉換為最佳化引數中所指定的PDF版本。 它會傳回包含最佳化PDF的OptimizePDFResult物件。
...
Distiller服務會將PostScript、Encapsulated PostScript (EPS)和印表機文字檔(PRN)轉換為PDF檔案。 Distiller服務通常用於將大量列印檔案轉換為電子檔案，例如發票和結算單。 將檔案轉換為PDF也可讓企業傳送檔案的紙本和電子版本給客戶。 支援的檔案格式為.ps、.eps和.prn。 此服務支援下列API：
...
<table>
 <tbody>
  ...
  <tr>
   <td>inputFileName</td>
   <td>指定輸入檔案的完整名稱以及檔案的副檔名。 此為必要引數。</td>
  </tr>
  <tr>
   <td>pdfSettings</td>
  ...
  <tr>
   <td>xmpDoc </td>
   <td>此檔案包含已產生PDF檔案的中繼資料資訊。 此為選用引數。</td>
  </tr>
 </tbody>
</table>
...
自動標籤PDF API透過在檔案中新增標籤來增強PDF協助工具，確保符合協助工具標準。 這不僅可改善使用者體驗，亦可維持各檔案的正確性和一致性。 自動標籤API支援標籤下列元素：

* 文字區塊（段落）
* 一個運運算元中的專案符號清單
* 目錄（目錄）

![自動標籤的PDF檔案](assets/auto-tag-api.png)

下列Java程式碼範例示範如何將PDF檔案轉換為已標籤的PDF檔案。

**語法**： `Document tag(final Document inDoc)`
