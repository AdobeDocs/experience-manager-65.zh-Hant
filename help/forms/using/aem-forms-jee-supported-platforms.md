...
Adobe已發行 [完整安裝程式](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html) 在JEE上使用AEM 6.5 Forms Service Pack 12 (6.5.12.0)以及修補程式安裝程式。 完整版安裝程式支援新平台，而修補程式安裝程式僅包含錯誤修正。

如果您要在JEE環境中執行全新安裝或計畫使用適用於您的AEM 6.5 Forms的最新軟體，Adobe建議使用 [AEM 6.5.12.0 Forms on JEE完整安裝程式](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html) 2022年3月3日發行，而非2019年4月8日發行的AEM 6.5 Forms安裝程式。
...
>
>隨著6.5版的推出，我們不再支援使用量最低的客戶之基礎結構元件，如下所示：
>
>- IBM DB2資料庫
>- IBM AIX和Sun Solaris作業系統
>
>若為新安裝，建議在可行的情況下，將AEM Forms部署於現代OSGi棧疊上，以利用有關回應式Adaptive Forms的最新創新，用於行動、多頻道互動式通訊和使用表單資料模型的後端資料整合。
>
>我們瞭解現有使用者需要繼續在JEE棧疊上部署AEM Forms。 在這種情況下，Adobe需要在支援的基礎架構上部署AEM Forms JEE，如本檔案所述。 如果您要升級至AEM 6.5 Forms並使用原版AEM Forms上不支援的平台，請聯絡Adobe支援以取得升級至支援平台的協助。

### Java虛擬機器器(JVM) {#java-virtual-machines-jvm}

Adobe Experience Manager Forms需要由Java開發套件(JDK)散發提供的Java虛擬機器器才能執行。 Adobe Experience Manager使用下列版本的Java虛擬機器器運作：
...
>[!NOTE]
>
>- 建議您追蹤Java廠商的安全公告，以確保生產環境的安全與保障，並安裝最新的Java更新。
...
- 全新安裝不支援IBM DB2。 僅現有升級至AEM 6.5 Forms的客戶支援此功能。
...
- Adobe客戶服務將協助確認與AEM使用MongoDB相關的問題。 如需詳細資訊，請參閱 [適用於Adobe Experience Manager的MongoDB頁面](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager).
- 「檔案系統」包括符合POSIX的區塊儲存裝置。 這包括網路儲存技術。 請注意，檔案系統效能可能會有所差異，並影響整體效能。 建議將負載測試AEM與網路/遠端檔案系統結合。
...
- JEE上的AEM Forms除了支援軟體的指定主要和次要版本以外，也支援更新、修補程式和修正套件。 不過，除非另有指定，否則不支援更新至下一個主要或次要版本。
- 叢集式安裝不支援TarMK持續性。 如需有關支援的持續性的資訊，請參閱 [為AEM Forms安裝選擇持續性型別](/help/forms/using/choosing-persistence-type-for-aem-forms.md).
- JEE上的AEM Forms支援各種協力廠商軟體，如 [協力廠商軟體支援政策](../../forms/using/aem-forms-jee-supported-platforms.md#p-third-party-patch-support-policy-p).
...
- 只有Windows和Linux才支援OpenOffice的PDF Generator轉換。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Intel® Xeon® E5-2680、2.4 GHz或同等處理器<br /> VMWare ESX 5.1或更新版本<br /> RAM：6 GB （64位元作業系統搭配64位元JVM）<br /> 可用磁碟空間：15GB暫存空間加上22GB<br /> 適用於AEM Forms on JEE</td>
  </tr>
  <tr>
   <td>SUSE Linux Enterprise Server</td>
   <td>Intel Xeon E5-2670v2,1顆vCPU，2.5 GHz處理器<br /> AWS m3.medium （3個ECU）<br /> RAM：6 GB （64位元作業系統搭配64位元JVM）<br /> 可用磁碟空間：6 GB的暫存空間加上22GB<br /> 適用於AEM Forms on JEE</td>
  </tr>
  <tr>
   <td>Red Hat Enterprise Linux</td>
   <td>Intel Xeon E5-2670v2,1顆vCPU，2.5 GHz處理器<br /> AWS m3.medium （3個ECU）<br /> RAM：6 GB （64位元作業系統搭配64位元JVM）<br /> 可用磁碟空間：6 GB的暫存空間加上22GB<br /> 適用於AEM Forms on JEE<br /> </td>
  </tr>
  <tr>
   <td>小型生產環境的硬體需求</td>
   <td>
    <ul>
     <li><strong>Intel支援的環境</strong>：Intel® Xeon® E5-2680,2.4 GHz或以上。 使用雙核心處理器可進一步提升效能</li>
...
- 硬體需求：Intel® Pentium® 4或相當於AMD處理器，1 GHz處理器
...
- 16 GB的磁碟空間，適用於32位元或20 GB的磁碟空間，適用於64位元作業系統
...
>[!NOTE]
>
Acrobat DC產品系列為Acrobat和Reader引進了兩個軌道，本質上是不同的產品：「傳統」和「持續」。 如需詳細資訊和兩個曲目的比較，請參閱 [https://www.adobe.com/go/acrobatdctracks.](https://www.adobe.com/go/acrobatdctracks)
...
| Google Android | Android 5.1和更新版本。 AEM Forms應用程式已通過7英吋及10英吋Samsung Galaxy平板電腦及熱門智慧型手機的認證。 |
| Microsoft Windows | Microsoft Surface裝置、平板電腦、筆記型電腦和桌上型電腦執行Microsoft Windows 10作業系統。 |
...
JEE版AEM Forms的協力廠商軟體需求記錄在各自產品檔案的「系統需求」一節。 所有檔案都可從以下位置存取： [https://adobe.com/go/learn_aemforms_documentation_65](https://adobe.com/go/learn_aemforms_documentation_65) .

JEE上的AEM Forms第三方參考平台會說明在JEE上AEM Forms開發和發行期間第三方基礎架構的特定修補程式層級，以及該JEE上AEM Forms版本支援之基礎架構的最低修補程式/Service Pack層級。

Adobe支援第三方廠商在發行時發行的緊急或建議修補程式，並假設第三方廠商保證與AEM Forms on JEE支援的版本回溯相容。 Adobe僅支援在AEM Forms on JEE檔案中所述的最低修補程式層級後發行的修補程式。

在某些情況下，Adobe不支援變更主要功能的協力廠商更新，因此不支援完整的回溯相容性。 如需支援的更新詳細資訊，請參閱 [支援的修補程式定義](https://helpx.adobe.com/aem-forms/aem-forms-third-party-software-patch.html) 適用於特定廠商產品及Adobe支援的修補程式型別。

在Adobe無法控制的情況下，聲稱回溯相容性的協力廠商修補程式可能會對Adobe產品或客戶環境造成負面影響。 在這種情況下，Adobe建議客戶在將第三方提供的任何緊急修補程式應用於關鍵系統之前，先評估其影響。 Adobe將與第三方合作，透過正常的Adobe支援計畫或第三方修正修補程式中的問題，透過合理的業務努力來解決此類問題。 這並不保證Adobe支援的新發行協力廠商修補程式將可如供應商紀錄或在JEE上與AEM Forms搭配運作。
...
>[!NOTE]
>
標示為 [AEM Forms 6.5.12.0和6.5.10.0版本已棄用，並繼續支援，直到AEM Forms 6.5 Service Pack 18 (6.5.18.0)發行為止](https://helpx.adobe.com/tw/support/programs/eol-matrix.html).

## 修訂歷史記錄 {#revision-history}

- 2022年9月01日

   - 新增對應用程式伺服器JBoss EAP 7.4的OracleJava™ SE 11 （64位元） SDK的支援。
