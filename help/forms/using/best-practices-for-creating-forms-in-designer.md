...
若要讓表單可供存取，它必須 [可感知](https://www.w3.org/TR/WCAG20/#perceivable) 使用輔助技術。 例如，大部分的熒幕助讀程式不會考量表單的視覺版面，而是考慮基礎結構。
...
![協助工具浮動面板](/help/forms/using/assets/image-3.png)
...
* **自訂文字**，此資訊是您在「協助工具」浮動視窗的「自訂熒幕Reader文字」欄位中所設定。 此選項可讓您指定您希望輔助技術（例如熒幕閱讀程式）使用的任何文字。 大多數情況下最好使用註解設定 — 只有在無法使用註解或工具提示時，才應將建立自訂熒幕Reader文字視為一個選項。
* **工具提示**，此資訊是您在「協助工具」浮動視窗的「工具提示」欄位中設定。 對於大多數物件，當使用者將指標暫留在物件上時，工具提示會在執行階段出現。 工具提示只會在熒幕閱讀器使用中時才會顯示給某些唯讀物件，例如紙張表單的條碼物件。
* **註解**，這會讓LiveCycle Designer使用表單欄位的相關（視覺）標籤作為熒幕助讀程式文字。
* **名稱**，可在「繫結」標籤的「名稱」欄位中設定。 請注意，此名稱不得包含任何空格。
...
![指定可見標籤不足時的自訂熒幕Reader文字](/help/forms/using/assets/image-4.png)

圖4： **指定可見標籤不足時的自訂熒幕Reader文字**

### 標籤選項按鈕

當具有視力障礙的使用者進入選項按鈕時，熒幕助讀程式需要讀取兩件事：
* 選項按鈕群組用途的指示
* 每個選項按鈕的有意義標籤若要使用按鈕標題來存取選項按鈕：
   1. 在階層浮動視窗中，選取排除群組。
...
![典型順序表單預設閱讀順序的視覺效果](/help/forms/using/assets/image-7.png)

圖7： **典型順序表單預設閱讀順序的視覺效果**
...
![視覺輔助工具反白標示索引標籤順序](/help/forms/using/assets/image-8.png)
...
![預設標籤順序由左至右](/help/forms/using/assets/image-9.png)
...
![使用座標，透過「版面」調色盤精確定位物件](/help/forms/using/assets/image-10.png)

圖10： **使用座標，透過「版面」調色盤精確定位物件**
...
![使用子表單的預設定位鍵順序](/help/forms/using/assets/image-11.png)
...
* WCAG 1.0
   ...
   * 2.2當有色彩缺乏的人檢視或在黑白熒幕上檢視時，請確定前景和背景色彩的組合可提供足夠的對比度。 [影像的優先順序2，文字的優先順序3] (P2)。
* WCAG 2.0
...
* 表格將表格的角色指定給選取的子表單。 當使用者導覽到這個子表單時，大多數熒幕助讀程式會將其識別為表格，並指示列數和欄數。
* 表頭列將表頭列的角色指定給選取的子表單或表格列。 說內文列儲存格的內容時，大多數熒幕閱讀程式會先識別標題列中對應儲存格的內容。
* 內文列將內文列的角色指定給選取的子表單或表格列。 如果儲存格包含子表單，熒幕助讀程式通常會讀出標題列中對應儲存格的內容，後面接著子表單中的欄位。
* 頁尾列將頁尾列的角色指定給選取的子表單或表格列。
* （無）指定傳送表格或其內容相關資訊的列。 此列不被視為表格的一部分；不過，熒幕助讀程式會讀取其內容。
...
* 表格標題列中適當儲存格的內容
* 區段標題列中適當儲存格的內容
* 來自所選儲存格的內容不過，有些熒幕閱讀程式可能不會從兩個標題列讀取內容。
...
* 使用輔助技術的使用者有時需要額外的時間來完成工作。 在計時常式即將到期的任何情況下，都會顯示可存取的訊息以允許副檔名。 輔助技術可使用透過JavaScript建立的警報方塊。 也可以部署新視窗，其中包含通知使用者即將逾時的訊息。
...
* WCAG 1.0
   ...
   * 8.1讓程式設計元素（例如指令碼和Applet）可直接存取或與輔助技術相容 [優先順序1 （如果功能重要且未顯示於其他位置）]，否則(P2)。
   * 9.3對於指令碼，請指定邏輯事件處理常式，而非裝置相依的事件處理常式(P2)。
...
若為Flash媒體，請參閱 [連結](/help/forms/using/best-practices-for-creating-forms-in-designer.md) 以取得有關提供註解的資訊。
...
![在表單屬性對話方塊中變更表單地區設定](/help/forms/using/assets/image-17.png)
