...
當您依順序將物件移至新位置時，LiveCycle Designer會重新指派定位鍵順序編號。 雖然位於主版頁面上的物件標籤順序會顯示在「設計檢視」標籤上，但您只能在「主版頁面」標籤上變更這些物件的順序。 如果您在表單中使用片段參考，則在檢視表單順序時會顯示片段內的Tab順序。 若要變更片段內的索引標籤順序，您必須開啟片段來源檔案進行編輯、進行變更並儲存檔案。 使用此片段的任何表單都會受到此變更的影響。
...
在LiveCycle Designer中，若要確保您的控制項可使用鍵盤，最簡單的方式是使用「物件庫」浮動視窗中「常用」標籤下列出的控制項。 預設情況下，這些控制項會回應滑鼠和鍵盤的輸入。 如需詳細資訊，請參閱本指南中的2.3選擇正確的控制項。
...
* WCAG 2.0
   ...
   * 1.4.3對比（最小）：文字的視覺呈現方式與文字影像的對比率至少為4.5:1，以下除外： （AA級）
