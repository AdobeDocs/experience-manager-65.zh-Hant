...
description: 調整您的XFA架構HTML5Forms，以獲得最佳效能。
seo-description: 瞭解如何調整您的XFA架構HTML5Forms，以獲得最佳效能。
...
AEM Forms有一個稱為HTML5表單的元件。 它可協助以HTML5格式轉換現有的XFAPDF forms（XDP檔案）。 本檔案提供指引和建議，以縮短載入時間並改善行動裝置上HTML5表格的效能。
...
雖然本文中討論的建議是針對HTML5表單，但這些建議同樣適用於XFAPDF forms。 這些最佳實務可共同協助HTML5表格的整體效能。 它需要謹慎規劃，以開發有效率且具生產力的表單。 我們開始吧：
...
* 使用Configuration Manager(Day CQ HTML Library Manager)AEM中的minify選項來壓縮JavaScript和CSS檔案。 如需詳細資訊，請參閱[OSGi組態設定](/help/sites-deploying/osgi-configuration-settings.md)。
* 啟用網頁壓縮。 它可減少源自表單的請求和回應的大小。 如需詳細資訊，請參閱[ Performance tuning of AEM forms server](https://helpx.adobe.com/aem-forms/6-3/performance-tuning-aem-forms.html)。
...
* 評估將大型表單分割為多個表單。 您也可以使用表單集將所有較小的表單群組在一起，並以單一單位呈現。 表單集僅載入所需的表單。 此外，在表單集中，您可以設定不同表單中的常用欄位，以共用資料系結。 資料系結可協助使用者只填寫一次一般資訊；這些資訊會自動填入後續的表格，大幅提升效能。 有關表單集的詳細資訊，請參閱[表單集AEM](https://helpx.adobe.com/aem-forms/6-3/formset-in-aem-forms.html)。
...
* 如果上述建議未產生多項改進，請評估將資料導向表單分割為[表單集](https://helpx.adobe.com/aem-forms/6-3/formset-in-aem-forms.html)。 例如，如果一個表格有1000多行，則每100行移動一次到不同的表格。 它有助於改善表單的載入時間和效能。  另請注意，表單集會為所有表單產生整合的提交XML。 若要區分每種形式的資料，請使用不同的資料根目錄。 如需詳細資訊，請參閱「AEM Forms中設定的[表單」。](https://helpx.adobe.com/aem-forms/6-3/formset-in-aem-forms.html)
...
Adobe Experience Manager(AEM)表格可協助您將複雜的交易轉換為簡單、愉悅的數位體驗。 但是，它需要共同努力，以開發高效和有生產力的表單。 除了HTML5Forms，以下是一般最佳實務的建議讀AEM取：

* [部署和維護的最佳做法AEM](/help/sites-deploying/best-practices.md)
...
[ ![HTML5Forms最佳做法快速參考卡](do-not-localize/best-practices_reference_card.png)](assets/html5_forms_best_practices_reference_card.pdf)
