...
### 若要自訂圖示並啟動影像{#to-customize-icon-and-launch-images}

#### 對於iOS {#for-ios}

1. 在Xcode中開啟`Capture.xcodeproj`專案。
1. （***對於自定義表徵圖***）在Capture的導航視圖中，導航至&#x200B;**[!UICONTROL Capture > Capture > Supporting Files > Capture-info.plist]**。 按一下「圖示檔」旁的下拉式清單。 指定圖示檔案(.png)的名稱，並在&#x200B;**[!UICONTROL 擷取>擷取>資源>圖示]**&#x200B;上傳檔案。 目前支援的維度包括：29x29、50x50、58x58、72x72、100x100和144x144。
1. （***用於自訂啟動影像***）請確定影像的檔名為：
...
1. 在Xcode中開啟`Capture.xcodeproj`專案。
1. 在Capture的導航器視圖中，導航至&#x200B;**[!UICONTROL Capture > Capture > Supporting Files > InfoPlist.strings]**。
...
1. 重建AEM Forms應用程式。
   ...
   如需建立Android適用應用程式的詳細資訊，請參閱[「設定Eclipse專案並建立Android應用程式」](/help/forms/using/setup-eclipse-project-build-installer.md)。
...
1. 重建AEM Forms應用程式。
   ...
   如需建立Windows適用應用程式的詳細資訊，請參閱[「設定Visual Studio專案並建立Windows應用程式」](/help/forms/using/setup-visual-studio-project-build-installer.md)。

## 自訂登入頁面{#customizing-images-on-the-login-page}上的影像
...
1. 若要變更標誌，請將預設的`LC-logo.png`檔案取代為自訂的`LC-logo.png`檔案。
1. 若要變更背景，請將預設的`Landing_bg.jpeg`檔案取代為自訂的`Landing_bg.jpeg`檔案。
1. 在iOS裝置或iOS模擬器上建立並執行AEM Forms應用程式。

### 若要使用Eclipse {#to-customize-images-on-the-login-pages-using-eclipse}自訂登入頁面上的影像
...
### 使用Visual Studio {#to-customize-images-on-the-login-pages-using-visual-studio}自訂登入頁面上的影像
...
## 自訂應用程式選單{#customizing_images_on_the_login_page-1}中的標誌
...
1. 導覽至`www/wsmobile/images`資料夾。
1. 若要變更標誌，請將預設的`aem_icon.png`檔案取代為自訂的`aem_icon.png`檔案。
1. 在iOS裝置或iOS模擬器上建立並執行AEM Forms應用程式。

### 若要使用Eclipse {#to-customize-images-on-the-login-pages-using-eclipse-1}自訂登入頁面上的影像
...
### 使用Visual Studio {#to-customize-images-on-the-login-pages-using-visual-studio-1}自訂登入頁面上的影像
