...
1. 按一下右鍵/etc/clientlibs子資料夾，然後按一下&#x200B;**Create** > **Create Node**。 輸入名稱（此處為afTestRegistration），將節點類型指定為cq:ClientLibraryFolder，然後按一下&#x200B;**確定。**

   clientlibs資料夾包含應用程式（JS和Init）的註冊方面。 建議您在clientlibs資料夾中註冊所有特定於表單的霍布斯測試套裝物件。

1. 在新建立的節點（此處為afTestRegistration）中指定以下屬性值，然後按一下&#x200B;**Save All**。 這些屬性可協助霍布斯將資料夾識別為測試。 若要在其他用戶端程式庫中重複使用此用戶端程式庫作為相依性，請將其命名為granite.testing.calvin.tests。
...
1. 按一下右鍵測試節點（此處&#x200B;**afTestRegistration）**，然後按一下&#x200B;**Create** > **Create File**。 將檔案命名為js.txt，然後按一下「**確定**」。
1. 在js.txt檔案中，新增下列文字：
...
1. 按一下「**儲存全部**」 ，然後關閉js.txt檔案。
1. 按一下右鍵測試節點（此處&#x200B;**afTestRegistration）**，然後按一下&#x200B;**Create** > **Create File**。 將檔案命名為init.js，然後按一下&#x200B;**OK**。
1. 將下列程式碼複製到init.js檔案中，然後按一下&#x200B;**Save All**:
...
1. 按一下&#x200B;**建立** > **建立節點**&#x200B;以在要測試的每個表單的clientlib資料夾下建立節點。 此範例使用名為&#x200B;**testForm**&#x200B;的節點來測試名為&#x200B;**testForm**&#x200B;的最適化表單。 指定以下屬性，然後按一下&#x200B;**OK**:
...
1. 按一下右鍵為測試表單建立的資料夾（此處為testForm），然後選擇&#x200B;**Create** > **Create File**。 將檔案命名為scriptingTest.js，並將下列程式碼新增至檔案中，然後按一下「**儲存全部」。**
...
測試AEM最適化表單的步驟與測試AEM UI的步驟類似。 如需測試最適化表單的詳細資訊，請參閱[測試UI](https://helpx.adobe.com//experience-manager/6-3/help/sites-developing/hobbes.html)中的下列主題：
