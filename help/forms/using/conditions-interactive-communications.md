...
1. 選取 **[!UICONTROL 建立規則]** 以視需要新增顯示或隱藏資產的規則。 若要在規則中使用變數，請參閱 [建立變數](#variables). 如需詳細資訊，請參閱 [將規則新增至條件](#ruleeditor).
...
1. 在建立或編輯條件時，選取 ![ruleeditoricon](assets/ruleeditoricon.png) （規則編輯器）圖示以取得相關資產。
...
1. 插入評估運算式、字串、資料模型屬性、變數或日期。
   ...
   * 在建立或編輯規則時，您也可以選取 ![icon_resize](assets/icon_resize.png) （調整大小）以展開「建立規則/編輯規則」對話方塊。 展開的完整視窗對話方塊可讓您建立 [變數](#variables) 以建構規則。 再次選取調整大小以返回一般的建立規則對話方塊。
