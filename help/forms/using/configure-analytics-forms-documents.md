...
1. 找到 **報表套裝ID** 並新增報表套裝ID。 例如，JJEsquire。 報表套裝ID會出現在「報表套裝ID」欄位下方。 它包含自動前置詞，通常是公司名稱。
...
1. 按一下 **連線至Analytics**. 會出現一個對話方塊，其中包含連線成功的訊息。 按一下&#x200B;**「確定」**。
...
1. 為下列欄位指定適當的值，然後按一下 **儲存**.
   ...
   * **欄位時間追蹤基準線**：以秒數指定必須追蹤欄位造訪的持續時間。 預設值為 0。當值大於0 （零）時，會將兩個個別的追蹤事件傳送至Adobe Analytics伺服器。 第一個事件會指示Analytics伺服器停止追蹤退出欄位。 第二個事件會在指定的持續時間過後傳送。 第二個事件會指示Analytics伺服器開始追蹤已造訪的欄位。 使用兩個不同的事件有助於準確測量在欄位上逗留的時間。 當值為0 （零）時，單一追蹤事件會傳送至Adobe Analytics伺服器。
