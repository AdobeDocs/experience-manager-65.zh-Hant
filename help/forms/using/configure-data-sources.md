...
資料整合支援OAuth2.0([授權代碼](https://oauth.net/2/grant-types/authorization-code/)， [使用者端認證](https://oauth.net/2/grant-types/client-credentials/))、基本驗證和API金鑰驗證型別為現成可用，並允許實作自訂驗證以存取Web服務。 雖然RESTful、SOAP型和OData服務是在AEM雲端服務中設定，但關聯式資料庫的JDBC和AEM使用者設定檔的聯結器是在AEM Web主控台中設定。
