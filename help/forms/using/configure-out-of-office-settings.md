---
title: 設定「外出」設定
description: 設定AEM Forms執行個體的「外出」設定。
...
1. 選取 **[!UICONTROL 在此期間轉寄我的專案]** 核取方塊。 如果您未選取選項且未指定受指派人，則您的專案不會轉寄給任何使用者。 雖然您已離開並啟用此設定，但專案仍會保留在您的收件匣中。
1. 點選 **[!UICONTROL 新增被指定者]**. 在中指定使用者 **[!UICONTROL 被指定者]** 要委派專案的目標欄位。 指定 **[!UICONTROL 工作流程模型]** 以委派給指定的使用者。 您可以選取多個工作流程模型。
...
>[!NOTE]
>
>(僅適用於Forms中心的工作流程專案)啟用 **允許受分派者使用「休假中」設定進行委派** 的選項 **指派任務** 步驟。 只有已啟用上述選項的專案才會委派給其他使用者。
