...
1. 選取 **[!UICONTROL 在此期間轉寄我的專案]** 核取方塊。 如果您未選取選項也未指定受託人，則您的專案不會轉寄給任何使用者。 雖然您離開並啟用了設定，但專案仍會保留在您的「收件匣」中。
1. 選取 **[!UICONTROL 新增被指定者]**. 在中指定使用者 **[!UICONTROL 被指定者]** 欄位，讓您可以委派專案。 指定 **[!UICONTROL 工作流程模型]** 以便委派給指定的使用者。 您可以選取多個工作流程模型。
...
>[!NOTE]
>
>(僅適用於Forms中心的工作流程專案)啟用 **允許受分派者使用「休假中」設定進行委派** 的選項 **指派任務** 步驟。 只有已啟用前述選項的專案才會委派給其他使用者。
