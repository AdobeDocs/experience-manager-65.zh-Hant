...
>[!CAUTION]
>
>如果您[預填](../../forms/using/prepopulate-adaptive-form-fields.md)表單範本、表單資料模型或基於結構的適用性表單（含XML或JSON資料）向結構（XML結構、JSON結構、表單範本或表單資料模型）投訴，該結構為資料不包含&lt;afData>、&lt;afBoundData>和&lt;/afUbnoundedData>標籤，則無界欄位的資料（無界欄位為沒有[binf](../../forms/using/prepopulate-adaptive-form-fields.md)屬性的適用性表單欄位）會遺失。
...
如下圖所示， `param1`和`param2`會以參數的形式傳遞，其值是從下一個動作的&#x200B;**textbox**&#x200B;和&#x200B;**numeric box**&#x200B;欄位複製而來。
...
如需Forms入口網站和提交動作的詳細資訊，請參閱[草稿和提交元件](../../forms/using/draft-submission-component.md)。
...
使用&#x200B;**叫用AEM Workflow**&#x200B;提交動作之前，請[配置Experience ManagerDS設定](../../forms/using/configuring-the-processing-server-url-.md)。 如需建立AEM工作流程的詳細資訊，請參閱OSGi](../../forms/using/aem-forms-workflow.md)上的[表單導向工作流程。
