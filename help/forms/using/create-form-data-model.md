...
AEM [!DNL Forms] 資料整合模組可讓您從不同的後端資料來源建立表單資料模型，例如AEM使用者設定檔、REST風格的web services、SOAP架構的web services、OData服務和關聯式資料庫。 您可以在表單資料模型中配置資料模型對象和服務，並將其與自適應表單關聯。 最適化表單欄位會系結至資料模型物件屬性。 這些服務可讓您預先填寫最適化表單，並將提交的表單資料寫回資料模型物件。
...
* OSGi Bundle for [!DNL MySQL] JDBC driver，如捆綁JDBC數 [據庫驅動程式中所述](/help/sites-developing/jdbc.md#bundling-the-jdbc-database-driver)
...
執行下列操作以配置數 [!DNL MySQL] 據庫：

1. 將資料庫的JDBC驅 [!DNL MySQL] 動程式作為OSGi包安裝：

   1. 以管理員身分登入「AEM [!DNL Forms] Author Instance」，然後前往AEM網頁主控台組合。 預設URL為 [https://localhost:4502/system/console/bundles](https://localhost:4502/system/console/bundles)。

   1. 點選 **[!UICONTROL 安裝／更新]**。 出現 [!UICONTROL 「上載／安裝組合] 」對話框。

   1. 按一下 **[!UICONTROL 選擇檔案]** ，瀏覽並選擇 [!DNL MySQL] JDBC驅動程式OSGi包。 選擇「 **[!UICONTROL 啟動Bundle]** 」和「重 **[!UICONTROL 新整理套件]**」，然後點選「 **[!UICONTROL 安裝」或「更新」]**。 確保的 [!DNL Oracle Corporation's] JDBC驅動程式 [!DNL MySQL] 處於活動狀態。 已安裝驅動程式。

1. 將數 [!DNL MySQL] 據庫配置為資料源：
   ...
   1. 在設定對話方塊中，指定下列詳細資訊：
      ...
      * **JDBC驅動程式類**:指定JDBC驅動程式的Java類名。 對於數 [!DNL MySQL] 據庫，請指 **定com.mysql.jdbc.Driver**。
      * **JDBC連接URI**:指定資料庫的連線URL。 對於 [!DNL MySQL] 在埠3306和架構weretail上運行的資料庫，URL為： `jdbc:mysql://'server':3306/weretail?autoReconnect=true&useUnicode=true&characterEncoding=utf-8`
      * **使用者名稱：** 資料庫的用戶名。 必須啟用JDBC驅動程式才能與資料庫建立連接。
      * **密碼：** 資料庫的口令。 必須啟用JDBC驅動程式才能與資料庫建立連接。
      * **借閱測試：** 啟用「 **[!UICONTROL 借閱時測試]** 」選項。
      * **回訪時測試：** 啟用「 **[!UICONTROL Test on Return]** 」選項。
      * **驗證查詢：** 指定SQL SELECT查詢以驗證池中的連接。 查詢至少必須返回一行。 例如，從 **客戶詳細資料中選擇***。
      * **事務隔離**:將值設定為 **READ_COMMITTED**。

         將其他屬性保留為預設 [值](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html) ，然後點 **[!UICONTROL 選「儲存]**」。
...
AEM提 [!DNL Forms] 供直覺式使用者介面， [從設定的資料來源建立表單資料模型](data-integration.md) 。 您可以在表單資料模型中使用多個資料來源。 對於我們的使用案例，我們將使用已配置的 [!DNL MySQL] 資料源。

執行下列動作以建立表單資料模型：

1. 在AEM作者例項中，導覽至「表 **[!UICONTROL 單]** >資 **[!UICONTROL 料整合」]**。
1. Tap **[!UICONTROL Create]** > **[!UICONTROL Form Data Model]**.
1. 在「建立表單資料模型」對話框中，為表 **單資料模** 型指定名稱。 例如， **customer-shipping-billing-details**。 點選「 **[!UICONTROL 下一步]**」。
1. 選取資料來源畫面會列出所有已設定的資料來源。 選擇 **WeRetailMySQL** 資料源並點選 **[!UICONTROL 建立]**。
...
1. 在AEM作者例項上，導覽至「表 **[!UICONTROL 單]** >資 **[!UICONTROL 料整合」]**。 預設URL為 [https://localhost:4502/aem/forms.html/content/dam/formsanddocuments-fdm](https://localhost:4502/aem/forms.html/content/dam/formsanddocuments-fdm)。
...
1. 為資料模型對象配置讀和寫服務。
   ...
   1. 選擇客戶詳 **細資料** ，並點選「編 **[!UICONTROL 輯屬性」]**。
   1. 從「 **[!UICONTROL 讀取服務]** 」下拉式清單中選取「取得」。 自 **動添加** id引數，該引數是customerdetails資料模型對象中的主鍵。 點選 ![aem_6_3_edit](assets/aem_6_3_edit.png) ，然後依如下方式設定引數。

      ![read-default](assets/read-default.png)

   1. 同樣地，選擇 **[!UICONTROL 更新]** 作為寫入服務。 customerdetails **對象** 會自動添加為參數。 參數的配置如下。
   ...
   1. 點選「 **[!UICONTROL 完成]** 」以儲存資料模型物件屬性。 然後點選「 **[!UICONTROL 儲存]** 」以儲存表單資料模型。

      get **** 和 **** update服務會新增為資料模型物件的預設服務。

      ![資料模型——對象](assets/data-model-object.png)

1. 前往「服務 **[!UICONTROL 」標籤]** ，並設 **[!UICONTROL 定get]** and **[!UICONTROL update services]** 。

   1. 選擇get服務 **[!UICONTROL 並點選]** 「編輯 **[!UICONTROL 屬性」]**。 屬性對話框開啟。
   1. 在「編輯屬性」對話方塊中指定下列項目：

      * **標題**:指定服務的標題。 例如：檢索發運地址。
      * **說明**:指定包含服務詳細功能的說明。 例如：

         此服務從資料庫中檢索發運地址和其他客戶詳細 [!DNL MySQL] 資訊
   ...
   1. 選擇更新 **[!UICONTROL 服務]** ，然後點選「 **[!UICONTROL 編輯屬性」]**。 屬性對話框開啟。

   1. 在「編輯屬性」對話 [!UICONTROL 框中指定] :
      ...
      * **引數**:選擇名為 **ID的引數** 和 **客戶詳細資訊**。
      點選「 **[!UICONTROL 完成]**」。 配置 **[!UICONTROL 了更新]** ，以在資料庫中更新客戶 [!DNL MySQL] 詳細資訊。
...
1. 轉至「模 **[!UICONTROL 型]** 」標籤，選取 **customerdetails** 資料模型物件，並點選「 **[!UICONTROL 測試模型物件」]**。
1. 在「測 [!UICONTROL 試模型／服務] 」窗口中，從「選擇模型／服務」(Select Model/Service ******** )下拉式清單中選擇「讀取模型對象」(Read model object)。
1. 在customerdetails **節中，為已配置資料庫中存在的** id **參數指定一個值，然** 後點選 [!DNL MySQL] 測試 ****。

   會擷取與指定ID相關的客戶詳細資訊，並顯示在「輸 **[!UICONTROL 出]** 」區段中，如下所示。
