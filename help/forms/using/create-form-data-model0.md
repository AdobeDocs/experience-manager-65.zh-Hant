...
1. 以OSGi套件形式安裝MySQL資料庫的JDBC驅動程式：
   ...
   1. 點選 **選擇檔案** 瀏覽並選取MySQL JDBC驅動程式OSGi套件。 選取 **開始套件組合** 和 **重新整理封裝**，然後點選 **安裝** 或 **更新**. 請確定Oracle Corporation的MySQL JDBC驅動程式處於作用中狀態。 已安裝驅動程式。

1. 將MySQL資料庫設定為資料來源：

   1. 前往AEM網頁主控台，位於 [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr).
   1. 尋找 **Apache Sling連線集區資料來源** 設定。 點選以在編輯模式中開啟設定。
   1. 在設定對話方塊中，指定下列詳細資訊：
   ...
   保留其他屬性為預設值 [值](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html) 然後點選 **儲存**.
...
1. 展開 **MySQL** 資料來源樹狀結構 從以下資料模型物件和服務中選取 **teleca** 綱要：
   ...
   * **服務:**
   ...
   點選 **新增選取專案** 將選取的資料模型物件和服務加入至表單資料模型。
...
1. 選取上方的核取方塊 **帳單** 資料模型物件，以選取並點選 **建立子屬性**.
1. 在 **建立子屬性** 窗格：
...
1. 在數學運算式中，選取 **callcharges** 和 **confcallcharges** 分別做為第一和第二物件。 選取 **加** 作為運運算元。 在數學運算式中點選，然後點選 **延伸運算式** 以新增 **smscharges**， **網際費用**， **roamingnational**， **roamingintel**、和 **vas** 物件至運算式。
...
1. 選取上方的核取方塊 **客戶** 資料模型物件，以選取並點選 **新增關聯**. 此 **新增關聯** 屬性窗格開啟。
1. 在 **新增關聯** 窗格：
   ...
   * 點選 **新增** 以連結 **客戶** 資料模型物件至 **呼叫** 使用屬性的資料模型物件。 根據使用案例，呼叫資料模型物件必須連結至客戶資料模型物件中的行動電話號碼屬性。 此 **新增引數** 對話方塊開啟。
...
1. 選取上方的核取方塊 **客戶** 資料模型物件，以選取並點選 **編輯屬性**. 此 **編輯屬性** 窗格隨即開啟。
1. 指定 **客戶** 作為 **頂層模型物件**.
...
1. 選取上方的核取方塊 **呼叫** 資料模型物件，以選取並點選 **編輯屬性**. 此 **編輯屬性** 窗格隨即開啟。
...
1. 前往 **服務** 標籤。
1. 選取 **get** 服務並點選 **編輯屬性**. 此 **編輯屬性** 窗格隨即開啟。
1. 在 **編輯屬性** 窗格：
...
1. 選取 **更新** 服務並點選 **編輯屬性**. 此 **編輯屬性** 窗格隨即開啟。
1. 在 **編輯屬性** 窗格：
