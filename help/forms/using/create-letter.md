...
1. 表單設計人員使用AdobeForms Designer建立版面和片段版面，並將其上傳至CRX存放庫。 版面配置包含典型的表單欄位、版面配置功能（例如頁首與頁尾），以及用來放置內容的空白「目標區域」。 稍後，應用程式專家會對應這些目標區域所需的內容。 有關[設計配置](/help/forms/using/layout-design-details.md)的詳細資訊。
...
1. 輸入通訊的內容並選取&#x200B;**儲存：**
   ...
   * **Post程式（選用）：**&#x200B;選取要套用至信函範本的貼文程式。 開箱即用的發佈程式，以及您使用AEM建立的程式，例如電子郵件和列印版。
...
1. 選取&#x200B;**新增資產**。
1. 在「選取資產」畫面中，選取要附加字母的資產，並選取&#x200B;**完成**。 您必須先將資產上傳至Assets。 建議您只附加PDF和Microsoft Office檔案，但您也可以附加影像。 如需在DAM中上傳資產的詳細資訊，請參閱[上傳Assets](/help/assets/manage-assets.md)。
