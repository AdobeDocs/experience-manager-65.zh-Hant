...
1. 作者預覽信件，並提交該信件以進行後處理。 有關[後處理](/help/forms/using/submit-letter-topostprocess.md)的詳細資訊。

#### 使用信函管理{#using-letter-templates-provided-with-correspondence-management}提供的信函範本

您可以選擇修改和重複使用「對應管理」提供的範本，而不是從頭建立版面範本。 您可以使用設計人員來快速修改範本的品牌、資料和內容欄位，以符合您組織的需求。 有關「通信管理」模板的詳細資訊，請參閱[參考信函模板](/help/forms/using/reference-cm-layout-templates.md)。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Data<br />由一線員工輸入</td> 
   <td>在發送信件之前由自定義信件的一線員工提供的資料。<br /> </td> 
   <td><p>未受保護的DD元素<br />可編輯的文欄位落<br />變數／佔位符<br /> </p> </td> 
  </tr> 
  <tr> 
   <td>預先核准的<br />文欄位落</td> 
   <td>預先核准的文字內容。 法律、財務或業務領域的專家若瞭解信件的商業背景，通常會編寫文字內容。 大部分信件都會使用頁首、頁尾、免責聲明和問候等內容。 但是，「終止原因」等內容將是特定字母的特定內容。</td> 
   <td><p>Text\Lists\<br />條件\配置</p> <p> </p> </td> 
  </tr> 
  <tr> 
   <td>資料<br />以自訂邏輯為基礎？</td> 
   <td>對於某些信件（如要求索取更多索賠資訊的信件），諸如索賠調整器之類的用戶可以添加自定義文本內容。</td> 
   <td>Document<br />類型條件片段 </td> 
  </tr> 
  <tr> 
   <td>從中央儲存庫儲存的<br />映像</td> 
...
## 在構造字母前分析它{#analyze-a-letter-before-you-construct-it}
...
## 執行分析{#benefits-of-performing-the-analysis}的優點

**內容** 重複使用您擁有產生通訊所需新內容的整合清單。許多字母常見的內容，例如頁首、頁尾、免責聲明和簡介，而且可在各字母間重複使用。 所有此類常見內容都可由專家建立並核准一次，然後在許多信件中重複使用。

**建立資料** 字典有許多字母共有的資料值，例如「客戶ID」和「客戶名稱」。您可以準備所有此類資料值的統一清單。 通常，在規劃結構時會咨詢企業中間件團隊的人員。 這構成建立資料字典的基礎。

**從後端企業系統採** 購資料您也會知道所有需要的資料值，以及從何處取得企業系統資料。然後，您可以架構實施，從企業系統擷取資料並饋送至Letters解決方案。

**估計字母的復** 雜性確定建立特定對應的複雜性非常重要。此分析有助於確定建立字母模板所需的時間和技能集。 這進而有助於評估實施信件解決方案的資源和成本。

## 對應複雜性{#correspondence-complexity}

通過分析以下參數，可以確定對應的複雜度：

**版面** 複雜性版面有多複雜？「取消通知」等字母的版面很簡單。 但是，諸如「報銷申請確認」之類的字母的佈局很複雜，具有多個表和60多個表格欄位。 建立複雜的版面需要更多時間，而且需要進階的版面設計技巧。

**文欄位落和條** 件數貸款合同可長達10頁，包含40多條文字條款。其中許多條款將取決於「貸款參數」。 根據確切的條款和條件，條款將包括或排除在合同之外。 建立此類信件需要對複雜情況進行周密規劃和仔細界定。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>低複雜度</p> </td> 
   <td><p>低. 版面沒有幾個表單欄位(&lt;15)。</p> <p>通常為1頁<span class="acrolinxCursorMarker"></span>。</p> </td> 
   <td><p>8</p> </td> 
   <td><p>1</p> </td> 
   <td><p>中型設計人員技能。</p> </td> 
  </tr> 
...
* 要在通信中使用的[資料模組](/help/forms/using/document-fragments.md)。
* [測](/help/forms/using/data-dictionary.md#p-working-with-test-data-p) 試XML檔案的測試資料已移植到其中。如果您使用資料字典，則需要測試資料。
...
1. 點選版面並點選「**Next**」以選取版面。

   ![選取版面以建立字母](assets/selectlayout.png)

1. 輸入「Correponsements（對應）」的屬性，然後點選「Save（保存）」:****
...
1. 系統會顯示一則訊息：&quot;Letter created successfully.&quot; （在警報訊息中）點選&#x200B;**Open**&#x200B;以設定其中的資料模組和配置片段。 或點選&#x200B;**Done**&#x200B;返回上一頁。

   ![警報消息：已成功建立字母](assets/createcorrespondencecreated.png)

   **下一個**:當您點選「開 **啟**」時，「對應管理」會顯示版面的呈現方式，並列出版面(XDP)中的所有元件。繼續插入[資料模組和佈局片段並配置它們](/help/forms/using/create-letter.md#p-insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them-p)。

### 將資料模組和版面片段插入字母中，並加以設定{#insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them}
...
1. 點選每個子表單的&#x200B;**插入**，並選取「資料模組」或「版面片段」以插入每個子表單。
...
1. 若要編輯模組，請點選其旁的「編輯」圖示。 編輯模組後，按一下&#x200B;**保存**。
...
1. 「資料」頁面詳細說明資料欄位和變數在範本中的使用方式。 資料可以連結至資料來源，例如資料字典或使用者輸入。 每個欄位定義屬性，資料字典會從中對應資料，或為使用者輸入欄位顯示標題。
   ...
   * **欄位**&#x200B;元素可連結至文字、資料字典元素、資產或使用者指定的值。 您也可以將欄位元素系結至「忽略」選項，以忽略欄位元素。
...
1. 點選&#x200B;**Save**。 您的信件會建立並列在「信件」頁面上。

在「通信管理」中建立信函模板後，最終用戶／代理／索賠調整器可以在CCR用戶介面中開啟信函，並通過輸入資料、設定內容和管理附件來建立信件。 如需詳細資訊，請參閱[建立對應](/help/forms/using/create-correspondence.md)。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>明</strong> <strong>文</strong></td> 
   <td>是</td> 
   <td>僅文字</td> 
   <td>是</td> 
   <td>是</td> 
...
## 建立字母模板{#createcopylettertemplate}的副本
