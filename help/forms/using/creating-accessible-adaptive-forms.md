...
description: AEM Forms為您提供工具並建立可訪問的自適應表單，並幫助遵守輔助功能標準。
...
可訪問表單是每個人都可以使用的表單，包括有特殊需要的用戶。 自適應Forms包括許多增強不同能力用戶的可用性的特性和功能。 將輔助功能構建到適應性表單中不僅允許盡可能廣泛的內容受眾，而且在要求遵守輔助功能標準的地理區域提供文檔時，這也是一項要求。 AEM Forms幫助形成符合輔助功能標準的開發人員。

在建立自適應表單時，作者應考慮以下幾點以建立可訪問的自適應表單：
...
Ultramarine-Accessible主題是一個參考主題。 它有助於演示如何以自適應形式修復顏色對比度和其他輔助功能相關問題。 Adobe建議根據組織批准的樣式為生產環境建立自定義主題。 執行以下步驟將主題上載到實AEM例：
...
應重點關注四個關鍵方面：鍵盤導航、顏色對比度、影像的有意義的替代文本，以及表單控制項的相應標籤，以使自適應表單可訪問。 執行以下步驟，使現有自適應表單可訪問：

### 1。應用可訪問的主題並執行其他修復

將Ultramarine-Accessable主題應用於現有的自適應窗體。 應用主題：
...
1. 在自適應窗體中為徽標影像添加有意義的替代文本。

   為自適應表單模板的頁眉和頁腳元件中的影像提供有意義的替代文本。 修復模板並使用它建立自適應表單時，自適應表單將繼承應用於模板頁眉和頁腳的所有與輔助功能相關的修復。  對於現有的自適應表單，在自適應表單級別進行更改。 對自適應表單模板所做的更改不會自動流入現有自適應表單。

1. 將包含窗體名稱的標題元件添加到自適應窗體。 如果您的表單設計指定了公司名稱，請為公司名稱添加單獨的標題元件。

   大多數輔助工具都會通知用戶內容的層次結構，以幫助他們瞭解網頁的結構。 在自適應表單上為組織名稱和表單名稱文本設定不同的標題級別，以便為這些文本提供層次結構。 此外，在每個面板和節之前使用一個帶有相應標題級別的文本元件來建立層次。
...
1. 對自適應表單元件使用簡短說明而不是長說明。 長說明添加幫助按鈕。 確保自適應表單沒有任何幫助按鈕。

1. 將自定義輔助功能文本添加到表的所有只讀單元格。 此外，禁用表的所有只讀單元格。

1. 刪除自適應表單中的任意手寫簽名欄位。 配置自適應表單以使用Adobe Sign提供無縫的數字簽名體驗。
...
**標題** 使用此選項可讓AEM Forms將與表單域關聯的可視標籤用作螢幕閱讀器文本。
...
>* 對於基於XFA的自適應表單，輔助功能選項從XDP中設定的輔助功能選項中繼承。 XDP中的工具提示映射到「簡短說明」，標題映射到「標題」。 其他選項同樣有效。
...
請參閱 [為自適應表單建立自定義主題](/help/forms/using/creating-custom-adaptive-form-themes.md)，以獲取有關更改自適應表單的顏色對比度和主題的詳細資訊。
...
| 隱藏日期選取器元件選項 | Esc 鍵 |
| <ul><li>通過日期選取器元件中可用的選項將游標向前移動。</li><li>當日期輸入欄位處於活動狀態時，將標籤焦點設定在日曆表徵圖上</li> | 定位字元 |
...
可訪問名稱和描述檢查器(ANDI)可幫助您以自適應形式確定和修復與可訪問性相容性相關的問題。 要使用ANDI工具以自適應形式查找輔助功能問題，請執行以下操作：

1. 在預覽模式下開啟自適應窗體。
1. 按一下標有書籤的ANDI工具表徵圖。 ANDI工具分析自適應表單並顯示輔助功能問題。 有關如何使用該工具的詳細資訊，請參見 [ANDI文檔](https://www.ssa.gov/accessibility/andi/help/howtouse.html)。
1. 審查並解決ANDI報告的問題。
