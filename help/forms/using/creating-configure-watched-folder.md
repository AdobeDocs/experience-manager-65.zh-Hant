...
管理員可以設定網路資料夾，稱為&#x200B;*watched資料夾*，這樣當使用者將檔案(例如PDF檔案)放入watched資料夾時，就會啟動預先設定的作業並操作檔案。 執行指定的作業後，該作業會將修改的檔案儲存在指定的輸出資料夾中。 如需有關管理watched資料夾的詳細資訊，請參閱[管理說明](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md)。
...
1. 選取&#x200B;**進階**。 進階標籤包含更多欄位。 這些欄位大多包含預設值。
   ...
   * **節流開啟：**&#x200B;啟用此選項時，它會限制AEM表單在任何指定時間處理的watched資料夾工作數目。 「批次大小」值決定作業的最大數量。 如需詳細資訊，請參閱[節流](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#about-throttling)
