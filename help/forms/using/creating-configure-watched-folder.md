...
* 監視的資料夾具有各種屬性，請參 [閱「監視資料夾屬性」](watched-folder-in-aem-forms.md#watchedfolderproperties)。
...
1. 點選「 **進階**」。 進階標籤包含更多欄位。 這些欄位中，大部分都包含預設值。
   ...
   * **在調節時刪除逾時分段檔案**:如果啟用，則 **只有在為受監視資料夾開啟頻寬限制時，才激活「Time Out Staged Files After** （超時暫存檔案after）」機制。
