...
1. [上傳新的標誌檔](#uploadlogo) ，位於您已在CRX中建立的資料夾中。
...
1. 在應用程式資料夾中，建立名為的資料夾 `css` 路徑/結構類似於css資料夾（位於ccrui資料夾）。
   ...
   1. 用滑鼠右鍵按一下 **css** 資料夾並選取「 」 **覆蓋節點**： `/libs/fd/cm/ccr/gui/components/admin/clientlibs/ccrui/css`
...
1. 在下列位置建立名為customcss.css的檔案（不能使用不同的檔案名稱）：
   ...
   1. 用滑鼠右鍵按一下 **css** 資料夾並選取 **「建立」>「建立檔案」**.
   1. 在「新增檔案」對話方塊中，將CSS的名稱指定為 `customcss.css`（您無法使用其他檔案名稱），然後按一下 **確定**.
