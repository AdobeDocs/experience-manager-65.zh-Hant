---
title: 自訂表單入口網站元件的範本
seo-title: 自訂表單入口網站元件的範本
description: 以表單清單顯示自訂中繼資料
seo-description: 以表單清單顯示自訂中繼資料
...
## 自定義模板{#technical-specifications-for-custom-templates}的技術規範
...
若要包含可重複的項目，請將屬性&#x200B;**data-repeatable**&#x200B;的值配置為&#x200B;**true**。

*在討論的範例中，自訂範本的頂端有兩個Div元素。第一個類別搭配「__FP_boxes-container」CSS類別，可作為所清單單的容器元素。 第二個包含「__FP_boxes」CSS類是基本實體的範本，在此例中為「表單」。 Div元素中存在的&#x200B;**data-repeatable**屬性具有值&#x200B;**true**。*

每個佔位符都有一個專用的OOTB元資料集。 若要在表單上的特定位置顯示自訂中繼資料，請在該位置新增&#x200B;**${metadata_prop}屬性**。

*在範例中，中繼資料屬性用於多個例項。例如，它以規定方式用於&#x200B;**description**、**name**、**formUrl**、**htmlStyle**、**pdfUrl**、**pdfStyle**和&#x200B;**路徑**。*
...
1. **本地化支援**:若要將任何靜態文字本地化， `${localize-YOUR_TEXT}` 請使用屬性，並讓本地化值可供使用（如果尚未存在）。
   *在討論的範例中，屬性和 `${localize-Apply}` 用 `${localize-Download}` 於將「套用」和「下載」文字當地化。*

1. **支援排序**:按一下HTML元素來排序搜尋結果。若要在表格版面中實施排序，請在特定表格標題上新增「data-sortKey」屬性。 此外，將其值新增為您要排序的中繼資料。
例如，對於格線檢視中的「Title」標題，「data-sortKey」標題的值為「title」。 按一下標題以排序特定欄中的值。

1. **使用配置屬性**:Search &amp; Lister元件有數個可在使用者介面上使用的設定。例如，要顯示通過編輯對話框保存的HTML工具提示文本，請使用`${config-htmlLinkText}`屬性。 **同樣地，對於PDF工具提示文本，請使** `${config-pdfLinkText}` 用屬性。
...
1. 對於「草稿與提交」元件下「草稿」區段中的刪除選項，將CSS類別命名為「__FP_deleteDraft」。 此外，還包括值&#x200B;**${draftID}**&#x200B;的屬性&quot;draftID&quot;，該值是對應草稿的草稿ID。

1. 在建立連結以開啟草稿和提交時，您可以將&#x200B;**${path}.html**&#x200B;指定為錨點標籤的&#x200B;**href**&#x200B;屬性的值。
...
1. 請勿在任何自訂範本中使用單引號(&#39;)。
1. 若為自訂中繼資料，請將此屬性僅儲存在&#x200B;**jcr:content/metadata**&#x200B;節點上。 如果您將其儲存在任何其他位置，Forms Portal將無法顯示中繼資料。
1. 請確定任何自訂中繼資料或現有中繼資料的名稱不包含冒號(:)。 若有，則無法在使用者介面上顯示。
1. **data-** repeatable對Linkcomponent沒有任何 **** 意義。Adobe建議您避免在連結元件的範本中使用此屬性。
