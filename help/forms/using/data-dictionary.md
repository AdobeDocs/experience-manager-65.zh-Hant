...
>[!NOTE]
>
>對於需要類似元素的多個字母，您可以建立通用資料字典。 但是，如果大資料字典含有大量元素，在使用資料字典和載入元素（例如字母和檔案片段）時，可能會導致效能問題。 如果您遇到效能問題，請嘗試為不同的字母建立個別的資料字典。
...
1. 在「屬性」畫面中，新增下列內容：
   ...
   * **延伸屬性**：（選擇性）點選 **新增欄位** 指定資料字典的中繼資料屬性。 在「屬性名稱」欄中，輸入唯一的屬性名稱。 在「值」欄中，輸入要與屬性關聯的值。

   ![德文指定的資料字典屬性](do-not-localize/1_ddproperties.png)

1. （選用）若要上傳資料字典的XSD結構描述定義，請在「資料字典結構」窗格下點選 **上傳XML資料結構**. 瀏覽至XSD檔案，選取該檔案，然後點選 **開啟**. 系統會根據上傳的XML結構描述建立資料字典。 您需要調整資料字典中元素的顯示名稱和說明。 要執行此操作，請點選元素名稱並編輯其說明、顯示名稱和其他在右窗格欄位中的詳細資訊。
...
1. （選擇性）選取元素後，您可以在進階標籤中新增屬性（屬性）。 您也可以點選 **新增欄位** 和擴充DD元素的屬性。
...
1. 點選「 複製」。UI會顯示「貼上」圖示。
1. 點選「貼上」。 「貼上」對話方塊隨即顯示。 系統會自動指定新資料字典的名稱和標題。
1. 必要時，請編輯儲存資料字典副本的標題和名稱。
1. 點選「貼上」。 資料字典的副本隨即建立。 現在，您可以在新建的資料字典中進行所需的變更。
...
1. 在資料字典中，點選簡單元素加以選取。 複合與集合元素沒有參照。
...
1. 輸入資料後，您可以在預覽含有測試資料的信函時，使用此XML檔案。
   ...
   您可以使用DD （選取DD並點選「上傳測試資料並上傳此xml檔案」）新增此測試資料。因此，當您正常預覽信函時（非自訂），此XML資料就會用於信函中。 您也可以點選「自訂」，然後上傳此XML。
