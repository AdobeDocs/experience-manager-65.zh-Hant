...
1. 為什麼HTML窗體中顯示的某些字型與設計窗體時使用的字型不同？
   ...
   答案：HTML5表單不嵌入字型(與字型嵌入到表單中的PDF forms形式不同)。 對於要按預期呈現的表單的HTML版本，請確保在XDP中指定的字型在伺服器和客戶端電腦上可用。 如果伺服器上不提供所需字型，則使用回退字型。 此外，如果在「表單模板」中使用客戶端設備上不可用的字型，則瀏覽器的預設字型將用於呈現文本。

1. vAlign和hAlign屬性是否在HTML表單中受支援？

   是，支援vAlign和hAlign屬性。 Internet Explorer和多行欄位中不支援vAlign屬性。

1. HTML5表單是否支援希伯來文字元？

   HTML5表單支援除MicrosoftInternet Explorer外的所有瀏覽器中的希伯來字元。
...
1. 為什麼HTML形式比PDF forms大？
   ...
   要將XDP呈現到HTML窗體，需要許多中間資料結構和對象，如窗體dom、資料dom和佈局dom。
...
1. 當在支援的瀏覽器中開啟HTML5窗體時，相鄰放置的域的邊界不正確對齊，或子窗體出現重疊。 當在Forms設計器中預覽同一HTML5窗體時，欄位和佈局不會出現不對齊，子窗體出現在正確的位置。 如何解決問題？
   ...
   當子窗體設定為流內容且該子窗體具有隱藏的邊框元素時，相鄰放置的域的邊框不正確對齊或子窗體出現重疊。 要解決此問題，可刪除或注釋隱藏 &lt;border> 元素。 例如，以下 &lt;border> 元素標籤為注釋：
...
1. 為什麼螢幕閱讀器不能正確處理日期/時間欄位對象？
   ...
   螢幕閱讀器不支援日期/時間欄位。 但是，您可以手動在欄位中輸入日期/時間，以使螢幕閱讀器讀取。 使用工具提示或螢幕閱讀器文本指示用戶手動選擇欄位的日期/時間。
...
1. HTML5是否支援浮動欄位？
   ...
   是的，HTML5號Forms支援浮地。 要啟用浮動欄位，請將以下屬性添加到渲染配置檔案：
...
1. HTML5表單是否多次執行初始化指令碼和表單就緒事件？
   ...
   可以，初始化指令碼和表單就緒事件將多次執行，至少一次在伺服器上執行，一次在客戶端執行。 建議根據某些業務邏輯（表單或欄位資料）編寫諸如初始化或表單：就緒事件之類的指令碼，以便根據資料和冪等元的狀態（如果資料相同）執行該操作。
