...
1. **行動檢視**：We.Gov使用者可透過調整大小的瀏覽器來復寫行動檢視。
...
**本節**：CX銷售機會的Camila擁有現有的PDF式表單，此表單是紙張式流程的一部分。 Camila想要使用此PDF表單，在現代化工作中自動建立新的現代化Adaptive Forms。
...
1. 登入方式：
   * **使用者**：camila.santos
   * **密碼**：密碼
1. 從首頁面中，選取「Forms > Forms &amp; Documents > AEM Forms We.gov Forms > AFC」。
...
1. Camilla接著選取PDF表單並按一下 **開始自動轉換** 以開始轉換程式。 您可能需要按一下 **覆寫轉換** 如果您已轉換表單。
   ...
   >[!NOTE]
   >
   >AFC中的設定已針對一般使用者預先設定，這表示這些設定不應變更。
...
1. Camilla接著開始編輯表單，並選取「根面板>編輯（扳手） >從「面板配置」下拉式選單中選取「標籤在頂端」 >選取「核取方塊」。
...
**本節**：轉換檔案並產生最適化表單後，Camila必須將最適化表單連結至資料來源。
...
1. Camila選取最適化表單欄位並按一下 ![設定圖示](assets/configure-icon.svg) 和建立與表單資料模型實體的繫結，使用 **繫結參考** 欄位。 Camila會對最適化表單中的所有欄位重複此步驟。
...
**本節**：Aya是公民，從朋友那裡得知她可能有資格獲得政府機構提供的服務。 Aya從行動電話瀏覽至We.Gov網站，進一步瞭解她有資格使用的服務。
...
Aya必須先在家中檢視一些檔案，她才能完成服務請求申請。 她儲存並從行動裝置退出應用程式。
...
**本節：** 回到家後，Aya找到她需要的資訊，並從她的案頭繼續執行應用程式。 Aya會導覽至線上Forms入口網站以繼續執行其應用程式。 代理商也可以透過一些簡單的自訂功能，自動產生連結並以電子郵件方式繼續使用應用程式。
...
1. 以滑鼠右鍵按一下「圓CTA」以移動到下一個截面。
   ...
   提交後，Aya會收到「感謝您」頁面。 或者，Aya也會收到電子郵件，她可以開啟該電子郵件，以使用Adobe Sign以電子方式簽署記錄檔案。
...
**本節：** CX業務負責人Camila與Aya建立歡迎電話，說明如何使用她所核准的政府服務。
...
1. 然後Camila提供連絡人id的測試服務，然後按一下 **測試**. 例如，1或2 （如果您已提交表單）。 如果您尚未提交表單，則不會傳回任何資料。
...
**本節：** Camila會導覽至儀表板，她可以看到跨機構KPI，例如開始填寫服務請求表單並放棄的公民百分比、從請求提交到核准/拒絕回應的平均時間長度，以及她已傳送給公民的福利手冊的參與統計資料。
