...
此封裝是為建立的 **AEM Forms 6.4 OSGI作者**&#x200B;已經過測試，因此在以下平台版本上受到支援：
...
* **we-gov-forms.pkg.all-&lt;version>.zip** - *完整示範套件*
   ...
   * **we-gov-forms.ui.apps-&lt;version>.zip** *— 包含所有元件、使用者端程式庫、範例使用者、工作流程模型等。*
      ...
      * **grid-aem.ui.apps-1.0-SNAPSHOT.zip** - *網站頁面欄控制項的AEM Sites Grid版面配置套件*
...
1. 允許安裝程式完成。
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/content/we-gov/home.html？wcmmode=disabled* 以確保安裝成功。

### 從本機ZIP檔案 {#from-a-local-zip-file}

1. 下載並找到 **we-gov-forms.pkg.all-&lt;version>.zip** 檔案。
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/crx/packmgr/index.jsp*.
1. 選取「上傳套件」選項。
...
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/libs/granite/security/content/groupadmin.html*
1. 以管理員身分登入以執行下列工作。
1. 向下捲動至頁面結尾以載入所有使用者群組。
1. 搜尋&quot;**工作流程**「。
1. 選擇&quot;**workflow-users**」群組，然後按一下「屬性」。
1. 導覽至「成員」標籤。
...
1. 搜尋&quot;，重複步驟2至7 **forms使用者**&quot;，選取&quot;**forms-power-users**「群組，並新增」**We.Gov Forms使用者**&quot;群組作為成員。
...
1. 以管理員身分登入以執行此工作。
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/system/console/configMgr*
...
1. 瀏覽到:
   ...
   *https://&lt;aemserver>：&lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >用來存取AEM伺服器的URL應該符合Adobe Sign OAuth重新導向URI中設定的URL，以避免設定問題(例如 *https://&lt;aemserver>：&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*)
...
1. 瀏覽至 [https://portal.azure.com/](https://portal.azure.com/) 和登入。
1. 從左側功能表中選取「所有服務」。
1. 搜尋或導覽至「應用程式註冊」。
1. 建立或選取現有的應用程式註冊。
1. 複製 **應用程式ID** 將用作OAuth **使用者端ID** 在AEM雲端設定中
...
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/aem/forms.html/content/dam/formsanddocuments-fdm/we-gov*
...
1. [測試服務](work-with-form-data-model.md#test-data-model-objects-and-services) 以確保他們成功連線至已設定的資料來源
   ...
   * 若要測試連線，請選取 **HOMEMORTGAGEACCOUNT** 並提供get服務。 測試服務，系統管理員可以檢視正在擷取的資料。
...
1. 瀏覽至 *https://&lt;aemserver>：&lt;port>/libs/cq/core/content/tools/cloudservices.html*
...
1. 若要更新框架設定，請從相同頁面按一下「We.Gov Adobe Analytics Framework (Analytics Framework)」(請參閱 [啟用AEM製作](../../forms/using/forms-install-configure-gov-reference-site.md#enableauthoring) 以啟用編寫)。
...
所有網頁都可從下列位置取得： *https://&lt;aemserver>：&lt;port>/sites.html/content/we-gov*
...
註冊最適化表單提交至OSGI工作流程進行處理。 此工作流程可在以下位置找到： *https://&lt;aemserver>：&lt;port>/conf/we-gov/settings/models/we-gov-process.html*.

由於某些限制，此工作流程包含數個指令碼和自訂OSGI工作流程處理步驟。 這些工作流程步驟是作為一般步驟建立的，且尚未使用設定對話方塊建立。 目前，工作流程步驟的設定依賴流程引數。

所有工作流程步驟Java™程式碼都包含在 **we-gov-forms.core-&lt;version>.jar** 套件組合。
...
封裝管理員，使用者可以透過導覽至「 」存取AEM Forms中的Ultramine主題 **Forms** > **主題** > **參考主題** > **超海洋 — 可存取**.
