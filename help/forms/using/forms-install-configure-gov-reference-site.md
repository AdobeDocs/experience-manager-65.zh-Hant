...
1. [MicrosoftDynamics OData配置](https://docs.adobe.com/content/help/en/experience-manager-64/forms/form-data-model/ms-dynamics-odata-configuration.html)
1. [為Microsoft配置AEM Forms](https://helpx.adobe.com/experience-manager/kt/forms/using/config-dynamics-for-aem-forms.html)
...
1. 對於此配置，我們將兩個複選框值留空。
   ...
   * 要瞭解有關這些選項的詳細資訊，請參閱 [配置雲服務](https://docs.adobe.com/content/help/en/aem-forms-automated-conversion-service/using/configure-service.html#configure-the-cloud-service)。
...
automated forms conversion服務包括 [最佳做法，已知的複雜模式](https://docs.adobe.com/content/help/en/aem-forms-automated-conversion-service/using/styles-and-pattern-considerations-and-best-practices.html), [已知問題](https://docs.adobe.com/content/help/en/aem-forms-automated-conversion-service/using/known-issues.html)。 在開始使用AEM FormsAutomated forms conversion服務之前，請先查看這些內容。
...
為了首先執行輔助功能測試，您需要安裝Chrome插件，可以找到 [這裡](https://chrome.google.com/webstore/detail/accessibility-developer-t/fpkknkljclfencbdbgkenhalefipecmb?hl=en)。

安裝後，在Chrome瀏覽器中載入要test的頁面(注：開啟多個制表符可能會影響分數，最好只開啟一個制表符)。 載入頁面後
**右鍵** 選擇 **審計** 按鈕 開發人員可以選擇要由輔助功能插件執行的審核類型。 選擇所有所需選項後，用戶就可以選擇「生成報告」按鈕。 這將生成一個PDF文檔，其中顯示總體輔助功能評級以及可用於提高總體輔助功能評級的內容。
