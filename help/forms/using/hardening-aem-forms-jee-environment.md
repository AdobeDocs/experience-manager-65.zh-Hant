...
<table> 
 ...
 <tbody>
  ...
  <tr> 
   <td><p>/workspace/*</p> </td> 
   <td><p>Workspace一般使用者網頁應用程式</p> </td> 
   <td><p>是</p> </td> 
   <td><p>是</p> </td> 
  </tr> 
  <tr> 
...
### 緩解序列化問題 {#mitigating-serialization-issues}

Java還原序列化攻擊會利用將不受信任的資料還原序列化的應用程式，可能允許伺服器上的遠端程式碼執行。 JEE上的AEM Forms包含還原序列化防火牆，可在任何嘗試還原序列化物件之前執行預檢檢查。 此檢查會針對防火牆樣式的允許清單、封鎖清單或兩者來測試類別名稱，並拒絕已知可透過還原序列化攻擊利用的類別。

在執行&#x200B;**JDK 11或更新版本**&#x200B;的安裝上，此保護是由平台的原生序列化篩選所啟動，不需要手動步驟。 在執行&#x200B;**JDK 8**&#x200B;的安裝上，原生序列化篩選無效，因此NotSoSerial代理程式必須在啟動時明確附加至JVM。

瀏覽至還原序列化篩選器健康情況檢查，確認保護作用中：

```text
https://<server>:<port>/system/console/healthcheck?tags=deserialization
```

如果健康情況檢查在JDK 8執行個體上報告失敗，請附加並設定代理程式，如[緩解AEM Forms JEE中的序列化問題](/help/forms/using/mitigating-serialization-issues-forms-jee.md)中所述。
...
1. 開啟JBOSS_HOME/standalone/configuration/standalone.xml檔案進行編輯。
   ...
   在&lt;subsystem xmlns=&quot;urn:jboss:domain:web:1.1&quot; native=&quot;false&quot; default-virtual-server=&quot;default-host&quot;>元素之後，新增下列詳細資料：
