...
您可以核准同一類型的多個工作。 在選擇一個要批准的任務後，將只啟用具有相同進程、具有相同任務名稱和相同路由選項的任務。 如需實作詳細資訊，請參閱[使用待辦事項清單](/help/forms/using/todo-lists.md)。
...
在AEM Forms工作區中，預設的演算和送出服務（在預設動作設定檔中）與XDP表單關聯已變更，並已引入新服務。 如需詳細資訊，請參閱[新演算和送出服務](/help/forms/using/new-render-submit-service.md)。 要遷移與XDP表單一起使用的現有進程，要使用這些服務，可以遵循[這些步驟](new-render-submit-service.md)。
