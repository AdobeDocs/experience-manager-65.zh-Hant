---
title: HTML5表單的架構
description: HTML5表單會部署為內嵌AEM執行個體中的套件，並使用RESTful Apache Sling架構透過HTTP/S公開作為REST端點的功能。
...
HTML5 Forms功能會部署為內嵌AEM執行個體中的套件，並使用RESTful作為HTTP/S上的REST端點公開 [Apache Sling架構](https://sling.apache.org/).

![02-aem-forms-architecture_large](assets/02-aem-forms-architecture_large.jpg)

### 使用Sling架構 {#using-sling-framework}

[Apache Sling](https://sling.apache.org/) 以資源為中心。 它會使用請求URL來先解析資源。 每個資源都有一個 **sling：resourceType** (或 **sling：resourceSuperType**)屬性。 接著，會根據此屬性、要求方法和要求URL的屬性，選取sling指令碼來處理要求。 此Sling指令碼可以是JSP或servlet。 對於HTML5表單， **個人資料** 節點會充當sling資源和 **設定檔轉譯器** 會當作sling指令碼，處理使用特定設定檔轉譯行動表單的請求。 A **設定檔轉譯器** 是一種JSP，可從請求讀取引數並呼叫Forms OSGi服務。
...
Mobile Form會維護兩種不同的快取層級：PreRender快取和轉譯器快取。 preRender快取包含已解析範本的所有片段和影像，而轉譯器快取包含HTML等轉譯內容。
...
* **預先填入的Forms**：在轉譯時，如果使用者請求具有預先填入資料的表單，Forms OSGi轉譯服務會呼叫Forms服務容器並產生具有合併資料的新表單狀態。 不過，由於版面配置已在上述步驟中產生，因此此呼叫的速度會比第一次呼叫快。 此呼叫只會執行資料合併，並對資料執行指令碼，

如果表單中有任何更新，或表單內使用任何資產，表單快取元件會偵測更新，且特定表單的快取會失效。 Forms OSGi服務完成處理後，設定檔轉譯器jsp會新增JavaScript程式庫參考和樣式至此表單，並將回應傳回給使用者端。 典型的Web伺服器，例如 [Apache](https://httpd.apache.org/) 可用於此處並開啟HTML壓縮。 網頁伺服器可大幅減少回應大小、網路流量，以及伺服器與使用者端電腦之間資料串流所需的時間。

當使用者提交表單時，瀏覽器會將表單的狀態以JSON格式傳送至 [送出服務Proxy](../../forms/using/service-proxy.md)；然後提交服務Proxy會使用JSON資料產生資料XML並提交該資料XML以提交端點。
...
此OSGi服務包含將XDP轉譯為HTML的邏輯，並處理表單提交以產生資料XML。 此服務使用Forms服務容器。 Forms服務容器會在內部呼叫原生元件 `XMLFormService.exe` 會執行處理作業的DSP。
...
<table>
 <tbody>
  ...
  <tr>
   <td>積極進取</td>
   <td>快取演算後的HTML內容<br /> 快取在Conservative層級快取的所有成品。<br /> <strong>注意</strong>：此策略可產生最佳效能，但耗用更多記憶體來儲存快取的成品。</td>
  </tr>
 </tbody>
</table>

HTML5表單使用LRU策略執行記憶體中的快取。 如果快取策略設為「無」，將不會建立快取，並將清除現有的快取資料（如果有的話）。 除了快取策略之外，您也可以設定記憶體中的快取大小總計，這有助於設定快取大小的最大限制，如果超過此限制，將使用LRU模式來釋放快取資源。
...
在轉譯時，FormCalc指令碼會在伺服器上轉譯（及快取）為JavaScript，對使用者或設計人員而言是透明的。

此指令碼引擎使用ECMAScript5的某些功能，例如Object.defineProperty。 引擎/程式庫會以CQ Client Lib的形式提供，且類別名稱為 **xfaforms.profile**. 它也提供 **FormBridge API** 啟用外部入口網站或應用程式與表單互動。 使用FormBridge，外部應用程式便能以程式設計方式隱藏特定元素、取得或設定其值，或變更其屬性。

如需詳細資訊，請參閱 [Form Bridge](/help/forms/using/form-bridge-apis.md) 文章。

#### 版面引擎 {#layout-engine}

HTML5表單的版面和視覺方面是根據SVG1.1、jQuery、BackBone和CSS3功能。 表單的初始外觀會在伺服器產生並快取。 使用者端會管理該初始版面的調整，以及表單版面的任何進一步增量變更。 為此，Runtime套件包含以JavaScript撰寫並以jQuery/Backbone為基礎的版面引擎。 此引擎會處理所有動態行為，例如新增/移除可重複例項、可成長物件配置。 此版面引擎一次轉譯一頁的表單。 起初，使用者只檢視一個頁面，而水準卷軸只代表第一個頁面。 但是，當使用者向下捲動時，下一頁開始呈現。 此逐頁轉譯可減少在瀏覽器中轉譯第一頁所需的時間，並增強表單的感知效能。 此引擎/程式庫屬於具有類別名稱的CQ Client Lib的一部分 **xfaforms.profile**.

版面引擎也包含一組用於從使用者擷取表單欄位值的Widget。 這些Widget的模型化為 [jQuery UI Widget](https://api.jqueryui.com/jQuery.widget/) 可實作特定附加合約，以順暢地與版面引擎搭配運作。

如需有關Widget和對應合約的詳細資訊，請參閱 [HTML5表單的自訂Widget](/help/forms/using/introduction-widgets.md).
...
HTML5表單支援義大利文(it)、西班牙文(es)、巴西葡萄牙文(pt_BR)、簡體中文(zh_CN)、繁體中文（僅有限支援） (zh_TW)、韓文(ko_KR)、英文(en_US)、法文(fr_FR)、德文(de_DE)和日文(ja)語言。 根據在請求標頭中接收的地區設定，相應的資源包被傳送到使用者端。 此資源套件會作為具有類別名稱的CQ使用者端程式庫新增到設定檔JSP中 **xfaforms.I18N**. 您可以覆寫在設定檔中擷取地區設定套件的邏輯。
...
設定檔節點具有屬性 **sling：resourceSuperType** 含值 **xfaforms/profile**. 此屬性會在內部將要求轉寄給中的設定檔節點的sling指令碼 **/libs/xfaforms/profile** 資料夾。 這些指令碼是JSP頁面，是用來組合HTML表單和必要JS/CSS成品的容器。 這些頁面包含對下列專案的參照：

* **xfaforms.I18N。&lt;locale>**：此資料庫包含當地語系化的資料。
