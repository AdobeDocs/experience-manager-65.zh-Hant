...
如果使用基於UNIX的作業系統，請從相應作業系統的安裝介質中安裝以下32位軟體包：
<table>
 <tbody>
  <tr>
   <td>
...
1. 運行PDF生成器實用程式批處理檔案，為PDF生成器服務配置Acrobat:
   ...
   1. 導航到 `[extracted-zip-file]\jcr_root\etc\packages\day\cq60\fd\adobe-aemds-common-pkg-[version]\jcr_root\etc\packages\day\cq60\fd\`
   1. 解壓縮 `adobe-aemfd-pdfg-common-pkg-[version]`。
   1. 導航到 `[downloaded-adobe-aemfd-pdfg-common-pkg]\jcr_root\libs\fd\pdfg\tools\adobe-aemfd-pdfg-utilities-[version]` 的子菜單。 運行以下批處理檔案：
...
的 [系統就緒性工具](#srt-configuration) 檢查電腦是否配置正確以運行PDF生成器轉換。 該工具在指定路徑上生成報告。 要運行工具，請執行以下操作：
...
1. 從命令提示符運行以下命令：
   ...
   該命令生成報告，並建立srt_config.yaml檔案。 可以使用它來配置SRT工具的選項。 為SRT工具配置選項是可選的。
...
可以使用srt_config.yaml檔案為SRT工具配置各種設定。 檔案格式為：
...
* **區域設定：** 它是強制參數。 它支援英語(en)、德語(de)、法語(fr)和日語(ja)。 預設值為en。 它對OSGi上在AEM Forms運行的PDF發電機服務沒有影響。
* **aemTempDir:** 它是可選參數。 它指定了Adobe Experience Manager的臨時儲存位置。
* **用戶：** 它是可選參數。 您可以指定用戶以檢查用戶是否具有運行PDF生成器所需的目錄所需的權限和讀/寫訪問權限。 如果未指定用戶，則會跳過用戶特定的檢查，並在報告中顯示為失敗。
* **輸出目錄：** 指定保存SRT報告的位置。 預設位置是SRT工具的當前工作目錄。

## 疑難排解

如果即使在修復了SRT工具報告的所有問題後仍面臨問題，請執行以下檢查：

執行以下檢查之前，請確保 [系統就緒性工具](#SRT) 不報告任何錯誤。
...
* 確保32位 [支援的版本 ](aem-forms-jee-supported-platforms.md#software-support-for-pdf-generator) 已安裝Microsoft辦公室，並取消所有應用程式的開啟對話。
* 確保在PDF配置UI中添加PDF生成器用戶。
...
* 確保用戶在PDF生成器UI中配置，並執行以下操作：
   ...
   1. 開啟Microsoft® Office或OpenOffice應用程式並取消所有對話框。
   1. 將AdobePDF設定為預設打印機。
...
* 安裝 [支援的版本](aem-forms-jee-supported-platforms.md#software-support-for-pdf-generator) 開啟。 AEM Forms支援32位和64位版本。 安裝後，開啟所有OpenOffice應用程式，取消所有對話窗口，然後關閉這些應用程式。 重新開啟應用程式並確保開啟OpenOffice應用程式時不顯示對話框。

* 建立環境變數 `OpenOffice_PATH` 並將其設定為指向OpenOffice安裝 [控制台](https://linuxize.com/post/how-to-set-and-list-environment-variables-in-linux/) 或dt（設備樹）配置檔案。
...
* 如果你有Adobe Acrobat的現有許可證，而且已經過期， [下載最新版本的Adobe Application Manager](https://helpx.adobe.com/in/creative-suite/kb/aam-troubleshoot-download-install.html)，並遷移序列號。 之前 [遷移序列號](https://www.adobe.com/devnet-docs/acrobatetk/tools/AdminGuide/licensing.html#migrating-your-serial-number)。
   ...
   * 使用以下命令生成prov.xml並使用prov.xml檔案而不是中提供的命令來保留現有安裝 [遷移序列號](https://www.adobe.com/devnet-docs/acrobatetk/tools/AdminGuide/licensing.html#migrating-your-serial-number) 編號文章。
...
要解決此問題，請參見 [無法在Windows Server上將Word或Excel檔案轉換為PDF](/help/forms/using/disable-uac-for-pdfgconfiguration.md)。
