...
| 屬性 | 說明 | 預設 | 可接受的值 |
|---|---|---|---|
| 顯示預留位置 | 選取核取方塊以啟用顯示最適化表單和互動式通訊中所包含欄位的預留位置。 | 已選取 | 不適用 |
| 快取專案上限 | 設定可使用快取記憶體擷取的最適化表單和互動式通訊數目上限。 | 100 | 數字 |
| 將檔案名稱設為唯一 | 選取此核取方塊，可為Adaptive Forms和互動式通訊中作為附件包含的檔案指定唯一名稱。 | 未選取 | 不適用 |
