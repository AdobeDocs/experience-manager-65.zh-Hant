...
* **使用表單資料模型**
   [資料整合](../../forms/using/data-integration.md) 可讓您將不同資料來源的實體和服務整合到表單資料模型中，以便用來建立最適化表單。 如果您建立的最適化表單涉及從多個資料來源擷取資料並將其寫入多個資料來源，請選擇表單資料模型。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>新增欄位以擷取電話號碼。 電話元件允許作者配置以下電話號碼類型之一。 每個類型都與驗證的預設規則運算式相關聯。</p>
    <ul>
     <li>「國際類型」通過 <code>^[+][0-9]{0,14}$</code>.</li>
...
* **模擬器** ![尺標](assets/ruler.png):可讓您針對不同的顯示大小（例如平板電腦和手機）模擬表單的外觀。
...
* **編輯**:可讓您選取其他模式，例如： **[!UICONTROL 編輯]**, **[!UICONTROL 樣式]**, **[!UICONTROL 開發人員]**，和 **[!UICONTROL 設計]**.
   ...
   * **Design**. 可讓您啟用或停用自訂元件，或側邊欄中未列出的現成元件。
...
答：**設定**:當您點選 **[!UICONTROL 設定]**，元件屬性會顯示在側邊欄中。 設定這些屬性可讓您自訂資料擷取體驗。 您可以變更元件的元素名稱，在元件的「標題」欄位中指定標籤文字。 元素名稱可讓您擷取使用者使用元件輸入的值。 在元件屬性中，您可以指定元件的行為，並管理使用者輸入。 在側欄中設定屬性以擷取使用者資料，並用於進一步處理。 最適化表單容器的屬性可讓您指定用戶端程式庫、配置、主題、記錄檔案設定、儲存設定、提交設定和中繼資料設定。
...
* **guideContainer**:適用性表單的根，標示為 **[!UICONTROL 最適化表單開始]** 在適用性表單UI中。 在此元件中，您可以指定：
