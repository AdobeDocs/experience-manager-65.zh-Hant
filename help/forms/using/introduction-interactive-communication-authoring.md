...
* **屬性瀏覽器**
  ...
  可讓您編輯元件的屬性。屬性會根據元件而變更。 例如，若要檢視檔案容器的屬性：請選取元件，然後選取 ![欄位層級](assets/field-level.png) > **檔案容器**，然後選取 ![cmppr](assets/cmppr.png).
...
| **Component** | **Print Channel** | **網頁管道** | **功能** |
|---|---|---|---|
...
* 以元件的元素名稱來識別元件。 當您選取 ![cmppr](assets/cmppr.png)中，您可以透過變更屬性瀏覽器中的元素名稱欄位值來變更元件名稱。 「元素名稱」欄位僅接受字母、數字、連字型大小(-)和底線(_)。 不允許使用其他特殊字元，元素名稱應以字母開頭。
...
答：**設定**：當您選取 **設定**，元件屬性會顯示在側邊欄中。

B.**編輯規則**：選取編輯規則時，規則編輯器會出現，您可以在其中編輯和建立所選元件的規則。 在規則編輯器中，您也可以選取其他表單物件（元件）並編輯/建立這些表單物件的規則。
