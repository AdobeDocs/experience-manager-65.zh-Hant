...
seo-description: 各種裝置上最適化表單的版面和外觀由版面設定控制。 了解各種版面及如何套用。
...
# 適用性表單的版面功能{#layout-capabilities-of-adaptive-forms}
...
**面板** 佈局控制面板內的項目或元件在設備上的顯示方式。

**行動** 版面控制行動裝置上表單的導覽。如果裝置寬度為768像素或以上，則版面會視為行動版面，並針對行動裝置最佳化。

**工** 具欄佈局控制窗體中工具欄或面板工具欄中「操作」按鈕的放置。
...
### 回應式 — 單一頁面上的所有項目，不含導覽{#responsive-everything-on-one-page-without-navigation-br}
...
使用`Panel adaptive form`元件在面板內提供逐步導覽。 使用此版面時，使用者只有在目前步驟完成後才會移至下一個步驟
...
### 折疊式功能表設計的佈局{#layout-for-accordion-design}

使用此版面，您可以將`Panel adaptive form`元件放置在具有折疊式面板樣式導覽的面板中。 使用此配置，您也可以建立可重複的面板。 可重複的面板可讓您視需要動態新增或移除面板。 您可以定義面板重複的次數下限和上限。 此外，基於在面板項中提供的資訊，可動態地確定面板的標題。
...
使用此佈局，可以將`Panel adaptive form`元件放置在帶有頁簽導航的面板中。 標籤會放置在面板內容的左側。

![在「頁簽式佈局」中，頁簽顯示在左側](assets/tabbed_layout_left.png)

顯示在面板左側的標籤

### 頁簽式佈局 — 頁簽顯示在{#tabbed-layout-tabs-appear-on-the-top}頂部

使用此佈局，可以將`Panel adaptive form`元件置於帶有頁簽導航的面板中。 標籤會放置在面板內容上方。
...
此版面會使用導覽列和導覽功能表來控制導覽。 導航欄顯示&#x200B;****&#x200B;和&#x200B;****&#x200B;表徵圖，以表單指示&#x200B;**next**&#x200B;和&#x200B;**previous**&#x200B;導航步驟。

在`/libs/fd/af/layouts/mobile/`位置可使用「行動配置」。 依預設，適用性表單中提供下列行動配置。

![最適化表單中的行動配置清單](assets/mobile-navigation.png)

最適化表單中的行動配置清單

使用行動版面時，點選![aem6forms_form_menu](assets/aem6forms_form_menu.png)圖示即可使用表單功能表，以存取各種表單面板。

### 表單標題{#layout-with-panel-titles-in-the-form-header}中面板標題的版面配置
...
### 表單標題{#layout-without-panel-titles-in-the-form-header}中沒有面板標題的佈局
...
### 工具欄{#default-layout-for-toolbar}的預設佈局
