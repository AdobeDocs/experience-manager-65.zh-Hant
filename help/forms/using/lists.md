...
1. 點選 **下一個**. 「通訊管理」會顯示「編輯器」頁面，您可以在其中新增文欄位落和資料元素至文字。
...
1. 文字編輯器開啟，輸入文字。 使用頁面頂端的工具列來格式化文字、插入條件、連結和分頁符號。
   ...
   * **條件**：點選以插入條件。 根據條件插入文字。 如果條件為true，則信函中會顯示文字，否則不會顯示。
...
1. 從資料面板，拖放資料字典元素和預留位置元素至文字。
   ...
   * 在文字中新增資料字典元素，從清單中選取資料元素，然後點選「插入」( ![插入](assets/insert.png))。 如果您選取「受保護」，資料字典元素為唯讀，且會出現在信函編輯器中，但不會出現在「建立通訊」使用者介面或「通訊建立者」中。
   * 在文字中新增預留位置元素，在「資料元素」面板中，點選「新建」，輸入新資料元素的詳細資料，然後點選「建立」以將新元素新增至清單。 新預留位置可以與資料字典元素一樣插入文字中。 若要編輯預留位置，請選取預留位置並點選「編輯」。
...
1. 為清單指定下列資訊：
   ...
   * **標籤（選擇性）**：選取要套用的標籤。 您也可以輸入新標簽名稱並建立它。 (新標籤會在您點選時建立 **儲存**.)
...
1. 資產會新增至「清單專案」頁面。
若要變更清單中資產的順序，請點選並按住箭頭圖示( ![拖放](assets/dragndrop.png) )和拖放。 當使用者在「建立通訊」使用者介面中開啟信函範本時，內容會依照您在此處定義的順序進行組裝。
...
1. 您可以選取下列選項，指定清單中每個資產在執行階段時的行為：
   ...
   * **複合：** 選取後，組合編號會套用為外部（父項）清單的樣式和它自己的樣式的組合。 此巢狀「清單」上的複合編號是根據此巢狀「清單」出現在外部「清單」中的順序而定。
...
1. 為清單指定下列資訊：
   ...
   * **標籤（選擇性）**：選擇性地選取要套用的標籤。 您也可以輸入新標簽名稱並建立它。 (新標籤會在您點選時建立 **儲存**.)
...
1. 若要變更條件中資產的順序，請點選並按住箭頭圖示( ![拖放](assets/dragndrop.png) )和拖放。 當使用者在「建立通訊」使用者介面中開啟信函範本時，內容會依照您在此處定義的順序進行組裝。
1. 點選 **刪除** 以刪除列。 如果您點選刪除以取得預設列，只會清除資產資訊。
1. 點選 **複製** 以複製列。
1. 點選 **編輯** 以變更資產或編輯運算式。
...
1. 選取 **建立** > **佈局片段**.
1. 通訊管理顯示可用的XDP。 選取您要作為佈局片段基礎的XDP，然後點選 **下一個**.
1. 為版面配置指定下列資訊：
