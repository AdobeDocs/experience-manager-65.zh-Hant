...
如果您是開發人員或網站擁有者，您可以自訂AEM Forms的一般使用者介面 — Forms Portal，以反映您在組織中使用的中繼資料。 如需Forms入口網站的詳細資訊，請參閱 [在入口網站上發佈表單簡介](../../forms/using/introduction-publishing-forms.md).
...
1. 使用下列其中一種方式開啟屬性頁面：
   ...
   1. 按一下檢視屬性 ![e_reviewmode_properties_n](assets/e_reviewmode_properties_n.png) 圖示加以存取。

      >[!NOTE]
      >
      >快速動作是指滑鼠懸停時縮圖上顯示的動作專案。

   1. 選取表單並按一下檢視屬性 ![e_reviewmode_properties_n](assets/e_reviewmode_properties_n.png) 圖示顯示在工具列中。
   1. 不在選擇模式中時，按一下表單縮圖，導覽至表單詳細資訊頁面。 現在，按一下 ![aem6forms_eye_viewon](assets/aem6forms_eye_viewon.png) 右上角的眼睛圖示，然後按一下下方清單中的「屬性」 。
...
對於所有表單型別，您都可以選擇按一下 **[!UICONTROL 上傳影像]** 並從本機目錄中瀏覽影像檔案。 選取的影像會用作縮圖，而非預設的縮圖。
...
提供的工具（中繼資料結構編輯器）可定義中繼資料配置的結構；也就是說，顯示在 **[!UICONTROL 屬性]** 表單頁面。 中繼資料結構編輯器可讓您新增或修改資產的自訂結構。
...
1. 任何按下的資產型別都會開啟一個清單，內含 `extendedmetadata` 選項。 編輯此結構描述。

1. 選取旁邊的核取方塊 `extendedmetadata` 然後按一下編輯 ![aem6forms_edit](assets/aem6forms_edit.png) 圖示顯示在工具列中。
...
1. AEM Forms會開啟所選資產型別的中繼資料結構編輯器/表單產生器（在此案例中為調適型表單）。
   ...
   1. 您可以從以下位置拖曳欄位元件，新增所需型別的自訂欄位： **[!UICONTROL 建置表單]** 區段至「綱要」頁面。
...
1. 從拖曳所需型別的元件 **[!UICONTROL 建置表單]** 區段至左側面板，並放置在方便的位置。
...
1. 按一下您剛才拖曳的元件。 在右側面板中開啟的「設定」標籤中，填寫下列欄位的資訊：
   ...
   1. 在「對應至屬性」欄位下方，您可以看到預填的值 **&#39;./jcr：content/metadata/default&#39;**. 變更&#39;**預設**&#39;至所需的屬性名稱，此屬性用於儲存crx存放庫中的屬性(例如： &#39;。/jcr：content/metadata/department&#39;)
