...
* 對於使用[XFA表單範本](../../forms/using/prepopulate-adaptive-form-fields.md)的適用性表單，請使用與XFA範本資料結構相容的預填XML。
* 對於使用[XML架構](#xml-schema-af)的適用性表單，請使用與XML架構結構相容的預填XML。
* 針對使用[JSON結構](#json-schema-based-adaptive-forms)的適用性表單，請使用符合JSON結構的預填JSON。
* 對於使用FDM結構的適用性表單，請使用與FDM結構相容的預填JSON。
* 對於沒有表單模型](#adaptive-form-with-no-form-model)的適用性表單，沒有系結資料。 [每個欄位都是未綁定的欄位，使用未綁定的XML預填。
...
針對以JSON結構為基礎的適用性表單，預填JSON和已提交JSON的結構說明如下。 如需詳細資訊，請參閱[使用JSON結構](../../forms/using/adaptive-form-json-schema-form-model.md)建立最適化表單。
...
1. 為&#x200B;**資料檔案位置**&#x200B;輸入資料位置或規則運算式（規則運算式）。 有效資料檔案位置的範例包括：
...
* SERVICE_NAME表示OSGI預填服務的名稱。 請參閱[建立並執行預填服務](../../forms/using/prepopulate-adaptive-form-fields.md#create-and-run-a-prefill-service)。
...
## 在客戶端{#prefill-at-client}預填資料

預填最適化表單時，AEM Forms伺服器會合併資料與最適化表單，並將已填入的表單傳送給您。 依預設，資料合併動作會在伺服器上發生。

您可以設定AEM Forms伺服器以在用戶端（而非伺服器）執行資料合併動作。 大幅縮短預填及演算最適化表單的所需時間。 預設情況下，該功能為禁用狀態。 您可以從Configuration Manager或命令行啟用它。
...
* 要從命令行啟用或禁用：
   ...
   * 若要停用，請執行下列cURL命令：
   ...
   若要充分利用「用戶端預填資料」選項，請更新預填服務以傳回[FileAttachmentMap](https://helpx.adobe.com/experience-manager/6-5/forms/javadocs/com/adobe/forms/common/service/PrefillData.html)和[CustomContext](https://helpx.adobe.com/experience-manager/6-5/forms/javadocs/com/adobe/forms/common/service/PrefillData.html)
