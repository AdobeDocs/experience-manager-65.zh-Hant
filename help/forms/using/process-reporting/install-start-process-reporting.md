...
流程報表可讓AEM Forms使用者查詢目前在AEM Forms實作中定義的AEM Forms流程相關資訊。 不過，程式報告不會直接從AEM Forms存放庫存取資料。 資料會先依排程發佈至Process Reporting存放庫(*由ProcessDataPublisher和ProcessDataStorage服務* s)。 接著，會從發佈至存放庫的「程式報告」資料產生「程式報告」中的報告和查詢。 程式報告會安裝為Forms Workflow模組的一部分。
...
>[!NOTE]
>
>如需「清除公用程式」的秘訣與技巧，請參閱Adobe Developer Connection文章，位置在： [永久刪除處理與工單](/help/forms/using/admin-help/purging-process-data.md).
...
1. &#x200B;URL
   ...
   * (適用於Linux®、AIX®及Solaris™) `[JBoss root]/bin/run.conf.sh` 編輯器中儲存的檔案。
...
1. 在「伺服器基礎結構」下，按一下 **Java™和流程管理** > **程式定義**.

1. 在其他屬性下，按一下 **Java™虛擬機器器**.
...
1. 在「領域結構」下，按一下 **環境** > **伺服器** 在右窗格中，按一下受管理的伺服器名稱。
...
您可以使用管理主控台來設定根(**預設**： `/content/reporting/pm`)位置和子資料夾(**預設**： `/yyyy/mm/dd/hh/mi/ss`)要儲存程式資料的階層格式。
...
1. 瀏覽至 **首頁** > **服務** > **應用程式和服務** >**服務管理** 並開啟 **ProcessDataStorageProvider** 服務。
...
1. 登入 **組態管理員** 具有CRX管理員認證。 Configuration Manager的預設URL為 `https://'[server]:[port]'/lc/system/console/configMgr`
1. 開啟 **報告組態** 服務。
1. **記錄數**
...
1. 瀏覽至 **首頁** > **服務** > **應用程式和服務** >**服務管理** 並開啟 **ProcessDataPublisher** 服務。
...
當您導覽至「程式報表」URL (https://)時&lt;server>：&lt;port>/lc/pr)，則會顯示登入畫面。
...
**報告：** 此專案包含隨附於「處理報告」的現成報表。

如需預先定義報表的詳細資訊，請參閱 [程式中的預先定義報表](/help/forms/using/process-reporting/pre-defined-reports-in-process-reporting.md).

**臨機查詢：** 此專案包含用於執行以篩選為基礎搜尋流程和任務的選項。
...
**程式報告標題列：** 程式報告標題列包含一些通用選項，您可以在使用者介面中使用這些選項。

**程式報告標題：** 程式報告標題會顯示在標題列的左角。

隨時按一下標題以返回首頁畫面。

**上次更新時間：** 程式資料會依排程從AEM Forms資料庫發佈到Process Reporting存放庫。

「上次更新時間」會顯示資料更新已推送至「程式報表」存放庫的上次日期與時間。

如需資料發佈服務以及如何排程此服務的詳細資訊，請參閱 [排程程式資料發佈](/help/forms/using/process-reporting/install-start-process-reporting.md#p-schedule-process-data-publishing-p) 在程式報告快速入門文章中。

**程式報告使用者：** 登入的使用者名稱會顯示在上次更新時間的右側。

**程式報表標題列下拉式清單：** 「程式報表」標題列右角的下拉式清單包含下列選項：
