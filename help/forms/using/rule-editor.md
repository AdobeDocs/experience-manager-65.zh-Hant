...
規則編輯器會取代AEM 6.1 Forms和舊版中的指令碼功能。 不過，您現有的指令碼會保留在新規則編輯器中。 有關在規則編輯器中使用現有指令碼的詳細資訊，請參閱[規則編輯器對現有指令碼的影響](../../forms/using/rule-editor.md#p-impact-of-rule-editor-on-existing-scripts-p)。
...
* 當用戶更改應用「時間」規則的對象值時，將評估「時間」規則中的條件。 不過，如果您想要在伺服器端的值變更時觸發動作，例如預先填入值，建議您撰寫在欄位初始化時觸發動作的「時機」規則。
...
**叫用** 服務叫用在表單資料模型中配置的服務。選擇「調用服務」操作時，將顯示一個欄位。 點選欄位時，會顯示在AEM例項上所有表單資料模型中設定的所有服務。 在選擇表單資料模型服務時，會出現其他欄位，您可以在其中使用指定服務的輸入和輸出參數映射表單對象。 請參閱用於調用表單資料模型服務的示例規則。

除了表單資料模型服務外，您還可以指定直接WSDL URL來叫用Web服務。 不過，表單資料模型服務有許多優點，並建議使用建議的方式來叫用服務。

如需在表單資料模型中設定服務的詳細資訊，請參閱[AEM Forms Data Integration](/help/forms/using/data-integration.md)。

**設定值** ofComputes並設定指定對象的值。可以將對象值設定為字串、另一個對象的值、使用數學表達式或函式的計算值、對象的屬性值或配置表單資料模型服務的輸出值。 當您選擇web services選項時，它會顯示在AEM例項上所有表單資料模型中設定的所有服務。 在選擇表單資料模型服務時，會出現其他欄位，您可以在其中使用指定服務的輸入和輸出參數映射表單對象。
...
規則編輯器使用者介面左側的窗格包含兩個標籤： **[!UICONTROL 形成對象]**&#x200B;和&#x200B;**[!UICONTROL 函式]**。
...
>[!NOTE]
>
>您可以在「表單對象」和「函式」頁籤中對對象和函式名稱和標題執行文本搜索。
...
1. 前往`https://[host]:[port]/system/console/configMgr`的AEM網頁主控台。
1. 按一下以編輯&#x200B;**[!UICONTROL Adaptive Form Configuration Service]**。
...
1. 在運算式欄位中：
   ...
   * 從「表單對象」頁籤中，選擇或拖放第一個&#x200B;**拖放對象中的** Salary **欄位，或選擇此處**&#x200B;欄位。

   * 從&#x200B;**選擇運算子**&#x200B;欄位中選擇&#x200B;**Plus**。

   * 從「表單對象」頁籤中選擇或拖放其他&#x200B;**拖放對象中的**&#x200B;配偶薪金&#x200B;**欄位，或選擇此處**&#x200B;欄位。
...
1. 點選「**新增條件**」以新增When陳述式。
   ...
   * 從「表單對象」頁籤中，選擇或拖放第一個&#x200B;**拖放對象中的**&#x200B;婚姻狀態&#x200B;**欄位，或選擇此處**&#x200B;欄位。
...
* 選項按鈕&#x200B;**您是Geometrixx的現有客戶嗎？**，它提供「是」和「否」選項。Yes的值為&#x200B;**0**,No為&#x200B;**1**。
