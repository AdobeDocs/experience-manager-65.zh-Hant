...
規則編輯者取代 AEM 6.1 Forms 和更早版本中的指令碼功能。 但現有的腳本會保留在新的規則編輯者中。 如需在規則編輯器中使用現有指令碼的詳細資訊，請參閱 [規則編輯器對現有指令碼的影響](#impact-of-rule-editor-on-existing-scripts).
...
此 **時間** 規則型別會遵循 **condition-action-alternate action** 規則建構，或有時僅 **condition-action** 建構。 在此規則類型中，您首先指定求值的條件，其後加上要觸發的動作（ `True` ）。 使用規則類型時，您可以使用多個 AND 和 OR 運算子來建立 [ 嵌套運算式 ](#nestedexpressions) 。

使用規則類型時，您可以評估表單物件上的條件，並對一或多個物件執行動作。
...
撰寫When規則時，您可以觸發「清除值」動作。 清除操作值會清除指定物件的值。 在 When 語句中，將明確值用作選項，可讓您使用多個欄位建立複雜的條件。
...
請注意，並非所有表單物件（例如面板和工具列按鈕）都可使用「設定值」規則型別。 規則的標準集值具有下列結構：
...
>[!NOTE]
>
>此外，您可以使用規則的「設定值」，從表單資料模型服務或Web服務的輸出，填入下拉式清單元件中的所有值。 不過，請確定您選擇的輸出引數屬於陣列型別。 陣列中傳回的所有值都可用於指定的下拉式清單。
...
「表單物件」標籤會顯示最適化表單中包含之所有物件的階層檢視。 它會顯示物件的標題和型別。 撰寫規則時，您可以將表單物件拖放至規則編輯器上。 在建立或編輯規則時，將物件或函數拖曳至預留位置時，預留位置會自動採用適當的數值型別。

具有一或多個有效規則的表單物件以綠點標示。 如果無效套用至表單物件的規則之一，表單物件會標記為黃色點。
