...
此 **時間** 規則型別會遵循 **condition-action-alternate action** 規則建構，或有時僅 **condition-action** 建構。 在此規則類型中，首先指定評估條件，然後在滿足條件時觸發操作 （ `True` ）。 使用 When 規則 類型時，可以使用多個 AND 和 OR 運算子來創建 [ 嵌套運算式 ](#nestedexpressions) 。
...
撰寫When規則時，您可以觸發「清除值」動作。 清除值操作將清除指定物件的值。 在 When 語句中使用「清除值」選項可以創建具有多個欄位的複雜條件。
...
請注意，並非所有表單物件（例如面板和工具列按鈕）都可使用「設定值」規則型別。 標準的「規則集值」具有下列結構：
...
以下示例將欄位中的值作為輸入，並將欄位的值 `dependentid` `Relation` 設置為表單資料模型服務參數 `getDependent` 的 `Relation` 輸出。
...
>[!NOTE]
>
>此外，您可以使用規則的「設定值」，從表單資料模型服務或Web服務的輸出，填入下拉式清單元件中的所有值。 不過，請確定您選擇的輸出引數屬於陣列型別。 陣列中返回的所有值都可在指定的下拉清單中使用。
...
「表單物件」標籤會顯示最適化表單中包含之所有物件的階層檢視。 它會顯示物件的標題和型別。 撰寫規則時，您可以將表單物件拖放至規則編輯器上。 創建或編輯規則時，將物件或函數拖放到預留位置中時，預留位置會自動採用適當的數值型別。

應用了一個或多個有效規則的表單物件用綠點標記。 如果無效應用於表單物件的任何規則，則表單物件將標有黃點。

函數標籤包括一組內建函數，如總和、最小值、最大值、平均值、數量和驗證表單。 您可以使用這些函數來計算可重複面板和表行中的值，並在編寫規則時在操作和條件陳述式中使用它們。 不過，您也可以建立 [ 自訂函數 ](#custom-functions) 。
...
在表單物件的左側樹狀結構中，您可以點選表單物件以顯示套用至每個物件的規則。 您不僅可以瀏覽各種表單物件的規則，也可以在表單物件之間複製 — 貼上規則。 如需詳細資訊，請參閱 [複製貼上規則](#copy-paste-rules).
...
1. 首先，根據使用者為「婚姻狀況」選項按鈕選取的選項，撰寫規則以控制「配偶薪資」欄位的可見度。
   ...
   以編寫模式開啟貸款申請表單。 點選 **婚姻狀況** 元件並點選 ![edit-rules](assets/edit-rules.png). 下一步，點選 **[!UICONTROL 建立]** 以啟動規則編輯器。
...
1. 拖放 **配偶薪資** 欄位。 **將物件放下或選取這裡** 欄位。 或者，點選 **將物件放下或選取這裡** 欄位並選取 **配偶薪資** 欄位，其中列出表單中的所有表單物件。
...
當您點選物件並點選時，表格物件上的任何現有規則都會列出 ![edit-rules1](assets/edit-rules1.png). 您可以檢視標題並預覽規則摘要。 此外，UI可讓您展開並檢視完整的規則摘要、變更規則順序、編輯規則及刪除規則。

![list-rules](assets/list-rules.png)

您可以對規則執行下列動作：

* **展開/摺疊**：規則清單中的「內容」欄會顯示規則內容。 如果預設檢視中看不到整個規則內容，請點選 ![expand-rule-content](assets/expand-rule-content.png) 以展開它。
...
* **編輯**：若要編輯規則，請選取規則標題旁的核取方塊。 會出現編輯和刪除規則的其他選項。 點選 **編輯** 以視覺化或程式碼編輯器模式，在規則編輯器中開啟選取的規則（視用於建立規則的模式而定）。
...
1. 點選您要從中複製規則的表單物件，然後在元件工具列上點選 ![editrule](assets/editrule.png). 規則編輯器使用者介面會出現，並選取表單物件，而現有規則會出現。
...
1. 選取另一個要貼上規則的表單物件，然後點選 **貼上**. 此外，您可以編輯規則以對其進行變更。
...
您也可以拖放規則中的條件以進行編輯。 點選並將滑鼠懸停在控點上( ![控點](assets/handle.png))。 指標變成如下所示的手形符號後，請將條件拖放至規則內的任何位置。 規則結構會變更。
