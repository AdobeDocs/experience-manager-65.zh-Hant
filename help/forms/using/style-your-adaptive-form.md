...
您可以使用主題為最適化表單提供獨特的外觀和樣式。 您可以套用隨附於最適化表單編輯器的方塊外主題，或建立您自己的自訂主題。 AEM提 [!DNL Forms] 供主題 [編輯器](https://helpx.adobe.com/experience-manager/6-3/forms/using/themes.html) ，以建立自訂主題。 單一主題可針對在行動裝置、平板電腦或桌上型電腦上開啟的相同最適化表單提供不同的外觀。 使用主題編輯器時，不需具備任何CSS或LESS的先前知識，但需要它。
...
1. 開啟最適化表 **[!UICONTROL 單容器的屬性]**。 在屬性瀏覽器中，導覽至「基 **[!UICONTROL 本]** >最適 **[!UICONTROL 化表單主題」]**。 「 **[!UICONTROL 最適化表單主題]** 」欄位會列出所有現成可用的主題和自訂主題。 依預設，會套用「畫布」主題。
1. 從「最適化表單主題」 **[!UICONTROL 欄位中選取主題]** 。 例如，「調 **查」主題**。 點 ![選aem_6_3_forms_save](assets/aem_6_3_forms_save.png) ，以套用選取的主題。
...
1. 變更標題的現有標誌和文字。 若要移除標誌：
   ...
   1. 在頁首元件中點選標誌 [!UICONTROL 影像] ，然後點選 ![cmppr](assets/cmppr.png)**[!UICONTROL 屬性]**。 在image屬 [!UICONTROL 性中] ，點選X以移除現有的標誌影像。
   1. 點 **[!UICONTROL 選「upload]**」 ![、選取logo.png，然後點選](assets/aem_6_3_forms_save.png) aem_6_3_forms_save，以儲存變更。 影像已下載至「開始 [之前](/help/forms/using/style-your-adaptive-form.md#before-you-start) 」區段。
   1. 點選標題文 `We.Retail`字，點 ![選aem_6_3_edit](assets/aem_6_3_edit.png) **[!UICONTROL edit]**。 將頁首文字變更為 `we retail`。 僅將粗體格式套用 `we`於 `we retail`。
...
1. 移除標題並新增預留位置文字：
   ...
   1. 將「標題」欄位的內 **[!UICONTROL 容複製]** 到「預留位 **[!UICONTROL 置文字」欄位]** 。
   1. 刪除「標題 **[!UICONTROL 」欄位的內容，並]** 點選aem_6_3_forms_save ![](assets/aem_6_3_forms_save.png)。
...
1. 登入AEM作者例項，並導覽至 **[!UICONTROL Adobe Experience Manager]** > **[!UICONTROL Forms]** >主 **[!UICONTROL 題]**。 預設URL為 [http://localhost:4502/aem/forms.html/content/dam/formsanddocuments-themes](http://localhost:4502/aem/forms.html/content/dam/formsanddocuments-themes)。
1. 點選「 **[!UICONTROL 建立]** 」並選 **[!UICONTROL 取「主題」]**。 此時 [!UICONTROL 會顯示「建立主題] 」頁面，其中包含建立主題所需的欄位。 「標 **[!UICONTROL 題]** 」和「 **[!UICONTROL 名稱]** 」欄位是必填欄位：
...
1. 點選「 **[!UICONTROL 建立]**」。 將建立一個主題，並出現一個用於開啟表單進行編輯的對話框。 點選 **[!UICONTROL 「開啟]** 」，在新標籤中開啟新建立的主題。 主題在主題編輯器中開啟。 對於樣式，主題編輯器會使用AEM隨附的現成可用最適化表單 [!DNL Forms]。

   如需使用主題編輯器UI的詳細資訊，請參 [閱關於主題編輯器](/help/forms/using/themes.md#aboutthethemeeditor)。

1. 點選 **[!UICONTROL 主題選項]**![主題選項](assets/theme-options.png) >設 **[!UICONTROL 定]**。 在「預 **[!UICONTROL 覽表單]** 」欄位中，選取 **shipping-address-add-update-form** adaptive表單、點選 ![aem_6_3_forms_save](assets/aem_6_3_forms_save.png)、點選 **** SaveJost。 現在，主題編輯器已設定為使用您自己的最適化表單，而非預設的最適化表單。 點選「 **[!UICONTROL 取消]** 」可返回主題編輯器。
...
1. 導覽「選 **[!UICONTROL 擇器]** 」面板中的「頁首 **[!UICONTROL >文字]** 」選項。 「選擇器」面板位於主題編輯器的左側。 如果面板不可見，請點選「 ![切換側面板](assets/toggle-side-panel.png) 」「切換側面板」。

1. 在 **[!UICONTROL Text]** accordion中設定下列屬性，並點 ![選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 點選頁首 [!UICONTROL 介面工具] ，然後點 **[!UICONTROL 選頁首]**。 標題介面工具集樣式的選項會顯示在左側。 展開「 **[!UICONTROL Dimensions &amp; Position]** 」(尺寸與位置 **[!UICONTROL )accordion、將「]** Height `120px`」（高度）設 ![定為，然後點選](assets/aem_6_3_forms_save.png)aem_6_3_forms_save。
1. 展開頁 **[!UICONTROL 首介面工具]** 集的「背景」收合式面板，將「背景 **[!UICONTROL 顏色」設為]** `F6921E.`

   將滑鼠指標暫 **[!UICONTROL 留在「影像與漸層]** > **[!UICONTROL +新增」上，點選「]**&#x200B;影像 ****」。 設定下列屬性， ![然後點選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 在主題編輯器中，點選頁首中的標誌，然後點選「頁首標 **[!UICONTROL 志」]**。 展開「維度與位置」accordion，設定下列屬性並點 ![選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 點選頁尾Widget並點選頁尾 **[!UICONTROL Footer]**。 展開 **[!UICONTROL Background]** accordion，將「Background Color **[!UICONTROL 」（背景顏色）設為]** ，然後點選 `F6921E`aem_6_3_forms_save ![](assets/aem_6_3_forms_save.png)。
...
1. 點選「客 **[!UICONTROL 戶ID]** 」欄位，然後點 **[!UICONTROL 選「欄位介面工具集]** 」選項。 設定下列屬性， ![然後點選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 點選「Customer ID（客戶ID）」欄位上方的空白區 **[!UICONTROL 域]** ，然後點 **[!UICONTROL 選「Responsive Panel Container（回應面板容器）]**」。 將「背 **[!UICONTROL 景]** >背 **[!UICONTROL 景顏色]** 」設定為F1F2F2。 點 ![選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 點選「 **[!UICONTROL Submit]** 」按鈕並點選「 **[!UICONTROL Button]** 」選項。 設定下列屬性， ![然後點選aem_6_3_forms_save](assets/aem_6_3_forms_save.png)。
...
1. 在頂欄上，選取「樣式 **[!UICONTROL 」選項]** 。

   ![樣式——選項](assets/style-option.png)

1. 點選「 **[!UICONTROL Attach]** （附加）」按鈕並點 ![選「aem_6_3_](assets/aem_6_3_edit.png)editicon」。 在「維度與位置」accordion中 **[!UICONTROL 設定下列屬性]** :
...
1. 點選「 **[!UICONTROL Submit]** （送出）」按鈕並點 ![選「aem_6_3_edit](assets/aem_6_3_edit.png) 」圖示。 設定下列屬性：
