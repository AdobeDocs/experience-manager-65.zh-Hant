...
用於欄位的HTML專案&#x200B;**label**&#x200B;包含類別&#x200B;**left**&#x200B;或&#x200B;**top**，這取決於標籤是在頂端還是左側。
...
視其型別而定，Widget也包含類別。 通常Widget包含`guideFieldWidget`類別。 HTML隨附的Widget通常使用標準HTML元素輸入並選取。 樣式會據此來設定。 您無法藉由變更變數來設定自訂Widget的樣式。
...
以下是標籤導覽器元素的HTML程式碼（類似於「啟動程式」標籤）：
