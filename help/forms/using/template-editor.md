...
1. 在範本主控台中，選取 **建立** 在您建立的資料夾內。
1. 在「挑選範本型別」區段中，選取 **自適應表單範本** 並選取 **下一個**.

1. 在「範本詳細資訊」區段中，提供範本標題並選取 **建立**.
...
另請參閱 [製作調適型表單簡介](../../forms/using/introduction-forms-authoring.md) 來瞭解觸控式UI編輯器。
...
![結構圖層中的配置容器](assets/header-layer-selector.png)

在結構圖層中配置容器

**答：** 頁首元件的配置容器 **B.** 頁尾元件的配置容器
...
表單作者可將其用作建立表單的基礎。 內容流程結構是在範本的「初始內容」層中所指定。 若要切換到編輯表單範本的初始內容，在頁面工具列中的「預覽」之前，選取 ![畫佈下拉式清單](assets/canvas-drop-down.png) **>初始內容**.
...
* 您可以點選「 」以新增面板 **+** 按鈕(當您選取 **將元件拖曳到這裡** 選項。

* 您可以從側邊欄中的元件瀏覽器拖放面板元件。
* 您可以新增的子面板 `guideRootPanel` 元件工具列中的。

若要建立「一般資訊」和「專業資訊」標籤，請在 `guideRootPanel`. 選取面板並選取 ![cmppr](assets/cmppr.png) 以開啟側邊欄中的屬性。 將元素名稱變更為 `general-info` 和 `professional-info`和標題分別作為「一般資訊」和「專業資訊」。 在側邊欄中，選取內容以開啟內容瀏覽器。 在「表單物件」標籤中，選取 `guideRootPanel`. 在編輯器中，選取guideRootPanel。 選取 ![cmppr](assets/cmppr.png) 以開啟其屬性。 在「面板配置」欄位中，選取 **索引標籤在頂端** 並選取 **完成**. 標籤範本結構即會套用。

#### 在索引標籤中新增內容 {#adding-content-in-tabs}

![在自適應表單範本中新增欄位](assets/template-edit-initial-content.png)

新增面板並將面板結構為索引標籤後，您即可在索引標籤內新增欄位。 在編輯器中選取索引標籤時，您可以看到 **將元件拖曳到這裡** 選項。 您可以拖放元件，例如文字方塊、清單專案和按鈕。 您可以從側邊欄中的元件瀏覽器拖放元件。

每個元件都有可增強資料擷取和操作的屬性。 例如，您可以啟用 **必要欄位** 元件的屬性。 您的作者可以指定客戶在略過填寫必填欄位時看到的訊息。 在中指定訊息 **必填欄位訊息** 屬性。
...
1. 瀏覽至 **Adobe Experience Manager >工具>範本**，然後開啟您已建立範本的資料夾。
...
表單可與其範本搭配使用。 下載使用自訂範本建立的最適化表單時，不會下載範本。 當您在其他AEM Forms執行個體上匯入表單時，會匯入表單而不包含其範本。 如果表單已匯入，但其範本無法使用，則不會轉譯表單。 您可以封裝自訂範本，從 `/conf` 中的節點 `https://<server>:<port>/crx/packmgr`，並將其移植到您要上傳表單的AEM Forms執行個體中。
...
1. 登入AEM Forms作者例項並導覽至 **工具** > **作業** > **網頁主控台**.

   >[!NOTE]
   >
   >AEM網頁主控台的URL為https://&#39;[伺服器]：[連線埠]&#39;/system/console/configMgr
