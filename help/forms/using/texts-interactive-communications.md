...
* 若要將表單資料模型屬性插入文字中，請將游標置於要插入屬性的位置，然後選取 **[A]** 屬性，點選並選取 **[!UICONTROL [B] 新增選取專案]**. 您也可以按兩下屬性，將其插入至 **[C]** 游標位置。 表單資料模型屬性會以棕色背景顏色反白顯示。
...
* 允許代理程式編輯時於代理程式UI中的表單資料模型屬性值 [準備和傳送互動式通訊](/help/forms/using/prepare-send-interactive-communication.md) 使用Agent UI，選取 **[D]** 該屬性的鎖定圖示，並確保其處於解鎖狀態。 屬性的預設狀態為鎖定，代理程式無法在Agent UI中編輯屬性。
...
1. 若要在文字中插入變數，請將游標放在適當的位置，選取變數，然後選取 **[!UICONTROL 新增選取專案]**.
...
1. 選取 **[!UICONTROL 尋找和取代]**.

1. 輸入要搜尋的文字 **[!UICONTROL 尋找]** 文字方塊與新文字（取代文字） **[!UICONTROL 取代]** 文字方塊並選取 **[!UICONTROL 取代]**.

1. 如果找到搜尋的文字，該文字會由取代文字取代。

   * 如果找到搜尋文字的另一個例項，該例項會在文字檔案片段中反白顯示。 如果您選取 **[!UICONTROL 取代]** 同樣地，如果找到第三個例證，則反白的例證會被取代，且游標會向前移動。
   * 如果找不到另一個執行個體，「尋找和取代」對話方塊會顯示訊息：模組已到達終點。

   您也可以選取「全部取代」來一次取代所有相符專案。

   「尋找和取代」也包含強大的規則運算式搜尋。 若要在搜尋中使用規則運算式，請選取「 」 **[!UICONTROL 規則運算式]** 然後選取 **[!UICONTROL 尋找]** 或 **[!UICONTROL 取代]**.
