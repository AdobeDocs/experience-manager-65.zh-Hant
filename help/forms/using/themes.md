  ...
  <tr>
   ...
   <td>
    <ul>
     ...
     <li>更改此值時，clientlib節點結構將移到輸入的新位置。<br /> 注 <em><strong>意：</strong> 如果更改了預設clientlib位置，則在CRXDE儲存庫中 <code>crx:replicate, rep:write, rep:glob:*, rep:itemNames:: js.txt, jcr:read </code>將 <code>forms-users</code> 分配給和， <code>crx:replicate</code>在新 <code>jcr:read </code><code>fd-service</code> 位置中將分配給。 另外，通過添加 <code>deny jcr:addChildNodes</code><code>forms-user</code></em></li>
    </ul> </td>
  </tr>
  <tr>
   <td>13.</td>
...
樣式中所做的修改會立即反映在畫布中載入的表單或互動式通訊中。 即時預覽可讓您以互動方式定義並檢視樣式的影響。 當您變更元件的樣式時，側欄中會 **啟用** 「完成」按鈕。 若要保留變更，請使用「完 **成** 」按鈕。
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>尺寸及位置</p> </td>
   <td><p>可讓您在主題中設定元件對齊方式、大小、定位和放置方式。 </p> <p>您的選項有顯示設定、間距、邊界、寬度、高度和Z索引。</p> <p>您也可以使用簡易的拖放介面，使用「版面」模式來定義元件的寬度。 如需詳細資訊，請參 <a href="../../forms/using/resize-using-layout-mode.md">閱「使用版面模式調整元件大小</a>」。</p> </td>
  </tr>
  <tr>
   <td><p>文字</p> </td>
...
1. 在側欄的「**基本**」下方，從「主題」下拉式清單中選取您的主題，然後按一下「完成 ********![」核取按鈕](assets/check-button.png)
