...
特定元件會影響特定類別的對象。 當您對主題中的數字框元件進行樣式設定時，只有該樣式中的數字框對象繼承該樣式。
   ...
   * **檢視CSS**: 可讓您檢視所選元件的CSS
   此外，在側欄中，底部有箭頭。 當您按一下箭頭時，您會再看到兩個選項： **模擬成功** , **模擬錯誤。** 這些選項以及上述選項將在下文中詳 [細討論](../../forms/using/themes.md#using-rail)。

[ ![反白顯示「邊欄」和「畫布」的主題編輯器。](assets/themes.png)](assets/themes-1.png) **答：** 側 **欄B.** 畫布
