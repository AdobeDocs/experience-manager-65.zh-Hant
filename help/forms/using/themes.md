...
欄位可以是數值方塊欄位，例如年齡，或文字方塊欄位，例如位址。當您設定欄位樣式時，所有欄位（例如年齡、名稱、位址）都會設定樣式。
   ...
   * **檢視CSS**:可讓您檢視所選元件的CSS
   此外，在側欄底部還有箭頭。 當您按一下箭頭時，您會再看到兩個選項：**模擬成功**&#x200B;和&#x200B;**模擬錯誤。** 這些選項以及上述選項將在下文中詳 [細討論](../../forms/using/themes.md#using-rail)。
...
主題編輯器可讓您編輯您建立或上傳的主題。 導覽至「**表單與檔案>主題**」，然後選取一個主題並開啟它。 主題在主題編輯器中開啟。
...
使用側欄中的**Dimension &amp; Position**&#x200B;選項來自訂元件的大小。 確保狀態為&#x200B;**Default**。

不選擇&#x200B;**數值框Widget**，而是選擇元件工具欄中的&#x200B;**欄位Widget**，並執行上述步驟。 當您為&#x200B;**欄位介面工具集**&#x200B;選項選取維度時，除數值方塊外的所有文字方塊大小都相同。
...
您現在可以以最適化形式或互動式通訊方式，在web-service中使用可用的字型。 Adobe的網頁字型服務[現成可用的設定。 ](https://fonts.adobe.com/)若要使用Adobe Fonts，請建立套件、在其中新增字型，並從[Adobe Fonts](https://fonts.adobe.com/)取得套件ID。

執行下列步驟，在AEM中設定Adobe字型：

1. 在作者例項中，按一下「![adobeexperiencemanager](assets/adobeexperiencemanager.png)Adobe Experience Manager >工具![hammer](assets/hammer.png) >部署>雲端服務」。
1. 在&#x200B;**雲端服務**&#x200B;頁面上，導覽並開啟&#x200B;**Adobe Fonts**&#x200B;選項。 開啟配置資料夾，然後按一下&#x200B;**建立**。
...
1. 在&#x200B;**Adobe Fonts Configuration**&#x200B;欄位中，選取套件，然後按一下&#x200B;**Save**。
...
1. 在製作實例中，導覽至「**Adobe Experience Manager >表單>主題」。**
1. 開啟「主題庫」檔案夾。
...
* **使用基本clientlib、主題編輯器和內嵌樣式**
   ...
   * **基本clientlib**:
      ...
      1. 導覽至「**Experience Manager > Forms > Themes**」。
