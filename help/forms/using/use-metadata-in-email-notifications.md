...
1. 將程式碼新增至.ecma檔案，該檔案具有在索引鍵值配對中產生自訂中繼資料的邏輯。 例如，下列ECMAScript程式碼會產生保單的自訂中繼資料：
...
1. （選用）指定指令碼的標題：
   ...
   1. 新增具有以下值的屬性。
      ...
      |---|---|
      | 類型 | 字串 |
      | 值 | 指定指令碼的標題。 例如，原則持有者的自訂中繼資料。 指定的值會顯示在指派工作步驟中。 |
...
1. 在新Maven專案中，保留預設值，然後按下一步。 選取原型並按一下「下一步」。 例如，maven-archetype-quickstart。 指定專案的群組識別碼、人工因素識別碼、版本和封裝，然後按一下完成。 專案已建立。

1. 開啟pom.xml檔案進行編輯，並將檔案的所有內容取代為下列內容：

1. 新增使用WorkitemUserMetadataService Java介面為電子郵件範本新增自訂中繼資料的原始程式碼。 以下列出範常式式碼：
