...
在典型的[!DNL Adobe Sign]和最適化表單場景中，用戶填充自適應表單以申請服務。 例如，房貸和信用卡申請需要所有借款人和共同申請人的合法簽名。 若要針對類似案例啟用電子簽名工作流程，您可以將[!DNL Adobe Sign]與AEM [!DNL Forms]整合。 還有一些示例，您可以使用[!DNL Adobe Sign]來：
...
[!DNL Adobe Sign] 與AEM整合支 [!DNL Forms] 援：
...
1. 在&#x200B;**[!UICONTROL Basic]**&#x200B;標籤中：
   ...
   1. 選擇在使用AEM [!DNL Forms]設定[!DNL Adobe Sign]時建立的[設定容器](../../forms/using/adobe-sign-integration-adaptive-forms.md#configure-adobe-sign-with-aem-forms)。
...
1. 在&#x200B;**[!UICONTROL Basic]**&#x200B;標籤中，選取在使用AEM [!DNL Forms]設定[!DNL Adobe Sign]時建立的[設定容器](../../forms/using/adobe-sign-integration-adaptive-forms.md#configure-adobe-sign-with-aem-forms)。
1. 在&#x200B;**[!UICONTROL 表單模型]**&#x200B;標籤中，選擇以下選項之一：
...
1. 從元件瀏覽器拖放&#x200B;**[!UICONTROL Adobe Sign Block]**&#x200B;元件至最適化表單。 [!DNL Adobe Sign]塊元件具有所有支援的[!DNL Adobe Sign]欄位。 預設情況下，它將&#x200B;**Signature**&#x200B;欄位添加到最適化表單中。
...
1. 點選「**[!UICONTROL Adobe Sign]欄位** ![ aem_6_3_adobesign](assets/aem_6_3_adobesign.png)」圖示。 它顯示選擇和添加[!DNL Adobe Sign]欄位的選項。
   ...
   展開&#x200B;**[!UICONTROL Type]**&#x200B;下拉式欄位以選取[!DNL Adobe Sign]欄位，並點選「完成![ aem_6_3_forms_save](assets/aem_6_3_forms_save.png)」圖示，將選取的欄位新增至[!DNL Adobe Sign]區塊。 **[!UICONTROL Type]**&#x200B;下拉式欄位包含「簽名」、「簽署者資訊」和「資料」欄位類型。 [!DNL Adobe Sign] 與「Type」下拉 [!DNL Forms] 式方塊中所列的AEM  支援欄位整合。如需[!DNL Adobe Sign]欄位的詳細資訊，請參閱[Adobe Sign檔案](https://helpx.adobe.com/sign/help/field-types.html)。
...
1. 在屬性瀏覽器中，展開&#x200B;**[!UICONTROL 電子簽名]** accordion，然後選取「啟用Adobe Sign ]**」選項。**[!UICONTROL &#x200B;它為自適應表單啟用[!DNL Adobe Sign]。

### 選擇[!DNL Adobe Sign]雲端服務與簽署順序{#selectadobesigncloudserviceforanadaptiveform}

您可以為AEM [!DNL Forms]的例項設定多個[!DNL Adobe Sign]服務。 建議為每個職能（人力資源、財務等）分別提供一套服務。 它讓追蹤和報告已簽署的檔案變得更輕鬆。 例如，銀行有多個部門。 您可以為每個部門分別設定一個設定，以便更好地追蹤檔案。
...
1. 在「內容」瀏覽器中，點選「表單容器」**[!UICONTROL ，然後點選「設定****![設定](assets/configure.png)」圖示。]**&#x200B;它會開啟屬性瀏覽器並顯示最適化表單容器屬性。
1. 在屬性瀏覽器中，展開&#x200B;**[!UICONTROL 電子簽名]** accordion，然後選取「啟用Adobe Sign ]**」選項。**[!UICONTROL &#x200B;它為自適應表單啟用[!DNL Adobe Sign]。
1. 從[!DNL Adobe Sign]雲端服務的已設定清單中選取雲端服務。

   如果&#x200B;**[!UICONTROL Adobe Sign Cloud Service]**&#x200B;清單為空白，請依照[「使用AEM Forms](../../forms/using/adobe-sign-integration-adaptive-forms.md)設定Adobe Sign」文章來設定服務。
...
1. 按一下&#x200B;**Edit** ![ aem_6_3_edit](assets/aem_6_3_edit.png)圖示，以指定與簽署者相關的下列資訊：
   ...
   * **[!UICONTROL 簽署者電子郵件地址]：指** 定簽署者的電子郵件地址。簽署者會收到指定電子郵件地址上的已簽署檔案／表格。 您可以選擇使用表單欄位中提供的電子郵件地址、登入使用者的AEM使用者設定檔，或手動輸入電子郵件地址。 這是必要的步驟。 請確定第一個簽署者或唯一簽署者（如果是單一簽署者）的電子郵件地址與用於設定AEM雲端服務的[!DNL Adobe Sign]帳戶不相同。
...
在您新增[!DNL Adobe Sign]欄位至最適化表單、從表單容器啟用[!DNL Adobe Sign]、選擇[!DNL Adobe Sign]雲端服務並新增[!DNL Adobe Sign]簽署者後，請為最適化表單選擇適當的提交動作。 有關自適應表單提交操作的詳細資訊，請參閱[配置提交操作](../../forms/using/configuring-submit-actions.md)。
...
1. 從元件瀏覽器拖放&#x200B;**[!UICONTROL Adobe Sign Block]**&#x200B;元件至最適化表單。 [!UICONTROL Adobe Sign Block]元件包含所有支援的[!DNL Adobe Sign]欄位。 預設情況下，它將&#x200B;**[!UICONTROL Signature]**&#x200B;欄位添加到最適化表單中。
...
1. 選擇新添加的「簽名」步驟元件並點選&#x200B;**Configure** ![configure](assets/configure.png)表徵圖。 它會開啟屬性瀏覽器並顯示「簽名」步驟屬性。 設定下列屬性：
   ...
   * **[!UICONTROL 使用舊版電子簽名元件]**:如果您在 [AEM Forms Workspace](../../forms/using/introduction-html-workspace.md)、AEM應用程式或基礎的最適化表單具有舊版電子簽名元件，請選取「 [!DNL Forms] Use legacy E-sign元件」（使用舊版電子簽名元件） **** 選項。

   * **[!UICONTROL 配置]**:選取設定([!DNL Adobe Sign] 雲端服務)。只有啟用了&#x200B;**使用舊版電子簽名元件**&#x200B;選項時，下拉框才可用。
...
[!DNL Adobe Sign]**Ans:** 否，AEM不 [!DNL Forms] 支援使用可內嵌啟用最適化表單以 [!DNL Adobe Sign] 進行簽署的最適化表單
...
**問：我** 可以在最適 [!DNL Adobe Sign] 化表單的靜態文字元件中使用文字標籤嗎？**Ans:** 是的，您可以在文字元件中使用文字標籤，將欄 [!DNL Adobe Sign] 位新增至 [Document of Record](../../forms/using/generate-document-of-record-for-non-xfa-based-adaptive-forms.md) （僅限自動產生的記錄檔案選項）啟用的最適化表單。若要瞭解建立文字標籤的程式和規則，請參閱[Adobe Sign Documentation](https://helpx.adobe.com/sign/using/text-tag.html)。 另請注意，最適化表單對文字標籤的支援有限。 您只能使用文字標籤來建立[Adobe Sign Block](../../forms/using/working-with-adobe-sign.md#configure-cloud-signatures-for-an-adaptive-form)支援的欄位。

**問：** AEM提 [!DNL Forms] 供 [!UICONTROL Adobe Sign區塊] 和簽名步驟元件。這些功能是否可同時以最適化形式使用？
**Ans:** 您可以在表單中同時使用這兩個元件。以下是使用這些元件的一些建議：

**Adobe Sign Block：您** 可以使用 [!UICONTROL Adobe Sign ] Block，在最適化表單的任意  位置新增Adobe Sign欄位。此外，也有助於為簽署者指派特定欄位。 預覽最適化表單或發佈[!UICONTROL 時，預設不會顯示Adobe Sign]區塊。 這些區塊僅在簽署檔案中啟用。 在簽署檔案中，只會啟用指派給簽署者的欄位。 [!UICONTROL Adobe ] Signblock可與第一個及後續的簽署者搭配使用。
...
### 為[!DNL Adobe Sign]啟用的最適化表單設定的AEM [!DNL Forms]工作流程不會啟動{#adobe-sign-aem-form-workflow-failures}
...
* 當您使用[!DNL Adobe Sign]而不使用「簽名」步驟或表單需要多人簽名時，AEM [!DNL Forms]伺服器會等待排程器確認所有人已簽署表單。 調度器只有在所有人完成簽名後才提交自適應表單，而工作流只在成功提交自適應表單後才啟動。 您可以縮短[排程器](adobe-sign-integration-adaptive-forms.md)的間隔，以快速間隔檢查表單簽署狀態並加速表單提交。
