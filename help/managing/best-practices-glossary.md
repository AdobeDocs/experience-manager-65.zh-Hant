...
此字彙表列出（依字母順序）所有交付專案檔案的詳細資訊。 [專案檢查清單](/help/managing/best-practices-checklist.md).
...
您規劃及設計驗收測試的時間越早，最終部署就越容易。 應與客戶及您的品質保證團隊一起定義。
...
此 [Adobe安全性檢查清單](/help/sites-administering/security-checklist.md) 是提供的官方檢查清單，用於確保Adobe Experience Manager (AEM)在安裝時安全無虞。 它包含您為確保執行個體的完整性而必須執行的安全性措施和驗證步驟。
...
此 [應用程式架構](/help/managing/best-practices.md#development-preparation) 應清楚定義建議應用程式的行為。
...
除了標準Adobe Experience Manager (AEM)維護工作之外，您必須定義任何其他為了持續維護解決方案而必須執行的操作工作。
...
業務KPI定義可衡量的價值，可顯示公司達成關鍵業務目標的效率。 請務必選擇適合您業務/案例的KPI，並清楚定義其內容、測量方式、使用方式及使用對象。
...
風險評估和滲透測試程式可能會產生在解決方案架構或開發中必須解決的問題和結果。

任何因這些流程而導致的調整必須由業務部門稽核和核准，並根據整體目標進行評量。
...
新應用程式中使用的範本和元件的輪廓。 包括繼承規則、許可權和關係等細節。
...
定義方法時，您應預先定義專案團隊為開發或維護您的應用程式而建立並完成的特定交付專案和成品。
...
通常已知有關這些函式的基本細節，因此本檔案不應為預估值。

### 高階解決方案設計 {#high-level-solution-design}

高階解決方案設計說明了用於開發解決方案的架構。 架構圖提供整個系統的概觀，識別針對產品及其介面開發的主要元件。
...
應識別並定義可能失敗的特定點。 與這些監督任務相關的任何監督任務也應加以定義。
...
概念性檔案，說明如何確保實施符合效能KPI，以及如何擴展解決方案以持續符合這些KPI。
...
角色型測試是根據體驗設計中所概述的不同角色的方法。 它也會測試帳戶及其相關許可權層級。
...
上線進入生產環境之前，必須授與生產簽核(PSO)。 這是審查將投入生產的版本的結果，包括所有已知問題。 登出會作為上線排程的一部分提供。
...
此 [初始預估](#project-efforts-initial-estimates) 是高水準的，並根據實作的高階要求進行。

這些資料現在會經過稽核、完善和擴充，以提供最終估計。 預估應由每個適當的專案主管提供，包括專案管理、諮詢、架構、測試和開發。

這些預估用於資源配置與預算編列。

### 專案工作 — 初始估計 {#project-efforts-initial-estimates}

初始估計是高水準的，並根據實作的高水準要求進行。 這將在稍後階段中檢閱和改良。
...
AEM會維護多個版本的資產和內容。 清除規則的設計及設定可定期移除舊版本，以維持存放庫健全狀態及大小。
...
強調與專案實作相關的特定合約條款，例如合約里程碑、發票期間或員工需求。
...
### 安全性型編碼准則 {#security-based-coding-guidelines}
...
專案特定的專案檢查清單，以安全性概念以及確保解決方案合規性所需的任何其他政策為基礎。

這通常也會包含在Runbook中的部署後步驟中。

### 安全性概念 {#security-concept}

定義並記錄應用程式、架構和基礎結構所需之安全性設定的詳細資訊。
...
簽核與接受程式會概述必須先滿足的條件，才能將解決方案發行至生產環境。
...
客戶簽核規格的明確程式應該到位。 此程式可確保需求範圍的明確和穩固。
...
Quality Lead的部分職責是確保有資源可支援測試時的任何使用者。 例如，在測試時協助使用者、在報告問題時協助使用者，以及協助根據測試環境驗證問題。
...
詳細說明系統架構的檔案；包括介面、網路位置，以及所有環境的整合以及其他資訊。
...
確保可使用所有協力廠商系統，並附上適當的檔案，以實施整合。

### 啟用協力廠商系統存取 {#third-party-systems-access-enabled}

授與協力廠商系統所用個別角色的必要存取權。
...
訓練課程使用的資料。 這些資料應專門針對解決方案建立，並設計成與使用手冊搭配使用。
...
預算計畫必須由所有利害關係人稽核和驗證。 他們必須檢查諸如開立商業發票、金額以及預算報告的方法/時間等細節。

### 白盒測試結果 {#white-box-test-results}

「白盒測試」是測試應用程式的內部結構或運作方式（與其功能相反）的方法。 在軟體測試流程的單元、整合和系統層級可套用白盒測試。

### 工作流程規格 {#workflow-specifications}

根據工作流程概念，這些規格應詳細定義建立完整工作流程的步驟。
