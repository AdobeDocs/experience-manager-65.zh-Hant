...
此辭彙表會列出（依字母順序）所有交付專案檔案的詳細資訊， [專案檢查清單](/help/managing/best-practices-checklist.md).
...
此 [Adobe安全性檢查清單](/help/sites-administering/security-checklist.md) 是提供的正式檢查清單，用於確保Adobe Experience Manager (AEM)安裝時的安全性。 它包含您必須執行的安全性措施和驗證步驟，以確保執行個體的完整性。
...
為將製作（撰寫）解決方案內容的員工提供培訓。 請參閱 [Adobe訓練服務](https://training.adobe.com/training/courses.html#solution=adobeExperienceManager) 以取得詳細資訊。

### AEM認證測驗 {#aem-certification-exam}

請確定已註冊適當的角色，以便 [認證考試](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager).
...
* 至少有一名AEM認證的潛在開發人員
* 至少有一個AEM認證架構師
* 至少75%的開發人員獲得AEM認證；這可讓獲得認證的開發人員指導初級開發人員，並確保知識分享和透明度
...
在檢查企業解決方案時，BRD應該回答以下問題：「企業想要做什麼？」
...
目前內容架構和格式的檔案。 這將用於產生未來的內容架構。 移轉概念也會用到它。
...
此 [初始預估](#project-efforts-initial-estimates) 都是高階，並根據實作的高階需求而打造。
