...
# SEO和URL管理最佳實踐{#seo-and-url-management-best-practices}

SEO(Search Engine Optimization)已成為許多營銷商的一個關鍵問題。 因此，許多項目必須解決SEOAEM問題。

本文檔首先介紹了 [SEO最佳做法](#seo-best-practices) 以及執行建AEM議。 接著，本文件會於第一節中再深入探討幾項更[複雜的實作步驟](#aem-configurations)。
...
「如果用戶看到此URL，而沒有看到頁面上的任何內容，他們能描述此頁面嗎？」
...
* 可行的話，避免使用查詢參數。如需使用，請不要超過兩個。
   ...
   * 如果可用，請使用目錄結構來指示資訊架構。
   * 如果無法使用目錄結構，請在 URL 中使用 Sling 選擇器，而非查詢字串。除了它們提供的SEO值外，sling選擇器還使頁面可以為Dispatcher快取。

* URL的人類可讀性越強越好。在URL中存在關鍵字會提升值。
...
* 每個網頁都必須只經由一個通訊協定來提供。
   ...
   * 有時，網站會透過 `http` 提供，直到使用者以結帳或登入形式抵達網頁後，才會切換為透過 `https` 提供。從此頁連結時，如果用戶可以返回 `http` 頁面，通過 `https`，搜索引擎將它們作為兩個單獨的頁面進行跟蹤。

   * 目前 Google 偏好使用 `https` 頁面，而非 `http` 頁面。它們有助於讓每個人的生活更輕鬆，為整個網站服務 `https`。
...
* 加入您網站的 Favicon。
* 為了使搜索引擎更容易地爬網您的內容，請實施XML站點地圖。 請務必加入行動 Sitemap 供行動及/或回應式網站使用。

## AEM 設定 {#aem-configurations}

本節介紹使用以下SEO建議AEM配置的實施步驟。

### 使用 Sling 選擇器 {#using-sling-selectors}

過去，業界在建置企業 Web 應用程式時普遍使用查詢參數。

近年來的趨勢是刪除參數，使URL更易讀。 在許多平台上，此刪除過程涉及在Web伺服器或內容分發網路(CDN)上實施重定向，但Sling使流程變得簡單明瞭。 Sling 選擇器：

* 改善 URL 可讀性。
* 讓您在Dispatcher上快取您的頁面並提高安全性。
* 允許您直接定址內容，而不是使用檢索內容的通用Servlet。 它為您提供了應用於儲存庫的ACL和應用於Dispatcher的篩選器的好處。
...
**Bin** servlet 符合許多開發人員在 J2EE 程式設計中慣用的模式。Servlet在特定路徑上註冊，AEM在中通常 `/bin`，並從查詢字串中提取所需的請求參數。
...
* URL中存在查詢參數意味著Dispatcher無法快取響應。
* 如果要保護此Servlet的安全，請在Servlet中實現您自己的自定義安全邏輯。
...
**Sling** servlet 可讓您以相反的方式註冊 servlet。您不應定址Servlet並指定希望Servlet基於查詢參數呈現的內容，而應定址所需的內容。 您還可以指定應基於Sling選擇器呈現內容的Servlet。
...
在這種情況下，URL地址的資源 —  `myPageType` 資源 — 可在servlet中自動訪問。 要訪問它，請調用以下命令：
...
* Dispatcher已配置為將此內容作為服務網站的功能。 無需額外設定。

### URL 重新寫入 {#url-rewriting}

在 AEM 中，所有網頁都儲存在下方 `/content/my-brand/my-content`。雖然從儲存庫資料管理的角度來看，此位置非常有用，但並不一定希望客戶看到您的站點。 而且，它可能與SEO指導相衝突，以盡可能縮短URL。 此外，您可能會從同一個 AEM 執行個體及不同網域名稱為多個網站供應內容。
...
對頁面名稱進行本地化的難點是，平台上的許多本地化工具都AEM依賴於跨語言環境匹配頁面名稱來保持內容同步。

的 `sling:alias` 房產讓你吃Adobe蛋糕，也吃。 可以添加 `sling:alias` 作為任何資源的屬性，以允許資源的別名。 在上一個示例中，您將具有以下功能：
...
雖然官方檔案和Sling文AEM件記錄了這種方法，但與使用 `SlingResourceResolver` 直接輸入。 此外，以這種方式實作對應可能會導致 Dispatcher 快取失效的問題。
...
1. 內容作者變更並啟動此頁面。
1. Dispatcher 排清代理程式傳送 `/content/my-brand/my-page`**的無效要求。** 由於Dispatcher在此路徑上沒有快取的頁，因此舊內容將保持快取並過時。
...
1. **SlingResourceResolver 規則**
   ...
   * **Apache Sling Resource Resolver Factory**
      ...
      `(org.apache.sling.jcr.resource.internal.JcrResourceResolverFactoryImpl)`。
   Adobe建議您構建將URL縮短為規則運算式所需的映射，然後在OsgiConfignode下定義這些配置， `config.publish` 包含在您的構造中。
...
1. **在網頁上對應 URL 輸出**
   ...
   在Apache Sling資源解析器中定義映射後，請在元件中使用這些映射以確保在頁面上輸出的URL短而整潔。 您可以使用 `ResourceResolver`。
...
到目前為止，您已經實現了映射以及元件中的邏輯，以便在將URL輸出到頁面時使用這些映射。

最後一個步驟，就是在這些縮短的 URL 進入 `mod_rewrite` 會發揮作用的 Dispatcher 時處理這些 URL。使用 `mod_rewrite` 的最大優點，就是 URL 在傳送到 Dispatcher 模組&#x200B;*之前*&#x200B;會先重新對應到原本的較長格式。此流表示Dispatcher從發佈伺服器請求長URL並相應地快取它。 因此，從發佈伺服器傳入的任何Dispatcher刷新請求都能夠成功使此內容無效。
...
搜尋引擎在對網站進行編目之前，*必須*&#x200B;先檢查網站根中是否有 `robots.txt` 檔案。雖然Google、雅虎或必應等主要搜索引擎都尊重此檔案，但一些外國搜索引擎卻不尊重。
...
附註： `robots.txt` 站點根上的檔案是Dispatcher刷新請求可能會清除此檔案。 此外，URL映射可能會將站點根放置在與 `DOCROOT` 如Apache HTTP Server配置中定義。 因此，常見的方式是將此檔案放置在網站根目的作者執行個體上，並複製到發佈執行個體。

### 在 AEM 上建置 XML Sitemap {#building-an-xml-sitemap-on-aem}

編目程式可透過 XML Sitemap 加強瞭解網站的結構。雖然提供 Sitemap 並不保證可改善 SEO 排名，但這是一項普遍採用的最佳做法。可以在Web伺服器上手動維護XML檔案，以用作站點地圖。 但是，Adobe建議您以寫程式方式生成站點地圖，以確保在作者建立內容時，站點地圖會自動反映其更改。
...
>[!NOTE]
>
>自Adobe Experience Manager6.5.11.0版以來，已作為產品功能提供。
> 
>對於較舊版本，您可以自行註冊Sling Servlet，以偵聽 `sitemap.xml` 呼叫。 使用Servlet API提供的資源查找當前頁及其子頁以輸出 `sitemap.xml` 的子菜單。
...
在預設設定中，頁面屬性對話框提供了一個選項來將頁面標記為 Sitemap 根，因此，如上所述，產生它本身 Sitemap 和其子系。此行為由 `SitemapGenerator` 的實作來實作，並且可以透過新增替代實作來擴展。但是，由於再生XML站點的頻率取決於內容創作工作流和工作負載，因此產品不會發運任何 `SitemapScheduler` 配置。 這樣，就使特徵有效地選擇加入。

啟用生成XML模板的後台作業 `SitemapScheduler` 必須配置。 若要這麼做，請為 PID `org.apache.sling.sitemap.impl.SitemapScheduler` 建立 OSGI 設定。排程器運算式 `0 0 0 * * ?` 可作為每天午夜一次重新產生所有 XML Sitemap 的起點。

![Apache Sling Sitemap - 排程器](assets/sling-sitemap-scheduler.png)

站點地圖生成作業可以在作者和發佈層實例上運行。 通常，建議在發佈層實例上運行生成，因為只能在該處生成適當的規範URL（因為Sling資源映射規則通常只存在於發佈層實例上）。 但是，可以通過實現URL來插入用於生成規範URL的外部化機制的自定義實現 [SitemapLinkExternalizer](https://javadoc.io/doc/com.adobe.cq.wcm/com.adobe.aem.wcm.seo/latest/com/adobe/aem/wcm/seo/sitemap/externalizer/SitemapLinkExternalizer.html) 。 如果自定義實現能夠在作者層實例上生成站點地圖的規範URL，則 `SitemapScheduler` 可以為作者運行模式配置。 並且，XML站點地圖生成工作量可以分佈在作者服務集群的實例中。 在此方案中，在處理尚未發佈、已修改或僅對受限用戶組可見的內容時必須小心。

AEM Sites 包含 `SitemapGenerator` 的預設實作，它周遊頁面樹以產生 Sitemap。它被預先設定為僅輸出網站的標準 URL 和任何可用的語言替代選項。如果需要，也可設定為包含頁面的上次修改日期。為此，請啟用 _添加上次修改時間_ 選項 _AdobeAEMSEO — 頁面樹站點地圖生成器_ 配置並選擇 _上次修改的源_。 在發佈層產生 Sitemap 時，建議使用 `cq:lastModified` 日期。
...
如果預設實現在特定用例上不起作用，或者如果擴展點不夠靈活，則實施自定義 `SitemapGenerator` 以完全控制生成的站點地圖的內容。 以下示例使用預設實現的AEM Sites邏輯。 它使用 [ResourceTreeSitemapGenerator](https://javadoc.io/doc/org.apache.sling/org.apache.sling.sitemap/latest/org/apache/sling/sitemap/spi/generator/ResourceTreeSitemapGenerator.html) 作為周遊頁面樹的起點：
