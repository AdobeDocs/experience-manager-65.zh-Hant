...
* 加入您網站的 Favicon。
* 若要讓搜尋引擎更容易抓取您的內容，請實作XML Sitemap。 請務必加入行動 Sitemap 供行動及/或回應式網站使用。
...
1. 內容作者變更並啟動此頁面。
1. Dispatcher排清代理程式傳送&#x200B;`/content/my-brand/my-page`**.**&#x200B;的失效請求 由於Dispatcher在此路徑上沒有經過快取的頁面，因此系統會維持對舊內容進行快取，導致內容過時。
