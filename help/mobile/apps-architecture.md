---
title: 行動應用程式的頁面範本
description: 請詳閱本頁內容，以進一步瞭解頁面範本。 您為應用程式建立的頁面元件是以/libs/mobileapps/components/angular/ng-page元件為基礎。
contentOwner: User
...
您為應用程式建立的頁面元件是以/libs/mobileapps/components/angular/ng-page元件為基礎（[在本機伺服器上的CRXDE Lite中開啟](http://localhost:4502/crx/de/index.jsp#/libs/mobileapps/components/angular/ng-page)）。 此元件包含下列元件繼承或覆寫的JSP指令碼：
...
Angular頁面的內文會根據是否偵測到wcmMode (!=WCMMode.DISABLED)而呈現不同的方式，以判斷頁面是開啟以供撰寫還是作為已發佈的頁面。

**作者模式**

在製作模式中，每個個別頁面會個別呈現。 Angular不會處理頁面之間的路由，也並非用來載入包含頁面元件之部分範本的ng-view。 而是透過`cq:include`標籤將頁面範本(template.jsp)的內容包含在伺服器端。

此策略可讓作者功能（例如在段落系統、Sidekick、設計模式等中新增及編輯元件）在不修改的情況下運作。 依賴使用者端轉譯的頁面（例如應用程式的頁面）在AEM作者模式中無法正常運作。
...
SPA中只有一個HTML頁面（包含`<html>`元素的頁面）。 此頁面稱為「版面配置範本」。 在Angular術語中，這是「……我們的應用程式中所有檢視的共同範本」。 將此頁面視為「最上層應用程式頁面」。 按照慣例，最上層應用程式頁面是您應用程式中最接近根的`cq:Page`節點（且不是重新導向）。

由於應用程式的實際URI在發佈模式下不會變更，因此從此頁面參考外部資產必須使用相對路徑。 因此，提供特殊的影像元件，在轉譯影像以進行匯出時，會將此頂層頁面列入考量。

做為SPA，此配置範本頁面只會產生具有ng-view指示詞的div元素。

```xml
 <div ng-view ng-class="transition"></div>
```

Angular Route服務使用此元素來顯示應用程式中每個頁面的內容，包括目前頁面的可授權內容（包含在template.jsp中）。
...
此屬性向Angular指出此DOM元素的內容應連結至下列模組。 此模組將檢視（在AEM中，這些將是cq:Page資源）與對應控制器連結。

此模組也會定義名為`AppController`的頂層控制器，將`wcmMode`變數公開至範圍，並設定擷取Content Sync更新裝載的URI。

最後，此模組會逐一檢視每個下級頁面（包括其本身），並呈現每個頁面的路由片段內容（透過angular-route-fragment.js selector &amp; extension），包括作為Angular \$routeProvider的設定專案。 換言之，\$routeProvider會告知應用程式當要求指定路徑時要轉譯的內容。
...
如有必要，您可以覆寫此指令碼以處理更複雜的路徑，包括包含變數的路徑。 範例可在隨AEM安裝的/apps/geometrixx-outdoors-app/components/angular/ng-template-page/angular-route-fragment.js.jsp指令碼中看到：
...
在Angular中，控制器會連線\$scope中的變數，讓這些變數公開至檢視。 angular-app-controllers.js.jsp指令碼遵循angular-app-module.js.jsp所述的模式，即逐一檢視每個下級頁面（包括其本身），並輸出每個頁面定義的控制器片段（透過controller.js.jsp）。 它定義的模組稱為`cqAppControllers`，必須列為最上層應用程式模組的相依性，才能讓頁面控制器可用。
...
`data`變數已指派給Angular `$http.get`方法傳回的Promise。 如果需要，此頁面中包含的每個元件都可以讓一些.json內容可用（透過其angular.json.jsp指令碼），並在解析時對此請求的內容採取行動。 在行動裝置上，要求非常快速，因為它只會存取檔案系統。

若要讓元件以這種方式成為控制器的一部分，它應該擴充/libs/mobileapps/components/angular/ng-component元件並包含`frameworkType: angular`屬性。

### template.jsp {#template-jsp}

在body.jsp區段中首先介紹的template.jsp僅包含頁面的parsys。 在發佈模式中，此內容會直接參考（在&lt;page-path>.template.html），並透過\$routeProvider上設定的templateUrl載入SPA。

此指令碼中的parsys可設定為接受任何型別的元件。 但是，在處理針對傳統網站（而非SPA）建置的元件時，必須謹慎。 例如，foundation影像元件只有在頂層應用程式頁面上才能正常運作，因為它的設計目的並非參考應用程式內的資產。
...
為了解決複雜的問題，PhoneGap應用程式會使用單頁應用程式(SPA)模式，讓基本URI （雜湊除外）永不變更。 因此，您參照&#x200B;**的每個資產、範本或指令碼都必須是您最上層頁面的相對專案。** 頂層頁面會利用`*<name>*.angular-app-module.js`和`*<name>*.angular-app-controllers.js`將Angular路由和控制者初始化。 此頁面應該是*不會*延伸sling:redirect的最接近存放庫根的頁面。
...
如AEM頁面範本中所述，每個元件都可以輸出JavaScript片段，以使用`data` Promise公開的JSON內容。 依照Angular慣例，控制器應僅用於指派變數至範圍。

#### angular.json.jsp {#angular-json-jsp}

此指令碼包含在全頁的「&lt;page-name>.angular.json」檔案中，該檔案會針對擴充ng-page的每個頁面匯出。 在此檔案中，元件開發人員可公開元件所需的任何JSON結構。 在「ng-text」範例中，此結構僅包含元件的文字內容，並加上指出元件是否包含RTF文字的標幟。
...
此目錄包含[CLI鉤點](https://cordova.apache.org/docs/en/10.x/guide/appdev/hooks/)。 Hooks目錄中的資料夾包含node.js指令碼，這些指令碼會在建置期間的特定時間點執行。
...
PhoneGap檔案(`https://docs.phonegap.com`)將此檔案稱為「全域組態檔」。 config.xml包含許多應用程式屬性，例如應用程式名稱、應用程式「偏好設定」（例如iOS webview是否允許過度捲動）以及PhoneGap Build僅使用&#x200B;*的*&#x200B;外掛程式相依性。
...
根據慣例，在AEM中，/etc節點包含靜態clientlib內容。 etc目錄包含Topcoat、AngularJS和Geometrixx ng-clientlibsall資料庫。

#### www/apps {#www-apps}

apps目錄包含與啟動顯示頁面相關的程式碼。 AEM應用程式啟動顯示頁面的獨特之處在於，它可以在不與使用者互動的情況下初始化應用程式。 因此，應用程式的clientlib內容（CSS和JS）為最小，以發揮最大效能。
