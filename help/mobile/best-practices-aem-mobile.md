...
description: 瞭解最佳做法和指引，協助有能力的網站Adobe Experience Manager (AEM)開發人員建立行動應用程式範本和元件。
contentOwner: User
...
* 在應用程式的Webview中執行範本時，您可以控制應用程式將支援的平台和版本，並須具備JavaScript支援的知識。 例如，與jQuery Mobile和Onsen UI相比，偏好使用Ionic （僅限CSS），而非Bootstrap。
...
* 如果您的文章未建置為SPA （單頁應用程式），則可能不需要包含Angular之類的完整棧疊程式庫
