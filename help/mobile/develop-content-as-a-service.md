...
Content Services可讓您存取AEM管理的資產，這些資產可能不會透過其他AEM內容參照。
...
## 已封裝的HTML內容 {#packaged-html-content}

客戶必須維護內容版面配置，因此需要HTML內容。 這對於使用Web容器（例如Cordova Webview）顯示內容的原生應用程式非常有用。

AEM Content Services會透過API將HTML內容提供給行動應用程式。 想要以HTML的形式公開AEM內容的客戶，可以建立指向AEM內容來源的HTML頁面實體。

考量到下列選項：

* **Zip檔案：**&#x200B;為了有在裝置上正確顯示的最佳機會，頁面參考的資料（css、JavaScript、資產等）會包含在含有回應的單一壓縮檔案中。 HTML頁面中的參照可以調整，以使用這些檔案的相對路徑。
* **串流：**&#x200B;從AEM取得必要檔案的資訊清單。 然後使用該資訊清單來要求所有檔案（HTML、CSS、JS等）與後續要求。
