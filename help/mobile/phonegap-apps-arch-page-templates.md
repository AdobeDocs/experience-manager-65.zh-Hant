...
如有必要，可以覆蓋此指令碼以處理更複雜的路徑，包括那些具有變數的路徑。 在/apps/weretail-app/components/angular/ng-template-page/angular-route-fragment.js.jsp指令碼中，可以看到以下示例AEM:
...
如所述 [頁AEM面模板](/help/mobile/apps-architecture.md)，每個元件都可以輸出JavaScript片段以使用由 `data` 答應。 遵循Angular約定，控制器只應用於為作用域分配變數。
...
We.Retail應用程式產品元件是一個更複雜的示例(/apps/weretail-app/components/angular/ng-product):
...
此目錄包含 [CLI掛接](https://gist.github.com/jlcarvalho/22402d013bc72f795d45a01836ce735c)。 掛接目錄中的資料夾包含在生成過程中精確點運行的node.js指令碼。
...
按慣例，在/etcAEM節點中包含靜態客戶端庫內容。 etc目錄包含Topcoat、AngularJS和We.Retail ng-clientlibsall庫。
