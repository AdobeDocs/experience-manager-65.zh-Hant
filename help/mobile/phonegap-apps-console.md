...
您也可以提供影像檔案，以用作應用程式縮圖、選取要使用的PhoneGap Build設定，並選取要使用的行動應用程式分析設定。 此影像僅能作為縮圖，在Experience Manager的行動應用程式主控台中代表您的行動應用程式。
...
* Analytics：按一下管理設定並設定您的[Adobe Mobile Services SDK](https://experienceleague.adobe.com/docs/mobile-services/using/home.html?lang=zh-Hant)雲端服務。 然後從下拉式清單，選取新建立的行動服務以整合至行動應用程式。
...
1. （選擇性）如果您使用PhoneGap Build來編譯應用程式，請在Build索引標籤上選取要使用的組態。
...
1. 在邊欄上，按一下「應用程式」 。
1. 按一下以深入探究您要在Admin Console應用程式中編輯的行動應用程式。 選取要設定的應用程式語言版本，然後按一下「檢視應用程式屬性」圖示。
...
這是Apps 6.1 Feature Pack 2提供的功能，可讓您輕鬆使用現有應用程式範本，在AEM中建立新的應用程式。
...
最新AEM應用程式6.1範例套件包含更新版本的Geometrixx應用程式和應用程式範本。 或者，您可以安裝也提供範本的StarterKit。
...
1. 提供應用程式ID和標題，不過您可能也要包含名稱和說明。
   ...
   1. 此外，您可以瀏覽AEM資產，提供PNG （支援的PhoneGap圖示格式）作為圖示。
...
>[!NOTE]
>
>如果您從AEM解除安裝Geometrixx Outdoors參考應用程式套件，並根據其範本建立應用程式，則該應用程式將無法再正常運作。 Geometrixx Outdoors應用程式可移除，但如果其他行動應用程式正在使用它，則必須保留應用程式範本。
...
製作行動應用程式的頁面時，Sidekick中可立即使用數個元件。 元件屬於PhoneGap元件群組。
...
* 排序依據：選取要用於排序頁面清單的頁面屬性。 例如，選取jcr:title，依標題的字母順序排序頁面。
