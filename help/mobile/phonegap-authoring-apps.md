...
>[!NOTE]
>
>Adobe建議針對需要單頁應用程式架構式用戶端轉譯（例如React）的專案使用SPA編輯器。 [深入了解](/help/sites-developing/spa-overview.md).
...
從目錄中選取AEM Mobile應用程式時，會顯示其控制面板。 您可以在此處管理應用程式、檢視統計資料、建立、部署及管理行動應用程式內容。
...
「管理應用程式圖磚」會顯示您的應用程式圖示、名稱、說明、支援的平台、呼叫首頁以取得更新URL和版本資訊。 您可以鑽取到此表徵圖，以編輯和維護PhoneGap應用程式配置(config.xml)，並準備您的應用程式以提交到各種應用程式儲存庫進行分發。
...
「內容套件」可讓AEM應用程式作者管理AEM中的頁面內容，並讓您的開發團隊變更您的PhoneGap Shell應用程式（即應用程式架構或基礎架構），然後快速將這些變更推出給客戶，而不需要請開發人員重新提交至各種商店進行發佈。
...
內容可以以語言呈現，或以應用程式特定部分呈現，應用程式會使用多個內容發行套件。 您選擇的內容捆綁方式靈活且完全取決於您希望如何管理應用程式的內容。

此 **已修改** 欄會指出最近修改頁面的時間。

此 **已轉移** 欄顯示上次內容更新的建立時間。 若要建立內容更新並預備變更，請開啟圖磚中的任何記錄並建立更新。
...
此 **PhoneGap Build圖磚** 連線 [https://build.phonegap.com](https://build.phonegap.com) 來建置和托管遠端組建。 建置後，組建即可供下載使用，或透過QR碼直接供您的裝置使用。
...
AEM Mobile透過 [AdobeMobile Services SDK](https://experienceleague.adobe.com/docs/mobile.html?lang=en) (AMS)。
...
「管理實體內容」圖磚可讓您新增及管理應用程式定義。 應用程式定義是識別哪些空格（和其他設定）適合應用程式的方法。 如此即可新增空間，而無須重新編譯應用程式。 應用程式定義會更新，其中包含任何新空格的資訊。
