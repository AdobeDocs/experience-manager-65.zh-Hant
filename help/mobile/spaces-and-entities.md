...
使用者首次尚未將Content Services設定為使用空間，AEM Mobile儀表板在選取後僅顯示應用程式 **內容服務**.

>[!CAUTION]
>
>**新增空間的先決條件**
>
>檢查 **啟用AEM內容服務** 以使用空間，並在AEM Mobile應用程式儀表板中將其啟用。
>
>另請參閱 [管理內容服務](/help/mobile/developing-content-services.md) 以取得更多詳細資料。

在控制面板中設定空間後，請依照下列步驟建立空間：

1. 選擇 **空間** 從Content Services取得。
...
此 **語言副本** 功能可讓作者複製主要語言副本，然後建立專案和工作流程以自動翻譯內容。 語言副本會建立正確的結構。 在分享空間新增資料夾後，您就可以新增語言副本至分享空間。
...
1. 選擇 **內容封裝語言&amp;ast；** 並輸入 **標題&amp;ast；** 在 **建立語言副本** 對話方塊。
...
1. 選擇圖元的模型。 這是您要建立的實體型別。 按一下「下一步」。
   ...
   >[!NOTE]
   >
   >您可以選擇透過 **資產模型**， **頁面模型**&#x200B;或您之前建立的圖元型別模型。
   >
   >另請參閱 [建立模型](/help/mobile/administer-mobile-apps.md)，以建立自訂實體。

1. 輸入 **標題**， **名稱**， **說明**、和 **標籤** 實體的。 按一下&#x200B;**建立**。
...
1. 選取要編輯的實體，然後按一下 **編輯**.
   ...
   ***如果您選擇用來建立實體為頁面模型的範本***，按一下 **編輯** 可讓您新增資產，如下圖所示：
   ...
   >[!NOTE]
   >
   >將資產新增至實體後，您可以選擇 **儲存** 儲存變更或選擇 **儲存並關閉** 儲存並重新導向至已定義實體的空間控制檯清單。

   此外，從空間控制檯清單中選取一個實體，然後按一下 **屬性** 以檢視和編輯已定義圖元的屬性。
...
1. 按一下 **刪除** 從工具列移除空間中的特定實體

### 發佈實體 {#publishing-an-entity}

您可以選擇以下選項 **發佈樹狀結構** 或 **快速發佈** 以發佈您的實體。

1. 從空間控制檯清單中選取一個實體，然後按一下**發佈樹狀結構**以發佈該實體及其子系。
