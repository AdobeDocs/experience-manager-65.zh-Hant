...
[!DNL Adobe Experience Manager] 6.5 Service Pack每季提供新功能、客戶要求的增強功能以及效能、穩定性和安全性改進。每季的上市情況讓您輕鬆存取和採用新功能和創新。

本文重點介紹最新版Service Pack中包含的功能、前6.5版Service Pack中包含的[主要功能，以及自上一版Service Pack](#key-releases-since-last-sp)以來的[主要版本。](#key-features-previous-service-packs)
...
#### 頁面移動和MSM推出的可用性作為非同步操作(6.5.7.0){#page-moves-msm-asynchronous}
...
* 不允許匿名存取CRXDE Lite以增強安全性。 而是將使用者導向登入畫面。 請參閱[使用CRXDE Lite開發](/help/sites-developing/developing-with-crxde-lite.md)。
...
* [!DNL Assets] 並提供 [!DNL Dynamic Media] 多種協助工具增強功能。這些增強功能與鍵盤導覽、螢幕閱讀程式的使用，以及類似的增強功能，以利使用輔助技術(AT)有關。 請參閱[[!DNL Assets] 增強功能](/help/release-notes/sp-release-notes.md#assets-6570)和[[!DNL Dynamic Media] 增強功能](/help/release-notes/sp-release-notes.md#dynamic-media-6570)(6.5.7.0)
...
[!DNL Assets] 使用者可以搜尋外觀相似的影像。Experience Manager顯示來自DAM儲存庫的智慧標籤映像，該映像與用戶選擇的映像類似。 請參閱[視覺搜尋](../assets/search-assets.md)。
...
#### 適用於Dynamic Media的智慧映像{#smart-imaging}
...
#### Dynamic Media(6.5.3.0){#smart-crop-video}的視訊設定檔智慧裁切

視訊智慧裁切（視訊描述檔中的選用功能）是一種工具，可運用Adobe Sensei的人工智慧功能，自動偵測並裁切您上傳的任何最適化視訊或漸進式視訊中的焦點，而不論其大小。 請參閱[關於在視訊描述檔中使用智慧裁切](../assets/video-profiles.md)。

### Experience ManagerForms{#aem-forms-previous-service-packs}

#### 效能改進(6.5.7.0){#performance-improvements-forms}
...
#### 在「配置」模式(6.5.7.0){#reset-option-layout-mode}中，每個元件的「重置選項」的可用性
...
#### 新增對Automated forms conversion服務(6.5.6.0){#sign-integration-acroform-afcs}中[!DNL Adobe Sign]文字標籤的支援
...
#### 支援將彩色PDF forms轉換為可調式表單(6.5.6.0){#colored-PDF-forms}

您可以使用[!DNL Automated Forms Conversion service]將彩色PDF forms轉換為可調式表單。
...
#### 自定義「Adobe Experience Manager收件箱」列(6.5.5.0){#customize-aem-inbox-columns}
...
Adobe Experience Manager Forms在JEE上新增了對Adobe Experience Manager Forms的[!DNL Oracle WebLogic 12]支援。 您可以從舊版升級，或在[!DNL Oracle WebLogic] 12.2.1.4及更新版本的JEE伺服器上設定新的Experience Manager6.5Forms。 稍後的版本會與次要版本變更相對應，其中12.2.1.x中的x會以版本號碼取代。
...
* 關於JEE Document Security的Experience Manager6.5Forms現在以[!DNL Apache Struts 2]為基礎。

* 新增對[!DNL Oracle Real Applications Cluster (RAC) 19c]的支援。

#### 在Experience ManagerForms工作流程(6.5.4.0){#generate-printable-output}中產生可列印的輸出

使用「生成可打印輸出」工作流步驟，可以將源模板檔案與資料檔案整合。 此整合可讓您列印或儲存範本檔案的不同副本。 此步驟會產生PCL、PostScript、ZPL、IPL、TPCL或DPL輸出。 有關此功能的詳細資訊，請參見[OSGi —— 步驟參考](../forms/using/aem-forms-workflow-step-reference.md)上的以Forms為中心的工作流。
...
FormsExperience Manager包含下列協助工具增強功能：

* 螢幕閱讀程式會在最適化表單中正確發佈核取方塊、連結、「日期選擇器」和「日期輸入」欄位。

* 最適化表單的每一頁現在都包含一個標題和一個主要地標標籤。

#### 共用並請求訪問Experience ManagerForms用戶(6.5.3.0){#share-request-access}的收件箱項目
...
#### 為Experience ManagerForms用戶(6.5.3.0){#configure-out-of-office}的收件箱項目配置不在辦公室的設定
...
#### 使用批次API產生多種互動式通訊，以Experience ManagerForms(6.5.3.0){#generate-multiple-ic}
...
## [!DNL Adobe Experience Manager] 6.5 SP7 {#key-releases-since-last-sp}以來的主要版本

在2020年11月26日至2021年2月25日之間，Adobe除了發佈Service Pack和Cumulative Fix Pack外，還發佈了以下內容：

* [!DNL Adobe Experience Manager] Cloud Service [2020.11.0](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/release-notes/2020/release-notes-2020-11-0.html)、 [2020.12.0](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/release-notes/2020/release-notes-2020-12-0.html)和 [2021.1.0](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/release-notes/release-notes-current.html?lang=en#release-date)。

* [[!DNL Experience Manager] 案頭應用程式2.1(2.1.0.0)](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/release-notes.html)。

* [Experience Manager Screens:功能套件202011](https://experienceleague.adobe.com/docs/experience-manager-screens/user-guide/release-notes/release-notes-fp-202011.html)
