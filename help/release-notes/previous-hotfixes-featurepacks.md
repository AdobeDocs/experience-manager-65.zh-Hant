...
* 訪問控制和拒絕服務失效 `DSRPReindexServlet` 這會將Communities網站拉下來，直到索引完成(NPR-35591)。
...
* 帶引號的本地化標籤名稱(`"`)無法正確顯示在使用者介面上(CQ-4305439)。
...
* [!UICONTROL 管理出版物] 將內容從製作復寫到發佈執行個體的作業失敗並產生JavaScript錯誤(NPR-33872)。
