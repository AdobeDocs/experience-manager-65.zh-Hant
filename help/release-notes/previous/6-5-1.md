...
[!DNL Adobe Experience Manager] 6.5.1.0為重要版本，包含自[!DNL Adobe Experience Manager] 6.5於2019年4月&#x200B;*正式發行以來所發行的效能、穩定性、安全性，以及重要客戶修正和增強功能。*&#x200B;它可以安裝在[!DNL Experience Manager] 6.5之上。

此Service Pack版本的部分主要重點為：

* 啟用在追蹤事件中納入動態UI狀態作為自訂屬性的功能。
* 包含在[!DNL Dynamic Media]-Scene7模式下傳送360度視訊資產的支援。
...
* 升級至`Only empty tenantId is currently supported` 6.5後，轉譯預覽產生[!DNL Experience Manager]錯誤。NPR-29986：CQ-4272353的Hotfix
...
* Scene7視訊播放器展開至全熒幕時出現問題。 CQ-4266700 的 Hotfix
...
* 將影像預設集發佈至[!DNL Brand Portal]失敗，出現500個錯誤代碼。 NPR-29510：CQ-4268659 的 Hotfix
...
* 從[!DNL Experience Manager Assets]作者/content/dam/mac資料夾發佈[!DNL Experience Manager]至[!DNL Brand Portal]無法運作。 NPR-29819：CQ-4271118 的 Hotfix
...
* 使用64位元JDK將CAD檔案轉換至PDF失敗。 NPR-29924、NPR-29925：CQ-4272113 的 Hotfix
* 已取代PhantomJS轉換為WebToPDF以進行HTMLtoPDF轉換的名稱。 NPR-29933：CQ-4234545 的 Hotfix
* 將zip檔案轉換為PDF時發生錯誤。 CQ-4268628 的 Hotfix

**Forms - Designer**

* 在使用[!DNL Experience Manager Forms Designer]建立的靜態PDF上執行完整的協助工具檢查時，主要語言檢查會因為遺失語言屬性而失敗。 CQ-4272923、CQ-4271002
...
* 使用PDF進階電子簽章(PAdES)在PDF中驗證簽名會產生InvalidOperationException。 NPR-29842：CQ-4244837 的 Hotfix
* 新增Office 2019適用的Document Security Extension支援。 CQ-4254369、CQ-4259764

**Forms — 檔案服務**

* PDF無法轉換成PDF/A-1b，因為表單欄位沒有外觀指示。 NPR-29940：CQ-4269618 的 Hotfix
...
* 由[!DNL Experience Manager Forms]伺服器的輸出模組建立的靜態PDF不會將語言屬性/標籤填入已建立檔案的語言。 NPR-27332：CQ-4271002 的 Hotfix
...
適用於Experience Manager 6.5.1.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.1/)中使用。
...
* 已啟用[!DNL Experience Manager Assets]的多網站管理員支援。 如需詳細資訊，請參閱[使用Experience Manager Assets的MSM重複使用資產](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/reuse-assets-using-msm.html)。 NPR-29199：CQ-4259922 的 Hotfix

### 網站 — 包含

* 將[!DNL Experience Manager]體驗片段匯出至[!DNL Adobe Target]。 如需詳細資訊，請參閱[體驗片段連結重寫器提供者 — HTML](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/experience-fragments.html#the-experience-fragment-link-rewriter-provider-html)。 CQ-4265469 的 Hotfix
