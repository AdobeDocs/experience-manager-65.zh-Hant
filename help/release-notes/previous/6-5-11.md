...
* 如果[!DNL Scene7]發生失敗，PDF處理錯誤不會傳播至[!DNL Experience Manager] (CQ-4331145)。
...
* 在包含自訂啟動器且將`ActivationModel`作為工作流程的[!DNL Experience Manager]設定中，Experience Manager會因為上傳PDF檔案的記憶體問題而當機。 (CQ-4330512)。
...
B.使用封裝管理員](/help/sites-administering/package-manager.md#package-share)的[HTTP API。 使用`cmd=install&recursive=true`安裝巢狀套件。
