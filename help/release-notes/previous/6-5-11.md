---
title: ' [!DNL Adobe Experience Manager] 6.5.11.0 版發行說明'
...
[!DNL Adobe Experience Manager] 6.5.11.0包含自2019年4月6.5版發行以來所推出的新功能、客戶要求的重要增強功能，以及效能、穩定性和安全性改善專案。 Service Pack安裝在[!DNL Adobe Experience Manager] 6.5上。

[!DNL Adobe Experience Manager] 6.5.11.0中引入的主要功能與增強功能包括：
...
* 您現在可以使用[!DNL Dynamic Media]來設定發佈設定，而不必透過[!DNL Dynamic Media Classic]案頭應用程式來進行。 請參閱[設定Dynamic Media發佈設定](/help/assets/dm-publish-settings.md)。
...
* 在工作流程中使用的自訂對話方塊中，當嘗試驗證對話方塊時，Experience Manager會在瀏覽器主控台中失敗並出現錯誤(GRANITE-35049)。
...
* 手冊、傳單和InDesign範本的PDF轉譯不正確(NPR-36433)。

* 在參考[!DNL Adobe Target]報告量度時，以[!DNL Experience Manager]目標定位模式儲存[!DNL Adobe Analytics]活動會失敗(NPR-37167)。
...
* 在[!DNL Dynamic Media]中還原資產版本時，[!DNL Experience Manager]中未更新資產(NPR-37421)。
...
* 如果[!DNL Experience Manager]發生失敗，PDF處理錯誤不會傳播至[!DNL Scene7] (CQ-4331145)。

* 使用者無法看到.MOV資產的預設中繼資料屬性(CQ-4332546)。

* 無法使用[!DNL Dynamic Media]將.MXF視訊檔案上傳到[!DNL Experience Manager] (CQ-4329709)。

* 設定自訂公司根目錄時發生上傳問題(CQ-4332800)。

* 在包含自訂啟動器且將[!DNL Experience Manager]作為工作流程的`ActivationModel`設定中，Experience Manager會因為上傳PDF檔案的記憶體問題而當機。 (CQ-4330512)。
...
* 使用[!UICONTROL 稍後發佈]選項時，使用者介面未將狀態反映為[!UICONTROL 擱置發佈] (CQ-4334229)。
...
* 從[!DNL Experience Manager] 6.5.8.0升級為版本6.5.9.0時，安裝會覆寫`/content/dam/projects`上的屬性。 它會預設重設資料夾的指派中繼資料結構和屬性(NPR-37124)。
...
* `Deactivate`中的`InboxOmniSearchHandler`方法顯示Null指標例外狀況(NPR-37533)。
...
* 將PDF檔案轉換為PDF-A/1B PDF檔案後，資料中缺少右括弧(NPR-37608)。

**HTML5 Forms**

* 安裝AEM 6.5.10.0時，XDP表單的HTML預覽無法運作(NPR-37503、CQ-4331926)。

* 當使用者嘗試將XDP表單從PDF轉換為HTML5表單時。 轉譯適用於柬埔寨字型的HTML5表單期間發生的一些問題包括：
   * 換行符會在HTML5表單中轉譯。 (CQ-4322508)
   * 清單在HTML5表單中無法正確呈現。 (CQ-4322317)
   * 呈現HTML5表單轉譯時會出現額外的空格。 (CQ-4322509)

* 以各種語言將PDF forms移轉至HTML 5表單時出現文字重疊問題(NPR-37173)。
...
已在AEM Forms **中修正6.5.11.1**&#x200B;個問題

>[!NOTE]
>
>如果您尚未升級至AEM 6.5.11.0 Forms，請直接安裝AEM Forms 6.5.11.1附加元件套件。 如果您已安裝AEM 6.5.11.0 Forms，Adobe建議您升級至AEM 6.5.11.1 Forms。

* 安裝AEM 6.5.11.0附加元件套件後，提交動作、傳送電子郵件和叫用Forms工作流程停止運作。
* 安裝Microsoft 6.5.11.0附加元件套件後，CreatePDF作業停止將Forms Word檔案轉換成PDF檔案。
* (僅限 JEE) 針對 Apache Log4j2 回報的重大安全漏洞 (CVE-2021-44228 和 CVE-2021-45046)。
* （僅限JEE） 6.5.11.0修補程式中的組合器DSC包含不正確的維度，例如規格版本和實作版本。
* 當使用者嘗試將XDP表單從PDF轉換為HTML5表單時，部分已修復的問題為：
   * 支援Farsi轉譯為LTR。
   * 與LTR文字混合時支援RTL。
   * 支援影像旋轉。
   * 新增對柬埔寨語言的支援。 若要啟用支援Combodian支援，呈現HTML5表單時需要啟用標幟為`CTL=true`。
...
* Experience Manager 6.5.11.0需要Experience Manager 6.5。如需詳細指示，請參閱[升級檔案](/help/sites-deploying/upgrade.md)。
* 可在Adobe [軟體發佈](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)上取得Service Pack下載。
* 在具有MongoDB和多個執行個體的部署上，使用封裝管理員在其中一個作者執行個體上安裝Experience Manager 6.5.11.0。
...
在工作執行個體上自動安裝[!DNL Experience Manager] 6.5.11.0的方法有兩種：
...
1. 產品資訊頁(`/system/console/productinfo`)會在`Adobe Experience Manager (6.5.11.0)`已安裝產品[!UICONTROL 下顯示更新的版本字串]。
...
適用於Experience Manager 6.5.11.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.11/)中使用。
...
* 如果您使用內容片段和GraphQL，建議您升級至6.5.12.0並安裝適當的套件。
   * 如果無法升級至6.5.12.0，請連絡支援以評估是否可以同時執行因應措施。
...
* [Experience Manager 6.5.11.0中包含的OSGi套件組合清單](../assets/65110_bundles.txt)
* [Experience Manager 6.5.11.0中包含的內容套件清單](../assets/65110_packages.txt)
