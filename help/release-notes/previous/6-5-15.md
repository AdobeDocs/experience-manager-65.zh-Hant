...
[!DNL Experience Manager] 6.5.15.0包含自2019年4月6.5版首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[!DNL Experience Manager] 6.5上[安裝此Service Pack](#install)。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* 您現在可以使用[OAuth2驗證® Office 365郵件伺服器通訊協定（SMTP和IMAP）](/help/forms/using/oauth2-support-for-mail-service.md)。 (NPR-35177)
...
* 當最適化表單的核取方塊、下拉式清單或選項按鈕欄位中使用`=`符號並產生記錄檔案時，則產生的記錄檔案中不會顯示`=`符號。(NPR-38859)
* 6.5.11.0 Service Pack升級後，通知批次處理錯誤的數量會成倍增加。 (NPR-39636)
* 若您未提供測試資料，無法在Agent UI中載入「通訊管理」信件。 (CQ-4348702)
* 當使用者從使用® WebSphere®部署的AEM Forms套用AEM Forms Service Pack 14 (SP14)時，初始化資料庫時啟動載入失敗，並發生`java.lang.NoClassDefFoundError:org/apache/log4j/Logger`錯誤。(NPR-39414)
...
<!-- OAK VERSION -MAY- NEED TO BE UPDATED FOR EACH NEW RELEASE -->
...
* AEM Forms JEE 6.5.15.0 Service Pack未正確安裝在JBoss Linux環境中。 檢查記錄專案`[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component is not in the installation. Skipping Processing`的`PatchInstallerProcessing[1-9*].log`檔案，以驗證AEM Forms JEE 6.5.15.0 Service Pack的正確安裝。 此專案表示AEM Forms JEE 6.5.15.0 Service Pack的安裝不成功。 若要修正安裝問題，請手動[將RUP_BOM.xml檔案](/help/forms/using/linux-jboss-installation-issue-on-sp15.md)新增至XML目錄，即AEM Forms JEE 6.5.15.0 Service Pack。
