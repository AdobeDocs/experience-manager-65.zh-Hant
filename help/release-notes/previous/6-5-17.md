...
[!DNL Experience Manager] 6.5.17.0包括自2019年4月6.5首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[!DNL Experience Manager] 6.5上[安裝此Service Pack](#install)。
...
* 記憶體不足；由於`NotificationManagerImpl`，記憶體回收活動量很高。 `NotificationManager`套件組合升級至Experience Manager6.5。 (SITES-11440)
...
* 當使用者升級至Experience Manager 6.5.16.0 Service Pack，提交後會遺失CRX中繼資料屬性。 (FORMS-8205)
...
* 在Experience ManagerDesigner建立的PDF表單中，所有背景物件的PAC 2021工具傳回錯誤為`Path object not tagged`。 (FORMS-6707)
* 當使用者在收件匣中套用篩選器時，它會擲回`NullPointerException`錯誤。 (FORMS-6706)
* 當使用者匯入包含參考片段的範本(.tds)檔案時，Experience ManagerDesigner當機。 (FORMS-6702)
...
* 當使用者嘗試使用輸出服務產生PDF時，即使它標籤為`PDF/A-1a`，使用`Preflight`服務的符合性檢查也會失敗。 (LC-3920837)
* 安裝Experience Manager6.5.16.0 Service Pack後，Experience ManagerDesigner無法開啟。 (LC-3921000)
...
* 當使用者升級至JBoss® Turnkey伺服器上的Experience Manager6.5.16.0 Service Pack時，無法叫用Signature Service。 遇到的錯誤是： `java.lang.ClassCastException: com.adobe.xfa.TextNode cannot be cast to com.adobe.xfa.Element`。 (FORMS-7833)
* 升級至Experience Manager 6.5.14.0 Service Pack後，將CRX節點從一個位置移動至另一個位置的Workbench程式無法使用。 錯誤發生為`ALC-CRX-30000-000: com.adobe.ep.crx.client.exceptions.CRCException: ALC-CRX-030-000-[Internal Server Error]`。 (FORMS-7713)
...
* 當使用者嘗試使用包含`Chinese Full Width Characters`的PDF叫用LCA處理序時，`ValidateForm`處理序會發生問題。 (FORMS-7464)
...
* 正在新增`modifyProperties`許可權給Adobe Sign設定的`fd-cloudservice`系統使用者。 (FORMS-6164)
...
* 提供虛擬路徑時，Sling `ResourceMerger`會耗用大量CPU，造成拒絕服務。 (NPR-40338)
...
* 在JBoss® 7.1.4平台上，當使用者安裝Experience Manager6.5.16.0或更新版本的Service Pack時，`adobe-livecycle-jboss.ear`部署失敗。
...
* [Experience Manager6.5.17.0](/help/release-notes/assets/65170_bundles.txt)中包含的OSGi套件組合清單<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager6.5.17.0](/help/release-notes/assets/65170_packages.txt)中包含的內容套件清單<!-- UPDATE FOR EACH NEW RELEASE -->
