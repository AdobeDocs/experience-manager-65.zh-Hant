...
[!DNL Experience Manager] 6.5.17.0包含自2019年4月6.5版首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[&#x200B; 6.5上](#install)安裝此Service Pack[!DNL Experience Manager]。
...
* 當使用者按一下`Configure`元件的`Form Container`選項時，瀏覽器會停止回應。 (FORMS-7605)
...
* 具有`InboxSharingCache`的類別`user-reader-service`中的工作階段洩漏。 (CQ-4352513)
...
1. 產品資訊頁(`/system/console/productinfo`)會在`Adobe Experience Manager (6.5.17.0)`已安裝產品[!UICONTROL 下顯示更新的版本字串]。<!-- UPDATE FOR EACH NEW RELEASE -->
