...
[!DNL Experience Manager] 6.5.18.0包含自2019年4月6.5版首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[&#x200B; 6.5上](#install)安裝此Service Pack[!DNL Experience Manager]。
...
* 為了在清單檢視中快速尋找您可能有許多專案的專案，Adobe現在支援伺服器端排序。 專案節點會在使用者介面中呈現之前，根據使用者選取的欄在後端排序。 (NPR-41027)
...
* Experience Manager 6.5的GraphQL實作與主要版本不符，且遺漏了數個重要修正。 (SITES-13096)
...
* 在Experience Manager 6.5 Service Pack 17中為錨點和內文標籤新增的樣式已導致CSS問題。 在Author中，錨點標籤未顯示底線。 (SITES-14261)
...
* 在欄檢視中編輯和註釋影像時，Experience Manager會顯示空白熒幕。 (ASSETS-24108)
...
* 修正動態媒體元件中顯示停用的檢視器預設集。 (ASSETS-22922)
...
* **檔案服務**
   ...
   * 如果使用者嘗試使用AssemblerService.Invoke方法合併PDF，則組合器無法執行此工作。 (FORMS-9550)
...
* **互動式通訊**
   ...
   * 在伺服器上安裝Experience Manager Service Pack 16後，如果互動式通訊字母嘗試編輯這些字母，所有互動式通訊字母都會開始計時。 如果他們提供任何範例裝載來預覽或檢視/編輯屬性頁面，就有效果。 但是，他們無法編輯字母。 (FORMS-9067)
...
* 安裝vanilla Experience Manager Service Pack 17後，`stderr.log`中會顯示錯誤。 Vanilla安裝應該不會發生錯誤。 (CQ-4353637)
* 「標籤」(Tagging)畫面中的「建立」(Create)按鈕不遵守ACL （存取控制清單）。 (NPR-40973)
* 無法在Experience Manager上建立、存取（或兩者）ContextHub快取節點。 (NPR-40515)
...
* 如[設定瀏覽器檔案](https://experienceleague.adobe.com/docs/experience-manager-65/administering/introduction/configurations.html?lang=zh-Hant#using-configuration-browser)中所述，_名稱會變成存放庫中的節點名稱_。 不過在設定瀏覽器中，「設定標題」會用於CRXDE Lite中的路徑，而設定名稱會遭忽略。 (NPR-40607)
...
* `NullPointerException`中的記錄檔已填滿`EMailNotificationService`，工作流程應傳送的電子郵件未傳送。 (NPR-40898)
...
>[!NOTE]
>
>Experience Manager 6.5.18.0不支援Bootstrap安裝。<!-- UPDATE FOR EACH NEW RELEASE -->

**驗證安裝**

若要瞭解經過認證可搭配此版本使用的平台，請參閱[技術需求](/help/sites-deploying/technical-requirements.md)。

1. 產品資訊頁(`/system/console/productinfo`)會在`Adobe Experience Manager (6.5.18.0)`已安裝產品[!UICONTROL 下顯示更新的版本字串]。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* 升級至Service Pack 18 (**)後，6.5.18.0頁面發佈在頁面編輯器中無法運作**

  <!-- https://jira.corp.adobe.com/browse/SITES-15856 REMOVE FOR 6.5.19.0-->

  將AEM 6.5.0.0—6.5.17.0的執行個體升級為AEM 6.5.18.0後，當您在頁面編輯器中按一下&#x200B;**發佈頁面**&#x200B;時，系統會將您重新導向至不存在的URL。
...
* 升級至Windows Server 2022上的AEM Forms 6.5.18.0 JBoss Turnkey完整安裝程式環境後，使用Java 11編譯輸出使用者端應用程式程式碼時，可能會發生下列編譯錯誤：
  ...
  若要解決此問題，請執行以下步驟：
   ...
   1. 從class-path屬性中移除專案`Manifest.mf`，以更新`${clover.jar.name}`檔案。
...
* 安裝AEM Service Pack 6.5.18.0完整安裝程式後，EAR部署在使用JBoss® Turnkey的JEE上失敗。
...
* [Experience Manager中包含的OSGi套件組合清單6.5.18.0](/help/release-notes/assets/65180_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager中包含的內容套件清單6.5.18.0](/help/release-notes/assets/65180_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
