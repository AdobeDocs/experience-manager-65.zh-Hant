...
* **[AEM Forms Designer中的協助工具增強功能](https://helpx.adobe.com/tw/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**： AEM Forms Designer引進下列協助工具增強功能：

   * AEM Forms Designer現在支援透過新的`<TOC>`和`<TOCI>`標籤存取的目錄，讓作者可透過協助工具索引標籤將這些角色指派給子表單。 此功能支援多層級目錄結構、標籤專案、編號參照和跨區子結構，讓熒幕閱讀器能夠正確識別並導覽目錄內容。

   * AEM Forms Designer現在支援為表格儲存格設定協助工具屬性，包括範圍、列範圍和欄範圍，讓熒幕閱讀器和輔助裝置能夠正確解譯產生PDF中的表格結構。 作者可以直接從「儲存格屬性」對話方塊設定這些屬性，並在XML Source檢視中驗證。 設定的屬性會準確反映在標籤的PDF輸出中，並可使用Acrobat表格編輯器進行驗證。
...
#### 無障礙功能{#sites-accessibility-6519}
...
* 如果頁面上文字的亮度對比率小於4.5:1，則會影響視力不佳的人。 (ASSETS-25756)
...
* 即使在Forms Designer中的標籤正確，標籤順序在PDF中仍會隨機變更。(LC-3921313) 
...
1. OSGi套件`org.apache.jackrabbit.oak-core`是1.22.17版或更新版本（使用Web主控台： `/system/console/bundles`）。
<!-- NPR-41292 for 6.5.19.0 -->
