...
[!DNL Experience Manager] 6.5.19.0包括自2019年4月6.5首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[!DNL Experience Manager] 6.5上[安裝此Service Pack](#install)。
...
* 在簽章元件(`/apps/fpl/components/campaign/signature`)上，連結Externalizer無法運作。 如果影像標籤上方的HTML註解已移除，則不會將網域附加至影像來源。 此問題僅存在於生產環境中的簽名元件中，而非中繼環境中。 (SITES-16120)
...
* 從AEM 6.5.17和AEM 6.5.18升級執行個體後，如果您在頁面編輯器中選取&#x200B;**[!UICONTROL Publish頁面]**，系統會將您重新導向至不存在的URL。 系統會將使用者重新導向至Publish精靈。 (SITES-15856)
* 在作業系統剪貼簿貼上期間，從AEM剪貼簿複製多餘的復本。 (SITES-15704)
...
* 資產上傳至AEM後，會觸發`update_asset`工作流程。 工作流程永遠不會完成。 檢視工作流程例項，工作流程會完成至產品上傳步驟。 下一步是Scene7批次上傳。 使用者可透過Dynamic Media Classic應用程式檢視資產是否位於Scene7中。 (ASSETS-30443)
...
* 即使在資料夾層級關閉同步之後，記錄檔仍會顯示「Scene7 ReplicateOnModifyListener」的觸發程式。 `ReplicateOnModifyListener/Worker`應略過對非Dynamic Media資料夾資產和內容片段的處理。 (ASSETS-26705)
...
* 如果您要升級至AEM Service Pack 6.5.15.0或更新版本，而AEM執行個體在Tomcat 8.5.88上運作，則必須在&#x200B;*之前安裝servlet片段*，然後繼續安裝Service Pack 6.5.15.0或更新版本。
...
* 安裝AEM Service Pack 6.5.14.0及更高版本後，使用者在導覽至「`Home` > `Services` > `PDF Generator` > `Adobe PDF Settings`」時，無法從JEE管理員UI中選取PDF檔案的字型，因為字型清單顯示為空白。 (FORMS-12095)
...
* [Experience Manager6.5.19.0](/help/release-notes/assets/65190_bundles.txt)中包含的OSGi套件組合清單<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager6.5.19.0](/help/release-notes/assets/65190_packages.txt)中包含的內容套件清單<!-- UPDATE FOR EACH NEW RELEASE -->
