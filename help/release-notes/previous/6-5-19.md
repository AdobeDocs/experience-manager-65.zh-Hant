...
[!DNL Experience Manager] 6.5.19.0包含自2019年4月6.5版首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 在[ 6.5上](#install)安裝此Service Pack[!DNL Experience Manager]。
...
* 透過NPR-41291更新`org.json`程式庫，導致`DefaultDataTypeConverter`套件組合的`cfm-impl`發生資料錯誤轉換。 資料型別轉換必須更靈活。 (SITES-16473)
...
* 使用捷徑鍵&#x200B;**導覽時，熒幕助讀程式（NVDA/JAWS/朗讀程式）無法辨識**&#x200B;編輯資產`D/R`頁面中的地標。 (ASSETS-25744)
...
* 在`Something went wrong while adding the private key.`編輯使用者&#x200B;**>**&#x200B;金鑰存放區&#x200B;**中新增錯誤的私密金鑰檔案時，未當地語系化字串**。 (NPR-41366)
...
1. 產品資訊頁(`/system/console/productinfo`)會在`Adobe Experience Manager (6.5.19.0)`已安裝產品[!UICONTROL 下顯示更新的版本字串]。<!-- UPDATE FOR EACH NEW RELEASE -->
