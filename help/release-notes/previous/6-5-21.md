...
[!DNL Experience Manager] 6.5.21.0包含新功能、客戶要求的重要增強功能和錯誤修正。 此外，還包含自2019年4月6.5首次推出以來的效能、穩定性和安全性改善專案。 在[&#x200B; 6.5上](#install)安裝此Service Pack[!DNL Experience Manager]。
...
#### 無障礙功能 {#sites-accessibility-6521}
...
* 當使用者在OSGi上按一下AEM Forms Service Pack 15 (**)上的**&#x200B;列印到PDF6.5.15.0時，使用者端驗證會失敗，這由開發人員工具主控台視窗中顯示的錯誤訊息所證明。 (FORMS-14029)
...
* AEM 6.5 Service Pack 15會產生持續性錯誤記錄，其中包含重要的專案。 已報告下列問題：
   ...
   * 由於`NullPointerException`的`CampaignsDataSourceServlet.java:147`，未攔截到的SlingException的錯誤記錄專案
...
* 從AEM 6.5移除Adobe Search&amp;Promote的程式碼和相依性，該程式碼和相依性已根據通知[於2022年9月](https://experienceleague.adobe.com/en/docs/discontinued/using/search-promote)日結束生命週期。 (NPR-41855)
...
* `Unclosed resource resolver`正在經歷`com.day.cq.mailer.impl.DefaultMailService`錯誤。 `MessageGatewayService`類別是現成可用的類別，且不使用資源解析程式。 任何具有表單提交按鈕的頁面上，發生使用此類別傳送電子郵件的問題。 (NPR-41853)
...
* 選取資產時，位於`pathfield`的Granite `/libs/granite/ui/components/coral/foundation/form/pathfield`元件無法啟用&#x200B;**[!UICONTROL 選取]**&#x200B;按鈕。 在路徑欄位彈出且使用者選取資產核取方塊後，**[!UICONTROL 選取]**&#x200B;按鈕未啟用；它不會從灰色變更為藍色。 (NPR-41970)
...
1. 產品資訊頁(`/system/console/productinfo`)會在`Adobe Experience Manager (6.5.21.0)`已安裝產品[!UICONTROL 下顯示更新的版本字串]。<!-- UPDATE FOR EACH NEW RELEASE -->
