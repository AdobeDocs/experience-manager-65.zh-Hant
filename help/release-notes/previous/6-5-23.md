...
exl-id: e6066ff2-d0e5-41dd-99a9-fc6ca6ab4dfd
...
[!DNL Experience Manager] 6.5.23.0包括新功能、客戶請求的關鍵增強功能和錯誤修復。 它還包括自2019年4月首次推出6.5以來發佈的效能、穩定性和安全性改進。 在[&#x200B; 6.5上](#install)安裝此Service Pack[!DNL Experience Manager]。
...
* [靜態PDF中具有混合文本樣式的可訪問超連結](https://helpx.adobe.com/tw/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)：靜態PDF中包含混合文本樣式的超連結現在被標籤為單個可訪問元素。 此增強功能簡化了標籤樹結構、改善了熒幕助讀程式導覽，並支援更出色的協助工具合規性。

* [已更新支援的平台清單](/help/forms/using/aem-forms-jee-supported-platforms.md)

  最新版本將更新引入支援的平台清單，確保與較新技術相容。
...
* 修復了頁面編輯器中的AEM一個關鍵輔助功能問題，在該頁面編輯器中，跨多個對話框和模型（例如，資產導軌或佈局預覽）的畫布元素不能僅使用鍵盤操作。 所有互動式畫布元素現在支援僅鍵盤導覽，確保符合WCAG 2.1成功標準2.1.1 (SITE-25256)
...
* 在「權限」對話框中通過表標識修復了輔助功能問題。 中的權限表現AEM在使用正確的ARIA角色和屬性來確保像JAWS這樣的螢幕閱讀器正確地將其標識為表。 該修復程式提高了可訪問性合規性並確保用戶收到準確的導航和內容通告。 (SITES-27140)
...
* 在Service Pack 22升級後修復損壞的Classic UI映像資產查找器功能。 Classic UI影像資產尋找器現在可以正確處理包含空格或特殊字元的資產名稱。 此更新可確保資產尋找器可正確編碼檔案名稱、避免搜尋失敗，並允許作者找到並選取影像資產，而不會發生錯誤。 (SITES-29151)

#### [!DNL Content Fragments]{#sites-contentfragments-6523}

* 修正`DeleteVariationIT.testUpdateBasic`的驗證測試失敗。 在Service Pack驗證運行期間，`DeleteVariationIT.testUpdateBasic`測試不再失敗。 修復程式可糾正JSON處理邏輯中缺少的文本映射問題，確保測試穩定性並避免不必要的測試中斷。 (SITES-28022)
...
* 在12月25日至12月31日之間顯示的固定錯誤發佈日期。 啟動UI現在顯示日期為12月25日至12月31日，年份正確。 此修正可確保日期不再錯誤地顯示下一年，避免在行銷活動規劃和排程期間產生混淆。 (SITES-28706)
...
* 低螢幕解析度下的固定AssetPicker載入問題。 當用戶以較低螢幕解析度（1728×1117或更小）滾動時，AssetPicker現在可以正確載入資產。 用戶在滾動時不再感受到缺少的資產，從而改進了不同設備斷點的資產管理。 (SITES-28065)
* 已修復頁面鎖定和解鎖操作的丟失螢幕閱讀器公告。 當用戶激活鎖定/解鎖按鈕時，頁面編輯器現在會正確宣佈「資訊：頁面已鎖定/解鎖」消息。 該修復程式提高了輔助功能合規性並確保螢幕閱讀器用戶在頁面編輯期間接收動態更新。 (SITES-27143)
* 改進了創作中元件操作的鍵AEM盤焦點行為。 「作者」工具中增強AEM的鍵盤導航功能，可確保在「配置」、「刪除」或「轉換」等操作後，焦點仍在新建立或選定的元件上。 以前，焦點轉移到頁面頂部，導致輔助功能符合性問題。 此更新改進了鍵盤和輔助技術用戶的用戶體驗。 它通過在編輯工作流中維護邏輯焦點進度來實現。 (SITES-26549)
...
* 隱藏/取消隱藏操作後，佈局容器內元件的調整大小問題已解決。 在隱藏和取消隱藏佈局容器後，頁面編輯器現在可以正確計算列值。 用戶可以無錯誤地調整元件大小，並且在調整大小操作期間可以正確顯示列。 (SITES-28463)
...
* 從AEM6.5.23或更早版本直接升級到(AEM6.5.19和更高版本)。
...
* 在AEM Forms6.5.22.0中，使用Workbench中的輸出服務生成帶標籤的PDF，在目錄項的引用標籤下添加意外的標籤標籤。 (LC-3922756)

* 當使用者在 AEM Forms Designer 中將欄位註解設定為底部或右側對齊時，標記樹狀結構僅包含註解而沒有相應的值，造成不完整的無障礙標記。(LC-3922619)

* 從AEM Forms6.5 Service Pack 6升級到AEM FormsService Pack 20後，生成的PDF中的QR碼變得不可讀。 QR 碼的替代文字也未能通過無障礙測試，影響螢幕閱讀器的相容性。(LC-3922551)

* 當用戶在AEM FormsService Pack 18的代理UI中呈現字母時，由於FormService.render()API，內容無法正確顯示。 (LC-3922461)
...
* 當用戶使用AEM部分交鑰匙方法使用MySQL 8.4設定6.5時，LiveCycle配置管理器(LCM)無法識別所需的MySQL連接器驅動程式。 這會導致資料庫連接測試和設定失敗。 (FORMS-19442)
...
* 在AEM Forms6.5.23.0上，(PDFG)PS到PDF和HTML到PDF(WebKit)轉換失敗。 要解決此問題，請從[Adobe Experience Manager Forms修補程式](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/release-notes/aem-forms-hotfix)&#x200B;(FORMS.-21721)下載並安裝修補程式

* 在AEM Forms6.5.23.0上，(PDFG)影像到PDF轉換失敗。 要解決此問題，請從[Adobe Experience Manager Forms修補程式](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/release-notes/aem-forms-hotfix)&#x200B;(FORMS.-22029)下載並安裝修補程式
...
1. 如果執行個體處於更新模式（從舊版更新執行個體時），請在安裝前重新啟動執行個體。 Adobe建議在實例的當前正常運行時間較長時重新啟動。

1. 在安裝之前，請拍攝快照或[!DNL Experience Manager]實例的新備份。

1. 從[軟體分發](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.23.0.zip)下載服務包。<!-- UPDATE FOR EACH NEW RELEASE -->

1. 開啟包管理器，然後選擇&#x200B;**[!UICONTROL 上載包]**&#x200B;以上載包。 有關詳細資訊，請參閱[包管理器](/help/sites-administering/package-manager.md)。
...
[!DNL Experience Manager] 6.5.23.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.23/)中使用。<!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
請參閱[已棄用和已刪除功能](/help/release-notes/deprecated-removed-features.md/)，瞭解6.5中已棄用或已刪除的所有功能AEM的詳細清單。

### SPA 編輯器 {#spa-editor}

[從SPA6.5版6.5.23開始的新項目已棄用編輯器](/help/sites-developing/spa-overview.md)AEM。編輯SPA器仍然支援現有項目，但不應用於新項目。
...
* **在AEM6.5.21-6.5.23和6.AEM5 LTS GA中使用JSP指令碼包問題**
AEM6.5.21、6.5.22、6.5.23和AEM6.5 LTS GA隨`org.apache.sling.scripting.jsp:2.6.0`捆綁包一起提供，其中包含已知問題。 通常在 AEM 實例同時處理許多請求而造成高負載的情況下，就會發生這個問題。
...
* 如果您將[!DNL Experience Manager]執行個體從6.5.0 - 6.5.4升級至Java™ 11上的最新Service Pack，您會在`RRD4JReporter`檔案中看到`error.log`例外狀況。 若要停止異常，請重新啟動[!DNL Experience Manager]的實例。<!-- THIS BULLET POINT WAS UPDATED AS PER CQDOC-20021, JANUARY 23, 2023 -->

* 用戶可以更名[!DNL Assets]中層次結構中的資料夾，並將嵌套資料夾發佈到[!DNL Brand Portal]。 但是，在重新發佈根資料夾之前，不會在[!DNL Brand Portal]中更新資料夾的標題。

* 在安裝[!DNL Experience Manager] 6.5.x.x期間，可能會顯示以下錯誤和警告消息：
   * &quot;當在[!DNL Experience Manager]中使用目標標準API（IMS身份驗證）配置Adobe Target整合時，將體驗片段導出到目標將導致建立錯誤的提供類型。 Target會建立多個型別為「HTML」/來源「Adobe Target Classic」的選件，而非「體驗片段」/來源「Adobe Experience Manager」型別。
...
* 安裝AEM FormsJEE Service Pack 21(6.5.21.0)後，如果在`(geode-*-1.15.1.jar and geode-*-1.15.1.2.jar)`資料夾(FORMS.-14926)下發現Geode Jars `<AEM_Forms_Installation>/lib/caching/lib`的重複條目，請執行以下步驟來解決此問題：

   1. 如果鎖定程式正在運行，請停止它們。
   2. 停止服AEM務器。
   3. 轉到`<AEM_Forms_Installation>/lib/caching/lib`。
   4. 刪除除`geode-*-1.15.1.2.jar`之外的所有Geode修補程式檔案。 確認只存在具有`version 1.15.1.2`的Geode Jar。
   5. 在管理員模式下開啟命令提示符。
   6. 使用`geode-*-1.15.1.2.jar`檔案安裝Geode修補程式。

* **FORMS.-15256**：當用戶從AEM6.5Forms Service Pack 18或19升級到Service Pack 20或21時，他們遇到JSP編譯錯誤。 此錯誤阻止他們開啟或建立自適應表單。 它還導致了其他介面AEM的問題。 這些介麵包含頁面編輯器、AEM Forms UI、工作流程編輯器和系統概覽UI。
...
* **FORMS。-16557**：在交互通信代理UI的打印預覽中，所有欄位值的貨幣符號（如美元符號$）顯示不一致。 它看起來是999以下的值，但1000及以上的值缺失。
* **FORMS-16575**：互動式通訊中對巢狀配置片段XDP所做的任何修改都不會反映在IC編輯器中。
* **FORMS-16603**：在互動式通訊代理程式UI的列印預覽中，部分計算值未正確顯示。
* **FORMS-15681**:When&#x200B;在預覽列印中檢視信件，內容已變更。 也就是說，某些空格會消失，而某些字母會以`x`取代。
...
>[!MORELIKETHIS]
>
>* [[!DNL Experience Manager] 產品頁](https://business.adobe.com/tw/products/experience-manager/adobe-experience-manager.html)
>* [[!DNL Experience Manager] 6.5文檔](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65)
