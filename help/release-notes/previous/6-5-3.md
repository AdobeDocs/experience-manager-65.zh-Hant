...
* 能夠在[工作流程中](/help/forms/using/configure-out-of-office-settings.md)設定[外出] [!DNL Experience Manager]偏好設定。

* 能夠與[工作流程中的其他使用者](/help/forms/using/configure-shared-queues-osgi.md)共用收件匣或收件匣專案[!DNL Experience Manager]。
...
* 在`ReplicationOnModifyListener`中區塊上傳期間，會針對區塊節點觸發[!DNL Dynamic Media]事件(CQ-4281279)。
...
* 從以[!DNL Experience Manager] — 混合模式執行的[!DNL Dynamic Media]下載的動態轉譯已損毀（其為文字型別，內容「找不到影像」，而非影像內容型別） (NPR-30876)。
...
* 使用移動按鈕將多個資產從一個資料夾移動到另一個資料夾，在[!DNL Experience Manager]-Scene7執行模式(CQ-4280384)上執行的[!DNL Dynamic Media]中失敗。
...
* 如果使用者在從[!DNL Experience Manager]端收到貢獻資料夾中的新資產後嘗試將貢獻資料夾從[!DNL Brand Portal]重新發佈到[!DNL Brand Portal]，系統擲回例外狀況(CQ-4279740)。
* 為了避免複雜性，禁止在「貢獻」資料夾（巢狀資料夾）中建立「貢獻」資料夾(CQ-4278391)。
* 系統上傳從[!DNL Brand Portal] Admin Console匯入的[!DNL Experience Manager]使用者清單（.csv檔案）時發生例外狀況。 .csv檔案中只有「電子郵件」、「名字」和「姓氏」欄位是必填欄位(CQ-4278390)。
...
* [!DNL Experience Manager] 6.5 UberJar檔案擲回例外狀況，`cq-social-translation` 6.5 UberJar檔案中缺少[!DNL Experience Manager]個套件(NPR-31186)。
