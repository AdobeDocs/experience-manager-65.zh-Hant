...
[!DNL Adobe Experience Manager] 6.5.3.0為重要版本，包含效能、穩定性、安全性，以及自2019年4月&#x200B;**日6.5版本正式發行以來所發行的重要客戶修正和增強功能。**&#x200B;它可以安裝在[!DNL Adobe Experience Manager] 6.5之上。
...
* 從[!DNL Dynamic Media]7-Scene7模式上傳至[!DNL Dynamic Media Classic]花費太長的時間才能完成(NPR-31048)。
...
* 上傳從[!DNL Experience Manager]Admin Console匯入的[!DNL Brand Portal]使用者清單（.csv檔案）時，系統擲回例外狀況。 .csv檔案中只有「電子郵件」、「名字」和「姓氏」欄位是必填欄位(CQ-4278390)。

## Communities {#communities-6530}

**修正**

* 社群管理員（群組管理員/網站管理員）看不到管理群組的快速連結(開啟/編輯/Publish/刪除群組) (NPR-31627)。
...
* 將信件從LiveCycleES4SP1移轉到[!DNL Experience Manager] 6.5時，不顯示標題和工具提示(NPR-31615)。
...
* AXIS 1.4 jar發現安全性漏洞2012-5784和2014-3596，並提供[AXIS1.4.1 jar](https://helpx.adobe.com/aem-forms/quick-fixes/6-5/jee-patch-0014.html)的修正(NPR-31015)。
...
* oracle18c的[!DNL Experience Manager] Forms支援(NPR-29155)。

## UberJar {#uber-jar}

適用於Experience Manager6.5.3.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.3/)中使用。
