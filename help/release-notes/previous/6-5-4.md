...
使用[!DNL Brand Portal]設定[!DNL Experience Manager Assets]的步驟因您的[!DNL Experience Manager]版本，以及您是第一次設定還是升級現有設定而異。 如需詳細資訊，請參閱[使用Brand Portal設定Experience Manager Assets](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/publish/configure-aem-assets-with-brand-portal.html)。
