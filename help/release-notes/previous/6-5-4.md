...
Adobe Experience Manager 6.5.4.0為重要更新，包含自2019年4月&#x200B;**日正式發行6.5版以來所推出的新功能、客戶要求的重要增強功能，以及效能、穩定性、安全性等方面的改善專案。**&#x200B;可安裝在Adobe Experience Manager 6.5上。
...
* 在HTML5表單中支援[RTF](/help/forms/using/designing-form-template.md)。
...
## 不同區域的效能改善(6.5.4.0) {#performance-improvements}
...
* 縮短[!DNL Sites]即時副本總覽&#x200B;**[!UICONTROL 中超過200個即時副本之]**&#x200B;頁面上的專案載入時間。

* 改善處理不完整或無效的URL。 此類URL可能會減慢範本編輯器的速度。

## Sites {#sites-fixes}

* 當Adobe Experience Manager Sites頁面的URL包含冒號(`:`)或百分比符號(`%`)時，瀏覽器會停止回應，而CPU使用量會達到尖峰(NPR-32369、NPR-31918)。
...
* 無法同步處理包含現成設計匯入工具元件的Adobe行銷活動中的行銷活動(NPR-31728)。
...
* 使用Dynamic Media Scene7設定，在Experience Manager中將資產從一個資料夾移至另一個資料夾時，SPS (Scene7 Publishing System)中會建立無名稱的資料夾(NPR-32440)。
...
* PDF資產的Assets詳細資訊頁面除了在Dynamic Media Scene7執行模式上執行的Experience Manager中的「至集合」和「新增轉譯」按鈕以外，不會顯示動作按鈕(CQ-4286705)。

* Assets耗費太長時間處理Scene7的批次上傳程式(CQ-4286445)。

* 當使用者未在Dynamic Media使用者端的設定編輯器中做出任何變更時，儲存按鈕不會匯入遠端設定(CQ-4285690)。

* 將支援的3D模型擷取到Experience Manager中時，3D資產縮圖無法提供資訊(CQ-4283701)。

* 智慧型裁切視訊檢視器預設集的未處理狀態會在預設集名稱旁的橫幅文字上顯示兩次(CQ-4283517)。

* 在資產的詳細資訊頁面上觀察到在3D檢視器中預覽的上傳3D模型的容器高度不正確(CQ-4283309)。

* 在Experience Manager Dynamic Media混合模式中，轉盤編輯器無法在IE 11中開啟(CQ-4255590)。 **對於Adobe Dynamic Media — 混合模式客戶：** Adobe將在2022年5月之後終止對Dynamic Media — 混合模式中的Internet Explorer 11的支援。
...
### 使用[!DNL Experience Manager Assets]設定[!DNL Brand Portal] (6.5.4.0) {#configure-assets-bp}

[!DNL Experience Manager Assets]與[!DNL Brand Portal]之間的授權通道已變更。 之前，[!DNL Brand Portal]是透過舊版OAuth閘道在傳統UI中設定，該閘道使用JWT權杖交換來取得IMS存取權杖以授權。 [!DNL Experience Manager Assets]現在已透過[!DNL Brand Portal]設定為[!DNL Adobe I/O]，這可取得IMS權杖以授權您的[!DNL Brand Portal]租使用者。

使用[!DNL Experience Manager Assets]設定[!DNL Brand Portal]的步驟因您的[!DNL Experience Manager]版本，以及您是第一次設定還是升級現有設定而異。 如需詳細資訊，請參閱[使用Brand Portal設定Experience Manager Assets](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/publish/configure-aem-assets-with-brand-portal.html?lang=zh-Hant)。
...
* 行動Forms：在HTML版本中轉譯XDP表單時，提交按鈕以展開的大小顯示(NPR-32514)。

* 檔案服務：套用Service Pack 2後，信件和某些其他頁面的URL存取問題(NPR-32508、NPR-32509)。

* 檔案服務：如果伺服器上的交易數量超過特定限制，HTML到PDF的轉換會失敗，且檔案型別設定會從[!DNL Forms]伺服器移除(NPR-32204)。
...
* Workbench：針對PDF公用程式服務使用取得PDF屬性操作時顯示錯誤訊息(NPR-32150)。
...
適用於Experience Manager 6.5.4.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/)中使用。
