...
* 在回應式格線的預留位置中選取「將元件拖曳到此處」選項時，會傳送GET要求，且要求會產生HTTP 403錯誤(NPR-31845)。
...
使用[!DNL Brand Portal]設定[!DNL Experience Manager Assets]的步驟因您的[!DNL Experience Manager]版本，以及您是第一次設定還是升級現有設定而異。 詳細資訊請參閱[設定 Experience Manager Assets 與 Brand Portal 搭配使用](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/publish/configure-aem-assets-with-brand-portal.html)。
