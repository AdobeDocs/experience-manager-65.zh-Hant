...
* 支援 [automated forms conversion服務中的Adobe Sign文字標籤](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=zh-Hant).

* 支援 [將彩色表單轉換為最適化表單](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=zh-Hant) 使用 [!DNL Automated Forms Conversion service].
...
* 在 [!DNL Sites] 或 [!DNL Screens]，選取專案並按一下 [!UICONTROL 管理出版物]. 使用者無法在以下位置前進： [!UICONTROL 管理發布] 精靈，因為使用者介面錯誤。 具體而言， [!UICONTROL 發佈] 選項無法運作(NPR-34099)。
* 取消選取後，iParsys （繼承的段落系統）的位置不會恢復為原始預設位置 [!UICONTROL 取消繼承] 或 [!UICONTROL 停用繼承] 選項(NPR-34097)。
...
* 此 `htmlwriter` 不會編碼回屬性值。 XF標籤中的標籤會以解碼的屬性值(亦即 `"` 而非 `&#34`)。 若使用已匯出XF的視覺化體驗撰寫器，這會導致Target端發生問題(NPR-34048)。
* 在中移動頁面時 [!DNL Experience Manager Sites]，增強記錄以擷取版本建立失敗的原因(NPR-34014)。
...
* A [!DNL Content Fragment] 無法當作 `TemplatedResource` 因為它在中失敗 `ContentFragmentUsePojo` (NPR-33911)。
...
* 當使用移動動作來重新命名中的網頁時 [!DNL Sites] 在主控台，它會在精靈的最後一步顯示兩個重疊的對話方塊(NPR-33831)。
...
* 在中的內嵌編輯模式 [!DNL RTE]，拖曳影像不適用於文字元件(NPR-33602)。
* 可以在Blueprint頁面中建立與頁面名稱同名的元件。 轉出時， `_msm_moved` 尾碼為以重新命名元件。 元件會移動至 [!UICONTROL 段落系統] (NPR-33535)。
* 在許多頁面或資產上設定offTime或onTime時，會耗用大量資源，且在啟動和關閉期間會減緩系統速度(NPR-33482)。
* 對具有CRUD許可權的使用者 `/content/experience-fragment` 無法刪除資料夾(NPR-33436)。
* 您可以選取 [!UICONTROL HTML和JSON] 作為的選項 [!UICONTROL Adobe Target匯出格式] 在中的父資料夾上 [!DNL Experience Fragments] 區段。 對於此上層資料夾的子資料夾，相同的屬性會顯示在觸控式UI中。 但在CRXDE中，針對 `cq:adobeTargetExportFormat`，只會顯示HTML而非顯示 `html,json` (NPR-33423)。
* 不支援從頁面別名發佈或取消發佈。 移除似乎未另外宣告的選項(NPR-33415)。
* 特定標籤可在中的不同位置之間移動 [!DNL Experience Manager]. 它也可以套用至移動前後的不同頁面。 編輯頁面的屬性時，即使標籤相同，也不會顯示標籤以供編輯(NPR-33353)。
* 從包含多個版面容器的範本中刪除版面容器時，頁面範本無法正確轉譯(NPR-33347)。
* 在範本編輯器中，嘗試刪除下超過100000個頁面使用的範本 `/content/`. 錯誤顯示且沒有錯誤訊息(NPR-33312)。
* 重新導向至 [!DNL Experience Manager] 含有錨點的頁面在製作例項上無法運作為 `PageRedirectServlets` 將查詢字串放在URL片段或錨點之後(NPR-34288)。
* 在下建立品牌 `/content/campaign` 導致無法讓您建立行銷活動的結構。 [!UICONTROL 建立品牌] 選項會讓新建立的品牌無法建立 [!UICONTROL 優惠方案與活動] 因為沒有 [!UICONTROL 建立] 選項(NPR-34113)。
