...
Adobe Experience Manager 6.5.6.0為重要更新，包含自2019年4月&#x200B;**日正式發行6.5版以來所推出的新功能、客戶要求的重要增強功能，以及效能、穩定性和安全性改善專案。**&#x200B;可安裝在Adobe Experience Manager 6.5上。

Adobe Experience Manager 6.5.6.0中引入的主要功能和增強功能包括：

* 使用[!DNL Experience Manager]快速發佈[!DNL Dynamic Media]或[!UICONTROL 管理出版物]精靈，選擇性地發佈或取消發佈資產到[!UICONTROL 或]。
...
* `test`和`aiprocess`的預設選項已從Dynamic Media先前的`Thumbnail`更新為`Rasterize`，以確保使用者只需要建立縮圖，並略過頁面擷取和關鍵字擷取。
...
* 支援自動錶單轉換服務[中的](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=zh-hant)Adobe Sign文字標籤。

* 支援[使用](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=zh-hant)將彩色表單轉換為最適化表單[!DNL Automated Forms Conversion service]。
...
* 在[!DNL Sites]或[!DNL Screens]中，選取專案並按一下[!UICONTROL 管理出版物]。 由於使用者介面錯誤，使用者無法在[!UICONTROL 管理出版物]精靈中前進。 尤其是[!UICONTROL 發佈]選項無法運作(NPR-34099)。
...
* 從Experience Manager 6.3.3升級至Experience Manager 6.5.3後，轉出程式需要更長的時間，且轉出會因逾時錯誤而失敗(NPR-34049)。
...
* [!UICONTROL 管理發布]將內容從作者復寫到發佈執行個體的作業失敗，並產生JavaScript錯誤(NPR-33872)。
...
* 複製和貼上作業期間會移除復本上`cq:acLinks`的`cq:acUUID`和[!DNL Adobe Campaign]屬性(NPR-33794)。
...
* 您可以在[!UICONTROL 區段的父資料夾中選取]HTML &amp; JSON[!UICONTROL 作為]Adobe Target匯出格式[!DNL Experience Fragments]的選項。 對於此上層資料夾的子資料夾，相同的屬性會顯示在觸控式UI中。 不過，在CRXDE中，對於`cq:adobeTargetExportFormat`，它只顯示HTML，而不是顯示`html,json` (NPR-33423)。
* 不支援從頁面別名發佈或取消發佈。 移除似乎未另外宣告的選項(NPR-33415)。
...
* URL中有`/`個字元的網頁在編寫時變成無回應。 在製作期間新增元件時，CPU使用量會增加且瀏覽器停止回應(CQ-4295749)。
...
* 在[!UICONTROL 中的資產詳細資料]檢視器[!UICONTROL 中導覽時，鍵盤焦點現在已成功移至]彈出式視窗[!UICONTROL 、]InlineZoom[!UICONTROL 、]Shoppable_Banner[!UICONTROL 、]Zoom_dark[!UICONTROL 、]Zoom_Light[!UICONTROL 、]ZoomVertical_dark[!UICONTROL 和]ZoomVertical_light[!DNL Dynamic Media]選項(CQ-4290605)。
...
* 在[!DNL Experience Manager]標題區段中，當以瀏覽模式導覽時，熒幕助讀程式現在會朗讀，
  ...
  頁首![中的](/help/release-notes/assets/Help_aem_header.png)說明功能表

  *圖： [!UICONTROL 在]說明[!UICONTROL 功能表中搜尋說明]。*

   * 如果在[!UICONTROL 使用者]選項下的[!UICONTROL 模擬為]欄位中輸入不正確的值，且焦點已正確移至文字欄位，便會出現錯誤訊息(NPR-33804)。

  頁首![中的](/help/release-notes/assets/User_aem_header.png)使用者功能表
...
* 文字擷取以及因此上傳之PDF檔案的全文搜尋無法運作(NPR-34164)。 若要修正此問題，請在安裝Service Pack 6後重新啟動[!DNL sAdobe Experience Manager]部署。
...
* 在[!UICONTROL 或]中使用[!DNL desktop app]開啟[!DNL Adobe Asset Link]選項更新資產，且同步回[!DNL Adobe Experience Manager]時，資產的靜態轉譯並未更新(CQ-4296279)。
...
* 當您在`/var/workflow/models/dam` 6.5上安裝[!DNL Experience Manager] 6.5 Service Pack 5或舊版時，會刪除資產自訂工作流程模型（建立於[!DNL Experience Manager]）的執行階段復本(NPR-34532)。 若要擷取執行階段副本，請使用HTTP API將工作流程模型的設計階段副本與執行階段副本同步：
  `<designModelPath>/jcr:content.generate.json`。

在Dynamic Media中修正了&#x200B;**個問題**
...
* 當使用者在設定Dynamic Media Scene7 (CQ-4299727)的[!UICONTROL 中]工具[!UICONTROL 內的]工作流程[!UICONTROL 索引標籤上的]工作流程封存[!DNL Adobe Experience Manager]頁面上向下捲動時，會觸發DAM更新資產工作流程。
...
* 從`bcpkix-1.51`目錄中刪除`bcmail-1.51`、`bcprov-1.51`和`crx-repository\launchpad\ext` JAR檔案。

* 從` sling.bootdelegation.class.org.bouncycastle.jce.provider.BouncyCastleProvider`檔案中刪除`sling.properties`屬性。
...
* 最適化表單欄位顯示段落HTML標籤的說明內容說明(NPR-34116)。
...
* [!DNL Experience Manager Forms]的[!DNL Windows]應用程式在提交表單後停止回應(NPR-34409)。

* 安裝AEM Service Pack時，**待辦事項**&#x200B;專案清單未顯示為連結。 **待辦事項**&#x200B;專案的文字包含HTML標籤(NPR-34317)。
...
* 當您下載包含Reader擴充功能互動功能的PDF檔案時，無法使用[!DNL Adobe Reader]將附件新增至PDF檔案(NPR-33729)。

**文件安全性**

* 安裝[!DNL Experience Manager] Service Pack後，無法在PDF檔案中使用HSM型憑證執行簽署操作(NPR-34310)。
...
### 已轉譯的最適化表單頁面(6.5.6.0)的增強快取 {#enhanced-caching-translated-adaptive-forms}
...
### 將表單資料模型服務的輸出儲存到變數(6.5.6.0) {#save-fdm-service-to-variable}
...
適用於Experience Manager 6.5.6.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.6/)中使用。
