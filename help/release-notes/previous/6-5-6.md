...
* 使用[!UICONTROL 快速發佈]或[!UICONTROL 管理出版物]精靈，選擇性地發佈或取消發佈資產到[!DNL Experience Manager]或[!DNL Dynamic Media]。
...
* `test`和`aiprocess`的預設選項已從Dynamic Media先前的`Rasterize`更新為`Thumbnail`，以確保使用者只需要建立縮圖，並略過頁面擷取和關鍵字擷取。
...
* 支援自動錶單轉換服務](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=zh-hant)中的[Adobe Sign文字標籤。
...
* 您可以在[!DNL Experience Fragments]區段的父資料夾中選取[!UICONTROL HTML &amp; JSON]作為[!UICONTROL Adobe Target匯出格式]的選項。 對於此上層資料夾的子資料夾，相同的屬性會顯示在觸控式UI中。 不過，在CRXDE中，對於`cq:adobeTargetExportFormat`，它只顯示HTML，而不是顯示`html,json` (NPR-33423)。
...
* 在[!DNL Experience Manager]標題區段中，當以瀏覽模式導覽時，熒幕助讀程式現在會朗讀，
  ...
  頁首](/help/release-notes/assets/Help_aem_header.png)中的![說明功能表

  *圖： [!UICONTROL 在[!UICONTROL 說明]功能表中搜尋說明]。*

   * 如果在[!UICONTROL 使用者]選項下的[!UICONTROL 模擬為]欄位中輸入不正確的值，且焦點已正確移至文字欄位，便會出現錯誤訊息(NPR-33804)。

  頁首](/help/release-notes/assets/User_aem_header.png)中的![使用者功能表
...
* 當使用者在設定Dynamic Media Scene7 (CQ-4299727)的[!DNL Adobe Experience Manager]中[!UICONTROL 工具]內的[!UICONTROL 工作流程]索引標籤上的[!UICONTROL 工作流程封存]頁面上向下捲動時，會觸發DAM更新資產工作流程。
