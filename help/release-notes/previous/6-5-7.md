...
[!DNL Adobe Experience Manager] 6.5.7.0中引入的主要功能與增強功能包括：
...
* 在[!DNL Microsoft SQL Server]中支援[!DNL Experience Manager Forms] 2019。
...
* 使用者無法透過[!UICONTROL 管理出版物]選項發佈頁面，因為[!UICONTROL 發佈]選項會消失(NPR-34542)。
...
* 當使用者在`Enter`全選[!UICONTROL 選項上選取]鍵時，鍵盤焦點未移動到[!UICONTROL 建立控制項]選項(CQ-4293599)。

* 選取`Esc`索引鍵時，焦點不會還原至上層控制項(CQ-4293593、CQ-4293590)。

* 改善[!DNL Sites] UI與核心元件(CQ-4293448)的WCAG合規性。

* 已針對[!UICONTROL 頁面停用]縮放[!UICONTROL 和]縮放[!DNL Sites Editor]功能(CQ-4282353)。
...
* 在[!UICONTROL 頁面上選取]動作[!DNL AEM Sites]區段後，鍵盤使用者無法以適當的順序移動定位鍵焦點(CQ-4307786)。

* 在編輯內容片段時，在RTE工具列的連結目標選單清單中選取選項後，內容片段作者對話方塊開始閃爍(CQ-4305532)。

* 鍵盤使用者無法使用向下方向鍵(CQ-4295097)選取[!UICONTROL 新增元件]下拉式清單中的選項。

* 從[!UICONTROL 頁面的]Assets[!DNL Sites]索引標籤的[行事曆]功能表選取日期時，索引標籤焦點不會以適當的順序移動(CQ-4293600)。

* 在刪除編輯Sites頁面時可用的連結或文字選項後，鍵盤使用者的索引標籤焦點不會移至下一個或上一個選項(CQ-4293597)。

* 在檢視可用選項並按下[!UICONTROL 鍵後，鍵盤使用者無法將Tab鍵焦點移回]動作`Esc`區段中的[更多]選項(CQ-4293592)。
...
* 此版本中的[!DNL Experience Manager Assets]已針對協助工具完成下列增強功能。 如需詳細資訊，請參閱[中的 [!DNL Assets]](/help/assets/accessibility.md)協助工具功能。
...
* 搭配[!DNL Adobe Asset Link]使用[!DNL Adobe InDesign]時，搜尋結果不包含資料夾和集合，但僅包含資產(NPR-34699、CQ-4303666)。
...
* [!DNL Dynamic Media]中的協助工具已完成下列增強功能(CQ-4290306)。 如需詳細資訊，請參閱[中的 [!DNL Dynamic Media]](/help/assets/accessibility-dm.md)協助工具功能。
...
* 無法選擇性將資料夾發佈到Dynamic Media (CQ-4304526)。
...
* 如果標籤名稱包含特殊字元(NPR-34357)，`findByTitle` API的`TagManager`函式將無法運作。
* 匯入使用者同步套件的程式失敗(NPR-34399)。
* 已新增對`ariaLabel`元件的`ariaLabelledby`和`Coral.Masonry`屬性支援(GRANITE-29962)。
...
* 如果使用IMS設定和[!DNL Experience Fragments]環境，則無法將[!DNL Adobe Target]匯出至[!DNL Adobe Target Standard] (NPR-34555)。
...
* 當您將PDF轉換為最適化表單時，無法在索引標籤版面配置上使用表單資料模型來設定巢狀面板的值。 此外，使用程式碼編輯器以靜態陣列動態設定選項按鈕群組的值時，也會出現問題(NPR-35062)。

* 在最適化表單的文字欄位元件中輸入日文字元時，可指定超過35個字元最大限制的字元(NPR-35039)。

* 最適化表單在提交表單後顯示的`owner`感謝您`status`頁面上顯示不需要的引數，例如&#x200B;**[!UICONTROL 和]** (NPR-34989)。
...
* [!UICONTROL 使用] 6.2資料XML檔案預先填寫最適化表單的「使用資料預覽」[!DNL Experience Manager]選項無法正常運作(NPR-35087)。
...
適用於Experience Manager 6.5.7.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.7/)中使用。
