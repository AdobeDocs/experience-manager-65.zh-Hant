...
[!DNL Adobe Experience Manager] 6.5.8.0中引入的主要功能與增強功能包括：
...
* 開始將AEM執行個體從AEM 6.3升級至AEM 6.5時，升級執行個體需要更長的時間才能啟動(NPR-35323)。
* 復寫包含中括弧()的AEM資產時。 在名稱中，復寫失敗(GRANITE-27004、NPR-35315)。
...
* 啟用縮制時，您無法編輯任何頁面或元件。 安裝AEM 6.5 Service Pack 7後開始的問題(CQ-4311133)。
...
適用於Experience Manager 6.5.8.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.8/)中使用。
...
## Experience Manager 6.5.8.0中的已知問題 {#known-issues-6580}

<!-- REMOVED AS PER CQDOC-20022, JANUARY 23, 2023 * If you install [!DNL Experience Manager] 6.5 Service Pack 10 or a previous service pack on [!DNL Experience Manager] 6.5, the runtime copy of your assets custom workflow model (created in `/var/workflow/models/dam`) is deleted.
To retrieve your runtime copy, Adobe recommends to synchronize the design-time copy of the custom workflow model with its runtime copy using the HTTP API:
`<designModelPath>/jcr:content.generate.json`. -->

* 如果您正在將您的[!DNL Experience Manager]執行個體從6.5版本升級為6.5.8.0版本，您可以在`RRD4JReporter`檔案中檢視`error.log`例外狀況。 重新啟動執行個體以解決問題。
* 如果您在使用[!UICONTROL 定義規則]對話方塊在[!UICONTROL 資料夾中繼資料結構描述Adobe編輯器]和[!UICONTROL 中繼資料結構描述Forms編輯器]中編輯和建立階層式規則時遇到問題，請聯絡Forms客戶支援。 已建立和儲存的規則可如預期運作。
