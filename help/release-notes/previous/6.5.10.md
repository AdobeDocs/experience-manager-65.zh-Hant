...
* **使用者體驗增強功能**：
   ...
   * [!DNL Experience Manager] 直接顯示資料夾下所有內容模型的清單，內容作者無需導覽檔案結構。 此功能現在所需的點按次數減少，並提高撰寫效率。

   * 路徑欄位位於 [!DNL Sites] 編輯器可讓作者從拖曳資產 [!DNL Content Finder].

* 新增的支援 `GuideBridge#getGuidePath` 中的API [!DNL AEM Forms].
...
* **支援使用常值選項來設定JSON型別變數的值**：您可以在AEM工作流程的設定變數步驟中，使用常值選項來設定JSON型別變數的值。 此常值選項可讓您以字串形式指定 JSON。

* [平台更新](/help/forms/using/aem-forms-jee-supported-platforms.md)： [!DNL Adobe Experience Manager Forms] on JEE已新增下列平台的支援：
...
* 輸入時焦點會移至另一個欄位 **[!UICONTROL 預設值]** 欄位位於 **[!UICONTROL 屬性]** 內容片段編輯器的索引標籤(NPR-36992)。

* 篩選時 [!DNL Content Fragment] 指定路徑下的模型， [!DNL Experience Manager] 搜尋傳回所有節點，包含 `cq:Template` 而不是只傳回的路徑和節點 [!DNL Content Fragment] 模型(SITES-1453)。
...
* 當使用者移動、重新命名或刪除時 [!DNL Content Fragments] 或媒體資產，已引用的 [!DNL Content Fragments] 不會自動更新(SITES-196)。
...
* [!DNL Experience Manager] 如果您使用「影像核心元件」將影像新增至頁面，或使用「基礎影像」元件調整大小，會在有透明度的部分PNG影像中新增灰色邊框(NPR-36879)。
* [!DNL Experience Manager Sites] 管理員UI包含大量範本會導致導覽速度緩慢(NPR-36870)。
* 自訂servlet篩選套件組合等等修改網頁的連結時，無法轉譯網頁。 (NPR-36857) &lt;! — 根據CQDOC-19203於2022年4月7日所做的附註更新 — >
...
* 含有常用大小寫和所有大寫字的新標籤不會顯示在內的標籤欄位中 [!DNL Content Fragments] (NPR-36742)。
* 新增「 」時顯示的「單一文字元素」選項 [!DNL Content Fragment] 造成文字遺失，並建立與清單和巢狀清單相關的奇數格式(NPR-36565)。
...
* 當使用者在包含空格（系統上不存在的標籤）的隨選標籤中鍵入並按下Enter鍵時，標籤會出現在欄位下方。 然而，當 [!DNL Content Fragment] 儲存並重新開啟，隨選標籤不會出現(NPR-36441)。
...
* [!DNL Experience Manager] 有一個在WCM或通知上沒有索引的查詢，這會減慢效能(NPR-36303)。
...
* 開啟現有的v3元件版本頁面時，頁面屬性對話方塊未開啟，且 `NullPointerException` 已記錄(SITES-1830)。
...
* 開啟 [!UICONTROL 編輯影像預設集] 和 [!UICONTROL 建立互動視訊] 頁面、熒幕助讀程式現在會在使用標題鍵盤快速鍵(CQ-4290730)(CQ-4290701)導覽頁面時，適當地宣告頁面標題。
...
* 中字元顯示的延遲 [!UICONTROL 資料] 索引標籤和HTML字母預覽(NPR-37020)。
...
適用於Experience Manager6.5.10.0的UberJar可在 [Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.10/).
