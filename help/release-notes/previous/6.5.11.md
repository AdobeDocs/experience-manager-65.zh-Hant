...
* 在 [!DNL Dynamic Media]，在介面中建立新的視訊編碼設定檔， [!UICONTROL 智慧型裁切比例] 選項標示為協助工具，讓熒幕助讀程式適當地朗讀它。
...
* 如果是內嵌容器工作流程，即使選取「 」，您也會收到多封工作流程完成電子郵件 `Notify on Complete of Container Workflow` 選項(NPR-37280)。
