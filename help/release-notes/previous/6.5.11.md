---
title: 的發行說明 [!DNL Adobe Experience Manager] 6.5.11.0
...
[!DNL Adobe Experience Manager] 6.5.11.0包含自2019年4月6.5版發行以來所發行的新功能、客戶要求的重要增強功能，以及效能、穩定性和安全性等改善項目。 Service Pack安裝在 [!DNL Adobe Experience Manager] 6.5。
...
* 當使用者嘗試將XDP表單從PDF轉換為HTML5個表單時。 在HTML5份柬埔寨字型表格時發生的一些問題是：
   * 分行符號以HTML5表單呈現。 (CQ-4322508)
   * 清單在HTML5表單中無法正確轉譯。 (CQ-4322317)
   * 呈現HTML5表單轉譯時會顯示額外空格。 (CQ-4322509)
...
* 當使用者嘗試將XDP表單從PDF轉換為HTML5表單時，有些已修正問題是：
   * 支援Farsi以LTR形式呈現。
   * 與LTR文本混合時支援RTL。
   * 支援影像旋轉。
   * 增加對柬埔寨語的支援。 若要啟用支援柬埔寨支援，您必須將標幟啟用為 `CTL=true` 呈現HTML5表單時。
