...
* 在設定遠端DAM與Sites部署之間的連線後，遠端DAM上的資產便可在Sites部署中使用。 您現在可以在遠端DAM資產或資料夾上執行更新、刪除、重新命名和移動操作。 這些更新會在Sites部署中自動提供，但會有一些延遲(NPR-37816)。
...
* AEM 6.5.7.0 Forms的多重執行緒PDF Generator服務在隨機時間後出現死結(NPR-38053)。
...
| 連接器 | Adobe 6.5已棄用適用於Microsoft®SharePoint 2010和Microsoft®SharePoint 2013的Experience ManagerJCR聯結器。 | N/A |
