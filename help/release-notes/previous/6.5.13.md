...
* 在最適化表單中使用隱藏的驗證碼：您現在可以使用隱藏的驗證碼，僅在可疑活動的情況下顯示驗證碼質詢。 如果未發現可疑活動，則不會顯示驗證碼質詢。 它有助於評估人工表單完成情況，而無需核取方塊要求，減少自訂工作，並改善終端使用者體驗。 (NPR-38500)
...
* 已棄用 `/oak:index/lucene` 索引可能會導致 `FileNotFoundException` 會將索引傳送至無止盡的回圈中。因應措施是新增系統屬性 `oak.indexTracker.autoRefresh` 3153600000000啟AEM時，依照下列步驟執行：
