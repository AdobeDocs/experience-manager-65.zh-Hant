...
* 在最適化表單中使用隱藏的驗證碼：您現在只有在可疑活動的情況下，才能使用隱藏的驗證碼來顯示驗證碼質詢。 如果未發現可疑活動，則不顯示驗證碼質詢。 它有助於評估人工表單完成情況而不需要核取方塊、減少自訂工作，並改善終端使用者體驗。 (NPR-38500)
...
| 區域 | 功能 | 替代方案 |
|---|---|---|
| 整合 | 此 **[!UICONTROL AEMCloud Service選擇加入]** 畫面已淘汰，因為 [!DNL Experience Manager] 和 [!DNL Adobe Target] 整合已更新於 [!DNL Experience Manager] 6.5.整合支援Adobe Target Standard API。 API透過Adobe IMS和以下方式使用驗證 [!DNL Adobe I/O]. 它可支援AdobeLaunch在樂器方面日益增加的作用 [!DNL Experience Manager] 頁面對於analytics和個人化，選擇加入精靈在功能上無關。 | 設定系統連線、Adobe IMS驗證和 [!DNL Adobe I/O] 透過個別 [!DNL Experience Manager] 雲端服務。 |
