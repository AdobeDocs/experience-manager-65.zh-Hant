...
新通訊協定DASH (Dynamic Adaptive Streaming over HTTP)支援在Dynamic Media視訊傳送（透過CMAF）中啟動最適化位元速率資料流 [通用媒體應用程式格式] 已啟用)。

* 自適應串流 (DASH/HLS) 可確保更好的一般使用者觀看影片體驗.
* DASH 是自適應影片串流的國際標準通訊協定，在業界被廣泛採用.
* 現已於亞太及北美推出（可透過支援票證啟用）；即將於歐洲 — 中東 — 非洲推出。
...
>[!IMPORTANT]
>
> Adobe不建議您移除或解除安裝 [!DNL Experience Manager] 6.5.16.0套件。 因此，在安裝套件之前，您應該建立 `crx-repository` 以防您必須將其復原。 <!-- UPDATE FOR EACH NEW RELEASE -->
...
* GraphQL查詢可使用 `damAssetLucene` 索引而非 `fragments` 索引。 因此，它可能會導致GraphQL查詢失敗或需要很長時間才能執行。
...
* 嘗試移動/刪除/發佈內容片段或網站/頁面時，由於背景查詢失敗，擷取內容片段參考時會發生問題。 也就是說，功能無法運作。
