...
oracleJava™技術的使用權(由Adobe分發)將於2022年12月底前到期。 [!DNL Experience Manager] 建議客戶規劃及實作 [!DNL Azul Zulu] 此日期之前最新的JDK。 如需關於 [!DNL Oracle Java™] 技術與 [!DNL Azul Zulu] 技術，請參閱 [常見問題集](https://experienceleague.adobe.com/docs/experience-manager-65/assets/Java_Policy_for_Adobe_Experience_Manager.pdf).
...
<!-- CQDOC-18116 -->*您無法從Experience Manager上資產的「詳細資料」頁面播放視訊轉譯 — Dynamic Media以混合模式執行。
<!-- CQDOC-18116 -->*您無法串流Experience Manager上的視訊 — Dynamic Media以混合模式執行。
