...
>[!NOTE]
>
>在排程的[!DNL Experience Manager] Service Pack發行日期一週後，[!DNL Experience Manager] Forms中的修正會透過個別附加元件套件傳送。 在此情況下，附加元件套件將於2025年12月4日星期四發行。 此外，本節還新增了Forms修正和增強功能的清單。
...
[!DNL Experience Manager] 6.5.24.0的UberJar可在[Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.22/)中使用。<!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* **AEM 6.5.21-6.5.24和AEM 6.5 LTS GA中的JSP指令碼套件組合問題**
AEM 6.5.21至6.5.24，以及AEM 6.5 LTS GA隨附`org.apache.sling.scripting.jsp:2.6.0`套件，其中包含已知問題。 通常在 AEM 實例同時處理許多請求而造成高負載的情況下，就會發生這個問題。
  ...
  請透過Adobe聯絡客戶支援，並參考此版本注意事項以尋求解決方案。
