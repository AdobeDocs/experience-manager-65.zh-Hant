...
| 類型 | Service Pack發行 |
| 日期 | 2024年5月23日（星期四） <!-- UPDATE FOR EACH NEW RELEASE --> |
...
[!DNL Experience Manager] 6.5.21.0包括自2019年4月6.5首次發行以來所推出的新功能、客戶要求的重要增強功能、錯誤修正，以及效能、穩定性和安全性改善專案。 [安裝此Service Pack](#install) 於 [!DNL Experience Manager] 6.5.
...
* 新的、更易於使用伺服器對伺服器驗證的認證，取代現有的服務帳戶(JWT)認證。 (NPR-41994)主要
...
* 將Adobe Experience Manager從6.5.19.0升級至6.5.20.0時，路徑為 `/libs/cq/graphql/sites/graphiql` 即將刪除。 (SITES-20098)嚴重




#### [!DNL Content Fragments] - GraphQL查詢編輯器{#sites-graphql-query-editor-6521}
...
#### 核心後端{#sites-core-backend-6521}

* 三

#### 核心元件{#sites-core-components-6521}

* I

#### Campaign整合{#sites-campaign-integration-6521}
...
* 從AEM 6.5中移除已達到的AdobeSearch&amp;Promote的程式碼和相依性 [2022年9月終止服務（依通知）](https://experienceleague.adobe.com/en/docs/discontinued/using/search-promote). (NPR-41855)
...
* 此 `Unclosed resource resolver` 發生下列錯誤： `com.day.cq.mailer.impl.DefaultMailService`. 此 `MessageGatewayService` 類別（現成可用）在沒有資源解析程式的情況下使用。 任何具有表單提交按鈕的頁面發生問題，該按鈕會使用此類別傳送電子郵件。 (NPR-41853)
...
您可以使用兩種不同的方法來自動安裝 [!DNL Experience Manager] 6.5.21.0。<!-- UPDATE FOR EACH NEW RELEASE -->
...
The UberJar for [!DNL Experience Manager] 6.5.21.0可在以下網址取得： [Maven中央存放庫](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.20/). <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* [Experience Manager6.5.21.0中包含的OSGi套件組合清單](/help/release-notes/assets/65200-bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager6.5.21.0中包含的內容套件清單](/help/release-notes/assets/65200-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
