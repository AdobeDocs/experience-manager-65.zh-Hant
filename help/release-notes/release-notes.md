...
## [!DNL Experience Manager] 6.5.25.0包含的內容 {#what-is-included-in-aem-6525}

[!DNL Experience Manager] 6.5.25.0包含新功能、客戶要求的重要增強功能和錯誤修正。 此外，還包含自2019年4月6.5首次推出以來的效能、穩定性和安全性改善專案。 在[!DNL Experience Manager] 6.5上[安裝此Service Pack](#install)。
...
* 頁面編輯器中的「開始工作流程」對話方塊現在會顯示當地語系化的工作流程動作標籤。 作者不會再看到工作流程選項（例如核准、發佈、請求和取消發佈動作）的英文字串。 (SITES-18103)
...
>[!NOTE]
>
>在排程的[!DNL Experience Manager] Service Pack發行日期一週後，[!DNL Experience Manager] Forms中的修正會透過個別附加元件套件傳送。 在此情況下，附加套件計畫於2026年5月28日星期四發行。 此外，本節還新增了Forms修正和增強功能的清單。
...
* AEM 6.5 LTS SP1執行個體升級為LTS SP2後，CRX Package Manager測試套件不再失敗。 縮圖清單servlet測試現在會如預期般完成。 (NPR-43534)
...
* **FORMS-15681**&#x200B;在「列印預覽」中檢視信函時，內容會變更；部分空格會消失，而某些信函會以`x`取代。
...
* **FORMS-23881**&#x200B;在使用6.5.23.0完整安裝程式設定的AEM Forms JEE部署上，當引動中提供自訂XCI檔案時，輸出服務無法處理要求。 若要解決此問題，請從[軟體發佈](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)入口網站安裝最新的AEM 6.5.25.0 Forms Service Pack。
* **FORMS-23789** （僅限JEE上的AEM Forms）：使用者在JEE SP24上的AEM Forms中遇到Log4j問題，導致企業客戶的記錄與監視中斷。 若要解決此問題，請在JEE Service Pack 6.5.25.0上[下載並安裝AEM Forms的Hotfix](/help/release-notes/aem-forms-hotfix.md)。
...
* **FORMS-23703**&#x200B;若未使用預設值設定`contains`規則，最適化表單的伺服器端驗證會失敗。 您可以安裝最新版的[AEM Forms 6.5.25.0 Service Pack](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases#aem-65-forms-releases)以修正問題。
* **GRANITE-63681**&#x200B;表單資料模型聯結器可能無法驗證，因為預設不允許必要的關鍵字和Regex模式。 若要解決此問題，請從[連結](/help/release-notes/aem-forms-hotfix.md)下載並安裝Hotfix。
* **FORMS-23979** HTML-PDF轉換(PDFG)可能會遇到間歇性逾時。 適用於SP24的較新版本Forms附加元件隨後發行，其中包含此修正。 如果您遇到此問題，請將您的環境更新為6.5.25.0[&#128279;](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases#aem-65-forms-releases)的最新發行的Forms附加元件。
* **FORMS-23717**&#x200B;升級至&#x200B;**AEM Forms6.5.25.0**&#x200B;後，`server.log`和`error.log`可能會充滿重複的警告訊息，例如&#x200B;*安全剖析器工廠建立失敗*&#x200B;或不支援&#x200B;*安全性屬性……*。 記錄檔可能會以每秒&#x200B;**5到10行的速度增加** （每小時數百個MB），這會填滿磁碟並封鎖生產轉出。
...
* [Experience Manager 6.5.25.0中包含的OSGi套件組合清單](/help/release-notes/assets/65250-bundles.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5.25.0中包含的內容套件清單](/help/release-notes/assets/65250-packages.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
