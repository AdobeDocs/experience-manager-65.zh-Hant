...
[!DNL Experience Manager] 6.5.24.0包含新功能、客戶要求的重要增強功能和錯誤修正。 此外，還包含自2019年4月6.5首次推出以來的效能、穩定性和安全性改善專案。 在[!DNL Experience Manager] 6.5上[安裝此Service Pack](#install)。
...
* 鍵盤使用者可以在Teaser強制回應視窗中操作格式功能表。 按&#x200B;**清單**&#x200B;或&#x200B;**段落格式**&#x200B;上的`Enter`或`Space`會開啟快顯視窗、方向鍵導覽選項，而`Enter`會套用選取專案。 `Escape`關閉功能表並將焦點還原到觸發控制項，產生一致的工具列工作流程。 (SITES-25235)
...
* 變更篩選器現在會將其狀態報告為展開或摺疊。 按鈕會以與篩選器面板同步的方式切換`aria-expanded`，並公開單一清除名稱(`Change filters`)，移除令人困惑的`filter?`宣告。 熒幕助讀程式的使用者可以預測啟動控制的結果。 (SITES-24713)
...
* 參考邊欄地標現在會向熒幕朗讀程式宣告其名稱。 區域會顯示唯一的`aria-label` (`References rail`)，以改進地標式導覽，並將它與其他區域區分開來。 (SITES-24973)
...
* DAM路徑包含括弧的內容片段取消發佈失敗。 管理發布精靈重寫了`(`和`)`，並中斷了資產路徑。 此修正會保留字元並解析正確的專案，以便完成取消發佈動作。 (SITES-35077)
...
* 體驗片段屬性現在會將&#x200B;**參考**&#x200B;索引標籤當地語系化。 標籤和欄標題（例如`Page,` `Page path,`和`Variation title`）會以作者的語言顯示。 此變更會移除僅英文字串，並讓全域團隊的屬性檢視保持一致。 (SITES-11203)
...
* 自訂標籤標籤在頁面編輯器中失敗，且UI一律顯示`Tags.`述詞現在會先評估`fieldLabel`再評估`labelText`秒，然後套用預設值。 作者會看到他們設定的標籤。 (SITES-32278)
...
* 使用者在AEM Forms Designer中使用分頁時遇到空白的PDF頁面。 為WireAdviceHeader選取`Top of the Next Page/Top of Page`時發生問題，中斷資料反複專案的配置。 (LC-3922997、LC-3922830)
...
* 使用者在接收訊息`path object not tagged.`時，在可攜式應用程式元件(PAC)中遇到標題`Dati Richiedente`的背景顏色錯誤(LC-3922912)
...
* 使用者在建立規則以顯示面板並將焦點設定在面板內的元素時發生問題。 setFocus規則會在可見度更新之前執行，導致焦點動作失敗。 (FORMS-19563)
* 使用者在AEM Forms Author中選擇元件時遇到問題。 在編輯模式下的標籤之間導覽時，某些容器會變成無法選取，導致無法輕鬆識別和互動。 (FORMS-18525)
* 使用者嘗試在AEM 6.5.22中為資產加上註解時遇到`Invalid URL`錯誤。 (NPR-42684)
...
* 「說明」圖示現在會顯示輔助技術的簡要標籤。 JAWS讀取`Help button`且不再新增多餘的`menu`字詞。 此更新將控制項帶入WCAG 4.1.2中，並簡化鍵盤和熒幕助讀程式的使用。 (GRANITE-55360)
...
* 修正「影像」面板中截斷的工具提示文字。 `Display caption as pop-up`說明在所有支援的區域設定中完全呈現，改善非英文作者的指南。 (SITES-10490)
* 網站管理欄檢視截斷的法文和西班牙文本地化標籤。 `End Time`和`Off Time`似乎已截斷，沒有顯示工具提示。 Adobe已修正翻譯，並在暫留時還原工具提示，因此標籤會完整閱讀。 (SITES-31318)
* Sites中的&#x200B;**移動**&#x200B;對話方塊顯示原始i18n金鑰而非可讀取的標籤。 專案（例如`Referencing pages,` `Created on,` `Created by,`和`Path`）看起來亂碼。 此修正會將對話方塊連結至正確的字典，並提供翻譯（含英文備援）。 (SITES-30881)
...
1. 產品資訊頁(`/system/console/productinfo`)會在[!UICONTROL 已安裝產品]下顯示更新的版本字串`Adobe Experience Manager (6.5.24.0)`。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* **FORMS-23881**&#x200B;在使用6.5.23.0完整安裝程式設定的AEM Forms JEE部署上，當引動中提供自訂XCI檔案時，輸出服務無法處理要求。 若要解決此問題，請從[軟體發佈](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)入口網站安裝最新的AEM 6.5.24.0 Forms Service Pack。

* **FORMS-23789** （僅限JEE上的AEM Forms）：使用者在JEE SP24上的AEM Forms中遇到Log4j問題，導致企業客戶的記錄與監視中斷。 若要解決此問題，請在JEE Service Pack 6.5.24.0上[下載並安裝AEM Forms的Hotfix](/help/release-notes/aem-forms-hotfix.md)。
...
* **FORMS-23979** HTML-PDF轉換(PDFG)可能會遇到間歇性逾時。 適用於SP24的較新版本Forms附加元件隨後發行，其中包含此修正。 如果您遇到此問題，請將您的環境更新至6.5.24.0[&#128279;](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases#aem-65-forms-releases)的最新發行的Forms附加元件。
...
若要減少記錄磁碟區，請在應用程式伺服器設定中或透過JVM引數`-Dlogging.level.com.adobe.util.XMLSecurityUtil=ERROR`，將`com.adobe.util.XMLSecurityUtil`的記錄層級設定為`ERROR`。 此功能只會隱藏訊息，不會修正根本原因。
...
* [Experience Manager 6.5.24.0中包含的OSGi套件組合清單](/help/release-notes/assets/65240-bundles.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5.24.0中包含的內容套件清單](/help/release-notes/assets/65240-packages.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
