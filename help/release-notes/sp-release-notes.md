...
* AEM工作流程 [現在提供新的「產生可列印的輸出](../forms/using/aem-forms-workflow-step-reference.md) 」步驟。
...
* [或者，在標籤欄位中輸入「平台標籤搜尋標籤」會顯示根邊界以外的標籤，而不會尊重標籤欄位的]`rootPath` 屬性(NPR-31895)。

* [如果文字欄位中新增無效路徑] ，平台UI路徑瀏覽器會中斷(NPR-31884)。
...
* 設計人員：如果啟用標籤選項，子表單邊框會在產生的PDF輸出中消失(NPR-32547)。

* 設計人員：如果啟用標籤選項，子表單邊框會消失在產生的PDF輸出中(NPR-31983、NPR-31950)。
