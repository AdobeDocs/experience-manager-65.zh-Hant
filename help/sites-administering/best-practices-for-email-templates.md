...
>
>建議使用者使用現代化的 [核心元件電子郵件元件。](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/email/introduction.html)
...
>[!NOTE]
>
>為Adobe Campaign建立郵件範本時，您必須包含屬性 **acMapping** 包含值 **mapRecipient** 在 **jcr：content** 範本的節點。 如果沒有該物件，則無法在中選取Adobe Campaign範本 **頁面屬性** Experience Manager（欄位已停用）。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>可藉由設計變更 <i>cq：doctype</i> 中的屬性<i>"/etc/designs/default/jcr：content/campaign_newsletterpage"</i></p> <p>預設值為「XHTML」：</p> <p>&lt;!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional/EN" "https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"&gt;</p> <p>可變更為「HTML_5」：</p> <p>&lt;!DOCTYPE HTML&gt;</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   <td><p>使用為所有結構編碼 &lt;table&gt;元素。 對於更複雜的版面，您應該巢狀內嵌表格，以建置複雜的結構。</p> <p>即使沒有CSS，電子郵件應該看起來不錯。</p> </td>
  ...
  <tr>
   ...
   <td><p>所有表格都包含必要的屬性，例如 <i>邊框</i>， <i>單元格內邊距</i>， <i>單元格間距</i>、和 <i>寬度</i>.</p> <p>若要協調表格內的元素位置，所有表格儲存格都具有屬性 <i>valign=」top」</i> 已設定。</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>為了更妥善地展示基礎HTML結構以及更輕鬆地自訂電子報結構，僅內嵌部分CSS定義。</p> <p>已將基本樣式和範本變數擷取至 &lt;head&gt; 頁面的「 」。 在最後提交Newsletter時，這些CSS定義會內嵌至HTML中。 已規劃自動內嵌機制，但目前無法使用。</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>此 <i>寬度</i> 在示範設計中，內容表的上限為600畫素。</td>
  </tr>
 </tbody>
</table>
...
| 使用 `<img>` 元素而非表格中的背景影像。 | 範本中不使用任何背景影像資料。 |
| 在圖片上新增attribute style=&quot;display block&quot;。 如此一來，它們就能在Gmail上正常顯示。 | 所有影像預設包含 *style=&quot;display block&quot;* 屬性。 |
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>RtfEditor （例如在文字元件中）現在支援選擇字型系列和字型大小並套用至選取的文字。 它們會呈現為標籤。</td>
  </tr>
  <tr>
   <td>使用基本的跨平台字型，例如 <i>Arial®、Verdana、喬治亞州</i>、和 <i>Times New Roman®</i>.</td>
...
| **最佳實務** | **實施** |
|---|---|
| 使用W3C驗證器修正HTML代碼。 請確定所有開啟的標籤皆已正確關閉。 | 程式碼已驗證。 僅適用於XHTML可轉換的Doctype，遺失的xmlns屬性 `<html>` 元素遺失。 |
...
所有都有 **頁首**， a **頁尾**，和 **內文** 區段。 在body區段下方，每個範本都不同 **欄設計** （一、二或三欄）。
...
| 文字個人化(&amp;P) | /libs/mcm/campaign/components/personalization |
...
這些元件的詳細說明請參閱 [Adobe Campaign元件](/help/sites-authoring/adobe-campaign-components.md).
