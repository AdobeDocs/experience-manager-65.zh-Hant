...
AEM與Campaign的整合需要在這兩個解決方案中執行許多步驟。
...
* Adobe Campaign Classic的管理員存取權
   ...
   * 如果您需要有關如何設定和配置Adobe Campaign Classic的更多詳細資訊，請參閱 [Adobe Campaign Classic檔案，](https://experienceleague.adobe.com/docs/campaign-classic/using/campaign-classic-home.html) 尤其是安裝及設定指南。
* AEM的管理員存取權
...
1. 於 **編輯** 標籤中，選取 **存取許可權** 子標籤，然後按一下 **編輯存取引數……** 連結。
...
1. 確保安全性區域不會被中的個別設定覆寫 `config-<server name>.xml` 檔案。

   * 如果組態檔包含單獨的安全區域設定，則變更 `allowUserPassword` 屬性至 `true`.

1. 如果您想要變更Adobe Campaign Classic伺服器連線埠，請取代 `8080` 使用所需的連線埠。

   >[!CAUTION]
   >
   >依預設，運運算元沒有設定安全性區域。 若要讓AEM連線到Adobe Campaign，您必須選取區域，如先前步驟所述。
   >
   >Adobe強烈建議建立專用於AEM的安全區域，以避免任何安全問題。 如需有關本主題的詳細資訊，請參閱 [Adobe Campaign Classic檔案。](https://experienceleague.adobe.com/docs/campaign-classic/using/installing-campaign-classic/additional-configurations/security-zones.html)

1. 在Campaign使用者端中，返回 `aemserver` 運運算元並選取 **一般** 標籤。

1. 按一下 **重設密碼……** 連結。

1. 指定密碼並將其儲存在安全位置以供日後使用。

1. 按一下 **確定** 儲存密碼給 `aemserver` 運運算元。

## 在AEM中設定Campaign整合 {#campaign-integration}

AEM使用 [您已在Campaign中設定的運運算元](#create-operator) 以便與Campaign通訊
...
為了讓Campaign與AEM通訊，您需要為 `campaign-remote` AEM中的使用者。
...
時間 [安裝 **AEM整合** 在Campaign中封裝，](#install-package) 已為AEM建立外部帳戶。 透過設定此外部帳戶，Adobe Campaign可以連線至AEM，啟用解決方案之間的雙向通訊。
...
1. 於 **一般** 標籤中，輸入您在 [設定行銷活動遠端使用者密碼](#set-campaign-remote-password) 步驟。

   * **伺服器** - AEM作者伺服器位址
      * AEM作者伺服器必須可從Adobe Campaign Classic伺服器執行個體連線。
      * 確定伺服器位址 **not** 以尾隨斜線結尾。
