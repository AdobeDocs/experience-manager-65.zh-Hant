...
* 在需要保護的樹上限制讀訪問，並且只允許讀取帶有指定CUG實例的清單主體或完全排除在CUG評估之外的主體。 這稱為 **授權** 元素。
* 在指定的樹上強制驗證，並可選地為該樹指定一個專用的登錄頁，該登錄頁隨後被排除。 這稱為 **驗證** 元素。
...
這些CUG原則會透過名為oak-authorization-cug的個別授權模組部署至AEM執行個體。 本模組隨附其自己的訪問控制管理和權限評估。 換句話說，預設AEM設定會提供結合多個授權機制的Oak內容存放庫設定。 如需詳細資訊，請參閱 [本頁面說明Apache Oak檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html).

在此複合設定中，新CUG不會替換附加到目標節點的現有訪問控制內容，而是設計為補充內容，以後也可以刪除，而不會影響原始訪問控制，預設情況下，AEM中的訪問控制清單將是訪問控制清單。

與前一實施不同，新的CUG策略始終被識別並視為訪問控制內容。 這表示使用JCR存取控制管理API來建立和編輯這些API。 如需詳細資訊，請參閱 [管理CUG策略](#managing-cug-policies) 區段。
...
CUG政策的權限評估以及與預設或任何其他授權模型的互動，會遵循Apache Jackrabbit Oak中為多個授權機制所設計的模式：只有在所有模型都授予存取權時，才會授予指定的一組權限。 請參閱 [本頁](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html) 以取得更多詳細資訊。
...
CUG功能的認證相關部分允許標籤需要認證的樹並可選地指定專用的登錄頁。 根據前一版本，新實施允許標籤需要在內容儲存庫中進行身份驗證的樹，並有條件地啟用與 `Sling org.apache.sling.api.auth.Authenticator`負責最終強制執行要求並將重新導向至登入資源。

這些要求會透過提供以下功能的OSGi服務向驗證者註冊 `sling.auth.requirements` 註冊屬性。 然後，這些屬性將用於動態擴展驗證需求。 如需詳細資訊，請參閱 [Sling檔案](https://sling.apache.org/apidocs/sling7/org/apache/sling/auth/core/AuthConstants.html#AUTH_REQUIREMENTS).

#### 使用專用混頻類型定義驗證需求 {#defining-the-authentication-requirement-with-a-dedicated-mixin-type}

基於安全理由，新實作會以稱為的專用混合類型取代剩餘JCR屬性的使用 `granite:AuthenticationRequired`，定義登入路徑的單一選用屬性類型STRING `granite:loginPath`. 只有與此混合類型相關的內容變更才會更新Apache Sling Authenticator註冊的需求。 持續保留任何暫時性修改時就會追蹤修改，因此需要 `javax.jcr.Session.save()` 呼籲生效。

同樣適用於 `granite:loginPath` 屬性。 只有由驗證要求相關的mixin類型定義時，才會接受它。 在非結構化JCR節點上添加具有此名稱的剩餘屬性將不會顯示所需的效果，而負責更新OSGi註冊的處理程式將忽略該屬性。

>[!NOTE]
>
>設定登錄路徑屬性是可選的，並且僅當需要身份驗證的樹不能回復為預設或繼承的登錄頁時才需要。 請參閱 [登錄路徑評估](/help/sites-administering/closed-user-groups.md#evaluation-of-login-path) 下方。
...
預設AEM設定現在可允許在製作執行模式中設定mixin，但只讓mixin在復寫至發佈執行個體時生效，以利用此設定。 請參閱 [本頁](https://sling.apache.org/documentation/the-sling-engine/authentication/authenticationframework.html) 如需詳細資訊，請參閱Sling如何執行驗證要求。

新增 `granite:AuthenticationRequired` 配置的支援路徑中的mixin類型將導致負責處理程式的OSGi註冊更新，該處理程式包含新的附加條目，該條目包含 `sling.auth.requirements` 屬性。 如果給定的驗證要求指定了可選 `granite:loginPath` 屬性，則值會以「 — 」首碼額外註冊給驗證器，以便從驗證需求中排除。
...
評估登入路徑並在驗證時重新導向至對應資源，目前是AdobeGranite登入選取器驗證處理常式( `com.day.cq.auth.impl.LoginSelectorHandler`)，此元件為Apache Sling AuthenticationHandler，預設會以AEM設定。

呼叫時 `AuthenticationHandler.requestCredentials` 此處理常式會嘗試判斷要將使用者重新導向的對應登入頁面。 這包括下列步驟：

* 區分過期的密碼以及需要定期登錄作為重定向原因；
* 若是一般登入，會測試登入路徑是否可依下列順序取得：

   * 從LoginPathProvider，如新 `com.adobe.granite.auth.requirement.impl.RequirementService`,
   * 從舊有已棄用的CUG實作，
   * ，以 `LoginSelectorHandler`,
   * 最後，回復至預設登入頁面，如 `LoginSelectorHandler`.

* 一旦透過上述呼叫取得有效的登入路徑，系統就會將使用者的要求重新導向至該頁面。

本檔案的目標是評估內部公開的登入路徑 `LoginPathProvider` 介面。 自AEM 6.3起提供的實作如下：

* 登錄路徑的註冊取決於密碼過期和需要定期登錄作為重定向原因之間的區別
* 若是一般登入，會測試登入路徑是否可依下列順序取得：

   * 從 `LoginPathProvider` 由 `com.adobe.granite.auth.requirement.impl.RequirementService`,
   * 從舊有已棄用的CUG實作，
   * ，如 `LoginSelectorHandler`,
   * 最後，依 `LoginSelectorHandler`.

* 一旦透過上述呼叫取得有效的登入路徑，系統就會將使用者的要求重新導向至該頁面。

此 `LoginPathProvider` 如Granite中新的驗證需求支援所實作，會公開由 `granite:loginPath` 屬性，而屬性又由如上所述的mixin類型定義。 保存登錄路徑的資源路徑和屬性值本身的映射被保存在記憶體中，並將被評估以為層次中的其他節點尋找合適的登錄路徑。
...
* 要避免指定，然後註冊冗餘登錄路徑：
   ...
   * 應用程式開發人員應識別應在與 `LoginSelectorHandler`.

## 在儲存庫中的表示 {#representation-in-the-repository}

### CUG在儲存庫中的策略表示 {#cug-policy-representation-in-the-repository}

Oak檔案涵蓋新CUG原則在存放庫內容中的反映方式。 如需詳細資訊，請參閱 [本頁](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#Representation_in_the_Repository).
...
使用JCR訪問控制管理API來管理用於限制CUG讀訪問的新類型的訪問控制策略，並遵循以下機制描述 [JCR 2.0規範](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).
...
編輯現有CUG策略需要執行以下步驟。 請注意，修改後的策略需要回復，而更改需要使用 `javax.jcr.Session.save()`.
...
JCR訪問控制管理定義了檢索在給定路徑生效的策略的最佳工作方法。 由於CUG策略的評估是有條件的，並且取決於要啟用的相應配置，請調用 `getEffectivePolicies` 是驗證給定CUG策略是否在給定安裝中生效的一種方便方法。

>[!NOTE]
>
>請注意 `getEffectivePolicies` 和後續的程式碼範例，向上游顯示階層，以尋找指定路徑是否已屬於現有CUG的一部分。
...
無論CUG是否生效，都會在指定路徑上找到已定義的所有巢狀CUG。 如需詳細資訊，請參閱 [配置選項](/help/sites-administering/closed-user-groups.md#configuration-options) 區段。
...
由定義的擴充功能 `JackrabbitAccessControlManager` 允許按主體編輯訪問控制策略的策略沒有通過CUG訪問控制管理實現，因為根據定義，CUG策略始終影響所有主體：那些跟 `PrincipalSetPolicy` 將被授予讀取訪問權限，而所有其他主體將被阻止讀取目標節點定義的樹中的內容。
...
>[!NOTE]
>
>只有在 `RequirementHandler` 已設定，且目標包含在受支援路徑定義的樹中（請參閱「配置選項」一節）。
>
>如需詳細資訊，請參閱 [分配混合節點類型](https://docs.adobe.com/docs/en/spec/jcr/2.0/10_Writing.html#10.10.3分配Mixin節點類型)和 [添加節點和設定屬性](https://docs.adobe.com/docs/en/spec/jcr/2.0/10_Writing.html#10.4添加節點和設定屬性)

#### 新增驗證需求 {#adding-a-new-auth-requirement}

建立新驗證需求的步驟如下所述。 請注意，若符合以下條件，則需向Apache Sling Authenticator註冊 `RequirementHandler` 已為包含目標節點的樹配置。
...
建立新驗證需求的步驟，包括登入路徑。 請注意，登入路徑的需求和排除項目只會向Apache Sling Authenticator註冊(若 `RequirementHandler` 已為包含目標節點的樹配置。
...
變更現有登入路徑的步驟於下文詳細說明。 只有在 `RequirementHandler` 已為包含目標節點的樹配置。 先前的登錄路徑值將從註冊中刪除。 與目標節點相關聯的驗證要求不會受此修改影響。
...
移除現有登入路徑的步驟。 只有在 `RequirementHandler` 已為包含目標節點的樹配置。 與目標節點相關聯的驗證需求不受影響。
...
移除現有驗證需求的步驟。 只有在 `RequirementHandler` 已為包含目標節點的樹配置。
...
沒有專用的公用API可讀取向Apache Sling Authenticator註冊的所有有效驗證需求。 不過，此清單會公開於 `https://<serveraddress>:<serverport>/system/console/slingauth` 在「**驗證需求配置**」部分。
...
如同登入路徑，沒有公用API可擷取內容中定義的繼承驗證需求。 以下示例說明如何列出已使用指定層次結構定義的所有身份驗證要求（無論這些要求是否有效）。 如需詳細資訊，請參閱 [配置選項](/help/sites-administering/closed-user-groups.md#configuration-options).

>[!NOTE]
>
>建議您同時依賴繼承機制來滿足驗證需求和登入路徑，並避免建立巢狀驗證需求。
>
>如需詳細資訊，請參閱 [認證需求的評估與繼承](#evaluation-and-inheritance-of-the-authentication-requirement), [登錄路徑評估](#evaluation-of-login-path) 和 [最佳實務](#best-practices).
...
新的授權相關部分包含在 **Oak CUG授權** 捆綁( `org.apache.jackrabbit.oak-authorization-cug`)，此為AEM預設安裝的一部分。 該捆綁定義了一個分離的授權模型，該授權模型旨在作為管理讀取訪問的一種附加方法進行部署。

#### 設定CUG授權 {#setting-up-cug-authorization}

如需設定CUG授權的詳細說明，請參閱 [相關Apache檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability). 依預設，AEM在所有執行模式中都部署了CUG授權。 該逐步指令還可用於在那些需要不同授權設定的安裝中禁用CUG授權。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    </ul> <p>另請參閱 <a href="#configuration-options">配置選項</a> 下方。</p> </td>
  </tr>
  <tr>
   <td>配置策略</td>
...
列出與CUG授權模組相關聯的可用配置選項，並在 [Apache Oak檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#configuration).
...
如需詳細資訊，請參閱 [自AEM 6.3起的預設設定](#default-configuration-since-aem) 一節。

「管理員」群組的排除可在以下位置的系統主控台中變更或展開： **Apache Jackrabbit Oak CUG排除清單**.

或者，您也可以提供並部署CugExclude介面的自訂實作，以因應特殊需求調整排除的主體集合。 請參閱 [CUG可插拔性](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability) 以取得詳細資訊和範例實作。

### 驗證：設定與設定 {#authentication-setup-and-configuration}

新的驗證相關部分包含在 **AdobeGranite驗證處理常式** 捆綁( `com.adobe.granite.auth.authhandler` 5.6.48版)。 此套件組合是AEM預設安裝的一部分。

為了為已棄用的CUG支援設定驗證需求替換，在指定的AEM安裝中，某些OSGi元件必須存在且處於活動狀態。 如需更多詳細資訊，請參閱 **OSGi元件的特性** 下方。
...
<table>
 <tbody>
  ...
  <tr>
   <td>說明</td>
   <td>針對驗證需求的專用OSGi服務，會針對影響驗證需求的內容變更向觀察者註冊(透過 <code>granite:AuthenticationRequirement</code> mixin類型)和的登入路徑會公開給 <code>LoginSelectorHandler</code>. </td>
  </tr>
  <tr>
   <td>組態屬性</td>
   <td>-</td>
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>設定&lt;string&gt;</td>
   <td>-</td>
   <td>此處理常式將接受驗證要求的路徑。 如果您要新增 <code>granite:AuthenticationRequirement</code> 混合類型至節點，但未強制執行（例如在製作例項上）。 如果缺少，則禁用該功能。 </td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>沒有 **Apache Jackrabbit Oak CUG排除清單** 和 **AdobeGranite驗證需求和登入路徑處理常式** 預設製作執行個體上存在。
...
請參閱 [CUG可插拔性](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability) 有關如何從複合授權設定中刪除CUG授權模型的詳細資訊的文檔。

### 禁用身份驗證要求 {#disable-the-authentication-requirement}

為了禁用對驗證要求的支援， `granite.auth.authhandler` 模組，足以移除與 **AdobeGranite驗證需求和登入路徑處理常式**.
...
為了反映CUG授權模型所使用的新類型存取控制政策，已擴充Apache Jackrabbit所定義的API。 自2.11.0版 `jackrabbit-api` 模組定義新介面，稱為 `org.apache.jackrabbit.api.security.authorization.PrincipalSetPolicy`，從 `javax.jcr.security.AccessControlPolicy`.

### Apache Jackrabbit FileVault {#apache-jackrabbit-filevault}

Apache Jackrabbit FileVault的匯入機制已調整，以處理類型的存取控制政策 `PrincipalSetPolicy`.
...
* 其他政策，例如 `org.apache.jackrabbit.api.security.authorization.PrincipalSetPolicy` 由CUG授權模型建立的實例將始終被複製，並配置選項 `DurboImportConfiguration.isImportAcl`()會被忽略。

複製CUG策略有一個限制。 如果刪除給定的CUG策略，而不刪除相應的混合節點類型 `rep:CugMixin,` 復寫時不會反映移除。 此問題已通過始終在刪除策略時刪除mixin來解決。 不過，如果手動新增混合類型，則可能會顯示限制。

### AdobeGranite驗證處理常式 {#adobe-granite-authentication-handler}

驗證處理常式 **AdobeGranite HTTP標題驗證處理常式** 隨 `com.adobe.granite.auth.authhandler` 捆綁包含對的引用 `CugSupport` 由相同模組定義的介面。 它用於在某些情況下計算「領域」，並回退到配置有處理程式的領域。

此項已調整，以參考 `CugSupport` 若指定的設定決定重新啟用已棄用的實施，則可選，以確保最大的回溯相容性。 使用實作的安裝將不再從CUG實作中擷取領域，但一律會依照定義顯示領域 **AdobeGranite HTTP標題驗證處理常式**.

>[!NOTE]
>
>依預設， **AdobeGranite HTTP標題驗證處理常式** 僅以「停用登入頁面」( `auth.http.nologin`)選項啟用。
...
這兩個元素都會在 `cq:Page`. 透過目前的設計，MSM只會處理位於 `cq:PageContent` (`jcr:content`)節點。
...
已棄用的OSGi元件 **AdobeGranite封閉使用者群組(CUG)支援** ( `com.day.cq.auth.impl.cug.CugSupportImpl`)取代為新元件，以便能夠分別處理前CUG功能的授權和驗證相關部分。
...
在新實施中，預設授權模型的訪問控制設定不受正在建立、修改或刪除的任何CUG的影響。 而是新類型的策略，稱為 `PrincipalSetPolicy` 會以其他存取控制內容的形式套用至目標節點。 此附加策略將作為目標節點的子節點定位，如果存在，則為預設策略節點的同級。

**在訪問控制管理中編輯CUG策略**

從剩餘JCR屬性移至專用的存取控制原則，會影響建立或修改CUG功能授權部分所需的權限。 由於這被視為存取控制內容的修改，因此需要 `jcr:readAccessControl` 和 `jcr:modifyAccessControl` 權限，以便寫入存放庫。 因此，只有有權修改頁面存取控制內容的內容作者才能設定或修改此內容。 這與舊版實作不同，舊版實作中寫入一般JCR屬性的能力已足夠，導致權限提升。
...
從舊的CUG支援移至其他授權模型，會變更評估有效讀取權限的方式。 如 [Jackrabbit檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)，指定的主體可檢視 `CUGcontent` 只有在Oak存放庫中所設定模型的權限評估已授予讀取存取權時，系統才會授予讀取存取權。

換言之，為評估有效權限， `CUGPolicy` 並且，將考慮預設訪問控制項條目，並且只有在CUG內容被兩種策略授予時，才授予該內容的讀訪問權。 在預設的AEM發佈安裝中，讀取完成的存取權 `/content` 樹授予每個人，CUG政策的效果將與舊實施相同。
...
* 只有在選項時才啟用權限評估 **啟用CUG評估** 已另外勾選。

在新的AEM預設CUG策略設定評估中，它只會以「發佈」執行模式啟用。 請參閱 [自AEM 6.3起的預設設定](#default-configuration-since-aem) 以取得更多詳細資訊。 這可以通過將給定路徑的有效策略與儲存在內容中的策略進行比較來驗證。 只有在啟用CUG的權限評估時，才會顯示有效策略。

如上所述，CUG訪問控制策略現在始終儲存在內容中，但只有在 **啟用CUG評估** 在Apache Jackrabbit Oak的系統主控台中開啟 **CUG配置。** 依預設，僅以「發佈」執行模式啟用。
...
在前一實施中，CUG的授權和驗證方面都是由單一JCR屬性( `cq:cugEnabled`)。 就驗證而言，這會導致更新Apache Sling Authenticator實作所儲存的驗證需求清單。 通過使用專用混合類型標籤目標節點( `granite:AuthenticationRequired`)。

#### 用於排除登錄路徑的屬性 {#property-for-excluding-login-path}

mixin類型定義了名為的單一選用屬性 `granite:loginPath`，這與 `cq:cugLoginPage` 屬性。 與先前的實施相反，只有在其聲明節點類型為上述mixin時，登入路徑屬性才會受到尊重。 在未設定mixin類型的情況下添加具有該名稱的屬性將無效，並且不會向驗證器報告登錄路徑的新要求或排除。

#### 身份驗證要求的權限 {#privilege-for-authentication-requirement}

添加或刪除混合類型需要 `jcr:nodeTypeManagement` 被授予權限。 在先前的實作中， `jcr:modifyProperties` 權限用於編輯剩餘屬性。

至 `granite:loginPath` 關注在添加、修改或刪除屬性時需要相同的權限。
...
兩者 `granite:AuthenticationRequired` mixin類型和granite:loginPath屬性只會在由 **支援的路徑** 設定選項與 **AdobeGranite驗證需求和登入路徑處理常式**. 如果未指定路徑，則完全禁用身份驗證要求功能。 在此情況下，mixin類型或屬性在添加到給定JCR節點或設定該節點時生效。
...
1. 前往 `https://<serveraddress>:<serverport>/system/console/cug-migration` 來存取工具。
1. 輸入要檢查CUG的根路徑，然後按 **執行乾式運行** 按鈕。 這會掃描所選位置中符合轉換資格的CUG。
1. 查看結果後，按 **執行移轉** 按鈕移轉至新實作。

>[!NOTE]
>
>如果您遇到問題，可以在 **除錯** 層級 `com.day.cq.auth.impl.cug` 以取得移轉工具的輸出。 請參閱 [記錄](/help/sites-deploying/configure-logging.md) 以取得如何執行此動作的詳細資訊。
