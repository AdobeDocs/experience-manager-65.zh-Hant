...
* 將剪貼簿內容貼為純文字。 在AEM元件中插入之前，會先從複製的內容移除所有樣式和格式元素。 使用下面的`plaintext`進行配置。
...
您可進一步設定「貼上方式-Microsoft-Word」(`paste-wordhtml`)模式，以便您明確定義從其他程式（例如Microsoft Word）在AEM中貼上時允許使用哪些樣式。
...
在標準AEM安裝中，當`misctools`外掛程式針對特殊字元(`specialchars`)啟用時，就會立即提供預設選擇供使用；例如，著作權和商標符號。
