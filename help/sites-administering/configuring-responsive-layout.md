...
>
>設定互動式模擬器 {#configuring-the-responsive-emulator}](/help/sites-developing/we-retail.md)[](#enable-the-layout-container-component-for-page)
...
指定裝置群組 {#specify-the-device-groups}](/help/sites-developing/responsive.md#registering-page-components-for-simulation)

### 要指定出現在模擬器「設備」清單中的設備組，請參 [閱指定設備組](/help/sites-developing/responsive.md#specifying-the-device-groups)。

將您的網站連結至指定的裝置群組 {#link-your-site-to-the-specified-device-groups}](/help/sites-developing/responsive.md#specifying-the-device-groups)

### 若要包含模擬器，您必須將網站連結至裝置群組。 請參 [閱新增裝置清單](/help/sites-developing/responsive.md#adding-the-devices-list) （適用於傳統和觸控最佳化的UI）。

啟用網站的版面模式 {#activate-layout-mode-for-your-site}](/help/sites-developing/responsive.md#adding-the-devices-list)
...
[用於互動式設計。](/help/sites-authoring/responsive-layout.md#selecting-a-device-to-emulate)
...
#### 使用CRXDE Lite（或相當等級），導覽至下列任一項目：{#configuring-breakpoints-using-crxde-lite}
...
1. 名稱: `cq:responsive`
   ...
   * 類型: `nt:unstructured`
   * 在此下，建立節點：`nt:unstructured`

1. 名稱: `breakpoints`

   * 類型: `nt:unstructured`
   * 在中斷點節點下，可以建立任意數量的斷點。 每個定義都是具有以下屬性的單個節點：`nt:unstructured`

1. 名稱: `<descriptive name>`

   * 類型: `nt:unstructured`
   * 標題: `String` * `<descriptive title seen in Emulator>`*
   * 寬度: `Decimal` * `<value of breakpoint>`*
   * 使用XML配置斷點 {#configuring-breakpoints-using-xml}`<value of breakpoint>`

#### 中斷點位於適當 `<jcr:content>` 範本(或內 `.context.html` 容)資料夾下方的區段內。

範例定義：`<jcr:content>``.context.html`
...
>[!NOTE]只有當頁面元件並非以基礎頁面元件為基礎時，才需要此功能。
>
...
### 若要將頁面的主要參數設為版面容器，您必須將參數定義為：{#set-layout-container-as-main-parsys}

`wcm/foundation/components/responsivegrid`

`wcm/foundation/components/responsivegrid`在下列任一項中：
...
#### AEM使用LESS來產生部分必要的CSS，這些必須包含在您的專案中。{#css-for-breakpoints-using-less}

您也需要建立用戶端程 [式庫[#$tu99] ，以提供額外的設定和函式呼叫。 以下LESS提取是您需要添加到項目中的最小值示例：
...
#### 根據自適應格線大小，將調整保持在自適應容器中的元件大小（連同其各自的HTML DOM元素）。 因此，在這些情況下，建議避免（或更新）固定寬度（包含）DOM元素的定義。{#styling-considerations}
...
#### 在網格中調整元件大小時，將根據需要觸發以下偵聽程式：{#resizing-and-adaptive-image-compliance}
...
例如：`afterEdit``REFRESH_PAGE``EditConfig`

`<cq:listeners jcr:primaryType="cq:EditListenersConfig" afteredit="REFRESH_PAGE" />`

`<cq:listeners jcr:primaryType="cq:EditListenersConfig" afteredit="REFRESH_PAGE" />`自適應影像機制通過控制窗口當前大小的正確影像選擇的指令碼來提供。 在DOM就緒或接收專用事件時啟動。 目前，頁面必須重新整理，才能正確反映使用者動作的結果。

[!CAUTION]

>[!CAUTION]自訂樣式表clientlibs必須載入為標題的一部分，才能在作者和發佈上正常運作。
>
...
### 若要允許作者在內容頁面中新增更多回應式格點，您需要為頁面啟用「版面容器」元件。 您可以使用下列其中一種方式：{#enable-the-layout-container-component-for-page-editing}
...
### 您可以設定每個特定版面容器例項可用的欄數：{#configure-the-grid-of-the-layout-container}

**作者環境**

1. **您可以設定每個特定版面容器例項可用的欄數。**
...
1. **回應式格線的定義在：**
   ...
   `etc/design/<*your-project-name*>/.content.xml`可定義下列參數：

   可用欄數：

   * `columns="{String}8"`

      * `columns="{String}8"`可添加到當前元件的元件：
