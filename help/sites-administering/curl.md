...
使用AEM as a Cloud Service時，某些情況下您可能需要轉出單一特定頁面，而不傳播其子頁面。 如果未正確設定，轉出頁面的典型curl命令可能會無意中包含子頁面。 本節說明如何調整curl指令，以實現指定頁面的淺層轉出，並排除任何其他子頁面。

若要執行淺層轉出，請執行下列步驟：

1. 將引數從`type=deep`變更為`type=page`，以修改現有的curl命令。
1. 對curl命令使用下列語法：
