...
有關如何使用控制台控制項的資訊，請參見[使用JMX控制台](#using-the-jmx-console)。 有關JMX的背景資訊，請參見Oracle網站上的[Java管理擴展(JMX)技術](https://www.oracle.com/technetwork/java/javase/tech/javamanagement-140525.html)頁。
...
>[!NOTE]
>
>有關其他工作流管理工具和可能的工作流實例狀態的說明，請參閱[工作流控制台](/help/sites-administering/workflows-administering.md)。
...
* 引數:
   ...
   * 重新啟動實例：（可選）指定`true`值，以在實例終止後重新啟動。 預設值`false`導致終止的工作流實例不會重新啟動。
   * 乾跑：（可選）指定`true`值，以查看操作結果，而不實際執行操作。 預設值`false`將導致執行操作。
...
* 引數:
   ...
   * `target`:（可選）一 `String` 個值，它表示要存檔儲存庫資料的ZIP檔案或目錄的名稱。若要使用ZIP檔案，請加入ZIP檔案副檔名。 要使用目錄，請不包括檔案副檔名。
      ...
      當您未指定值時，會使用預設值`backup-currentdate.zip`，其中`currentdate`的格式為`yyyyMMdd-HHmm`。
...
`org.apache.jackrabbit.api.stats.RepositoryStatistics`定義的每個統計類型的「時間系列」欄位的值。
...
## 使用外部JMX應用程式監視{#using-external-jmx-applications-for-monitoring}

CRX允許外部應用程式通過[Java管理擴展(JMX)](https://docs.oracle.com/javase/6/docs/technotes/guides/management/overview.html)與受管Bean(MBeans)交互。 使用通用控制台（如[JConsole](https://java.sun.com/developer/technicalArticles/J2SE/jconsole.html)或特定於域的監視應用程式），可以獲取和設定CRX配置和屬性，以及對效能和資源使用情況的監視。
...
有關設定密碼檔案的詳細說明，請參閱[相關的JMX文檔](https://docs.oracle.com/javase/6/docs/technotes/guides/management/agent.html)。
