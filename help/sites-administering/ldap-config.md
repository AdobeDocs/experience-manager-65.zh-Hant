...
可在LDAP伺服器和儲存庫之間同步用戶帳戶，並將LDAP帳戶詳細資訊保存在儲存庫中。 此功能可讓帳戶指派給存放庫群組，以分配所需的權限。

儲存庫使用LDAP驗證來驗證此類用戶，並將憑據傳遞到LDAP伺服器進行驗證，這是允許訪問儲存庫之前所需的。 為了改善效能，存放庫可快取成功驗證的憑證，並設定到期逾時，以確保在適當的期間後會進行重新驗證。

從LDAP伺服器中刪除帳戶後，將不再授予驗證，並拒絕對儲存庫的訪問。 也可清除儲存庫中儲存的LDAP帳戶的詳細資訊。

此類帳戶的使用對您的使用者是透明的。 也就是說，它們在從LDAP建立的用戶和組帳戶和僅在儲存庫中建立的帳戶之間沒有區別。
...
>[!NOTE]
>
>Watch [Oak的外部登入模組 — 使用LDAP及更新進行驗證](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-oak-external-login-module-authenticating-with-ldap-and-beyond.html?lang=en) 深入探索外部登入模組。
>
>若要閱讀使用Apache DS配置Experience Manager的範例，請參閱 [將Adobe Experience Manager 6.5設定為使用Apache Directory Service。](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/configuring-adobe-experience-manager-6-to-use-apache-directory/m-p/183805)
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>用於驗證的用戶的DN。 如果此欄位留空，則會執行匿名綁定。</td>
  </tr>
  <tr>
...
>[!NOTE]
>
>Apache Jackrabbit Oak外部登入模組會實作Java™驗證和授權服務(JAAS)規格。 請參閱 [官方OracleJava™安全參考指南](https://docs.oracle.com/javase/8/docs/technotes/guides/security/jaas/JAASRefGuide.html) 以取得更多資訊。

其工作是定義要使用的身份提供程式和同步處理程式，有效綁定這兩個模組。

可使用下列設定選項：

| **JAAS排名** | 指定此登錄模組條目的排名（即排序順序）。 條目會以遞減順序排序（亦即，值排名較高的組態會排在首位）。 |
|---|---|
| **JAAS控制標幟** | 指定LOGINModule是必需、必要、充分還是可選的屬性。 如需這些標幟含義的詳細資訊，請參閱JAAS設定檔案。 |
| **JAAS王國** | 註冊LoginModule的領域名稱（或應用程式名稱）。 如果未提供領域名稱，則LoginModule將註冊為Felix JAAS配置中配置的預設領域。 |
| **身份提供程式名稱** | 身份提供程式的名稱。 |
| **同步處理程式名稱** | 同步處理程式的名稱。 |

>[!NOTE]
如果您打算使用AEM執行個體建立多個LDAP設定，則必須為每個設定建立單獨的身份提供程式和同步處理程式。
...
1. 視需要在您的Java™ VM中安裝SSL憑證。 此安裝可使用鍵工具完成：
...
在設定AEM以透過SSL透過LDAP驗證時，可使用自簽名憑證。 以下是產生要與AEM搭配使用之憑證的工作程式範例。

1. 請確定您已安裝SSL程式庫且運作正常。 此程式以OpenSSL為例。

1. 建立自訂的OpenSSL設定(cnf)檔案。 複製預設的**openssl.cnf **配置檔案並自訂，即可完成此配置。 在UNIX®系統上，它位於 `/usr/lib/ssl/openssl.cnf`
...
可以為LDAP身份提供程式和外部登錄模組啟用調試日誌記錄，以解決連接問題。
...
透過LDAP同步的使用者可以是AEM中不同群組的一部分。 這些組可以是作為同步過程的一部分添加到AEM的外部LDAP組。 但是，它們也可以是單獨添加的組，而且不是原始LDAP組隸屬方案的一部分。

這些群組通常由本機AEM管理員或任何其他身分提供者新增。

如果從LDAP伺服器上的組中刪除了用戶，則同步時AEM端會反映該更改。 但是，未由LDAP添加的用戶的所有其他組從屬關係都保持不變。

AEM會使用 `rep:externalId` 屬性。 此屬性將自動添加到由同步處理程式同步的任何用戶或組中，並包含有關原始身份提供程式的資訊。

請參閱以下位置的Apache Oak檔案： [用戶和組同步](https://jackrabbit.apache.org/oak/docs/security/authentication/usersync.html).

## 已知問題 {#known-issues}

如果您打算使用LDAP over SSL，請確保您使用的證書是在沒有Netscape注釋選項的情況下建立的。 如果啟用此選項，驗證會因SSL握手錯誤而失敗。
