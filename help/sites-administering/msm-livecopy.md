...
1. Click or tap **Create**. 將會顯示確認訊息，您可從此處選擇「開 **啟** 」或 **「完成**」。
...
1. 指定轉出工作應立即(**Now**)或在其他日期／時間(**Later**)執行。

   ![推出藍圖](assets/rollout-blueprint.png)

退出將作為非同步作業進行處理，並可以在 [**Global Navigation **-](asynchronous-jobs.md#monitor-the-status-of-asynchronous-operations)> Async Jobs Status** Dashboard中的「非同步作業狀態」控制面板中檢查，該控制面板位於Global Navigation **-**********> Operations -> Jobs **
...
1. 系統會要求您確認推出的詳細資訊：
   ...
   * **計劃**:
      ...
      指定轉出工作應立即(**Now**)或稍後日期／時間(**Later**)執行。
...
>[!NOTE]
>
>非同步轉出處理需要AEM 6.5.3.0或更新版本。 在舊版中，頁面會立即同步處理，除非已勾選「背 **景轉出** 」選項。
...
1. 從工 **具欄中** ，選擇「繼續」。
1. 指出在回復繼承後是否要同步頁面，然後在「繼續即時副本」( **Resume Live** Copy **)對話框中確認「繼** 續」(Resume)操作。
...
要為元件啟用繼承，請按一下或點選元件工 **具欄上的「重新啟用繼承** 」(Re-enable Inheritance)表徵圖。
...
1. 使用頁面工具列上的「 **Sites** Console **」（網站控制台）或「Page Information** 」（頁面資訊）圖示的「View Properties **** 」（檢視屬性）選項，開啟即時復本頁面的屬性。
...
要啟用屬性的繼承，請按一下或點選屬性旁 **邊顯示的「恢復繼承** 」表徵圖。
