...
使用您的Blueprint設定時，您可以將其與轉出設定相關聯，以決定來源/Blueprint的即時副本同步的方式。 另請參閱 [指定要使用的轉出設定](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use).
...
* 如果您未指定任何轉出設定，MSM會決定要使用哪個轉出設定。 另請參閱 [指定要使用的轉出設定](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use).
...
唯一的差異是 **轉出** 「來源/Blueprint」頁面上的命令取決於Blueprint設定是否參考了來源：
...
1. 按一下或點選 **建立**. 將會顯示確認訊息，您可以從這裡選取 **開啟** 或 **完成**.
...
1. 在 **網站** 主控台，選取 **建立**，則 **網站** 從下拉式選擇器中。
...
## 監控您的即時副本 {#monitoring-your-live-copy}
...
Blueprint頁面（在Blueprint設定中參照）會為您提供使用目前(Blueprint)頁面作為來源的即時副本頁面清單。 使用此清單來追蹤即時副本。 清單會顯示在 **Blueprint** 的標籤 [頁面屬性](/help/sites-authoring/editing-page-properties.md).
...
1. 在 **網站** 控制檯中，選取Blueprint中的頁面並開啟屬性。
1. 開啟 **Blueprint** 標籤。
1. 選取 **轉出**.

   ![選取轉出](assets/chlimage_1-220.png)

1. 指定頁面和任何子頁面，然後使用核取記號確認：
...
1. 在 **網站** 控制檯中，選取即時副本中的頁面並開啟 **[引用](/help/sites-authoring/basic-handling.md#references)** 面板（從工具列）。
...
1. 系統會要求您確認轉出的詳細資訊：
   ...
   * **轉出範圍**:
     ...
     指定範圍是只用於選取的頁面，還是應該包含子頁面。

   * **計劃**:

     指定轉出工作是否應該立即執行(**現在**)或在以後的日期/時間(**稍後**)。
...
>[!NOTE]
>
>同步會執行使用下列專案的轉出設定： [轉出時](/help/sites-administering/msm-sync.md#rollout-triggers) 觸發器。
...
此 [即時副本概觀中也提供同步動作](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)，在選取即時副本頁面時。

1. 開啟 [即時副本概觀](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview) 並選取即時副本頁面。
1. 選取 **同步** （從工具列）。
1. 確認 **轉出** 指定是否要包含下列專案後，對話方塊中的動作：
...
>[!NOTE]
>
>在本機對標示為容器的元件進行的變更將不會被轉出時的Blueprint內容覆寫。 另請參閱 [MSM最佳作法](/help/sites-administering/msm-best-practices.md#components-and-container-synchronization) 以取得詳細資訊。
...
#### 暫停來自即時副本概述的繼承 {#suspending-inheritance-from-the-live-copy-overview}

此 [即時副本概觀也提供暫停動作](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)，在選取即時副本頁面時。
...
1. 從下列專案選取適當的選項：
   ...
   * **暫停子項**
   ...
   ![選取適當的「暫停」選項](assets/chlimage_1-226.png)
...
暫停頁面的即時副本繼承是暫時動作。 暫停後， **繼續** 動作將變為可用，可讓您恢復即時關係。

當您重新啟用繼承時，頁面不會自動與來源同步。 如果需要，您可以要求同步化，方法是：
...
一次 [已暫停](#suspending-inheritance-from-page-properties) 此 **繼續** 動作會變成位於頁面屬性工具列中：

![繼續](assets/chlimage_1-229.png)

選取後，將會顯示對話方塊。 您可以視需要選取同步，然後確認動作。
...
* 切換到淺層即時副本：
   ...
   * 將立即生效且不可還原。
      ...
      * 子頁面會明確從即時副本中分離。 如果還原，則無法保留對子項所做的進一步修改。
...
>[!NOTE]
>
>當您重新啟用繼承時，元件不會自動與來源同步。 如果需要，您可以手動要求同步。
...
1. 在「取消繼承」對話方塊中，確認動作 **是**.
   ...
   元件工具列會更新以包含所有（適當的）編輯指令。

### 重新啟用元件的繼承 {#re-enabling-inheritance-for-a-component}

若要啟用元件的繼承，請按一下或點選 **重新啟用繼承** 圖示加以檢視。
...
* 轉出時，元件順序將從Blueprint恢復。 如果在轉出前已將新元件新增到live copy，則這些新元件將會與其上方新增的元件一起重新排序。
* 如果取消段落系統的繼承，轉出時元件的順序將不會恢復，並將保持與即時副本中的相同。

>[!NOTE]
>
>在段落系統上恢復取消的繼承時，元件的順序 **將不會自動還原** 從Blueprint。 如果需要，您可以手動要求同步。
...
* **在即時副本的子頁面上分離**
  ...
  *但是*，子頁面仍受父分支的即時關係約束，因此藍圖頁面的進一步轉出將：
  ...
  >[!NOTE]
  >
  >另請參閱 [MSM轉出衝突](/help/sites-administering/msm-rollout-conflicts.md) 以取得這類情況的詳細資訊。
