...
seo-description: 了解如何處理多網站管理員轉出衝突。
...
在以下幾節中，我們使用在Blueprint和Live Copy分支中（手動建立）建立的新頁面`b`的範例來說明各種衝突解決方法：
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><code> /lc-level-1</code> <br /> （包含在Live Copy分支中手動建立的page<br /> child-level-1的內容）</td>
  </tr>
 </tbody>
</table>
...
這是使用&#x200B;**Day CQ WCM轉出管理器**&#x200B;的[OSGi設定](/help/sites-deploying/configuring-osgi.md)來完成的：
...
當衝突管理已停用](#behavior-when-conflict-handling-deactivated)時，AEM具有[預先定義的行為。
...
* 實施[自定義處理程式](#customized-handlers)的可能性。
...
* 此處理常式的服務排名設為低(即在`service.ranking`屬性的預設值下)，因為假設自訂處理常式需要較高的排名。 然而，排名並非確保必要時靈活性的絕對最小值。

此衝突處理程式優先於Blueprint。 即時副本頁面`/b`會移動（在即時副本分支內）至`/b_msm_moved`。
...
* 可設計為使用[OSGi配置](/help/sites-deploying/configuring-osgi.md)進行配置；尤其是：
...
### 已停用衝突處理{#behavior-when-conflict-handling-deactivated}時的行為

如果您手動[停用衝突處理](#rollout-manager-and-conflict-handling)，則AEM不會在任何衝突頁面上採取任何動作（非衝突頁面會如預期般推出）。

>[!CAUTION]
>
>AEM不會指出衝突正在遭到忽略，因為此行為必須明確設定，因此會假設這是必要行為。

在這種情況下，有效的即時副本優先。 Blueprint頁面`/b`不會複製，且即時副本頁面`/b`不會受到影響。
