...
* 有了MSM，您可以：
   ...
   * 只要建立內容一次，
   * 將此內容複製到相同或其他網站的其他區域（[即時副本](#live-copies)），並重新使用此內容。
...
* **跨國公司——全球對本地公司**
   ...
   >
   >
   >如果您想要擴充此範例，請參閱[多語言網站的翻譯內容](/help/sites-administering/translation.md)。
...
* **建立網站** (**網站**)
   ...
   * MSM可協助您管理多個共用內容的網站；例如，網站通常提供給國際觀眾，讓大部分內容在所有國家／地區都很常見，而且每個國家／地區都有特定內容的子集。 MSM允許您[建立即時拷貝，根據源站點](/help/sites-administering/msm-livecopy.md#creating-a-live-copy-of-a-site-from-a-blueprint-configuration)自動更新一個或多個站點。 這也可協助您強制建立共同的基本結構、跨多個網站使用共同內容、維持共同的外觀和感覺，並集中精力管理實際不同網站的內容。
...
* **參考** (**網站**)
   ...
   * [參考](/help/sites-authoring/basic-handling.md#references)邊欄提供有關&#x200B;**即時副本**&#x200B;的資訊，並可存取適當的動作。
...
* **Blueprint** (**工具** -站 **點**)
   ...
   * 此控制台允許您[建立和管理Blueprint配置](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration)。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>對給定資源的繼承進行有效定義；源副本和活動副本之間的連接。<br /> </td>
  ...
  <tr>
   ...
   <td>用於同步源和即時拷貝之間內容的通用術語（由<strong>Rolovate</strong>和<strong>Synchronize</strong>兩者共同使用）。</td>
   <td> </td>
  </tr>
  <tr>
...
>[!NOTE]
>
>有關對象名稱，請參見[Java API](/help/sites-developing/extending-msm.md#overview-of-the-java-api)的概述。
...
在上例中，`/content/we-retail/language-masters/en`是全域英文主體網站。 若要重複使用此網站的內容，請建立MSM即時副本：

* `/content/we-retail/language-masters/en`下方的內容是來源。

* `/content/we-retail/language-masters/en`下方的內容複製在`/content/we-retail/us/en/`、`/content/we-retail/gb/en`、`/content/we-retail/ca/en`和`/content/we-retail/au/en`節點下方。 這些是即時復本。

* 作者對`/content/we-retail/language-masters/en`下的頁面進行變更。
...
* 根據需求，更改可以是[synchronized](/help/sites-administering/msm-livecopy.md#synchronizing-your-live-copy)。
...
* MSM可以處理特殊情況。 例如，當您（或流程）在來源／藍圖和即時副本分支中建立位置和名稱相同的頁面時。 如需詳細資訊，請參閱[MSM轉出衝突](/help/sites-administering/msm-rollout-conflicts.md)。

![chlimage_1-368](assets/chlimage_1-368.png)

#### 巢狀即時副本{#nested-live-copies}

當您（或流程）在現有即時副本中建立[新頁面時，此新頁面也可設為不同藍圖的即時副本。 ](#live-copy-with-non-live-copy-pages)這稱為巢狀即時副本，其中第二個（內部）即時副本的行為受到第一個（外部）即時副本的影響，方式如下：
...
當即時副本作為淺層即時副本的子項建立時，即稱為堆疊即時副本。 其運作方式與[巢狀即時副本](#nested-live-copies)相同。
...
* 可以定義[轉出配置](#rollout-configurations)，使得特定[事件](/help/sites-administering/msm-sync.md#rollout-triggers)可自動轉出。
* 在製作藍圖頁面時，您可以使用[Rovolt](/help/sites-administering/msm-livecopy.md#rolling-out-a-blueprint)命令將變更推送至即時副本。

   **Rollout命** 令可用於藍圖配置所引用的Blueprint頁面。

   ![chlimage_1-370](assets/chlimage_1-370.png)

* 製作即時副本頁面時，您可以使用[Synchronize](/help/sites-administering/msm-livecopy.md#synchronizing-a-live-copy)命令將變更從來源提取到即時副本。
...
轉出設定可重新使用，因此多個即時副本可以使用相同的轉出設定。 標準安裝中包含幾個[轉出配置](/help/sites-administering/msm-sync.md#installed-rollout-configurations)。

### 轉出衝突{#rollout-conflicts}

推播可能會變得複雜，尤其是當作者同時在來源和即時副本中編輯內容時，因此請務必注意AEM如何處理在轉播期間可能發生的任何[衝突。](/help/sites-administering/msm-rollout-conflicts.md)
...
您也可以[將即時副本](/help/sites-administering/msm-livecopy.md#detaching-a-live-copy)從其藍圖中分離，以刪除所有連接。
...
>[!NOTE]
>
>如需詳細資訊，請參閱[分離即時副本](/help/sites-administering/msm-livecopy.md#detaching-a-live-copy);包括對子頁面和父頁面的相關影響。
...
1. 確定要使用的轉出配置。
   ...
   1. MSM [安裝了幾個可滿足多種使用情況的轉出配置](/help/sites-administering/msm-sync.md#installed-rollout-configurations)。
   1. 您也可以視需要建立轉出設定[。](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration)

1. 確定您需要在哪裡指定[的轉出配置以使用](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use)並視需要進行配置。
1. 如果需要，[會建立藍圖設定](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration)，以識別即時副本的來源內容。
1. [建立即時副本](/help/sites-administering/msm-livecopy.md#creating-a-live-copy)。
1. 視需要變更來源內容。 您應採用貴組織已建立的一般內容審閱與核准程式。
1. [將滑](/help/sites-administering/msm-livecopy.md#rolling-out-a-blueprint) 鼠指向藍圖，或 [將即時組](/help/sites-administering/msm-livecopy.md#synchronizing-a-live-copy) 排文字與變更同步。
...
* **自訂轉出設定**
   [當安裝的轉](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration) 出配置不符合您的要求時，請建立轉出配置。您可以使用任何可用的轉出觸發器和同步操作。

* **自定義同步操作**
   [當安裝的操作不](/help/sites-developing/extending-msm.md#creating-a-new-synchronization-action) 符合您的特定應用程式要求時，建立自定義同步操作。MSM提供Java API，用於建立自定義同步操作。
