...
seo-title: 「重複使用內容：多站點管理員和即時副本」
description: 了解如何使用Live Copy和Multi Site Manager來重複使用內容。
seo-description: 了解如何使用Live Copy和Multi Site Manager來重複使用內容。
...
# 重複使用內容：多站點管理器和即時副本{#reusing-content-multi-site-manager-and-live-copy}
...
* 透過MSM，您可以：
   ...
   * 先建立內容一次
   * 將此內容複製到相同或其他網站的其他區域([Live Copy](#live-copies))中，並重新使用此內容。
...
* **跨國公司 — 全球到本地公司**
   ...
   >
   >
   >如果您想要延伸此範例，請參閱[轉譯多語言網站的內容](/help/sites-administering/translation.md)。
...
## 從UI {#msm-from-the-ui}進行MSM

您可透過適當控制台的各種選項，直接在UI中存取MSM。 要介紹以下內容，請列出主要位置：

* **建立網站** (**網站**)

   * MSM可協助您管理多個共用相同內容的網站；例如，網站通常為國際受眾提供，因此大部分內容在所有國家/地區都是共有的，而且每個國家/地區都有特定內容的子集。 MSM可讓您[建立即時副本，以根據您的來源網站](/help/sites-administering/msm-livecopy.md#creating-a-live-copy-of-a-site-from-a-blueprint-configuration)自動更新一或多個網站。 這也可協助您強制建立通用的基礎結構、跨多個網站使用通用內容、維持通用的外觀和風格，並著重於管理實際上不同網站的內容。
...
* **建立即時副本** (**網站**)
   ...
   * MSM可讓您[建立個別頁面或網站](/help/sites-administering/msm-livecopy.md#creating-a-live-copy-of-a-page)子分支的隨選（一次性）即時副本；例如，複製子分支以提供關於產品的新版本/更新版本的資訊。
...
* **檢視屬性** (**網站**)
   ...
   * 如果適用，此選項可通過提供相關的&#x200B;**Live Cop** y或&#x200B;**Blueprint**&#x200B;的資訊，幫助您[監視即時副本](/help/sites-administering/msm-livecopy.md#monitoring-your-live-copy)。

* **參考** (**網站**)

   * [參考](/help/sites-authoring/basic-handling.md#references)邊欄提供關於&#x200B;**即時副本**&#x200B;的資訊，以及對適當動作的存取。

* **Live Copy概述** (**Sites**)

   * 此主控台可讓您[檢視及管理您的Blueprint及其Live Copy](/help/sites-administering/msm-livecopy-overview.md)。

* **Blueprint** (**工具**  -  **站點**)

   * 此控制台可讓您[建立和管理Blueprint配置](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration)。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>對給定資源的繼承的有效定義；源和即時副本之間的連接。<br /> </td>
  ...
  <tr>
   ...
   <td>用於同步源副本和即時副本之間內容的通用術語（由<strong>Rollout</strong>和<strong>Synchronize</strong>兩者使用）。</td>
   <td> </td>
  </tr>
  <tr>
...
>[!NOTE]
>
>如需物件名稱，請參閱[Java API](/help/sites-developing/extending-msm.md#overview-of-the-java-api)概觀。
...
在上一個示例中， `/content/we-retail/language-masters/en`是全局主網站，用英語表示。 若要重複使用此網站的內容，會建立MSM即時復本：

* `/content/we-retail/language-masters/en`下方的內容為來源。

* `/content/we-retail/language-masters/en`下的內容複製在`/content/we-retail/us/en/`、`/content/we-retail/gb/en`、`/content/we-retail/ca/en`和`/content/we-retail/au/en`節點下。 這些是即時副本。

* 作者對`/content/we-retail/language-masters/en`下方的頁面進行變更。
...
* 可以根據需求[已同步](/help/sites-administering/msm-livecopy.md#synchronizing-your-live-copy)更改。
...
* MSM可能會以特殊情況處理的情況。 例如，當您（或程式）在來源/藍圖和即時副本分支中建立位置和名稱相同的頁面時。 如需詳細資訊，請參閱[MSM轉出衝突](/help/sites-administering/msm-rollout-conflicts.md) 。

![chlimage_1-368](assets/chlimage_1-368.png)

#### 巢狀即時副本{#nested-live-copies}

當您（或程式）在現有即時副本中建立[新頁面時](#live-copy-with-non-live-copy-pages)這個新頁面也可以設定為不同藍圖的即時副本。 這稱為巢狀即時副本，其中第二個（內部）即時副本的行為會以下列方式受第一個（外部）即時副本影響：
...
即時副本建立為淺層即時副本的子項時，即稱為堆疊即時副本。 其行為方式與[巢狀即時副本](#nested-live-copies)相同。

### 源、藍圖和Blueprint配置{#source-blueprints-and-blueprint-configurations}
...
* 可定義[轉出設定](#rollout-configurations)，使得特定[事件](/help/sites-administering/msm-sync.md#rollout-triggers)可導致自動轉出。
* 編寫Blueprint頁面時，您可以使用[ Rollout](/help/sites-administering/msm-livecopy.md#rolling-out-a-blueprint)命令將變更推送至即時副本。

   **Rollout** 命令可在Blueprint配置引用的Blueprint頁面上使用。

   ![chlimage_1-370](assets/chlimage_1-370.png)

* 在編寫即時副本頁時，可以使用[ Synchronize](/help/sites-administering/msm-livecopy.md#synchronizing-a-live-copy)命令將更改從源提取到即時副本。

   Live Copy頁面上始終提供&#x200B;**Synchronize**&#x200B;命令（無論源/Blueprint頁面是否被Blueprint配置包圍）。
...
* **同步操作**
   ...
   會在即時副本上執行，以便與來源同步。 範例動作包括複製內容、排序子節點及啟動即時副本頁面。 MSM提供一些同步動作。

   >[!NOTE]
   >
   >您可以使用Java API建立執行個體的自訂動作。

轉出設定可重新使用，因此多個即時副本可以使用相同的轉出設定。 標準安裝中包含數個[轉出設定](/help/sites-administering/msm-sync.md#installed-rollout-configurations)。

### 轉出衝突{#rollout-conflicts}

轉出可能會變得複雜，尤其是當作者同時編輯來源和即時副本中的內容時，請務必注意AEM如何處理轉出](/help/sites-administering/msm-rollout-conflicts.md)期間可能發生的任何[衝突。
...
編輯個別頁面時，作者可以&#x200B;**取消元件的繼承**。 取消繼承時，即時關係會暫停，且該元件不會同步。 當需要自訂內容的子區段時，取消繼承和同步很實用。

### 分離即時副本{#detaching-a-live-copy}

您也可以從Blueprint中[分離即時副本](/help/sites-administering/msm-livecopy.md#detaching-a-live-copy)以移除所有連線。
...
>[!NOTE]
>
>如需完整詳細資訊，請參閱[分離即時副本](/help/sites-administering/msm-livecopy.md#detaching-a-live-copy);包括對子頁面和上層頁面的相關影響。
...
1. 決定要使用的轉出設定。
   ...
   1. MSM [安裝數個可滿足許多使用案例的轉出設定](/help/sites-administering/msm-sync.md#installed-rollout-configurations)。
   1. 您可以視需要選擇建立轉出設定](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration)。[

1. 決定您需要在哪個位置[指定要使用](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use)的轉出設定，並視需要進行設定。
1. 如果需要，請[建立Blueprint配置](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration)以標識即時副本的源內容。
1. [建立即時副本](/help/sites-administering/msm-livecopy.md#creating-a-live-copy)。
1. 視需要變更來源內容。 您應採用貴組織已建立的一般內容審核和核准程式。
1. [滾](/help/sites-administering/msm-livecopy.md#rolling-out-a-blueprint) 出Blueprint，或將 [即時副本](/help/sites-administering/msm-livecopy.md#synchronizing-a-live-copy) 與變更同步。
...
* **自訂轉出設定**
   [當安裝的轉](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration) 出設定不符合您的需求時，建立轉出設定。您可以使用任何可用的轉出觸發器和同步動作。

* **自訂同步動作**
   [當安裝的操作不](/help/sites-developing/extending-msm.md#creating-a-new-synchronization-action) 符合您的特定應用程式要求時，建立自定義同步操作。MSM提供用於建立自訂同步動作的Java API。

## 最佳作法 {#best-practices}

[MSM最佳實務](/help/sites-administering/msm-best-practices.md)頁面包含與您的實作相關的重要資訊。
