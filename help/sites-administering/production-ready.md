...
Adobe推出AEM 6.1的新功能 `"nosamplecontent"` 執行模式旨在自動化準備AEM執行個體以在生產環境中部署所需的步驟。
...
若要在生產就緒模式下執行AEM，您只需新增 `nosamplecontent` 透過 `-r` 執行模式切換至您現有的啟動引數：
...
更具體來說，當AEM在生產就緒模式下執行時，會執行下列設定變更：
...
1. **產生偵錯資訊** 已為停用 **Apache Sling JavaScript處理常式**.
