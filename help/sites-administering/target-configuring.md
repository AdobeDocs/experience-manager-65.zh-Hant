...
的 [選擇加入嚮導](/help/sites-administering/opt-in.md) 那 [與AEMAdobe Target](/help/sites-administering/target.md) 自動建立名為「已預配的目標配置」的目標雲配置。 該嚮導還為名為「已預配的目標框架」的雲配置建立目標框架。 如有必要，您可以修改雲配置和框架的屬性。
...
* **自定義AT.js:** 未指定以使用預設AT.js檔案。 根據需要指定要使用的自定義AT.js檔案。 僅當選擇了AT.js時才顯示。

>[!NOTE]
>
>在AEM6.3中，可以選擇目標庫檔案， [AT.JS](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/mboxcreate-atjs/)它是Adobe Target的一個新的實現庫，它既適用於典型的web實現，也適用於單頁應用程式。
>
...
為此，您指定用以下方式連接Adobe Target雲配置的A4T雲配置：
...
>[!NOTE]
目標庫檔案， [AT.JS](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/mboxcreate-atjs/)，是為Adobe Target設計的一個新的實現庫，它既適用於典型的web實施，也適用於單頁應用程式。 Adobe建議將AT.js而不是mbox.js用作客戶端庫。
AT.js對mbox.js庫提供了幾項改進：
* 用於Web實現的改進的頁載入時間
* 提高安全性
* 針對單頁應用程式的更好實施選項
...
您可以配置雲配置以同步來自Adobe Target的段。 如果啟用同步，則在保存雲配置時，會從後台的Target導入段。

請按下列步驟在中建立目標雲配AEM置：

1. 導航到 **Cloud Services** 通過 **AEM徽標** > **工具** > **Cloud Services** > **舊Cloud Services**。 ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))

   的 **Cloud Services** 的子菜單。
...
1. 在對話框中，提供這些屬性的值。
   ...
   * **密碼**:目標帳戶密碼。
   * **API類型**:REST或XML
   * **A4TAnalytics Cloud配置**:選擇用於目標活動目標和度量的Analytics Cloud配置。 如果在目標內容時使用Adobe Analytics作為報告源，則需要此配置。 如果看不到雲配置，請參閱中的注釋 [配置A4TAnalytics Cloud配置](#configuring-a-t-analytics-cloud-configuration)。

   * **使用準確的目標：** 預設情況下，此複選框處於選中狀態。 如果選中，則雲服務配置將在載入內容之前等待上下文載入。 請參閱下面的注釋。
   * **同步來自Adobe Target的段：** 選擇此選項，以便您可以下載在「目標」中定義的段以在中使AEM用。 當API Type屬性為REST時，選擇此選項，因為不支援內聯段，並且必須使用「目標」中的段。 (「AEM段」的術語等同於「目標受眾」。)
   * **客戶端庫：** 選擇是希望使用mbox.js還是AT.js客戶端庫。
   * **使用DTM提供客戶端庫**  — 選擇此選項可使用AT.js或DTM中的mbox.js或其他標籤管理系統。 配置 [DTM整合](/help/sites-administering/dtm.md) 選項。 Adobe建議您使用DTM而AEM不是交付庫。
   ...
   * **自定義AT.js**:如果選中DTM框或使用預設AT.js，則保留為空。 或者上載自定義AT.js。 僅當選擇了AT.js時才顯示。
   ...
   如果 ***已*** 建立了目標元件，並且您更改了此設定，您所做的更改不會影響這些元件。 直接更改這些元件。
...
可以為單個目標配置建立多個框架。 當必須將一組不同的參數發送到網站不同部分的「目標」時，多個框架非常有用。 為您發送的每組參數建立一個框架。 將網站的每個部分與相應的框架相關聯。 網頁一次只能使用一個框架。
...
1. 拖動表示要用於映射到放置目標的資料的客戶端上下文元件。 或者，拖動&#x200B;**上下文中心儲存** 框架的元件。
   ...
   例如，要使用 **配置檔案資料** 關於您的站點訪問者以控制您的目標市場活動，拖動 **配置檔案資料** 元件。 將顯示可用於映射到「目標」參數的配置檔案資料變數。
...
關聯您的 [AEM活動](/help/sites-authoring/activitylib.md) 目標雲配置，以便您能夠鏡像 [Adobe Target](https://experienceleague.adobe.com/docs/target/using/experiences/offers/manage-content.html)。
...
>
**附加說明：** **xt_only** option是應用於特定目標租戶（客戶端代碼）的設定，只能在Adobe Target直接修改。 您無法在AEM中啟用或停用此選項。
...
1. 點擊/按一下 **完成**。
1. 激活網站的根頁，以便將其複製到發佈伺服器。 (請參閱 [如何發佈頁面](/help/sites-authoring/publishing-pages.md)。)
