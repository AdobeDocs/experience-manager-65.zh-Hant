...
檢查這兩個解決方案(AEM > Adobe Campaign Classic、Adobe Campaign Classic > AEM)是否傳送及接收HTTP呼叫。 這是為了避免防火牆/SSL問題。

* 針對AEM功能，您會看到已從AEM作者介面要求JSON呼叫
   * 這些不應導致HTTP-500錯誤。
   * 如果您看到HTTP-500錯誤，請檢查 `error.log` 以取得相關資訊。
...
檢查HTML來源，並確認您可以從使用者端電腦開啟URL。 如果URL具有 `localhost:4503` 在其中，然後變更AEM編寫執行個體上Day CQ Link Externalizer的設定，以指向可從Adobe Campaign Classic主控台電腦存取的發佈執行個體。
...
* 如果尚未安裝，您可以修補 `/usr/local/neolane/nl6/env.sh` 指令碼並將地區設定變更為已安裝的地區設定。
...
如果AEM執行個體上有過時的Adobe Campaign Classic資訊，就會發生此情況。 您可以透過以下方式解決此問題：
...
如果您在「 」中看到以下內容，請向Adobe Campaign支援團隊提出票證 `error.log` 的AEM。
...
嘗試同步Adobe Campaign Classic傳送中的內容時，AEM會傳回電子報清單。 不過，清單中電子報的URL可能是HTTP位址，而不是HTTPS。 選取清單中的專案之一時，會發生錯誤。 這可能發生在以下設定中。
...
* AEM Dispatcher或反向Proxy需要設定為以標頭傳遞原始通訊協定。
* 此 **Apache Felix Http服務SSL篩選器** 在AEM的OSGi設定中，需要以必要的標頭設定進行設定。
   * `https://<host>:<port>/system/console/configMgr`
   * 另請參閱 [https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter](https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter)

## 無法在頁面屬性中選取自訂範本 {#if-the-custom-template-i-created-cannot-be-selected-in-page-properties}

在AEM for Adobe Campaign Classic中建立郵件範本時，您必須包含屬性 `acMapping` 包含值 `mapRecipient` 在 `jcr:content` 範本的節點，否則您將無法在中選取Adobe Campaign Classic範本 **頁面屬性** 的AEM。 該欄位將顯示為停用。
...
這是一個問題，如果 `acMapping` 屬性設定為以外的值 `recipient.firstName`，則會在Adobe Campaign Manager中建立空白值。

如果發生這種狀況，請從安裝AEM適用的Feature Pack 6576 [封裝共用](/help/sites-administering/package-manager.md#package-share).
