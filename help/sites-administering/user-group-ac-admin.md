...
* [存取許可權](#how-access-rights-are-evaluated)  — 如何定義和評估的概念
...
* 您可以指定使用者帳戶的存取許可權。
  ...
  但是，若要簡化管理Adobe，建議（大多數情況下）您指派存取權給群組帳戶。 為每個個別使用者指派存取許可權會迅速變得非常難以管理（只有一或兩個執行個體存在時，某些系統使用者是例外）。
...
* A **主體** 是一個具有存取許可權的實體。 主要專案包括：
   ...
   * 群組帳戶
     ...
     如果使用者帳戶屬於一或多個群組，則該帳戶也會與每個群組主參與者相關聯。
...
CRX處理要求時，會比較來自主體的存取要求與存放庫節點上的存取控制清單：
...
* 對於指定的主體，指定節點上最多有1個拒絕和1個允許專案。 實作一律會清除多餘的專案，並確保允許和拒絕專案中未列出相同的許可權。
...
<table>
 <tbody>
  ...
  <tr>
   <td><i>保持簡單</i></td>
   <td><p>在設定新安裝時投入一些時間和思考，將能收到良好的回報。</p> <p>套用清晰的結構將簡化持續的維護和管理，確保您目前的同事和/或未來的後續人員都能輕鬆瞭解正在實作的內容。</p> </td>
  </tr>
  <tr>
   <td><i>測試</i></td>
...
按一下GroupID （可用時）會開啟 [群組管理](#group-administration) 代表該群組。

**Impersonator**

透過模擬功能，使用者可以代表其他使用者工作。

這表示使用者帳戶可以指定可以與其帳戶一起操作的其他帳戶（使用者或群組）。 換言之，如果允許使用者B模擬使用者A，則使用者B可以使用使用者A的完整帳戶詳細資訊（包括ID、名稱和存取許可權）來執行動作。

這可讓模擬者帳戶完成工作，就像他們正使用自己所模擬的帳戶一樣；例如，在短期休假或共用過多負載時。

如果帳戶模擬其他帳戶，就很難檢視。 記錄檔不會保留事件上已發生模擬的資訊。 因此，如果使用者B模擬使用者A，則所有事件看起來都像是使用者A個人執行的。
...
1. 瀏覽樹狀結構。
1. 按一下所需的帳戶以開啟以進行編輯。
1. 進行變更，然後按一下該專案的「儲存」（綠色勾號）。
1. 按一下 **關閉** 完成，或 **清單……** 以返回所有使用者帳戶的清單。

### 移除使用者帳戶 {#removing-a-user-account}

1. 使用 **使用者管理** 對話方塊開啟所有帳戶的清單檢視。
1. 瀏覽樹狀結構。
1. 選取所需的帳戶，然後按一下 **移除使用者**；帳戶將立即刪除。
...
1. 按一下所需的帳戶以開啟以進行編輯。
1. 進行變更，然後按一下該專案的「儲存」（綠色勾號）。
1. 按一下 **關閉** 完成，或 **清單……** 以返回所有群組帳戶的清單。

### 移除群組帳戶 {#removing-a-group-account}

1. 使用 **群組管理** 對話方塊開啟所有帳戶的清單檢視。
1. 瀏覽樹狀結構。
1. 選取所需的帳戶，然後按一下 **移除群組**；帳戶將立即刪除。
...
使用 **存取控制** CRXDE Lite的頁簽，您可以定義存取控制原則並指派相關許可權。
...
* **目前路徑**
  ...
  如上述範例所示，在存放庫中選取資源。 將顯示此「目前路徑」的原則。
...
新增存取控制專案時，可以選取下列許可權(請參閱 [安全性API](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/security/Privilege.html) 以取得完整詳細資訊)：
...
1. 從工具列選取 **工具**，然後 **許可權** 以顯示目前登入的許可權。
...
1. 定義您的存取控制原則並按一下 **確定** 以儲存。 您的新原則將：

   * 列在 **本機存取控制原則**
   * 這些變更將會反映在 **有效的存取控制原則**.

CRX將會驗證您的選擇；對於指定的主體，在指定節點上最多1個拒絕和1個允許專案。 實作一律會清除多餘的專案，並確保允許和拒絕專案中未列出相同的許可權。
...
1. 此專案將從以下兩個表格中移除： **本機** 和 **有效的存取控制原則**.

### 測試存取控制原則 {#testing-an-access-control-policy}

1. 從CRXDE Lite工具列選取 **工具**，然後 **測試存取控制……**.
1. 新的對話方塊會在右上方的窗格中開啟。 選取 **路徑** 和/或 **主體** 要測試的物件。
1. 按一下 **測試** 若要檢視選取範圍的結果：
