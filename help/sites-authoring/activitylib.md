...
活動可在目標定位模式中用於 [製作目標內容](/help/sites-authoring/content-targeting-touch.md)，您也可以在此建立活動。 您在「鎖定目標」模式中建立的活動會顯示在「活動」主控台中。
...
>
>**其他附註：** **xt_only** options是套用至特定Target租使用者(clientcode)的設定，只能在Adobe Target中直接修改。 您無法在AEM中啟用或停用此選項。

>[!CAUTION]
>
>您必須保護活動設定節點 **cq：ActivitySettings** ，讓一般使用者無法存取。 活動設定節點應該只能由處理活動同步至Adobe Target的服務存取。
>
>另請參閱 [與Adobe Target整合的必要條件](/help/sites-administering/target-requirements.md#securingtheactivitysettings) 詳細資訊。

## 使用「活動」控制檯建立品牌 {#creating-a-brand-using-the-activities-console}

建立您要管理行銷活動的品牌。

當您使用「活動」主控台建立品牌時，它也會出現在 [選件主控台](/help/sites-authoring/offerlib.md) 您可以在此處建立活動體驗的選件。
...
1. 如果您使用Adobe Target作為目標定位引擎，請選取您要透過此活動測量的內容。 另請參閱 [設定活動和設定目標](/help/sites-authoring/content-targeting-touch.md) 以取得有關可用成功量度的詳細資訊。 您必須至少選取一個目標。
1. 按一下或點選 **儲存**.
...
* 作者執行個體上的活動與具有相同名稱的Target行銷活動對應， `_author` 字尾。
...
AEM與Adobe Target同步活動時，AEM會包含名為的活動屬性 `thirdPartyId`. 此屬性的值根據AEM存放庫中活動的路徑。 Adobe Target中的兩個行銷活動不能有相同的值 `thirdPartyId` 屬性。 因此，如果Adobe Target中的現有行銷活動（屬於不同型別AB、XT）使用的值與以下專案相同，則活動將無法同步 `thirdPartyId`.
...
若要避免同步化問題，請一律對活動使用唯一名稱。 如果活動同步失敗，您可以刪除Adobe Target中相同名稱的促銷活動（若未使用該促銷活動）。

>[!NOTE]
>
>在Adobe Target中建立行銷活動時，會指派名為的屬性 `thirdPartyId t`每個行銷活動的。 當您在Adobe Target中刪除行銷活動時， `thirdPartyId` 不會刪除。 您無法重複使用 `thirdPartyId` 適用於不同型別(AB、XT)的行銷活動，且無法手動移除。 為避免此問題，請為每個行銷活動命名一個唯一的名稱；因此，行銷活動名稱不能在不同行銷活動型別中重複使用。
>
