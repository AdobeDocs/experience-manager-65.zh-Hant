...
1. 如果您使用Adobe Target作為目標定位引擎，請選取您要透過此活動測量的內容。 另請參閱 [設定活動和設定目標](/help/sites-authoring/content-targeting-touch.md) 以取得可用成功量度的詳細資訊。 您必須至少選取一個目標。
1. 按一下或點選 **儲存**.
...
1. 若要發佈，請點選或按一下 **發佈**. 若要取消發佈，請點選或按一下 **取消發佈**. 您的活動已發佈或取消發佈，且狀態在活動主控台中變更（可能需要重新整理）。
...
1. 按一下或點選 **是** 若要確認您想要轉換成功者，請停用目前的體驗，並將其取代為成功體驗的內容。
