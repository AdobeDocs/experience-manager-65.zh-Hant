...
由萬維網聯盟工作組起草的[《網頁內容無障礙准則》(WCAG)2.1](https://www.w3.org/TR/WCAG/)由一套技術獨立准則和成功准則組成，幫助殘疾人能夠訪問和使用網頁內容。[](https://www.w3.org/Consortium/activity#Accessibility_Guidelines_Working_Group)
...
* Adobe解決方案的[協助工具符合性報表](https://www.adobe.com/accessibility/compliance.html)。
* [設定Rich Text Editor，以產生可存取的內容](/help/sites-administering/rte-accessible-content.md)
...
以下部分介紹了WCAG 2.1准則的[層以及A級和A級符合性級別](https://www.w3.org/TR/WCAG/#conformance-to-wcag-2-1)的相關成功標準。](https://www.w3.org/TR/WCAG/#wcag-2-layers-of-guidance)[

>[!NOTE]
>
>在本檔案中，我們使用：
>
>* WCAG 2.1准則的[簡短名稱](https://www.w3.org/TR/WCAG/#wcag-2-layers-of-guidance)。
...
對於靜態圖形，基本要求是為圖形提供等效的文本替代。 這可在&#x200B;**替代文字**&#x200B;欄位中完成；例如，請參閱核心元件&#x200B;**[Image](https://docs.adobe.com/content/help/zh-Hant/experience-manager-core-components/using/components/image.translate.html)**。

>[!NOTE]
>
>有些現成可用的核心元件，例如&#x200B;**[Carousel](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/components/carousel.html)**&#x200B;不提供用於為個別影像新增替代文字說明的&#x200B;**替代文字**&#x200B;欄位，但有&#x200B;**Label**&#x200B;欄位（**[Accessibility](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/components/carousel.html#accessibility-tab)**&#x200B;標籤）對整個元件。
>
>當為您的AEM例項實作這些版本時，您的開發團隊將需要設定這些元件以支援`alt`屬性，讓作者可以將它新增至內容（請參閱[新增支援其他HTML元素和屬性](/help/sites-administering/rte-accessible-content.md#add-support-for-more-html-elements-and-attributes)）。
...
* 說明性像片：這些是人物、物件或地點的影像。 請務必考慮像片在頁面中的角色，並建議一般描述影像內容，因為輔助技術會宣佈元素類型（例如`graphic`或`image`）;它可提高替代文字說明中使用`screenshot`或`illustration`的清晰度，但這取決於內容。 一致性是一個重要因素，因此應針對整個製作團隊做出決策，並套用至使用者體驗。
...
>
>准則[瞭解WCAG 1.2.1](https://www.w3.org/WAI/WCAG21/Understanding/audio-only-and-video-only-prerecorded.html)提供了詳細資訊。
...
#### 如何符合——標題（預先錄制）(1.2.2){#how-to-meet-captions-prerecorded}
...
對於隱藏字幕，您需要在視訊檔案旁建立並提供適當格式（例如[SMIL](https://www.w3.org/AudioVideo/)）的同步化字幕檔案(如如何做到的詳細資訊不在本指南的範圍之內，但我們提供了[更多資訊——標題（預錄）(1.2.2)](#more-information-captions-prerecorded)下的教學課程連結。 請確定您提供附註或啟用視訊播放器中的標題功能，讓使用者知道標題可用於視訊。

如果您必須使用開放標題，請將文字內嵌至視訊軌。 這可以使用影片編輯應用程式來實現，讓標題可以覆蓋在影片上。

#### 詳細資訊——標題（預先錄制）(1.2.2){#more-information-captions-prerecorded}
...
#### How to Meet - Audio Description or Media Alternation(Prerecorded)(1.2.3){#how-to-meet-audio-description-or-media-alternative-prerecorded}
...
#### 詳細資訊——音訊說明或媒體替代（預錄）(1.2.3){#more-information-audio-description-or-media-alternative-prerecorded}
...
此成功標準與[標題（預錄）](#captions-prerecorded)相同，因為它可解決聾人或聽障人士所經歷的無障礙環境障礙，但此成功標準涉及即時簡報，例如網路廣播。

#### How to Meet - Captions(Live)(1.2.4){#how-to-meet-captions-live}

請遵循上述[標題（預先錄制）](#captions-prerecorded)的指引。 但是，由於媒體的即時性質，必須盡快建立字幕布建，以因應目前的情況。 因此，您應考慮使用即時字幕或語音轉文字工具。
...
此成功標準與[音訊描述或媒體替代項目（預錄）](#audio-description-or-media-alternative-prerecorded)相同，但作者必須提供更詳細的音訊描述，以符合等級AA。

#### How to Meet - Audio Description(Prerecorded)(1.2.5){#how-to-meet-audio-description-prerecorded}

遵循[音訊說明或媒體替代項目（預錄）](#audio-description-or-media-alternative-prerecorded)的指引。
...
* **重點**:使用或 `<strong>` 元 `<em>` 素來指出強調。請勿使用標題來反白標示段落中的文字。
   * 反白標示您要強調的文字；
   * 按一下「屬性」面板中顯示的 **B** 表徵圖( `<strong>`for)或「屬性」面板中顯示 **的「I** 」表徵圖(for `<em>`)(請確定已選 **** 中HTML)。
      ...
      >
      >它們實際上相同，但`<strong>`和`<em>`更好，因為它們在語義上是正確的html。 您的開發團隊可以設定RTE在開發專案例項時使用`<strong>`和`<em>`（而非`<b>`和`<i>`）。
...
#### 如何符合——有意義的序列(1.3.2){#how-to-meet-meaningful-sequence}

遵循[如何符合成功准則1.3.2](https://www.w3.org/WAI/WCAG21/quickref/#meaningful-sequence)下的准則。
...
如果使用顏色做為提供資訊的提示，您應提供額外的視覺提示，例如變更樣式（例如粗體、斜體）或字型。 這可協助視力低下或色彩視覺缺乏者識別資訊。 但是，它不能完全依賴，因為它不會幫助根本看不到頁面的人。 因此，（有時）提供隱藏文字或使用程式化解決方案(例如[ Accessible Rich Internet Applications(ARIA)Web標準套件](https://www.w3.org/WAI/standards-guidelines/aria/))，將此資訊傳達給無視覺的使用者是很有用的。
...
#### 如何滿足——音頻控制(1.4.2){#how-to-meet-audio-control}

遵循[如何符合成功准則1.4.2](https://www.w3.org/WAI/WCAG21/quickref/#audio-control)下的准則。
...
* 對比度（最小值）:文字和影像的視覺呈現至少有4.5:1的對比率，但下列除外：
   ...
   * 附帶：屬於非作用中使用者介面元件的文字或影像，即任何人不可見的[純裝飾](https://www.w3.org/TR/WCAG/#dfn-pure-decoration)，或是包含重要其他視覺內容之圖片的一部分，則無對比需求。
   * Logotypes:屬於標誌或品牌名稱的文字沒有最低對比要求。

   >[!NOTE]
   >
   >如需詳細資訊，請參閱[瞭解非文字對比](https://www.w3.org/WAI/WCAG21/Understanding/non-text-contrast.html)，協助確保內容作者瞭解非文字元素（包括圖示、介面元素等）的其他需求。
...
除了遵循[如何符合成功准則1.4.4](https://www.w3.org/WAI/WCAG21/quickref/#resize-text)下的准則，您還可鼓勵內容作者在其頁面設計和字型大小（例如，互動式網頁設計）中使用流暢、有彈性的寬度和高度，讓讀者能夠調整文字大小。
...
遵循[如何符合成功准則2.1.1](https://www.w3.org/WAI/WCAG21/quickref/#keyboard)下的准則。
...
遵循[如何符合成功准則2.1.2](https://www.w3.org/WAI/WCAG21/quickref/#no-keyboard-trap)下的准則。
...
遵循[如何符合成功准則2.2.1](https://www.w3.org/WAI/WCAG21/quickref/#timing-adjustable)下的准則。
...
### 癲癇與生理反應(2.3){#seizures-and-physcial-reactions}
...
#### 如何符合——旁路塊(2.4.1){#how-to-meet-bypass-blocks}

遵循[如何符合成功准則2.4.1](https://www.w3.org/WAI/WCAG21/quickref/#bypass-blocks)下的准則。
...
#### 如何滿足——焦點順序(2.4.3){#how-to-meet-focus-order}
...
雖然建議提供識別連結目的的連結文字，而不需要額外的內容，但是可以認識到，這並非總是可能。 無上下文連結可用於下列情況，其HTML範例可在[如何符合成功准則2.4.4](https://www.w3.org/WAI/WCAG21/quickref/#link-purpose-in-context)中找到。
...
遵循[如何符合成功准則2.4.5](https://www.w3.org/WAI/WCAG21/quickref/#multiple-ways)下的准則。
...
在AEM中，建立頁面時會設定頁面的預設語言，但編輯[頁面屬性](/help/sites-authoring/editing-page-properties.md)時也可以變更。
...
遵循[如何符合成功准則3.2.1](https://www.w3.org/WAI/WCAG21/quickref/#on-focus)下的准則。
...
遵循[如何符合成功准則3.2.2](https://www.w3.org/WAI/WCAG21/quickref/#on-input)下的准則。
...
遵循[如何符合成功准則3.2.3](https://www.w3.org/WAI/WCAG21/quickref/#consistent-navigation)下的准則。
...
#### 如何滿足——一致性標識(3.2.4){#how-to-meet-consistent-identification}

遵循[如何符合成功准則3.2.4](https://www.w3.org/WAI/WCAG21/quickref/#consistent-identification)下的准則。
...
本成功准則的目的是確保使用者知道發生錯誤，並可判斷錯誤的內容。 錯誤訊息應盡可能具體。 如果表單提交失敗，重新顯示表單並指出有錯誤的欄位不足以讓部分使用者察覺到發生錯誤。 例如，螢幕閱讀程式使用者在遇到其中一個指示器之前，不會知道有錯誤。 他們可能會在遇到錯誤指示器之前完全放棄表單，認為頁面無法運作。 根據WCAG中的定義，[輸入錯誤](https://www.w3.org/TR/WCAG/#dfn-input-error)是用戶提供的不被接受的資訊。 這包括：
...
遵循[如何符合成功准則3.3.1](https://www.w3.org/WAI/WCAG21/quickref/#error-identification)下的准則。
...
本「成功准則」的目的，在於確保使用者在可能的情況下收到適當的建議，以修正輸入錯誤。 [input error](https://www.w3.org/TR/WCAG/#dfn-input-error)的WCAG定義表示系統「未接受用戶提供的資訊」。 未被接受的資訊的一些示例包括用戶需要但省略的資訊以及用戶提供但不屬於所需資料格式或允許值的資訊。
...
遵循[如何符合成功准則3.3.3](https://www.w3.org/WAI/WCAG21/quickref/#error-suggestion)下的准則。
...
遵循[如何符合成功准則3.3.4](https://www.w3.org/WAI/WCAG21/quickref/#error-prevention-legal-financial-data)下的准則。
...
遵循[如何符合成功標準4.1.1](https://www.w3.org/WAI/WCAG21/quickref/#parsing)下的准則。
...
遵循[如何符合成功標準4.1.2](https://www.w3.org/WAI/WCAG21/quickref/#name-role-value)下的准則。
