...
>[!NOTE]
>
>您可以為Adobe Experience Manager建立自訂轉盤元件，該元件會在AEM DAM中顯示數位資產。 另請參閱 [建立Adobe Experience Manager的自訂轉盤元件](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html).
...
>[!CAUTION]
>
>已棄用此基礎元件。 Adobe建議使用 [標題核心元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/list.html) 而非。
...
>[!CAUTION]
>
>已棄用基礎元件。 Adobe建議使用 [核心元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=zh-Hant) 而非。
...
>[!NOTE]
>
>基礎元件表單元件僅支援使用其他基礎元件表單元件（按鈕、文字、隱藏等等）。 使用 [核心元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=zh-Hant) 不支援基礎元件表單中的表單元件（反之）。
...
標準AEM安裝包含一系列動作型別：
   ...
   * **使用者端驗證**
     ...
     指示此表單是否需要使用者端驗證（伺服器驗證） *一直* 發生)。 可使用進行使用者端驗證 **Forms驗證碼** 元件。
