...
>[!NOTE]
>
>進階功能是使用目標內容的多站台支援。 若要使用此功能，您應該熟悉 [多站點管理員](/help/sites-administering/msm.md) 和 [Adobe Target整合](/help/sites-administering/target.md) 使用AEM。
...
您可在以下位置管理此專案： **區域**. 區域可區隔不同網站中使用的目標內容（活動、體驗和選件），並提供以MSM為基礎的機制，以建立和管理目標內容的繼承以及網站繼承。 這可讓您不需要依照6.2之前AEM的要求，在繼承的網站中重新建立目標內容。
...
您可以根據使用案例，以多種方式設定目標內容的多網站支援。 本節說明理論上此功能如何與單一品牌搭配運作。 此外，在 [範例：根據地理位置鎖定內容](#example-targeting-content-based-on-geography)，您會看到在多個網站中鎖定內容的實際應用程式。
...
舉例來說，透過對目標內容的多網站支援，您可以擁有兩個（或更多）具有 **一** 具備下列其中一項的品牌：

* 完全 *distinct* 目標內容集 — 在一個中編輯目標內容不會影響另一個內容。 連結至不同區域的網站會讀取和寫入其自己的設定區域。 例如：
...
* 一組不同的目標內容 *已繼承* 透過MSM從其他網站 — 內容可以從主版單向轉出到即時副本。 例如：
...
* 此 **美國** 網站位於左上角，是主要網站。 在此範例中，它會在「鎖定目標」模式中開啟。
* 此網站的其他三個版本為 **加拿大**， **英國**、和 **澳洲**，即都是即時副本。 這些網站會在預覽模式中開啟。
...
重新整理標籤後，加拿大網站會變更為新影像，我們無須採取任何動作。 因為與美國共用主區域。 在Great Britan和Australia網站，影像不會變更。

![chlimage_1-274](assets/chlimage_1-274.png)

行銷人員想要將這些變更推廣到歐洲地區，並 [轉出即時副本](/help/sites-administering/msm-livecopy.md) 點選或按一下 **轉出頁面**. 重新整理標籤後，Great Britain網站會有新影像，因為Europe區域繼承自主區域（轉出後）。
...
>[!NOTE]
>
>當您按一下品牌名稱時，預設會收合名為「主版區域」的預設區域 **直到** 您會建立另一個區域。 然後，當您在「活動」或「選件」控制台中選 **取品牌** 時，您會看到「 **區域****** 」控制台。
...
1. 在 **轉出設定** 在下拉式功能表中，選取適當的設定。

   另請參閱 [已安裝的轉出設定](/help/sites-administering/msm-sync.md#installed-rollout-configurations) 每個選項的說明。

   另請參閱 [建立和同步化即時副本](/help/sites-administering/msm-livecopy.md) 以取得即時副本的詳細資訊。
...
>[!NOTE]
>
>參考相同區域的頁面或網站正在使用 *相同* 共用的活動、體驗和選件集。 編輯多個網站共用的活動、體驗或選件會影響所有網站。

將網站連結至區域：

1. 導覽至您要連結至某個區域的網站（或頁面）。
1. 選取網站或頁面，然後按一下 **檢視屬性**.
1. 按一下 **個人化** 標籤。
1. 在 **品牌** 功能表，選取您要連結區域的品牌。 選取品牌後，中會顯示可用區域 **區域參考** 功能表。

   ![chlimage_1-283](assets/chlimage_1-283.png)

1. 從中選擇區域 **區域參考** 下拉式功能表並按一下 **儲存**.
...
1. 導覽至您要分離或暫停繼承的頁面，然後按一下 **目標定位** 在「模式」下拉式功能表中。
1. 如果您的頁面連結至即時副本區域，您會看到繼承狀態。 按一下 **開始定位**.
1. 若要在活動上暫停，請執行下列任一項作業：

   1. 選取活動的元素，例如對象。 AEM會自動顯示「暫停即時副本」確認方塊。 （您可以在定位程式中點選或按一下任何元素，以暫停即時副本。）
   1. 選取 **暫停即時副本** 從工具列的下拉式功能表中。
...
1. 導覽至您要從主版分離即時副本的頁面，然後按一下 **目標定位** 在「模式」下拉式功能表中。
1. 如果您的頁面連結至即時副本區域，您會看到繼承狀態。 按一下 **開始定位**.
1. 從工 **具列的下拉式選單中選取「分離即時副本** 」。AEM會確認您要分離即時副本。
1. 按一下 **分離** 將即時副本與活動中斷連結。 分離後，有關繼承的下拉式功能表不再顯示。 活動現在是本機活動。
...
1. 導覽至您要還原繼承的頁面，然後按一下 **目標定位** 在「模式」下拉式功能表中。
1. 按一下 **開始定位**.
