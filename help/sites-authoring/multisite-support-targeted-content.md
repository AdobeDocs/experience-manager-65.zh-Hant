...
>[!NOTE]
>
>使用目標內容的多網站支援是一項進階功能。 若要使用此功能，您應熟悉 [多網站管理員](/help/sites-administering/msm.md) 和 [Adobe Target整合](/help/sites-administering/target.md) 搭配AEM。
...
您可在 **區域**. 區域會區隔不同網站中使用的目標內容（活動、體驗和選件），並提供MSM型機制，以建立及管理目標內容的繼承與網站繼承。 如此一來，您就不必按照AEM 6.2之前的要求，在繼承的網站中重新建立目標式內容。
...
您可以根據您的使用案例，透過多種方式為目標內容設定多網站支援。 本節說明此功能在理論上如何搭配一個品牌運作。 此外， [範例：根據地理定位內容](#example-targeting-content-based-on-geography)，您可以在多個網站中看到鎖定目標內容的實際應用程式。
...
>[!NOTE]
>
>如需此功能的更多技術資訊，請參閱 [如何架構目標內容的多網站管理](/help/sites-authoring/technical-multisite-targeted.md).
...
重新整理索引標籤後，加拿大網站會變更為新影像，而我們不需採取任何動作。 它這樣做是因為它與美國共用主區域。 在大不列顛和澳大利亞的網站上，形象並不改變。

![chlimage_1-274](assets/chlimage_1-274.png)

行銷人員想要將這些變更推展至歐洲地區，以及 [轉出即時副本](/help/sites-administering/msm-livecopy.md) 點選或點選 **轉出頁面**. 重新整理索引標籤後，大不列顛網站會有新影像，因為歐洲區域會從主版區域繼承（轉出後）。
...
1. 在 **轉出設定** 下拉式選單中，選取適當的設定。
   ...
   請參閱 [安裝的轉出設定](/help/sites-administering/msm-sync.md#installed-rollout-configurations) 以取得每個選項的說明。

   請參閱 [建立和同步Live Copy](/help/sites-administering/msm-livecopy.md) 以取得即時副本的詳細資訊。
