...
此 **專案資訊** 「表徵圖」(tile)提供有關項目的一般資訊，包括說明、項目狀態（非活動或活動）、到期日和成員。 此外，您可以新增專案縮圖，該縮圖會顯示在主「專案」頁面上。
...
您可以從團隊中添加和刪除團隊成員。 此外，您也可以編輯 [使用者角色](#userroles) 已指派給團隊成員。
...
這些說明於 [使用專案工作流程。](/help/sites-authoring/projects-with-workflows.md)
...
任務可讓您監視任何專案相關任務的狀態，包括工作流程。 有關任務的詳細說明，請訪問 [使用任務](/help/sites-authoring/task-content.md).
...
1. 權限：使用者角色分為下列三個類別之一：觀察者、編輯、所有者。 例如，攝影師或文案撰寫者擁有與編輯相同的權限。 權限會決定使用者可以對專案中的內容執行什麼動作。
1. 工作流程：工作流程會決定專案中指派工作的人員。 這些任務可以與項目角色相關聯。 例如，任務可以分配給攝影師，這樣所有具有攝影師角色的團隊成員都將獲得該任務。
...
| 編輯者 | 此角色的使用者可以上傳和編輯專案的內容。 | 專案、相關中繼資料和相關資產的讀取和寫入存取權<br>上傳照片清單、像片拍攝，以及檢閱和核准資產的權限<br>寫入權限 `/etc/commerce`<br>修改特定專案的權限 | `workflow-users` 群組 |
| 擁有者 | 此角色的使用者可以建立專案、起始專案中的工作，以及將已核准的資產移至生產資料夾。 項目中的所有其他任務也可由所有者查看和執行。 | 寫入權限 `/etc/commerce` | `dam-users` 群組，以便建立專案<br>`project-administrators` 群組，以建立專案和移動資產 |
