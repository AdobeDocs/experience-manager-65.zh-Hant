...
如需如何在技術層面使用可編輯範本的詳細資訊，請參閱開發人員檔案 [頁面範本 — 可編輯](/help/sites-developing/page-templates-editable.md) 以取得詳細資訊。
...
* 靜態範本
   ...
   * 靜態範本可用於多個版本的AEM。
   * 它們是 [由您的開發人員提供](/help/sites-developing/page-templates-static.md)，因此作者無法建立或編輯它們。
...
* [發佈範本](#publishingatemplateauthor) 以使其可在發佈環境中使用
...
>[!CAUTION]
>
>切勿輸入任何需要的資訊 [國際化](/help/sites-developing/i18n.md) 放入範本中。 為了達到內部化的目的， [核心元件的本地化功能](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/get-started/localization.html) 建議使用。

### 建立範本資料夾 — 管理員 {#creating-a-template-folder-admin}

您應該為專案建立範本資料夾，以儲存專案特定的範本。 這是一項管理員任務，詳見檔案 [頁面範本 — 可編輯](/help/sites-developing/page-templates-editable.md#template-folders).
...
1. 選擇 **建立**。隨即顯示確認訊息，請選取 **開啟** 以開始 [編輯範本](#editingatemplate) 或 **完成** 以返回範本主控台。
...
* 影像
   ...
   * 要當做影像使用 [範本的縮圖](/help/sites-authoring/templates.md#template-thumbnail-image) 以輔助選取，例如「建立頁面」精靈中的選取。
...
1. 開啟 [頁面屬性](/help/sites-authoring/editing-page-properties.md) 用於您希望範本可用的分支根頁面。

1. 開啟 **進階** 標籤。

1. 在 **範本設定** 使用 **新增欄位** 以指定範本的路徑。
   ...
   >[!NOTE]
   >
   >如果 **允許的範本** 清單保留為空白，樹狀結構將遞增，直到找到值/清單為止。
   >
   >
...
* [結構](#editingatemplatestructure)
  ...
  鎖定元件後，您可以新增頁面作者無法編輯的內容。 您可以解鎖元件，讓您 [初始內容](#editingatemplateinitialcontent).
...
當 **頁面原則** 上的選項 **頁面資訊** 功能表可讓您 [選取所需的頁面原則](#editingatemplatepagepolicies)：
...
* 元件可解除鎖定並再次鎖定，以讓您定義 [初始內容](#editingatemplateinitialcontent).
...
* **原則與屬性（一般）**
  ...
  ***政策***
  ...
  >[!NOTE]
  >
  >如果將相同型別的多個元件新增為初始內容，則相同原則會套用至所有元件。 這反映了中的相同限制 [**設計模式** 適用於靜態範本](/help/sites-authoring/default-components-designmode.md).
  ...
  *功能*
  ...
  >[!CAUTION]
  >
  >請注意，在AEM中，裁切比例定義為 **高度/寬度**. 這和寬度/高度比的傳統定義不同，並且是由於舊有相容性的原因完成的。只要您定義「 」，頁面編寫使用者就不會察覺到任何差異 **名稱** 很清楚，因為這是UI中顯示的內容。
...
您可以為一系列裝置定義範本配置。 [回應式版面](/help/sites-authoring/responsive-layout.md) 適用於範本的運作方式與頁面製作相同。

>[!NOTE]
>
>配置變更將反映在 **初始內容** 模式，但看不到任何變更 **結構** 模式。
...
1. 從 **範本編輯器**，選取 **頁面資訊** 從工具列，然後 **頁面設計** 以開啟對話方塊。
...
使用 **初始頁面屬性** 選項，您可以定義 [頁面屬性](/help/sites-authoring/editing-page-properties.md) 建立結果頁面時使用。
...
1. [建立您自己的資料夾](#creatingatemplatefolderdeveloper) 您網站專屬範本的URL。
