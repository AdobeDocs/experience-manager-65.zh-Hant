...
依預設，AEM會將 **Alt Text** 新增至您的影像。 對於傳統UI，預設屬性的建立方式有兩種不同的情況(雖然預設值可能不足以作為替代值，而且很可能需要在 **Advanced** image properties頁籤中編輯):
...
* **使用清單**:您可以使用HTML來指定三種不同的清單類型：
   ...
   * The `<ul>` element is used for *unordered* lists (bulleted) lists. 使用元素來識別個別清單 `<li>` 項目。

      在RTE中，使用「項目 **清單」圖示** 。

   * The `<ol>` element is used for *numbered* lists. 使用元素來識別個別清單 `<li>` 項目。
...
* [如何符合成功標準2.2.2](https://www.w3.org/WAI/WCAG20/quickref/#qr-navigation-mechanisms-title)
