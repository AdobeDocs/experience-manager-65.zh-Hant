...
>[!CAUTION]
>
>由於Classic UI在6.4AEM中已棄用，因此尚未更新WCAG 2.1的此頁上的內容。
>
...
根據以下三個一致性級別對這些准則進行分級：A級（最低）、A級和AAA級（最高）。 簡而言之，這些級別定義如下：

* **** A級：您的網站達到基本的最低協助功能等級。要達到此級別，將滿足所有A級成功標準。
* **AA級：** 希望達到的理想可訪問性級別，在此級別，您的站點達到了增強的可訪問性級別，因此在大多數情況下使用大多數技術的大多數人都可以訪問。 要達到此級別，將滿足所有A級和A級成功標準。
* **AAA級：** 您的站點達到了高度可訪問性。 要達到此級別，將滿足所有A級、AA級和AAA級成功標準。
...
網頁上的資訊可以以多種不同的非文本格式提供，如圖片、視頻、動畫、圖表和圖形。 失明或視力嚴重受損的人無法看到非文本內容，但他們可以通過讓螢幕閱讀器讀取文本內容或通過盲文顯示設備以觸覺形式呈現來訪問文本內容。 因此，通過提供圖形格式內容的文本替代選項，無法看到圖形內容的人可以訪問內容所提供的資訊的等效版本。

一個有用的附加好處是，文本替代方案使非文本內容能夠通過搜索引擎技術進行索引。

#### 如何見面 — 非文本內容(1.1.1) {#how-to-meet-non-text-content}

對於靜態圖形，基本要求是為圖形提供等效的文本替換。 此方法在 **替代文字** 欄位：

>[!NOTE]
>
>有些現成可用的元件(例如 **Carousel** 和 **Slideshow** )不提供將替代文字說明新增至影像的方式。在為實例實施這些元件的版本AEM時，開發團隊應配置這些元件以支援 `alt` 屬性。 這樣做可確保作者可以將其添加到內容(請參見 [添加對附加HTML元素和屬性的支援](/help/sites-administering/rte-accessible-content.md#add-support-for-more-html-elements-and-attributes))。

的 **替代文字** 的 **高級** 影像屬性頁籤 **影像** 元件對話框：

![在經典UI中編輯影像元件對話框；顯示了「Alt文本」欄位。](assets/chlimage_1-17a.png)

添AEM加 **替代文字** 預設情況下。 對於傳統UI，預設屬性的建立方式有兩種不同的方案，但預設值可能不足以作為替代，而且可能必須在 **高級** 影像屬性頁籤：
...
從數字資產庫添加影像。 如果將影像從數字資產儲存庫拖動到網頁，則 **標題** 和 **替代文字** 該影像的值從該影像的元資料中提取。
...
>[!NOTE]
>
>如果您的影像純粹是裝飾性的(請參閱 [建立好文本選項](#creating-good-text-alternatives))，您可以在 **替代文字** 的子菜單。 這樣會建立空 `alt` 屬性，提示螢幕閱讀器忽略影像。

#### 建立好文本選項 {#creating-good-text-alternatives}

非文本內容有多種形式，因此文本替換的價值取決於圖形在網頁中所扮演的角色。 一些一般經驗法則包括：
...
* 對於給定空（空）的替代文本，但僅當影像沒有替代文本時，影像是可接受的。 例如，它是純裝飾性的圖形。 或者，如果頁面文本中已存在等效文本。

的 [W3C草稿：HTML5用於提供有用文本替代的技術](https://html.spec.whatwg.org/multipage/images.html#alt) 提供了更多詳細資訊和示例，說明了針對不同類型影像的適當替代文本設定。
...
傳輸特定資訊的小像形圖（圖形）。 必須在頁面和站點上始終使用它們。 頁面或站點上表徵圖的所有實例都應具有相同的簡短文本選項，除非這樣做會導致相鄰文本的不必要的重複。

* 圖表和圖形：

這些通常表示數值資料。 因此，提供文本替代選項的一個選項可能是包含圖表或圖形中顯示的主要趨勢的簡短摘要。 如有必要，還可使用 **說明** 的 **高級** 影像屬性頁籤。 此外，還可以在頁面或站點的其他位置以表格形式提供源資料。
...
>[!NOTE]
>
>上面的代碼段僅用於說明順序。 使用 **影像** 元件，而不是 `img src` 上面使用的引用。

在AEM中，可以使用 **替代文字** 和 **說明** 映像的「配置」對話框中的欄位 — 如中所示 [如何見面 — 非文本內容(1.1.1)](#how-to-meet-non-text-content)。

* 映射、圖、流程圖：

用於提供空間資料的圖形(例如。 若要支援描述物件或程式之間的關係)，請確定關鍵訊息是以文字格式提供。對於地圖，提供全文等效可能不現實，但如果提供地圖是幫助人們找到通往特定位置的途徑，則地圖影像的替代文本可以簡短地指出 *X地圖*，然後在頁面上其他位置或通過 **說明** 的 **高級** 頁籤 **影像** 元件。

* 驗證碼：

驗證碼是 *完全自動化的公共圖靈test將電腦和人區分開來*。 它是用於網頁上的安全檢查，用於區分人和惡意軟體，但會造成訪問障礙。 這些映像需要用戶描述他們看到的通過安全test的內容。 不可能為影像提供文本替代，因此您必須考慮其他非圖形解決方案。

W3C提供了以下幾種建議。 這些方法各有優缺點。
...
這些影像是使用層疊樣式表(CSS)而非HTML實現的。 無法指定替代文本值。 因此，背景影像不應提供重要的文本資訊 — 如果提供了，則還必須在頁面文本中提供此資訊。

但是，當無法顯示影像時，必須顯示替代背景。

>[!NOTE]
>
>背景文本和前景文本之間應有適當的對比度。 對此對比作了更詳細的討論， [對比度（最小值）(1.4.3)](#contrast-minimum)。
...
* [如何滿足成功標1.1.1](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#text-alternatives)
* [W3C:HTML5用於提供有用文本替代的技術](https://html.spec.whatwg.org/multipage/images.html#alt)
...
此資訊處理的Web內容 *基於時間*。 這包括用戶可以播放的內容（例如視頻、音頻和動畫內容），並且可以預先錄制或即時流。
...
* 如果內容是預錄的無視頻音頻（如播客）:
   ...
   * 在內容之前或之後提供指向音頻內容的文本記錄的連結。
   ...
   筆錄應是HTML頁，其文本相當於所有口語和重要的非口語內容。 它還應指明說話者、背景描述、聲音表達以及任何其他重要音頻的描述。
...
>[!NOTE]
>
>如果音頻或視頻內容被提供作為網頁上以另一種格式存在的內容的替代，則無需遵循上述要求。 例如，如果視頻說明了文本說明的清單，則此視頻不需要替代，因為文本說明已充當視頻的替代。
...
* [如何滿足成功標1.2.1](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#time-based-media)
...
聾人、聽力障礙者無法或難以獲取音頻內容。 字幕是在視頻期間的適當時間在螢幕上顯示的口語和非口語音頻的文本等效項。 它們讓聽不到音頻的人理解正在發生的事情。
...
盡可能使用隱藏字幕。 它讓用戶可以選擇是否查看字幕。

對於隱藏字幕，請以適當的格式建立並提供同步字幕檔案，如 [SMIL](https://www.w3.org/AudioVideo/)，以及視頻檔案。

請參閱中的教程 [詳細資訊 — 字幕（預錄）(1.2.2)](#more-information-captions-pre-recorded)。 確保您提供備注，讓用戶知道字幕可用於視頻。

如果必須使用開放字幕，請將文本嵌入視頻軌道。 該方法使用允許將標題疊置到視頻上的視頻編輯應用來實現。
...
* [如何滿足成功標1.2.2](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#time-based-media)
...
盲人或視覺受損者在視頻或動畫中的資訊僅在視覺上提供時，會遭遇無障礙。 或者，如果原聲帶沒有提供足夠的資訊，無法直觀地瞭解正在發生的情況。
...
1. 包括視頻內容的其他音頻說明。 您可以通過以下三種方式之一來完成此方法：
   ...
   * 提供包含原始音軌的新的、附加的和可選的音軌，同時還包括有關場景更改的附加音頻資訊。
      ...
      * 此方法可防止不需要附加說明的用戶中斷。
   * 建立視頻內容的第二版本，以允許擴展音頻說明。 這樣做通過在適當的點暫停音頻和視頻，減少了在現有對話間隙內提供詳細音頻描述的困難。 因此，在再次開始操作之前，可以給出更長的音頻描述。 如上例所示，最好將其作為可選的附加音頻軌道提供，以防止中斷不需要附加說明的用戶。


1. 提供與視頻或動畫的音頻和視覺元素等價的適當文本文字記錄。 它應酌情包括關於誰在說話的指示、對背景的描述、聲音表達。 視其長度而定，可以將文字記錄放在視頻或動畫的同一頁面，或放在單獨的頁面上；如果選擇後一個選項，請提供視頻或動畫旁邊的記錄的連結。
...
* [如何滿足成功標1.2.3](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-media-equiv-audio-desc)
* [Adobe EncoreCS5](https://helpx.adobe.com/premiere-pro/using/whats-new.html)
...
* [AccessIT（華盛頓大學）:能否使用語音識別自動生成字幕？](https://www.washington.edu/doit/programs/accessit?1209)
...
* [如何滿足成功標1.2.4](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-media-equiv-real-time-captions)
...
* [如何滿足成功標1.2.5](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-media-equiv-audio-desc-only)
...
殘疾人使用的許多輔助技術依靠結構資訊來有效顯示或輸出內容。 此結構資訊可以採用頁面標題、表行和列標題以及清單類型的形式。 例如，螢幕閱讀器允許用戶在從標題到標題的頁面中導航。 然而，當頁面內容似乎只通過視覺造型而非底層HTML具有結構時，則輔助技術沒有可用的結構資訊，限制了它們支援更方便瀏覽的能力。
...
只要啟用了RTE的輔助功能(請參見 [AEM和輔助功能](/help/sites-administering/rte-accessible-content.md))，提AEM供三級頁標題。 您可以使用這些項目來識別內容的區段和子區段。標題1是標題的最高級別，標題3是最低級別。系統管理員可以配置系統以允許使用更多標題級別。
...
使用或元素指示強調。 請勿使用標題來反白標示段落中的文字。
    ...
    *突出顯示要強調的文本；
    *按一下**B**表徵圖（用於&amp;lt;strong&amp;gt；）或**I**表徵圖（用於&amp;lt;em&amp;gt；）（在**Properties**面板中顯示）(確保已選擇HTML)。
...
此外，可訪問的表使用以下元素和屬性：
    ...
    * 「 」&lt;caption>&#39;元素用於為表提供可見的標題。 預設情況下，字幕顯示在表格的中心位置，但可以使用CSS進行適當定位。 字幕以寫程式方式與表相關聯，因此它是提供內容介紹的有用方法。
    * 「 」&lt;h3 class=&quot;summary&quot;>&quot;元素通過提供有視力的用戶可以看到的內容的概要，幫助無視的用戶更輕鬆地理解表中顯示的資訊。 在使用複雜或非常規表佈局時，這特別有用（此屬性不顯示在瀏覽器中，它只讀於輔助技術）。
    * 「 」的「scope」屬性&lt;th>&#39;元素用於指示單元格是代表特定行的標題還是代表特定列的標題。 類似的方法是在複雜表中使用頭和id屬性，其中資料單元可與一個或多個頭相關聯。
...
添加 **表格**，您可以配置 **表屬性** 對話框。
    ...
    *相應的**標題**。
    *最好刪除**Width**、**Height**、**Border**、**單元格填充**、**單元格間距**的任何預設值。 因為這些屬性可以在全局樣式表中設定。
...
有時，如果存在具有兩個或更多級別標題的複雜表，則基本表屬性可能不足以提供所有必要的結構資訊。 對於這些類型的複雜表，必須使用 **標題** 和 **ID** 屬性。 例如，在下表的標題和ID中，會對輔助技術使用者進行程式化關聯。
...
* [如何滿足成功標1.3.1](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-content-structure-separation-programmatic)
...
設計人員在展示資訊時通常關注視覺設計特徵，如顏色、形狀、文本樣式或內容的絕對或相對位置。 這些技術可以是傳遞資訊的強大設計技術，但是盲人或視覺障礙者可能無法訪問需要視覺識別屬性的資訊，如位置、顏色或形狀。
...
>[!NOTE]
如果理解描述性術語在非視覺上下文中具有含義，則使用描述性術語是可接受的。 例如，使用 *上* 和 *下* 通常可以接受，因為它們分別暗示某一內容項目之前和之後的內容。 當內容被大聲說出時，這仍然是有意義的。
...
* [如何滿足成功標1.3.3](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-content-structure-separation-understanding)
...
顏色是提高網頁審美感的有效途徑，也是資訊傳遞的有效手段。 但是，從盲目性到彩色視覺缺陷，都存在著一系列視覺缺陷，這意味著有些人無法區分某些顏色。 這個問題使顏色編碼成為提供資訊的不可靠方式。

例如，有紅綠色視覺缺陷的人，無法分辨出綠色的陰影和紅色的陰影。 他們可能將這兩種顏色都視為第三種顏色（例如，棕色），在這種情況下，他們無法區分紅色、綠色和棕色。
...
如果將顏色用作提供資訊的提示，則應提供額外的視覺提示，如更改樣式（例如，粗體、斜體）或字型。 這有助於視力低或視力不足的人識別資訊。 但是，它不能完全依靠，因為它根本無助於看不到頁面的人。
...
* [如何滿足成功標1.4.3](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-visual-audio-contrast-contrast)
...
* [如何滿足成功標1.4.5](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-visual-audio-contrast-text-presentation)
...
* 提供暫停滾動內容的方法，以便用戶有足夠的時間閱讀內容。 例如，新聞標題或自動更新的文本。
* 確保連結內容在五秒後停止閃爍。
...
* [如何滿足成功標2.2.2](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-time-limits-pause)
...
>[!NOTE]
因為任何不符合此成功標準的內容都可能干擾用戶使用整個頁面的能力，因此網頁上的所有內容（無論是否用於滿足其他成功標準）都必須符合此成功標準。 請參閱 [符合性要求5:無干擾](https://www.w3.org/TR/WCAG20/#cc5)。
...
* [如何滿足成功標2.3.1](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#seizure)
...
此成功標準可幫助每個人快速識別網頁內容，而不必閱讀該頁面。 在瀏覽器頁籤中開啟多個網頁時，此設計非常有用，因為該頁籤中顯示了頁面標題，因此可以快速定位。

#### 如何見面 — 標題為(2.4.2)的頁 {#how-to-meet-page-titled}

在中建立新HTML頁AEM時，可以指定頁標題。 確保標題充分描述了頁面的內容，以便訪問者能夠快速確定內容是否與其需要相關。
...
* [如何滿足成功標2.4.2](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-navigation-mechanisms-title)

### 連結用途（上下文）(2.4.4)  {#link-purpose-in-context}

* 成功標準2.4.4
* A級
* 連結用途（在上下文中）:每個連結的目的可以單獨地從連結文本中確定，也可以從連結文本與其寫程式確定的連結上下文一起確定。 在通常情況下，連結的目的對用戶不明確。

#### 目的 — 連結目的（在上下文中）(2.4.4) {#purpose-link-purpose-in-context}

對所有用戶來說，無論有何損害，通過適當的連結文本明確指示連結的方向至關重要。 此設計可幫助用戶確定他們是否真正想遵循連結。 對於有目光的用戶，當頁面上有多個連結時（尤其是當頁面上有大量文本時），有意義的連結文本非常有用，因為有意義的連結文本提供了對目標頁面功能的更清晰的指示。 而輔助技術的用戶可以更輕鬆地理解上下文之外的連結文本。
...
* 標題屬性中包含的文本僅作為工具提示彈出窗口可供滑鼠用戶使用，無法使用鍵盤訪問。
...
雖然建議提供標識連結目的的連結文本而不需要附加上下文，但人們認識到這並非總是可能的。 上下文自由連結可用於以下情況，其HTML示例可在 [如何滿足成功標2.4.4](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-navigation-mechanisms-refs)。
...
或者，可以使用指令碼，使得在連結本身內提供最少量的文本。 但是，當激活指向頁面頂部的適當控制項時，連結文本 *擴展* 進一步細節。 類似的方法是使用CSS *隱藏* 可視用戶的完整連結，但仍將其全部輸出給螢幕閱讀器用戶。 這不屬於本文檔的範圍，但有關如何實現此目標的詳細資訊，請參閱 [更多資訊 — 連結目的（在上下文中）(2.4.4)](#more-information-link-purpose-in-context) 的子菜單。
...
* [如何滿足成功標2.4.4](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-navigation-mechanisms-refs)
...
* [如何滿足成功標3.1.1](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-meaning-doc-lang-id)
...
* [如何滿足成功標3.1.2](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-meaning-other-lang-id)
...
提供幫助人們完成表單的說明是介面可用性方面良好實踐的一個基本部分。 對於視覺或認知障礙的人來說，這對他們很有幫助，因為他們本來很難理解表單的佈局以及在特定表單域中提供的資料類型。

在AEM中，添加表單元件時會添加預設標籤，如 **文本欄位**&#x200B;的雙曲餘切值。 此預設標題取決於元件類型。 您可以在 **標題和文本** 對話框。 必須確保標籤有助於用戶瞭解與每個表單元件關聯的資料。

![標題和文本頁籤（編輯對話框）;已添加標題「說明」。](assets/chlimage_1-22a.png)

此 **標題** 欄位元素必須用於欄位元素，因為它提供了可用於輔助技術的標籤。 僅在欄位旁邊的文本中寫入標籤是不夠的。

對於某些表單元件，您也可以使用「隱藏標題」核取方塊以視覺化方式 **隱藏標籤** 。以此方式隱藏的標籤仍適用於輔助技術，但無法顯示在螢幕上。雖然在某些情況下，這可以是一種好方法，但最好盡可能添加一個可視標籤。 有些用戶可能正在查看螢幕的一小部分（一次只看一個欄位），需要標籤來正確標識該欄位。
...
倘存在一組相關控制，例如 **無線電組**，組和個別控制項可能需要標題。 在AEM中新增一組選項按鈕時，「標題 **」欄位會提供此群組標題，而個別標題會指定為選項按鈕(** Items ****)。
...
* [如何滿足成功標3.3.2](https://www.w3.org/WAI/WCAG21/quickref/?versions=2.0#qr-minimize-error-cues)
