...
為標準網頁編寫內容時，會使用下列元件。 這些元件構成標準AEM安裝可用的元件子集。
...
無論您選擇哪種方法，您在清單中加入的頁面，皆應已有與頁面相關聯的影像。 這是輪播中顯示的影像。 如果該頁面的「頁面屬性」下沒有頁面的影像，您應該先將影像與頁面建立關聯，再開始。 否則，輪播會顯示空白頁面。 請參閱 [編輯頁面屬性](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
系統會根據您選擇的項目顯示新面板：
...
>[!NOTE]
>
>您可以為Adobe Experience Manager建立自訂輪播元件，在AEM DAM中顯示數位資產。
...
下載連結所顯示的簡短說明。
   ...
   * **檔案**
可在產生的網頁上下載的檔案。 從內容尋找器拖曳資產，或選取區域，以便上傳要供下載的檔案。
...
下列範例顯示 **清單** 元件，以顯示子頁面清單的方式（設計由網站設計的自訂CSS定義控制）。
...
除了參考特定段落外，還可以修改路徑以指定整個par-system。 在路徑的尾碼加上下列項目：
...
例如，如果您輸入 `Geometrixxe`，系統會顯示「您是說？ Geometrixx&quot;.
   ...
   * **類似頁面文字**
      ...
      類似頁面的結果旁顯示的文字。 要剪除影像的一部分，請按一下此連結。
...
然後，投影片放映元件會在一段短時間內，重複顯示每個元件，然後淡入下一張投影片：
...
>
>Adobe建議您使用 **表格** 表的元件，但也可使用 **文字** 元件。

此 **表格** 元件已預先設定，可讓您建構、填入表格並設定表格格式。 使用該對話框，可以從頭配置表並建立內容，或從外部編輯器（如Excel、OpenOffice或記事本）複製並貼上電子錶格或表。
...
當您開啟 **樣式** 的 **編輯** 對話框中，還可以設定以下內容：

* **空格字元**
* **文字樣式**

格式化文字隨即顯示在頁面上。 實際設計取決於網站CSS:
...
* 影片
   ...
   * **大小**
      ...
      視訊的原生大小(寬x高（以像素為單位）會出現在「大小」旁的方塊中（請參閱上方）。 如果您想要覆寫視訊的原生維度，請在此手動輸入寬度和高度維度。 選取 **確定** 會關閉對話方塊。
...
欄是控制AEM中內容版面的機制。 在標準安裝中，提供了用於建立兩列和/或三列的元件。

以下示例顯示正在使用的兩個列和三個列元件。 您可以為新元件使用預留位置：
...
>[!NOTE]
>
>有關開發和自訂表單的資訊，請參閱 [開發Forms頁面](/help/sites-developing/developing-forms.md). 此表單自訂包括新增動作、限制、預先載入欄位，以及使用指令碼來呼叫動作服務等。
...
* **限制**
   ...
   您可以在此處指定欄位是否為必要欄位，並將限制置於該欄位上，例如為數值。
...
此欄位為必填欄位，應僅包含下列字元：
   ...
   * **說明**
      ...
      可讓您視需要為使用者提供其他資訊。 在表單中，此說明會以比標題小的字型顯示在欄位下方。
...
* **限制**
   ...
   * **必要**
      ...
      此約束取決於表單元件類型，但提供一個或多個按一下框以指明此欄位是必需的，或此欄位的某些部分是必需的。

   * **必要訊息**

      通知使用者此欄位為必填的訊息。 必填欄位會標籤星號。
...
表單元件會使用 **表單開始** 和 **表單結尾** 元素。 開始和結束元素一律成對，以確保表單已正確定義。
...
此欄位是可選欄位，並指定儲存庫中節點的路徑。 如果此節點的屬性與欄位名稱匹配，則表單上的相應欄位將預載這些屬性的值。 如果不存在匹配項，則欄位包含預設值。
使用 **載入路徑** 您可以預先載入必填欄位中的值。 請參閱 [預先載入表單值](/help/sites-developing/developing-forms.md#preloading-form-values).

   * **用戶端驗證**

      指示此表單是否需要客戶端驗證（伺服器驗證） *always* 發生。) 驗證是透過 **Forms驗證碼** 元件。
...
指定所需的銷售機會清單。
      * **建立和更新帳戶**
         ...
         * **檢視資料...**
            ...
            選擇按鈕，以便在批量編輯器中訪問有關表單結果的資訊。 從這裡，您可以將資訊匯出至 `.tsv` （以Tab分隔）檔案（例如，在Excel試算表中使用）。
      * **郵件**
      ...
      * **存放區內容**
         ...
         * **檢視資料...**
            ...
            選擇按鈕，以便在批量編輯器中訪問有關表單結果的資訊。 從這裡，您可以將資訊匯出為.tsv（以Tab分隔）檔案（例如，在Excel試算表中使用）。
      * **透過上傳存放內容**
...
此元件可讓您建立隱藏欄位。 這些隱藏欄位可用於各種用途，例如提交表單後必須執行動作，或後續處理需要隱藏資料時。
