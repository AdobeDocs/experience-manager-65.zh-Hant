...
在為標準網頁創作內容時，以下元件將供使用。 這些元件構成了可用於標準安裝的現成的元件的子集AEM。
...
無論您選擇哪種方法，您清單中包含的頁面都應該已經與頁面相關聯。 顯示在旋轉傳送器中的是此影像。 如果該頁面的「頁面屬性」下沒有頁面的影像，則應在開始之前將影像與頁面相關聯。 否則，「旋轉傳送」(Carousel)將顯示空白頁。 請參閱 [編輯頁面屬性](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md)。
根據您選擇的項目，將出現一個新面板：
...
>[!NOTE]
>
>您可以為Adobe Experience Manager建立自定義旋轉木馬元件，該元件在DAM中顯示數AEM字資產。
...
隨下載連結顯示的簡短說明。
   ...
   * **檔案**
可在生成的網頁上下載的檔案。 從內容查找器中拖動資產，或選擇區域，以便上載要供下載的檔案。
...
* **裁切**
   ...
   選擇「裁剪」(Crop)，以便可以剪斷影像的一部分。 使用滑鼠裁剪影像。
...
以下示例顯示 **清單** 顯示子頁清單（設計由站點設計的自定義CSS定義控制）的方式。
...
除了引用特定段落外，還可以修改路徑以指定整個par-system。 對路徑添加尾碼，並包含以下內容：
...
例如，如果鍵入 `Geometrixxe`，系統顯示&quot;你是說？ Geometrixx&quot;.
   ...
   * **類似頁面文本**
      ...
      顯示在類似頁面結果旁邊的文本。 要剪切影像的一部分，請按一下此連結。
...
此元件允許您載入一系列影像，以便在頁面上以幻燈片的形式顯示。 您可以添加或刪除影像，並為每個影像分配標題。 在「高級」(Advanced)下，還可指定顯示區域的大小。
...
然後，幻燈片放映元件會在短時間內反複按順序顯示每個元件，然後才會淡入下一張幻燈片：
...
>
>Adobe建議您使用 **表格** 表的元件，但也可以使用 **文本** 元件。

的 **表格** 元件已預配置，以便構建、填充和格式化表。 使用該對話框，您可以從頭配置表並建立內容，或者通過從外部編輯器（如Excel、OpenOffice或記事本）複製和貼上電子錶格或表來建立內容。
...
開啟 **樣式** 頁籤 **編輯** 對話框，您還可以設定以下內容：

* **空格字元**
* **文字樣式**

格式化的文本隨即顯示在頁面上。 實際設計取決於站點CSS:
...
列是控制中內容佈局的機AEM制。 在標準安裝中，提供了用於建立兩列和/或三列的元件。

以下示例顯示了正在使用的兩個列和三個列元件。 可將佔位符用於新元件：
...
>[!NOTE]
>
>有關開發和自定義表單的資訊，請參見 [開發Forms頁](/help/sites-developing/developing-forms.md)。 此表單自定義包括添加操作、約束、預載入欄位和使用指令碼調用服務以執行操作等。
...
這些頁籤為您提供了必要的參數。 這些頁籤取決於各個元件類型，但可以包括以下內容：
...
此欄位為必需欄位，並且只應包含以下字元：
   ...
   * **說明**
      ...
      允許您根據需要為用戶提供其他資訊。 在窗體中，此說明以比標題小的字型顯示在欄位下面。
...
* **限制**
   ...
   * **必要**
      ...
      此約束取決於窗體元件類型，但提供一個或多個按一下框以指示需要此欄位或需要此欄位的某些部分。

   * **必要訊息**

      一條消息，通知用戶需要此欄位。 必填欄位用星號標籤。
...
「表單」元件使用 **窗體開始** 和 **表單結束** 元素。 起始元素和終止元素始終成對，以確保正確定義表單。
...
此欄位為可選欄位，並指定儲存庫中節點的路徑。 如果此節點具有與欄位名稱匹配的屬性，則表單上的相應欄位將預先載入這些屬性的值。 如果不存在匹配項，則欄位包含預設值。
使用 **載入路徑** 可以使用必填欄位中的值預載入窗體。 請參閱 [預載入窗體值](/help/sites-developing/developing-forms.md#preloading-form-values)。

   * **用戶端驗證**

      指示此表單是否需要客戶端驗證（伺服器驗證） *總是* 發生。) 通過 **Forms卡普查** 元件。
...
指定所需的潛在顧客清單。
      * **建立和更新帳戶**
         ...
         * **檢視資料...**
            ...
            選擇按鈕，以便您可以訪問有關批量編輯器中表單結果的資訊。 從此處，您可以將資訊導出到 `.tsv` （以制表符分隔）檔案（例如，在Excel電子錶格中使用）。
      * **郵件**
      ...
      * **存放區內容**
         ...
         * **檢視資料...**
            ...
            選擇按鈕，以便您可以訪問有關批量編輯器中表單結果的資訊。 在此處，可以將資訊導出到.tsv（以制表符分隔）檔案（例如，在Excel電子錶格中使用）。
      * **透過上傳存放內容**
...
此元件用於建立隱藏欄位。 這些隱藏欄位可用於各種目的，例如在提交表單後必須執行某項操作，或在後處理中需要隱藏資料。
