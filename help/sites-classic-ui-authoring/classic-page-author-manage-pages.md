...
如果您在建立新頁面時只提供頁面 **Title** ,AEM會從此字串衍生頁面 **Name**[ ，並根據AEM和JCR所強加的慣例來驗證名稱。](/help/sites-developing/naming-conventions.md)在這兩個UI中 **標題** 將接受包含無效字元的欄位，但派生的名稱將取代無效字元。 例如：
...
AEM隨附多種現成可用的範本。 提供的範本取決於個別網站，而需要提供的資訊（建立新頁面時）則取決於使用的UI。 主要欄位包括：
...
1. 此 **建立資料夾** 對話方塊將會開啟。 您可以在這裡輸入 **名稱** 和 **標題**：
