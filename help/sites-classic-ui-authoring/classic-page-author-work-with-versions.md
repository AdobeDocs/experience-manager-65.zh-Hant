...
>[!NOTE]
>
>如果從存放庫移除版本，則時間扭曲無法顯示正確的檢視。 此外，如果轉譯網站的元素（例如程式碼、css和影像）已變更，則檢視會與原本不同，因為這些專案不會在存放庫中建立版本。
...
1. 使用日期和時間選擇器指定您想要的日期/時間，然後按一下 **前往**.
   ...
   >[!NOTE]
   >
   >您無法編輯頁面的舊版本。 它僅供檢視。 如果您想要還原較舊的版本，請使用手動還原 [還原](/help/sites-classic-ui-authoring/classic-page-author-work-with-versions.md#restoring-a-page-version-from-sidekick).
...
在以下範例中，紅線表示頁面是在初始綠色版本的時間範圍內進行編輯，黃線表示頁面是在紅色版本期間進行編輯等。
...
* **時間扭曲為唯讀**  — 您無法編輯頁面的舊版本。 它僅供檢視。 如果您想要還原較舊的版本，請使用手動還原 [還原](#main-pars-title-1).

* **時間扭曲僅以頁面內容為基礎**  — 如果轉譯網站的元素（例如程式碼、css和影像資產）已變更，則檢視會與原本不同，因為這些專案不在存放庫中建立版本。

>[!CAUTION]
>
>Timewarp的設計目的是協助作者瞭解和建立其內容。 此日誌並非用作稽核記錄檔或作法律用途。
