...
call to action (CTA)是用來讓訪客在登陸頁面上立即採取動作，例如「立即訂閱」、「檢視此影片」、「僅限有限時間」等。
...
此CTA元件可用來在登入頁面上新增文字連結。 您可以按一下該連結，將使用者引導至元件屬性中指定的目標URL。 它是「Call to action」群組的一部分。
...
### call to action (CTA)銷售機會表單 {#call-to-action-cta-lead-form}
...
下節將說明Call-to-action銷售機會表單可用的元件。
...
此外，設計人員可能需要透過編輯或刪除內容及設定call-to-action元件，在登入頁面匯入後進行修改。 最後，行銷人員會想要預覽登入頁面，然後啟動行銷活動以確保登入頁面已發佈。
...
清除登入頁面時，您可以移除內容變更。 如果您按一下&#x200B;**否**，則會保留內容變更，也就是說，`jcr:content/importer`下的結構會保留，而且只會移除importer頁面元件和`etc/design`中的資源。 然而，若您按一下&#x200B;**是**，則會一併移除`jcr:content/importer`。
...
>[!NOTE]
>
>如果登入頁面上的元件無法編輯，則您需要在[修改HTML檔案之後重新匯入zip檔案。](/help/sites-administering/extending-the-design-importer-for-landingpages.md) 這表示在匯入期間，不可編輯的零件未轉換為AEM元件。
