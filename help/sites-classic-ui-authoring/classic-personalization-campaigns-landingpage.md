...
[Extending and Configuring the Design Importer](/help/sites-administering/extending-the-design-importer-for-landingpages.md)中介紹了準備要導入的設計軟體包。 [「整合著陸頁面與Adobe Analytics」中涵蓋與Adobe Analytics的整合。](/help/sites-administering/integrating-landing-pages-with-adobe-analytics.md)
...
>[!CAUTION]
>
>由於Design Importer需要存取`/apps`，因此在`/apps`不可變的容器化雲端環境中，它將無法運作。
...
著陸頁面匯入程式與一般著陸頁面的程式相同，著陸頁面設計有與行動著陸頁面對應的額外html檔案。 此html檔案也必須有畫布`div`，其中`id=cqcanvas`與案頭登陸頁面html類似，並支援案頭登陸頁面所描述的所有可編輯元件。
...
「文字」元件可讓您使用WYSIWYG編輯器輸入文字區塊。 如需詳細資訊，請參閱[文字元件](/help/sites-authoring/default-components.md#text)。
...
影像元件會顯示影像，您可從「內容搜尋器」拖放，或按一下以上傳。 如需詳細資訊，請參閱[image元件](/help/sites-authoring/default-components.md)。
...
**標** 簽使用者在圖中看到的文字。您可以使用RTF編輯器修改格式。
...
#### 許多銷售機會表單元件的常用設定{#settings-common-to-many-lead-form-components}
...
>您看到的欄位視個別元件而定。
>
>並非所有選項都可用於所有銷售機會表單元件。 有關這些[常用設定](/help/sites-authoring/default-components.md#formsgroup)的詳細資訊，請參閱表單。

#### 銷售線索表單元件{#lead-form-components}
...
由於匯入頁面需要準備HTML、驗證和測試頁面，因此匯入著陸頁面會當成管理工作。 身為管理員，執行匯入的使用者需要`/apps`的讀取、寫入、建立和刪除權限。 如果使用者沒有這些權限，匯入將會失敗。

>[!NOTE]
>
>由於設計匯入工具是以要求`/apps`讀取、寫入、建立和刪除權限的管理工具為目的，Adobe不建議在生產中使用設計匯入工具。
...
1. 選擇&#x200B;**匯入工具頁面**&#x200B;範本並新增標題和名稱，然後按一下「建立&#x200B;**a3/>」。**
...
在匯入設計套件之前，必須先準備HTML。 如需詳細資訊，請參閱[擴充和設定設計匯入](/help/sites-administering/extending-the-design-importer-for-landingpages.md)。

### 導入設計包{#importing-the-design-package}

建立匯入工具頁面後，您可將設計套件匯入至該頁面。 有關建立設計軟體包及其建議結構的詳細資訊，請參閱[擴展和配置設計導入](/help/sites-administering/extending-the-design-importer-for-landingpages.md)。
...
1. 將設計套件拖放至dropbox。 請注意，當將套件拖曳到其上時，箭頭會改變方向。
...
>[!NOTE]
>
>如果導入設計包時遇到問題，請參閱[ Troubleshooting](/help/sites-administering/extending-the-design-importer-for-landingpages.md#troubleshooting)。
...
當您匯入設計套件時，頁面的設定選單中會提供&#x200B;**Clear Design**&#x200B;和&#x200B;**Download Imported Zip**:
...
清除著陸頁面時，您可以移除內容變更。 如果按一下&#x200B;**否**，則保留內容更改，即保留`jcr:content/importer`下的結構，並僅刪除導入程式頁元件和`etc/design`中的資源。 但是，如果按一下「是」(**Yes**)，則`jcr:content/importer`也會被刪除。

>[!NOTE]
>
>如果您決定移除內容變更，則當您按一下&#x200B;**Clear**&#x200B;時，您在匯入的登陸頁面上所做的所有變更以及所有頁面屬性都會遺失。

### 修改和添加著陸頁面上的元件{#modifying-and-adding-components-on-a-landing-page}
...
>[!NOTE]
>
>如果無法編輯著陸頁面上的元件，則在[修改HTML檔案後，您必須重新匯入zip檔案。](/help/sites-administering/extending-the-design-importer-for-landingpages.md) 這表示在匯入期間，不可編輯的部件不會轉換為AEM元件。
