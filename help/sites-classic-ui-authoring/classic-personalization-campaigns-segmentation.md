---
title: 了解分段
description: 區段是建立行銷活動時的主要考量事項。 通常，在開始行銷活動之前，您必須先定義區段。
contentOwner: User
...
區段是建立行銷活動時的主要考量事項。 通常，在開始行銷活動之前，您必須先定義區段。
...
接著，即可根據訪客相符的區段，將內容鎖定在訪客的需求和興趣上。
...
**使用者**  — 使用者是在網站註冊以接收帳戶設定檔的訪客。 為了產生其設定檔，他們可提供額外的身分識別，例如電子郵件地址和性別等。 您也可以收集其他資訊，包括社群活動和購買模式等。 根據設定檔中提供的資訊，可以建立人口統計設定檔。
