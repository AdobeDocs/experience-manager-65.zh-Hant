...
title: 影片
seo-title: 影片
description: Assets提供集中式視訊資產管理，您可以直接將視訊上傳至Assets，以便自動編碼至Dynamic Media Classic，並直接從Assets存取Dy視訊以進行頁面製作。
contentOwner: User
...
* Dynamic Media Classic視訊元件會自動執行裝置和頻寬偵測，以在桌上型電腦、平板電腦和行動裝置上播放正確格式和適當品質的視訊。
* 資產——您可以包含可調式視訊集，而不只包含單一視訊資產。 最適化視訊集是所有必要視訊轉譯的容器，可在多種螢幕上順暢播放視訊。 「最適化視訊集」會針對以不同位元速率和格式（例如400 kbps、800 kbps和1000 kbps）編碼的相同視訊版本分組。 您使用Adaptive Video Set和S7視訊元件，在多種螢幕上（包括桌上型電腦、iOS、Android、Blackberry和Windows行動裝置）進行最適化視訊串流。 如需詳細資訊，請參閱[動態媒體經典檔案，瞭解最適化視訊集](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/quick-start-video.html#video)。

## 關於FFMPEG和Dynamic Media Classic {#about-ffmpeg-and-scene}

預設的視訊編碼程式是以使用FFMPEG與視訊設定檔整合為基礎。 因此，現成可用的[!UICONTROL DAM更新資產]工作流程包含下列兩個ffmpeg工作流程步驟：

* FFMPEG縮圖
* FFMPEG編碼

請注意，啟用和設定Dynamic Media Classic整合併不會從現成可用的[!UICONTROL DAM更新資產]擷取工作流程自動移除或停用這兩個工作流程步驟。 如果您已在AEM中使用以FFMPEG為基礎的視訊編碼，則您可能已在製作環境中安裝FFMPEG。 在此例中，使用「資產」收錄的新視訊會編碼兩次：一次是從FFMPEG編碼器，一次是從Dynamic Media Classic整合。

如果您已在AEM中設定FFMPEG視訊編碼並安裝FFMPEG,Adobe建議您從[!UICONTROL DAM更新資產]工作流程中移除兩個FFMPEG工作流程。
...
1. 在AEM中，存取「內容搜尋器」的&#x200B;**[!UICONTROL Dynamic Media Classic]**&#x200B;標籤中WCM中的視訊資產。
...
1. 在&#x200B;**[!UICONTROL Cloud Services]**&#x200B;中，導覽至您的&#x200B;**[!UICONTROL Dynamic Media Classic]**&#x200B;組態，然後按一下「編輯」。]****[!UICONTROL 
1. 選擇&#x200B;**[!UICONTROL Video]**&#x200B;標籤。

   >[!NOTE]
   >
   >如果頁面沒有雲端設定，則不會顯示&#x200B;**[!UICONTROL Video]**&#x200B;標籤。 請參閱[啟用WCM](#enablingscene7forwcm)的動態媒體經典。

1. 選擇最適化視訊編碼設定檔、現成可用的單一視訊編碼設定檔或自訂視訊編碼設定檔。

   >[!NOTE]
   >
   >如需視訊預設集的詳細資訊，請參閱[編碼視訊檔案的視訊預設集](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html?lang=en#video-presets-for-encoding-video-files)。
   >
...
如果您因為Dynamic Media Classic中已更新預設集，所以需要更新AEM中視訊的檢視器和編碼預設集，請導覽至雲端設定中的Dynamic Media Classic設定，然後按一下「更新檢視器和編碼預設集&#x200B;**」。**
...
1. 按一下&#x200B;**[!UICONTROL Upload]**&#x200B;上載主源視頻。 視訊上傳和編碼在[!UICONTROL DAM更新資產]工作流程完成後完成，而&#x200B;**[!UICONTROL 發佈至動態媒體經典]**&#x200B;具有核取標籤。

   >[!NOTE]
   >
   >產生視訊縮圖可能需要一些時間。

   將DAM主要來源視訊拖曳至視訊元件時，會存取Dynamic Media Classic編碼的Proxy轉譯的&#x200B;*all*&#x200B;以進行傳送。

### Foundation Video Component與Dynamic Media Classic Video Component {#foundation-video-component-versus-scene-video-component}

使用AEM時，您可以存取Sites中的Video元件和Dynamic Media Classic視訊元件。 這些元件不能互換。
...
即使建議使用Dynamic Media Classic視訊元件來檢視Dynamic Media Classic視訊，本節也會為了完整性，在AEM中說明使用Dynamic Media Classic視訊與[!UICONTROL Foundation Video Component]。

### AEM Video與Dynamic Media Classic Video比較{#aem-video-and-scene-video-comparison}

下表提供AEM Foundation Video元件與Dynamic Media Classic Video元件之間支援功能的高階比較：
...
1. 編輯新視訊設定檔。 先選取雲端設定。 然後選取與雲端設定中選取的編碼預設集相同。
   ...
   | 屬性 | 說明 |
   |---|---|
   | Dynamic Media Classic Cloud設定 | 用於編碼預設集的雲端設定。 |
   | 動態媒體經典編碼預設集 | 用來映射此視訊描述檔的編碼預設集。 |
...
1. 開啟基礎視訊元件的設計對話方塊，並變更至&#x200B;**[!UICONTROL Profiles]**&#x200B;標籤。 然後刪除現成可用的描述檔，並新增新的Dynamic Media Classic視訊描述檔。 設計對話方塊中描述檔清單的順序，也會定義在轉換時視訊來源元素的順序。
