...
description: 了解使用MongoDB成功部署Adobe Experience Manager所需的工作和考量事項。
...
本文旨在改善對使用MongoDB成功部署AEM(Adobe Experience Manager)所需工作和考量事項的了解。
...
最少的部署需要三個 `mongod` 配置為複製副本集的實例。 一個實例被選為主實例，而另一個實例是輔助實例，選舉由管理 `mongod`. 連接到每個實例的是本地磁碟。 因此，群集可以支援負載，建議每秒最小吞吐量為12 MB，每秒超過3000次I/O操作(IOPS)。
...
在此部署中，成功實施需要所有元件。 如果缺少任何元件，實作將無法運作。
...
如果運行項目的不同技術團隊之間有良好的溝通，則支援虛擬化環境。 此支援包括運行AEM的團隊、擁有作業系統的團隊以及管理虛擬化基礎架構的團隊。

MongoDB實例的I/O容量有特定要求，必須由管理虛擬化環境的團隊管理。 如果項目使用雲部署(如Amazon Web Services)，則必須為實例布建足夠的I/O容量和一致性，以支援MongoDB實例。 否則，MongoDB程式和Oak存放庫會以不可靠且不穩定的方式執行。

在虛擬化環境中， MongoDB需要特定的I/O和VM配置，以確保MongoDB的儲存引擎不會因VMWare資源分配策略而受損。 成功的實施可確保各個團隊之間沒有任何障礙，而且所有團隊都已註冊，以提供所需的效能。

## 硬體考量事項 {#hardware-considerations}

### 儲存空間 {#storage}

為了實現讀寫吞吐量以獲得最佳效能，而不需要過早的水準擴展，MongoDB通常需要SSD儲存或與SSD效能相當的儲存。

### RAM {#ram}

使用MMAP儲存引擎的MongoDB 2.6和3.0版要求資料庫及其索引的工作集與RAM相符。

RAM不足導致效能顯著下降。 工作集和資料庫的大小與應用程式高度相關。 雖然可以做出一些估計，但確定所需RAM量的最可靠方法是構建AEM應用程式並對其進行負載測試。
...
這些比率意味著，在SSD部署中，2 TB的資料庫需要200 GB的RAM。

雖然MongoDB 3.0中的WiredTiger儲存引擎也有同樣的限制，但工作集、RAM和頁故障之間的關聯性並不強。 WiredTiger使用記憶體映射的方式與MMAP儲存引擎不同。
...
由於MongoDB工作集限制，建議維護資料儲存獨立於MongoDB。 在大部分環境中， `FileDataStore` 應使用所有AEM執行個體都可用的NAS。 若是使用Amazon Web Services, `S3 DataStore`. 如果由於任何原因，資料儲存區將保留在MongoDB內，資料儲存區的大小應添加到總資料庫大小中，並適當調整工作集計算。 此調整可能意味著配置更多RAM以保持效能，而不會出現頁面故障。

## 監控 {#monitoring}

監測對於成功實施該項目至關重要。 只要具備足夠的知識，便可在MongoDB上執行AEM而不進行監控。 但是，這些知識通常在為部署的每個部分專門設計的工程師中提供。

這項專業知識通常涉及一名研發工程師，負責Apache Oak Core，以及一名MongoDB專家。

若不在所有層級進行監控，診斷問題就需要詳細了解程式碼庫。 實施團隊可針對異常情況作出適當反應，以便對主要統計資料進行監控和提供適當指導。

雖然可以使用命令行工具獲取群集操作的快速快照，但在許多主機上即時執行此操作幾乎是不可能的。 命令列工具很少會提供超過幾分鐘的歷史資訊，且絕不允許不同類型的量度之間產生交叉關聯。 一段短暫的緩慢背景 `mongod` 同步需要大量的手動工作，以便與來自明顯未連接的虛擬機的共用儲存資源的I/O等待或過多的寫入級別相關聯。
...
它需要安裝在MongoDB實例上的代理，該代理連接到監視伺服器。 代理程式有三個層級：
...
MongoDB Ops Manager與MongoDB Cloud Manager軟體相同。 註冊後，Ops Manager就可以下載並安裝到本地的專用資料中心，或任何其他筆記型電腦或台式電腦上。 它使用本地MongoDB資料庫來儲存資料，並以與Cloud Manager相同的方式與受控伺服器通信。 如果您的安全策略禁止監視代理，則應使用MongoDB Ops Manager。

### 作業系統監控 {#operating-system-monitoring}

運行AEM MongoDB群集需要作業系統級別監視。

Ganglia是此類系統的一個很好的示例，它提供了所需資訊的範圍和詳細資訊，這些資訊超出了CPU、負載平均值和可用磁碟空間等基本運行狀況指標。 要診斷問題，需要低級別資訊，如熵池級別、CPU I/O Wait、FIN_WAIT2狀態的套接字。
...
* `mongodburi`
MongoDB伺服器AEM必須連接到。 將連接到預設副本集的所有已知成員。 如果使用了MongoDB Cloud Manager，則會啟用伺服器安全性。 因此，連接字串必須包含適當的用戶名和密碼。 非企業版的MongoDB僅支援用戶名和密碼驗證。 如需連線字串語法的詳細資訊，請參閱 [檔案](https://docs.mongodb.org/manual/reference/connection-string/).
...
* `customBlobStore`
如果部署將二進位檔儲存在資料庫中，則它們是工作集的一部分。 因此，建議不要在MongoDB中儲存二進位檔，而偏好使用其他資料存放區，例如 
`FileSystem` NAS上的資料儲存。

* `cache`
快取大小(MB)。 此空間分佈於 
`DocumentNodeStore`。預設為256 MB。 不過，較大快取可讓Oak讀取效能得益。

* `blobCacheSize`
AEM可快取常用的Blob，以避免從資料存放區重新擷取。 這樣做對效能的影響更大，尤其是在MongoDB資料庫中儲存Blob時。 所有基於檔案系統的資料儲存都受益於作業系統級磁碟快取。

#### 資料儲存配置 {#data-store-configuration}

資料儲存區用於儲存大於臨界值的檔案。 低於該閾值時，檔案將作為屬性儲存在文檔節點儲存區中。 若 `MongoBlobStore` ，則會在MongoDB中建立專用集合以儲存blob。 此集合有助於 `mongod` 例項，並要求 `mongod` 有更多RAM以避免效能問題。 因此，建議的設定是避免 `MongoBlobStore` 供生產部署及使用 `FileDataStore` 由所有AEM實例共用的NAS作為備份。 由於作業系統級快取在管理檔案方面非常有效，因此應將磁碟上檔案的最小大小設定為接近磁碟的塊大小。 這樣可確保檔案系統的使用效率，而且許多小型檔案不會對檔案的工作集產生過多的貢獻 `mongod` 例項。
...
* `minRecordLength`
大小（位元組）。 小於或等於此大小的二進位檔會與Document Node Store一起儲存。 儲存二進位檔的內容，而非儲存blob的ID。 如果二進位檔大於此大小，則二進位檔的ID會儲存為節點集合中Document的屬性。 而且，二進位檔的內文會儲存在 
`FileDataStore` 在磁碟上。 4096位元組是典型的檔案系統塊大小。

* `path`
資料儲存根的路徑。 若是MongoMK部署，此路徑必須是可供所有AEM執行個體使用的共用檔案系統。 通常使用網路連接儲存(NAS)伺服器。 若是雲端部署(例如Amazon Web Services), 
`S3DataFileStore` 也可用。
...
建議您借由新增屬性來停用隨所有查詢傳送的查詢提示 `-Doak.mongo.disableIndexHint=true` 啟動AEM時。 這樣可確保MongoDB根據內部統計資料計算要使用的最適當索引。

如果未停用查詢提示，索引的任何效能調整不會影響AEM的效能。
...
MongoDB 2.6使用記憶體映射儲存引擎，該引擎對RAM和磁碟之間作業系統級別管理的某些方面很敏感。 MongoDB實例的查詢和讀取效能依賴於避免或消除慢速的I/O操作（通常稱為頁面故障）。 這些問題是套用至的頁面錯誤 `mongod` 過程。 請勿與作業系統層級的頁面錯誤混淆。

為了快速操作，MongoDB資料庫應僅訪問已在RAM中的資料。 它必須存取的資料由索引和資料組成。 此索引和資料集合稱為工作集。 當工作集大於可用RAM時，MongoDB必須從磁碟中將該資料頁面化，從而產生I/O成本，從而將記憶體中已有的其他資料逐出。 如果逐出操作導致資料從磁碟重新載入，則頁故障佔據主導地位，效能降低。 如果工作集為動態和變數，則會發生更多頁面錯誤以支援操作。

MongoDB在多種作業系統上運行，包括各種Linux®風格、Windows和macOS。 請參閱 [https://docs.mongodb.com/manual/installation/#supported-platforms](https://docs.mongodb.com/manual/installation/#supported-platforms) 以取得其他詳細資訊。 MongoDB根據您的作業系統選擇提供不同的作業系統級別建議。 有檔案記錄於 [https://docs.mongodb.com/manual/administration/production-checklist-operations/#operating-system-configuration](https://docs.mongodb.com/manual/administration/production-checklist-operations/#operating-system-configuration) 為方便起見。

#### Linux® {#linux}

* 關閉透明護頁和碎片整理。 請參閱 [透明大頁設定](https://docs.mongodb.com/manual/tutorial/transparent-huge-pages/) 以取得更多資訊。
* [調整預讀設定](https://docs.mongodb.com/manual/administration/production-notes/#readahead) 儲存資料庫檔案的設備上，以便適合您的使用案例。

   * 對於MMAPv1儲存引擎，如果工作集大於可用RAM，且文檔訪問模式是隨機的，請考慮將預讀數降低到32或16。 評估不同的設定，以便找到最佳值，最大化駐留記憶體並減少頁面故障數。
...
* 當RHEL 7/CentOS 7在虛擬環境中運行時，調諧的工具會自動調用從效能吞吐量派生的效能配置檔案，該效能配置檔案會自動將預讀設定設定為4 MB。 此設定可能會對效能造成負面影響。
* 使用SSD驅動器的noop或截止日期磁碟計畫程式。
* 在來賓VM中使用用於虛擬化驅動器的noop磁碟調度程式。
* 禁用NUMA或設定 `vm.zone_reclaim_mode` 到0並運行 [蒙古](https://docs.mongodb.com/manual/administration/production-notes/#readahead) 具有節點交錯的實例。 請參閱： [MongoDB和NUMA硬體](https://docs.mongodb.com/manual/administration/production-notes/#readahead) 以取得更多資訊。

* 調整硬體上的上限值，使其符合您的使用案例。 若為多個 [蒙古](https://docs.mongodb.com/manual/reference/program/mongod/#bin.mongod) 或 [蒙古](https://docs.mongodb.com/manual/reference/program/mongos/#bin.mongos) 例項在相同使用者下執行，請據以縮放上限值。 請參閱： [UNIX®限制設定](https://docs.mongodb.com/manual/reference/ulimit/) 以取得更多資訊。
...
* 請考慮禁用NTFS「上次訪問時間」更新。 此設定類似於在類似Unix的系統上禁用atime。

### WiredTiger {#wiredtiger}

自MongoDB 3.2起，MongoDB的預設儲存引擎是WiredTiger儲存引擎。 此引擎提供一些強大且可擴展的功能，使其更適合於所有一般資料庫工作負載。 以下幾節將介紹這些功能。

#### 文檔級併發 {#document-level-concurrency}

WiredTiger對寫操作使用文檔級併發控制。 因此，多個用戶端可以同時修改集合的不同檔案。

對於大多數讀寫操作，WiredTiger使用樂觀的併發控制。 WiredTiger僅在全局、資料庫和收集級別使用意圖鎖。 當儲存引擎檢測到兩個操作之間的衝突時，一個操作會產生寫入衝突，導致MongoDB透明地重試該操作。 某些全局操作（通常涉及多個資料庫的短期操作）仍需要全局「全實例」鎖。
...
寫入磁碟時，WiredTiger會以一致的方式在所有資料檔案中將快照中的所有資料寫入磁碟。 現在 —  [耐用](https://docs.mongodb.com/manual/reference/glossary/#term-durable) 資料會作為資料檔案中的查核點。 查核點可確保資料檔案在最後一個查核點之前（包括最後一個查核點）一致。 也就是說，查核點可以作為恢復點。

MongoDB配置WiredTiger以60秒或2 GB的日誌資料間隔建立檢查點（即將快照資料寫入磁碟）。
...
WiredTiger使用預寫事務登錄組合，與 [查核點](https://docs.mongodb.com/manual/core/wiredtiger/#storage-wiredtiger-checkpoints) 確保資料的持久性。
...
請參閱 [與WiredTiger聯繫](https://docs.mongodb.com/manual/core/journaling/#journaling-wiredtiger).
...
>
>針對 [獨立](https://docs.mongodb.com/manual/reference/glossary/#term-standalone) 例項，不使用日記帳表示當MongoDB意外退出查核點之間時，您會丟失某些資料修改。 若 [副本集](https://docs.mongodb.com/manual/reference/glossary/#term-replica-set)，複製過程可提供足夠的持久性保證。
...
使用WiredTiger時，MongoDB既使用WiredTiger內部快取，又使用檔案系統快取。

從3.4版開始，WiredTiger內部快取預設會使用下列其中一個較大的：
...
NUMA（非統一記憶體訪問）允許內核管理記憶體映射到處理器內核的方式。 雖然此過程試圖使內核的記憶體訪問速度加快，以確保內核能夠訪問所需資料，但NUMA會干擾MMAP引入額外的延遲，因為無法預測讀取。 因此，必須為 `mongod` 在所有功能強大的作業系統上執行。

本質上，在NUMA體系結構中，儲存器連接到CPU,CPU連接到匯流排。 在SMP或UMA體系結構中，儲存器連接到匯流排並由CPU共用。 當線程在NUMA CPU上分配記憶體時，它會根據策略分配記憶體。 預設值是分配連接到線程的本地CPU的記憶體，除非沒有空閒記憶體，此時它會以較高的成本使用空閒CPU的記憶體。 分配後，記憶體不會在CPU之間移動。 分配由從父線程繼承的策略執行，該策略最終是啟動進程的線程。

在許多將電腦視為多核統一記憶體體系結構的資料庫中，這種情況會導致初始CPU先滿，然後再填滿次要CPU。 如果中央線程負責分配記憶體緩衝區，則尤其如此。 解決方案是更改用於啟動的主線程的NUMA策略 `mongod` 執行下列命令以處理：

```shell
numactl --interleaved=all <mongod> -f config
```

此策略會以循環方式在所有CPU節點上分配記憶體，以確保在所有節點上均勻分配。 它不會像使用多CPU硬體的系統那樣，對記憶體產生最高效能的訪問。 大約一半的記憶體操作在匯流排上速度較慢，但 `mongod` 尚未以最佳方式寫入目標NUMA，因此這是一個合理的折衷。

### NUMA問題 {#numa-issues}

若 `mongod` 進程從以下位置啟動： `/etc/init.d` 資料夾中，可能尚未使用正確的NUMA策略啟動。 根據預設策略的不同，可能會出現問題。 原因是，MongoDB的各種Linux® Package Manager安裝程式也安裝了一個包含配置檔案的服務， `/etc/init.d` 執行上述步驟。 如果直接從歸檔檔案安裝並運行MongoDB( `.tar.gz`)，您必須在 `numactl` 程式。

>[!NOTE]
>
>有關NUMA政策的更多資訊，請查閱 [numactl檔案](https://linux.die.net/man/8/numactl).

MongoDB進程在不同分配策略下的行為不同：
...
* `-cpunodebind=<nodes>`
僅在節點上執行。 Mongod只在指定的節點上運行，並且只使用這些節點上可用的記憶體。

* `-physcpubind=<nodes>`
僅對列出的CPU（核心）運行。 Mongod只在列出的CPU上運行，並且只使用這些CPU上可用的記憶體。
...
由於資料庫的記憶體密集性質，必須禁用作業系統級別交換。 MongoDB流程避免了設計交換。

#### 遠程檔案系統 {#remote-filesystems}

不建議將NFS等遠程檔案系統用於MongoDB的內部資料檔案（mongod進程資料庫檔案），因為它們會導致太多延遲。 請勿將與建議使用NFS的Oak Blob(FileDataStore)儲存所需的共用檔案系統混淆。

#### 預讀 {#read-ahead}

調整「Read aew」（預讀），以便當使用隨機讀取的方式將頁面調入時，不會從磁碟讀取不必要的塊。 這種結果意味著I/O頻寬的不必要消耗。
...
建議對運行MongoDB資料庫的磁碟使用32的值。 此值等於16 KB。 您可以執行下列程式來設定：
...
請確保在承載MongoDB資料庫的電腦上安裝並運行NTP。 例如，您可以在CentOS電腦上使用yum Package Manager進行安裝：
...
Red Hat® Linux®使用名為「透明大頁」(THP)的記憶體管理算法。 如果您正在將作業系統用於資料庫工作負載，建議禁用它。
...
在大多數啟用NUMA的安裝中，如果MongoDB守護程式以服務的形式運行，則會自動禁用它 `/etc/init.d` 檔案夾。
...
Linux®允許通過 `ulimit` 命令。 此設定可由使用者執行，或依每個程式執行。
...
>[!NOTE]
>
>此 `mongoperf` 是MongoDB在其所運行的平台上效能的指標。 因此，不應將結果視為生產系統效能的最終結果。
>
>為獲得更準確的效能結果，您可以使用 `fio` Linux®工具。

**在組成部署的虛擬機上測試讀取效能**

安裝此工具後，切換到MongoDB資料庫目錄以運行測試。 然後，執行以開始第一個測試 `mongoperf`使用此設定：

```shell
echo "{nThreads:32,fileSizeMB:1000,r:true}" | mongoperf
```

對於所有MongoDB實例，所需輸出應高達每秒2 GB(2 GB/s)和每秒500.000 IOPS（以32個線程運行）。
...
1. 通過設定，保證承載MongoDB的電腦的CPU資源 [CPU預留](https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.hostclient.doc/GUID-6C9023B2-3A8F-48EB-8A36-44E3D14958F6.html?hWord=N4IghgNiBc4RB7AxmALgUwAQGEAKBVTAJ3QGcEBXIpMkAXyA)
...
為了正確提供MongoDB部署，托管Dispatcher的作業系統必須正在運行 **Apache httpd** **2.4版或更新版本。**

此外，請確定您組建中使用的所有程式庫都為最新狀態，以將安全性影響降至最低。

### Dispatcher設定 {#dispatcher-configuration}

一般的Dispatcher設定，其作用是單一AEM例項之請求總處理量的十至20倍。

由於Dispatcher沒有狀態，因此可輕鬆水準縮放。 在某些部署中，作者必須受限於無法存取特定資源。 建議您將Dispatcher與製作例項搭配使用。

在不使用Dispatcher的情況下執行AEM時，需要由其他應用程式執行SSL終止和負載平衡。 此為必要項目，因為工作階段必須與其建立所在的AEM例項有相關性，這個概念稱為黏著連線。 原因是為了確保內容更新顯示最低的延遲。

檢查 [Dispatcher檔案](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/dispatcher.html?lang=zh-Hant) 以取得如何設定的詳細資訊。
...
建議針對將請求路由至AEM例項的所有內層啟用黏著連線，鼓勵後續請求連至相同的AEM例項。 這麼做有助於將執行個體之間更新內容時可能注意到的延遲降至最低。

#### 長期過期 {#long-expires}

依預設，從AEM Dispatcher傳送的內容具有「上次修改時間」和「Etag」標題，且沒有內容過期的指示。 此流程可確保使用者介面一律取得資源的最新版本。 這也表示瀏覽器會執行GET操作，查看資源是否已變更。 因此，視頁面載入而定，可能會產生多個HTTP回應為304（未修改）的請求。 對於未過期的資源，設定「過期」標題並移除「上次修改時間」和「ETag」標題，會導致快取內容。 此外，在滿足「過期」標題中的日期之前，不會提出進一步的更新請求。

不過，使用此方法表示在過期標題過期之前，沒有合理的方式可讓資源在瀏覽器中過期。 為了緩解此工作流程，可將HtmlClientLibraryManager設定為對用戶端程式庫使用不可修改的URL。

這些URL必定不會變更。 當URL中包含的資源內文變更時，URL會反映變更，確保瀏覽器要求正確版本的資源。

預設配置將一個選擇器添加到HtmlClientLibraryManager中。 資源是選取器，會在Dispatcher中快取，且選取器保持不變。 此外，此選取器也可用來確保正確的過期行為。 預設選取器會遵循 `lc-.*?-lc` 模式。 下列Apache httpd設定指示可確保所有符合該模式的請求都以適當的到期時間提供。
...
如果內容以沒有內容類型的方式傳出，許多瀏覽器會透過讀取內容的前幾個位元組來嘗試猜測內容類型。 此方法稱為「嗅探」。 嗅探會開啟安全漏洞，因為可寫入儲存庫的用戶可能上傳沒有內容類型的惡意內容。

因此，建議您新增 `no-sniff` Dispatcher提供的資源標題。 不過，Dispatcher不會快取標題。 因此，這意味著從本地檔案系統提供的任何內容都由其擴展確定其內容類型，而不是使用其源AEM伺服器的原始內容類型頭。
...
預設的Dispatcher設定允許開啟的內容安全性原則，也稱為CSP。 這些設定可讓頁面從受瀏覽器沙箱預設原則限制的所有網域載入資源。

最好限制資源可從的載入位置，以避免從不受信任的或未驗證的外部伺服器將程式碼載入到JavaScript引擎中。

CSP可微調策略。 不過，在複雜的應用程式中，必須謹慎開發CSP標頭，因為限制太嚴格的原則可能會破壞使用者介面的部分。

>[!NOTE]
如需此功能的詳細資訊，請參閱 [內容安全策略上的OWASP頁](https://owasp.deteact.com/cheat/cheatsheets/Content_Security_Policy_Cheat_Sheet.html).
...
>[!NOTE]
請參閱 [MongoDB檔案](https://docs.mongodb.com/manual/reference/limits/) 以便您熟悉MongoDB的已知限制和臨界值。
