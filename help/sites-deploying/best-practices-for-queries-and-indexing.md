...
在AEM 6中轉換至Oak後，對查詢和索引的管理方式做了一些重大變更。 在Jackrabbit 2下，預設會建立所有內容的索引，並可自由查詢。 在Oak中，必須在`oak:index`節點下手動建立索引。 查詢可以在沒有索引的情況下執行，但對於大型資料集，執行速度會非常慢，甚至中止。

本文將概述何時建立索引以及何時不需要索引、避免在不需要查詢時使用查詢的技巧，以及最佳化索引和查詢以盡可能最佳化執行的提示。

此外，請務必閱讀[Oak說明檔案，說明如何編寫查詢和索引](/help/sites-deploying/queries-and-indexing.md)。 除了索引是AEM 6的新概念外，從舊版AEM安裝移轉程式碼時，Oak查詢中也有語法差異需要考慮。
...
此外，在設計分類時，務必考慮排序是否重要。 在不需要顯式排序且需要大量同級節點的情況下，最好使用無序節點類型，如`sling:Folder`或`oak:Unstructured`。 如果需要訂購，則`nt:unstructured`和`sling:OrderedFolder`更合適。

### 元件{#queries-in-components}中的查詢

由於查詢是AEM系統上較耗時的作業之一，因此建議您在元件中避免查詢。 每次呈現頁面時執行多個查詢通常會降低系統的效能。 在呈現元件時，有兩種策略可用來避免執行查詢：**遍歷節點**&#x200B;和&#x200B;**預取結果**。
...
`/content/myUnstructuredContent/parentCategory/childCategory`節點可以簡單地檢索，其子節點可以被解析並用於渲染元件。
...
運行未使用索引的查詢時，將記錄有關節點遍歷的警告。 如果此查詢將經常運行，則應建立索引。 要確定給定查詢使用的索引，建議使用[解釋查詢工具](/help/sites-administering/operations-dashboard.md#explain-query)。 如需詳細資訊，可針對相關搜尋API啟用DEBUG記錄。
...
* 透過[QueryBuilder APIs](/help/sites-developing/querybuilder-api.md)（建議）
* 使用XPath（建議）
* 使用SQL2
...
>[!NOTE]
>
>使用QueryBuilder時，會依預設判斷結果計數，與舊版Jackrabbit相比，Oak中的結果計數較慢。 要彌補這一缺點，可使用[guessTotal參數](/help/sites-developing/querybuilder-api.md#using-p-guesstotal-to-return-the-results)。

### Explain Query Tool {#the-explain-query-tool}

和任何查詢語言一樣，最佳化查詢的第一步是瞭解查詢的執行方式。 要啟用此活動，可以使用「操作儀表板」中的[解釋查詢工具](/help/sites-administering/operations-dashboard.md#explain-query)。 使用此工具，可插入並解釋查詢。 如果查詢將導致大型儲存庫以及執行時間和將使用的索引出現問題，則會顯示警告。 此工具也可以載入慢速和常用查詢清單，然後加以說明並最佳化。

### 查詢的DEBUG日誌{#debug-logging-for-queries}

若要取得有關Oak如何選擇要使用的索引以及查詢引擎如何實際執行查詢的其他資訊，可針對下列封裝新增&#x200B;**DEBUG**&#x200B;記錄設定：
...
您可以通過訪問`https://server:port/system/console/jmx`的JMX控制台訪問它

登錄到JMX控制台後，請搜索&#x200B;**Lucene Index Statistics**&#x200B;以查找它。 其他索引統計資訊可在&#x200B;**IndexStats** MBean中找到。

對於查詢統計資訊，請查看名為&#x200B;**Oak查詢統計資訊**&#x200B;的MBean。

如果您想使用[Luke](https://code.google.com/p/luke/)之類的工具來挖掘索引，則需要使用Oak控制台將索引從`NodeStore`轉儲到檔案系統目錄。 有關如何執行此操作的說明，請閱讀[Lucene文檔](https://jackrabbit.apache.org/oak/docs/query/lucene.html)。
...
為`oak.queryLimitInMemory`設定低臨界值(例如 10000)和橡樹。 `queryLimitReads` (例如5000)，並最佳化當點擊「查詢讀取的節點數超過x個……」的UnsupportedOperationException時，昂貴的查詢。
...
此外，索引只有在索引中包含的資料足夠唯一以保證其安全時才有用。 考慮一本書中的索引及其涵蓋的主題。 在為文字中的一組主題建立索引時，通常會有數百或數千個項目，這可讓您快速跳至頁面的子集，以快速找到您要尋找的資訊。 如果該索引只有兩三個條目，每個條目指向幾百個頁面，則該索引將不太有用。 這一概念同樣適用於資料庫索引。 如果只有幾個唯一值，則索引將非常有用。 話雖如此，一個指數也可能變得過大，無法發揮作用。 要查看索引統計資訊，請參見上面的[索引統計資訊](/help/sites-deploying/best-practices-for-queries-and-indexing.md#index-statistics)。
...
* Lucene索引提供的功能比屬性索引更多。 例如，屬性索引只能為單一屬性建立索引，而Lucene索引可以包含許多屬性。 有關Lucene索引中所有可用功能的詳細資訊，請參閱[文檔](https://jackrabbit.apache.org/oak/docs/query/lucene.html)。
...
>[!NOTE]
>
>採用整合的Solr搜索方法時，可以將索引卸載到Solr伺服器。 如果通過基於爬蟲的方法使用Solr伺服器的更高級功能，則需要額外的配置工作。 Headwire已建立[開放源碼連接器](https://www.aemsolrsearch.com/#/)以加速這些類型的實施。
...
Oak documentation for Lucene indexes列出了設計索引時要考慮的幾個事項：

* 如果查詢使用不同的路徑限制，請使用`evaluatePathRestrictions`。 這將允許查詢返回指定路徑下的結果子集，然後根據查詢篩選結果。 否則，查詢將搜索與儲存庫中的查詢參數匹配的所有結果，然後根據路徑進行篩選。
* 如果查詢使用排序，請為sorted屬性定義明確的屬性，並為其設定`ordered`至`true`。 這樣，結果就可以在索引中按此順序排列，並節省查詢執行時的昂貴排序操作。

* 只將所需內容放入索引中。 新增不需要的功能或屬性會導致索引增長並降低效能。
* 在屬性索引中，具有唯一的屬性名稱有助於減小索引的大小，但對於Lucene索引，應使用`nodeTypes`和`mixins`來實現連貫索引。 查詢特定的`nodeType`或`mixin`將比查詢`nt:base`更有效。 使用此方法時，請為相關的`nodeTypes`定義`indexRules`。
...
這可在OSGi控制台的&#x200B;**LuceneIndexProvider**&#x200B;服務下進行設定，並自Oak 1.0.13起預設啟用。

### 刪除索引{#removing-indexes}

刪除索引時，建議您先將`type`屬性設為`disabled`來暫時停用索引，然後進行測試，以確保應用程式在實際刪除前正常運作。 請注意，索引在停用時不會更新，因此，如果重新啟用索引，並且可能需要重新建立索引，則可能沒有正確的內容。

在刪除TarMK實例上的屬性索引後，將需要運行壓縮以回收任何正在使用的磁碟空間。 對於Lucene索引，實際索引內容存在於BlobStore中，因此需要資料存放區廢棄項目收集。

刪除MongoDB實例上的索引時，刪除成本與索引中的節點數成比例。 由於刪除大型索引可能會造成問題，建議的方法是使用例如&#x200B;**oak-mongo.js**&#x200B;的工具，停用索引並僅在維護視窗中刪除它。 請注意，此方法不適用於一般節點內容，因為它可能會造成資料不一致。
...
本節概述&#x200B;**only**&#x200B;重新索引Oak索引的可接受理由。

在下列原因外，開始Oak索引的重新索引將&#x200B;**not**&#x200B;變更行為或解決問題，並無須增加AEM的負載。
...
>
>* 查詢正確
>* 查詢解析為預期索引（使用[Explain Query](/help/sites-administering/operations-dashboard.md#diagnosis-tools)）
>* 索引程式已完成
...
* 如何解決：
   ...
   * [盧塞](/help/sites-deploying/best-practices-for-queries-and-indexing.md#how-to-re-index) 內指數
...
* 如何驗證：
   ...
   * 驗證是否使用Lucene Index統計資訊JMX Mbean(LuceneIndex)方法`diffStoredIndexDefinition`更改了索引定義。

* 如何解決：

   * Oak 1.6之前版本：

      * [盧塞](#how-to-re-index) 內指數
   * Oak 1.6+版

      * 如果現有內容不受變更影響，則只需重新整理

         * [通](https://jackrabbit.apache.org/oak/docs/query/lucene.html#stored-index-definition) 過設定 [oak:queryIndexDefinition]@refresh=true來刷新lucene索引
      * 否則，[re-index](#how-to-re-index)lucene指數

         * 注意：將使用上次正確重新索引（或初始索引）的索引狀態，直到觸發新的重新索引



### 搜索和異常情況{#erring-and-exceptional-situations}

下表說明重新建立Oak索引索引將解決此問題的唯一可接受的錯誤和例外情況。

如果AEM發生不符合下列標準的問題，請執行&#x200B;**not**&#x200B;重新索引任何索引，因為它不會解決問題。
...
#### Lucene索引二進位檔案缺少{#lucene-index-binary-is-missing}
...
#### Lucene索引二進位檔案損壞{#lucene-index-binary-is-corrupt}
...
* 如何解決：
   ...
   * 刪除lucene索引的本地副本
      ...
      1. 停止AEM
      1. 刪除`crx-quickstart/repository/index`處lucene索引的本地副本
      1. 重新啟動AEM
   * 如果這無法解決問題，而`AsyncIndexUpdate`例外會持續存在，則：

      1. [重新編](#how-to-re-index) 制索引
      1. 另外，請提交[Adobe支援](https://helpx.adobe.com/support.html)票證
...
>[!NOTE]
>
>在AEM 6.5中，[oak-run.jar是ONLY支援的方法](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree)，可在MongoMK或RDBMK儲存庫上重新建立索引。
...
* 使用Web控制台，透過&#x200B;**PropertyIndexAsyncReindex** MBean重新索引屬性索引；
...
>[!NOTE]
>
>上節摘要並框定AEM內容中[Apache Oak檔案](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)中的Oak重新索引指引。
...
啟用二進位擷取的重新索引&#x200B;**existing** lucene索引

* 對儲存庫中的&#x200B;**所有**&#x200B;候選內容重新編製索引；當要從中擷取全文的二進位檔數眾多或複雜時，AEM會增加執行全文擷取的運算負擔。 文字預先擷取將文字擷取的「運算量大、成本高」移入可直接存取AEM資料存放區的隔離程式，以避免AEM中的開銷和資源爭用。

支援在啟用二進位擷取的情況下，將&#x200B;**new** lucene索引部署至AEM
...
#### 使用文本預抽取{#prerequisites-to-using-text-pre-extraction}的先決條件
...
#### 如何執行文本預抽取{#how-to-perform-text-pre-extraction}
...
1a。 執行`oak-run.jar --generate`以建立預提取其文本的節點清單。

1b. 節點清單(1a)以CSV檔案形式儲存到檔案系統

請注意，每次執行`--generate`時，都會遍歷整個節點儲存（如oak-run命令中的路徑所指定），並建立&#x200B;**new** CSV檔案。 CSV檔案是&#x200B;**not**，在文本預提取過程的離散執行之間重新使用（步驟1 - 2）。
