...
由於查詢可能是在AEM系統上執行的最繁重的操作之一，因此最好在元件中避免查詢。 每次轉譯頁面時執行數個查詢通常會降低系統效能。 有兩種策略可用來避免在轉譯元件時執行查詢： **周遊節點** 和 **預先擷取結果**.
...
為了支援建立有效的JCR查詢和索引定義， [JCR查詢速查表](assets/JCR_query_cheatsheet-v1.1.pdf) 可供下載，並可在開發期間作為參考使用。 它包含QueryBuilder、XPath和SQL-2的範例查詢，涵蓋了在查詢效能方面表現不同的多個情境。 此外，也提供如何建立或自訂Oak索引的建議。 本速查表內容適用於AEM 6.5和AEMas a Cloud Service。
