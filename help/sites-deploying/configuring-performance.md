...
>
>此外，您也可以檢閱知識庫文章，網址為 [效能調整秘訣](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/configuring-performance.html?lang=zh-Hant).
...
>* 另請參閱 [效能調整秘訣。](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/configuring-performance.html?lang=zh-Hant)
...
| Component | 測試型別 | 否. 個使用者的 | Tx/秒（預期） | Tx/秒（已測試） | 說明 |
|---|---|---|---|---|---|
| 首頁單一使用者 | 平均 | 1 | 1 |  |  |
|  | 尖峰 | 1 | 3 |  |  |
| 首頁100位使用者 | 平均 | 100 | 3 |  |  |
...
| 情境 | Component | 否. 個使用者的 | Tx/秒（預期） | Tx/秒（已測試） | 說明 |
|---|---|---|---|---|---|
| 混合平均 | 首頁 | 10 | 1 |  |  |
|  | 搜尋 | 10 | 1 |  |  |
|  | 新聞 | 10 | 2 |  |  |
...
此 [Dispatcher](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/dispatcher.html?lang=zh-Hant) 是Adobe的快取和/或負載平衡工具。 使用Dispatcher時，請考慮將您的網站快取效能最佳化。
...
Dispatcher提供數種內建機制，如果您的網站利用這些機制來最佳化效能。 本節將說明如何設計網站，好讓快取的優點最大化。

>[!NOTE]
>
>記住 Dispatcher 會將快取儲存在標準網頁伺服器上可能會有所幫助。 瞭解此資訊表示您可以快取可以儲存為頁面並使用URL請求的所有內容。 此外，您無法儲存Cookie、工作階段資料和表單資料等其他資料。
>
...
有了Dispatcher版本4.1.11，您可以快取回應標頭。 如果您沒有在Dispatcher上快取回應標頭，如果您將頁面編碼資訊儲存在標頭中，可能會發生問題。 在此情況下，當 Dispatcher 從快取中提供頁面時，將會對此頁面使用網頁伺服器的預設編碼。 有兩種方式可避免此問題：
...
如果您將圖片用於導覽專案，方法基本上與標題相同，但稍微複雜一些。 將所有導覽影像與目標頁面一起儲存。 如果您將兩張圖片用於一般和活躍情境，可以使用以下指令碼：
...
>[!TIP]
>如需快取安全內容的相關資訊，請參閱 [快取安全內容](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/permissions-cache.html?lang=zh-Hant) 在Dispatcher指南中。
