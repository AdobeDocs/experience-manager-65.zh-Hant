...
本節說明安裝獨立AEM例項時可用的選項。 您也可以閱讀[Storage Elements](/help/sites-deploying/storage-elements-in-aem-6.md)，以取得在新安裝AEM 6後選擇後端儲存類型的詳細資訊。
...
* 更名檔案時，它必須以`cq;`開頭，如`cq5-publish-p4503.jar`中所示。
...
最後，如果您執行的是從AEM 6.3升級的例項，請確定下列屬性在`sling.properties`下方設定為&#x200B;**true**:

* `felix.bootdelegation.implicit`

## 執行模式 {#run-modes}

**執** 行模式，以針對特定用途調整AEM實例；例如，製作或發佈、測試、開發、內部網路等。這些模式也可讓您控制範例內容的使用。 此示例內容是在構建快速入門之前定義的，可包括包、配置等。 當您想要保持安裝精簡，而且不需要範例內容時，這對於生產就緒型安裝特別有用。 如需詳細資訊，請參閱：

* [執行模式](/help/sites-deploying/configure-runmodes.md)

## 添加檔案安裝提供程式{#adding-a-file-install-provider}

預設情況下，資料夾`crx-quickstart/install`為檔案而監視。
...
* 可在快速啟動之前建立`crx-quickstart/install`資料夾，並可將其他軟體包放在那裡。

>[!NOTE]
>
>如需範例，請參閱[如何在伺服器啟動時自動安裝CRX封裝。](https://helpx.adobe.com/experience-manager/kb/HowToInstallPackagesUsingRepositoryInstall.html)

## 將Adobe Experience Manager安裝並啟動為Windows服務{#installing-and-starting-adobe-experience-manager-as-a-windows-service}

>[!NOTE]
>
>以管理員身份登錄時，請務必執行以下過程，或使用&#x200B;**以管理員身份運行**&#x200B;上下文菜單選項啟動／運行這些步驟。
>
>以具有管理員權限的用戶身份登錄的&#x200B;**不足**。 如果完成這些步驟時您沒有以管理員身份登錄，則會收到&#x200B;**拒絕訪問**&#x200B;錯誤。
...
1. 為防止進程陷入多個進程，請增加最大堆大小和PermGen JVM參數。 找到`set jvm_options`命令並按如下方式設定值：
...
1. 開啟「命令提示」，將目前的目錄變更為AEM安裝的crx-quickstart/opt/helpers資料夾，然後輸入下列命令以建立服務：
   ...
   要驗證是否建立了服務，請在「管理工具」控制面板中開啟服務，或在「命令提示符」中鍵入`start services.msc`。 cq5服務會出現在清單中。
...
1. Windows表示服務正在運行。 AEM啟動，並且prunsrv可執行檔會出現在「任務管理器」中。 在您的網頁瀏覽器中，導覽至AEM，例如`https://localhost:4502`以開始使用AEM。
...
## Quickstart檔案{#further-options-available-from-the-quickstart-file}中提供的其他選項
...
在Amazon Elastic Compute Cloud(EC2)實例上安裝AEM時，如果您在EC2實例上同時安裝作者和發佈，則按照[安裝AEM Manager](#installinginstancesofaemmanager)實例上的步驟，可正確安裝作者實例；不過，「發佈」例項會變成「作者」。
...
* 請參閱[安全檢查清單](/help/sites-administering/security-checklist.md)以瞭解確保系統安全所需的任務。
* 檢視隨AEM WCM安裝的預設使用者和群組清單。 檢查您是否要對任何其他帳戶採取行動——如需詳細資訊，請參閱[安全性與使用者管理](/help/sites-administering/security.md)。
...
要開啟CRXDE Lite，可以從歡迎螢幕中選擇&#x200B;**CRXDE Lite** ，或使用瀏覽器導航到
...
若要存取Adobe CQ Web Console，您可從歡迎畫面中選取&#x200B;**OSGi Console**，或使用您的瀏覽器導覽至
