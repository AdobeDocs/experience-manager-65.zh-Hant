...
* `customBlobStore`：指出使用自訂資料存放區的布林值。 預設為 `false`。
...
您也可以使用，離線升級快取 `datastorecacheupgrade` oak-run的命令。 如需有關如何執行指令的詳細資訊，請核取 [讀我檔案](https://svn.apache.org/repos/asf/jackrabbit/oak/trunk/oak-run/README.md) 用於oak-run模組。
