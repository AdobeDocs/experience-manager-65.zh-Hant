...
您可以在公司環境的伺服器上安裝AEM。 典型安裝實例包括：開發、測試和發佈環境。 請參閱[快速入門](/help/sites-deploying/deploy.md#getting%20started)一節，以取得如何讓AEM軟體在本機安裝的基本詳細資訊。
...
在用戶端，AEM可與兩者上的所有現代瀏覽器(**Microsoft Edge**、**Internet Explorer** 11、**Chrome **51+** **、*Firefox **47+、**Safari** 8+)搭配使用和平板電腦作業系統。 如需詳細資訊，請參閱[支援的用戶端平台](/help/sites-deploying/technical-requirements.md#supported-client-platforms)。
