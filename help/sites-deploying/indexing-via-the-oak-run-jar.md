...
以下是可透過執行索引作業的使用案例清單 `oak-run` 工具。
...
[文字預先擷取](/help/sites-deploying/best-practices-for-queries-and-indexing.md#how-to-perform-text-pre-extraction) (AEM 6.3已有的功能)可用來縮短重新編列索引的時間。 文字預先擷取可用於所有重新索引方法。

根據 `oak-run.jar` 在下列圖表中，執行「重新索引」步驟的每一側都會有各種步驟。
...
這是在oak-run.jar的新索引功能推出之前使用的方法。 這是透過設定 `reindex=true` Oak索引的屬性。
