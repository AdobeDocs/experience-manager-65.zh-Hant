...
| 災難恢復計畫。 | 貴公司的災難恢復指導方針。 |  |
| 錯誤追蹤系統可用於報告問題。 | 例如，[bugzilla](https://www.bugzilla.org/)、[jira](https://www.atlassian.com/software/jira/)或其他許多項目之一。 |  |
...
>[!CAUTION]
>
>在實施生產實例的備份時，必須進行&#x200B;*測試，以確保能夠成功恢復備份。*
>
>如果沒有這一點，備份可能無用（最壞的情況）。

>[!NOTE]
>
>有關備份效能的詳細資訊，請閱讀[Backup Performance](/help/sites-deploying/configuring-performance.md#backup-performance)部分。
...
CRX文檔的[備份和還原](/help/sites-administering/backup-and-restore.md)部分涵蓋與CRX儲存庫備份相關的所有問題。

有關進行線上「熱」備份的完整詳細資訊，請參閱[建立線上備份](/help/sites-administering/backup-and-restore.md#online-backup)。
...
**清除版本**&#x200B;工具&#x200B;**[控制台**&#x200B;位於&#x200B;**版本控制**&#x200B;下，或直接位於：](/help/sites-administering/tools-consoles.md)

`https://<server>:<port>/etc/versioning/purge.html`

![screen_shot_2012-03-15at14418pm](assets/screen_shot_2012-03-15at14418pm.png)

**啟動** 路徑必須執行清除的絕對路徑。按一下儲存庫樹導航器可以選擇「開始路徑」。

**遞歸** 清除資料時，可通過選擇遞歸，在一個節點上或整個層次上執行操作。在最後一種情況下，給定路徑定義了層次的根節點。

**要保留的最** 大版本節點要保留的最大版本數。當這些數字超過此值時，會清除最舊的版本。

**最大版** 本時間節點版本的最大時間。當版本的年齡超過此值時，會清除它。

**乾** 式運行由於刪除內容版本是確定的，並且在不恢復備份的情況下無法還原，因此「清除版本」工具提供了乾式運行模式，允許您預覽已清除的版本。要啟動清除流程的乾式運行，請按一下「乾式運行」。

**清** 除啟動清除由開始路徑定義的節點上的版本。

### 清除網站{#purging-versions-of-a-web-site}的版本

要清除網站版本，請按如下步驟進行：

1. 導覽至&#x200B;**[工具](/help/sites-administering/tools-consoles.md)****控制台**，選擇&#x200B;**版本控制**&#x200B;並連按兩下清除版本。****
1. 設定要清除之內容的開始路徑(例如`/content/geometrixx-outdoors`)。

   * 如果只想清除路徑定義的節點，請取消選擇&#x200B;**Recursive**。
   * 如果要清除路徑及其子代定義的節點，請選擇&#x200B;**Recursive**。
...
* **[!DNL Shirts]**&#x200B;版本會因其版本年齡大於2天而被清除。
* **[!DNL Tonga Fashions!]**&#x200B;版本會因其版本數大於5而被清除。
...
此日誌僅在啟用時 [!DNL Dynamic Media] 使用。s7access日誌記錄對[!DNL Dynamic Media]通過`/is/image`和`/is/content`發出的每個請求。
   ...
   * `stderr.log`
保存在啟動期間生成的、嚴重性級別不同的錯誤消息。預設情況下，日誌級別設定為 
`Warning` ( `WARN`)
...
>[!NOTE]
>
>ImageServer和s7access記錄檔不包含在**下載完整**套件中，此套件是從**system/console/status-Bundlelist **頁面產生。 為了支援，如果您有[!DNL Dynamic Media]問題，請在聯絡客戶支援時附加ImageServer和s7access記錄檔。

### 激活DEBUG日誌級別{#activating-the-debug-log-level}

預設記錄層級([Apache Sling Logging Configuration](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingconfiguration))為「資訊」，因此不記錄除錯訊息。

要激活Logger的調試日誌級別，請將屬性`org.apache.sling.commons.log.level`設定為在資料檔案庫中調試。 例如，在`/libs/sling/config/org.apache.sling.commons.log.LogManager`上設定[全域Apache Sling Logging](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingconfiguration)。
...
1. 如果尚未存在，請為項目`/apps/<project-name>/config`建立新的配置資料夾(`sling:Folder`)。
1. 在`/apps/<project-name>/config`下，建立新[Apache Sling Logging Logger Configuration](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration)的節點：
...
1. 在此節點上設定以下屬性：
   ...
   * 視需要設定其他參數：
   ...
   >{5}記錄訊息
   >
   >如果日誌調用包含`Throwable` ，則stacktrace將附加到消息。
...
1. 只有在需要新寫入器時（即配置與預設寫入器不同），才需要此步驟。
   ...
   在`/apps/<project-name>/config`下，建立新[Apache Sling Logging Writer Configuration](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingwriterconfigurationfactoryconfiguration)的節點：

   * 名稱：`org.apache.sling.commons.log.LogManager.factory.writer-<identifier>`（因為這是寫入器）

      與Logger一樣，`<identifier>`會由您（必須）輸入以識別例項的自由文字取代（您無法忽略此資訊）。 例如， `org.apache.sling.commons.log.LogManager.factory.writer-MINE`
   ...
   * 視需要設定其他參數：
   ...
   >
   >* 可以用數字指定大小限制。 如果沒有提供大小指示器，則此指示器被用作位元組數，或者可以添加一個大小指示器- `KB`、`MB`或`GB`（忽略大小寫）。
   >* 時間／日期計畫可指定為`java.util.SimpleDateFormat`模式。 這定義了檔案旋轉的時段；也附加到旋轉檔案的字尾（用於識別）。
...
Felix Console也提供`../system/console/slinglog`;例如`https://localhost:4502/system/console/slinglog`。
...
您可以監視[複製隊列](/help/sites-deploying/replication.md)以檢測隊列何時關閉或被阻止——這反過來可能表示發佈實例或外部系統有問題：

* 所有必要的隊列都啟用了嗎？
* 是否仍然需要任何禁用的隊列？
* 所有`enabled`隊列應具有`idle`或`active`狀態，這表示正常操作；沒有隊列應該是`blocked` ，這通常是接收器端出現問題的一個標誌。
...
同樣地，您可以開發一個解決方案來檢測所有複製代理（位於`/etc/replication/author`或`/etc/replication/publish`下），然後檢查代理(`enabled`、`disabled`)和基礎隊列(`active`、`idle`、`blocked`)的狀態。

## 監視效能{#monitoring-performance}

[效能最](/help/sites-deploying/configuring-performance.md) 佳化是互動式程式，在開發過程中會受到重視。在部署後，通常會在特定間隔或事件後進行審查。

在收集資訊以進行優化時使用的方法也可以用於進行中的監控。

>[!NOTE]
>
>還可以檢查可用於提高效能的特定[配置。](/help/sites-deploying/configuring-performance.md#configuring-for-performance)
...
<table>
 <tbody>
  ...
  <tr>
   <td>系統呼叫</td>
   <td>識別時間問題。</td>
   <td><p>對<code>System.currentTimeMillis()</code>或<code>com.day.util</code>的呼叫。Timing會用來從您的程式碼產生時間戳記，或透過<a href="#html-comments">HTML-comments</a>產生時間戳記。</p> <p><strong>注意：</strong> 這些應該實作，以便視需要啟用／停用；當系統運行順利時，不需要收集統計資訊的開銷。</p> </td>
  </tr>
  <tr>
   <td>Apache Bench</td>
   <td>識別記憶體洩漏，選擇性分析回應時間。</td>
   <td><p>基本用法為：</p> <p><code>ab -k -n &lt;<em>requests</em>&gt; -c &lt;<em>concurrency</em>&gt; &lt;<em>url</em>&gt;</code></p> <p>如需詳細資訊，請參閱<a href="#apache-bench">Apache Bench</a>和<a href="https://httpd.apache.org/docs/2.2/programs/ab.html">ab手冊頁</a>。</p> </td>
  </tr>
  <tr>
   <td>搜尋分析</td>
   <td> </td>
  ...
  <tr>
   ...
   <td><p>若要查看頁面演算的時間統計資料，您可搭配使用<strong>Ctrl-Shift-U</strong>和URL中設定的<code>?debugClientLibs=true</code>。</p> </td>
  </tr>
  <tr>
...
我們建議將「慢速」頁面與`request.log`隔離，然後個別調整以取得更佳的效能。 這通常是透過為每個元件加入效能量度或使用效能分析工具（例如` [yourkit](https://www.yourkit.com/)`）來完成。
...
#### 使用request.log {#monitoring-search-response-times-with-the-request-log}監控搜索響應時間
...
例如，您可以執行它，指定`request.log`檔案為參數，並顯示持續時間最長的10個第一個請求：
...
如果您需要對大型資料示例執行此操作，則可能需要連接單個`request.log`檔案。

### Apache Bench {#apache-bench}

為了盡量降低特殊情況（如垃圾回收等）的影響，建議使用例如`apachebench`（如需進一步說明檔案，請參閱[ab](https://httpd.apache.org/docs/2.2/programs/ab.html)）的工具，以協助識別記憶體洩漏並選擇性地分析回應時間。
...
請求流量的相關資訊（特定時段內的請求數）會提供您例項負載的指示。 此資訊可從[request.log](#interpreting-the-request-log)擷取，不過使用計數器會自動收集資料，讓您看到：
...
建議每個項目都包含`html comments`以提高伺服器效能。 許多好的公開例子，選取頁面，開啟頁面來源以檢視並捲動至底部，可看到下列程式碼：
...
1. 開始AEM執行。
   ...
   >[!NOTE]
   >
   >如果使用Java 5，您可以將`-Dcom.sun.management.jmxremote`參數添加到啟動JVM的java命令行中。 JMX依預設會在Java 6中啟用。
...
1. 使用命令行：
   ...
   若要取得平均大小，請將全域大小除以`/var/dam`（上方取得）中的資產總數。
...
* 是否已配置[版本清除](/help/sites-deploying/version-purging.md)以及配置方式
* 知識庫：
