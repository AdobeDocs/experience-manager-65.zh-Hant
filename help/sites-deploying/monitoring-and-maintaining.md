...
# 監視和維護實AEM例{#monitoring-and-maintaining-your-aem-instance}

部署實AEM例後，將需要某些任務來監控和維護其操作、效能和完整性。

這裡的一個關鍵因素是，要識別潛在問題，您需要瞭解系統在正常情況下的外觀和行為。 這最好通過監測系統和收集一段時間的資訊來實現。

| 檢查 | 注意事項 | 注釋/操作 |
|---|---|---|
| 備份計畫。 |  | 瞭解如何 [備份實例](/help/sites-deploying/monitoring-and-maintaining.md#backups)。 |
| 災難恢復計畫。 | 您公司的災難恢復指導原則。 |  |
| 錯誤跟蹤系統可用於報告問題。 | 比如說， [布吉拉](https://www.bugzilla.org/)。 [吉拉](https://www.atlassian.com/software/jira/)或其他許多。 |  |
| 正在監視檔案系統。 | 如果可用磁碟空間不足， CRX儲存庫將「凍結」。 當空間可用時，它將恢復。 | &quot; `*ERROR* LowDiskSpaceBlocker`&quot;可用空間變低時，可以在日誌檔案中看到消息。 |
...
| [正AEM在監視效能](/help/sites-deploying/monitoring-and-maintaining.md#monitoring-performance)。 | 包括 [請求計數器](/help/sites-deploying/monitoring-and-maintaining.md#request-counters) 監視流量級別。 | 如出現重大或長期損失，應進行詳細調查。 |
...
您的公司可能會有一個備份策略，您需要遵循該策略，備份內容的其他考慮事項以及備份時間：

* 系統和資料有多重要。
* 對軟體或資料進行更改的頻率。
* 資料量；容量有時可能成為問題，執行備份所需的時間也是問題。
* 用戶線上時能否進行備份；如果可能，效能影響將如何。
* 用戶的地理分佈；例如，何時是備份的最佳時間（以最大限度地減少影響）?
* 您的災難恢復策略；有關備份資料必須儲存的位置（如非現場、特定介質等）的指導原則。

通常，完整備份是定期進行的（如每日、每週或每月），增量備份介於每小時、每日或每週之間。
...
>[!CAUTION]
>
>如果您正在操作第三方應用程式伺服器，則其他資料夾可能位於不同的位置，並且可能還需要備份。 請參閱 [如何使用應AEM用程式伺服器安裝](/help/sites-deploying/application-server-install.md) 有關安裝應用程式伺服器的資訊。
...
有關進行線上「熱」備份的完整詳細資訊，請參閱 [建立聯機備份](/help/sites-administering/backup-and-restore.md#online-backup)。
...
的 **清除版本** 工具在 **[工具](/help/sites-administering/tools-consoles.md) 控制台** 在 **版本控制** 或直接在以下地址：

`https://<server>:<port>/etc/versioning/purge.html`

![screen_shot_2012-03-15at14418pm](assets/screen_shot_2012-03-15at14418pm.png)

**起始路徑** 必須執行清除的絕對路徑。 按一下儲存庫樹導航器可以選擇「開始路徑」。

**遞歸** 清除資料時，您可以選擇在一個節點上或整個層次上執行該操作，方法是選擇「遞歸」。 在最後一種情況下，給定路徑定義層次的根節點。

**要保留的最大版本** 要為節點保留的最大版本數。 當這些數字超過此值時，將清除最舊的版本。

**最大版本期限** 節點版本的最大期限。 當版本的年齡超過此值時，它將被清除。

**乾跑** 由於刪除內容的版本是確定的，並且在不還原備份的情況下無法還原，因此「清除版本」工具提供了一種乾式運行模式，允許您預覽已清除的版本。 要啟動清除流程的乾式運行，請按一下「乾式運行」。

**清除** 啟動清除「啟動路徑」定義的節點上的版本。

### 清除網站的版本 {#purging-versions-of-a-web-site}

要清除網站的版本，請按如下步驟操作：

1. 導航到 **[工具](/help/sites-administering/tools-consoles.md)** **控制台**&#x200B;選中 **版本控制** 並按兩下 **清除版本。**
1. 設定要清除的內容的起始路徑(例如 `/content/geometrixx-outdoors`)。

   * 如果僅要清除路徑定義的節點，請取消選擇 **遞歸**。
   * 如果要清除由路徑定義的節點，並選擇其子體 **遞歸**。

1. 設定要保留的最大版本數（針對每個節點）。 留空以不使用此設定。

1. 設定要保留的最大版本年限（對於每個節點）。 留空以不使用此設定。

1. 按一下 **乾跑** 以預覽清除流程將執行的操作。
1. 按一下 **清除** 啟動進程。

>[!CAUTION]
>
>如果不還原儲存庫，則無法還原清除的節點。 您應該處理配置，因此我們建議您在清除之前始終執行乾式運行。

### 分析控制台 {#analyzing-the-console}

的 **乾跑** 和 **清除** 進程列出已處理的所有節點。 在此過程中，節點可以具有以下狀態之一：
...
* 的 **[!DNL Shirts]** 版本會被清除，因為其版本年齡超過2天。
...
審計記錄和與Adobe Experience Manager(AEM)有關的日誌檔案可在不同位置找到。 下面將概述您可以在何處找到的內容。
...
日誌檔案旋轉是指通過定期建立新檔案來限制檔案增長的過程。 在AEM中，名為 `error.log` 將根據給定的規則每天輪換一次：
...
>[!NOTE]
>
>如果升級AEM安裝，請注意，磁碟上將保留任何不再使用AEM的現有日誌檔案。 你可以不冒風險地移除它們。 所有新日誌條目都將寫入新日誌檔案中。
...
>[!NOTE]
>
>從**系統/控制台/狀態 — Bundlelist **頁生成的**下載完整**包中不包含ImageServer和s7訪問日誌。 為了支援目的，如果 [!DNL Dynamic Media] 問題，請在您與客戶支援聯繫時附加ImageServer和s7訪問日誌。
...
>[!CAUTION]
>
>不要將日誌保留在調試日誌級別的時間超過必要時間，因為它會生成大量日誌條目，因此會消耗資源。
...
1. 如果尚未存在，請建立新配置資料夾( `sling:Folder`) `/apps/<project-name>/config`。
1. 下 `/apps/<project-name>/config`，為新節點建立節點 [Apache Sling日誌記錄器配置](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration):
   ...
   * 名稱： `org.apache.sling.commons.log.LogManager.factory.config-<identifier>` （因為這是記錄程式）
...
1. 在此節點上設定以下屬性：
   ...
   * 根據需要配置其他參數：
   ...
   >{5}日誌消息
   >
   >如果日誌呼叫包括 `Throwable` stacktrace被附加到消息中。
...
1. 只有在需要新寫入程式（即配置與預設寫入程式不同）時，才需要執行此步驟。
   ...
   * 根據需要配置其他參數：
   ...
   >
   >以指示何時建立新檔案（以及根據名稱模式更名的現有檔案）。
   >
   >* 可以用數字指定大小限制。 如果未提供大小指示器，則將其作為位元組數，或者您可以添加一個大小指示器 —  `KB`。 `MB`或 `GB` （忽略案例）。
   >* 時間/日期計畫可以指定為 `java.util.SimpleDateFormat` 的上界。 這定義了檔案旋轉的時間段；也是附加到旋轉檔案（用於標識）的尾碼。

   >
   >預設值為「。」yyyy-MM-dd（用於日誌輪替）。
   >
   >因此，例如，在2010年1月20日午夜（或當此之後的第一條日誌消息準確無誤）,../logs/error.log將更名為../logs/error.log.2010-01-20。 1月21日的日誌記錄將輸出到（新的和空的）../logs/error.log，直到在下一天更改時滾動。
   >
   >| `'.'yyyy-MM` | 每月初的輪換 |
   >|---|---|
...
1. 使用所選工具讀取新日誌檔案。
   ...
   此示例建立的日誌檔案將 `../crx-quickstart/logs/myLogFile.log`。

Felix控制台還提供有關Sling Log Support的資訊，位於 `../system/console/slinglog`;例如 `https://localhost:4502/system/console/slinglog`。
...
1. 開啟頁面。
1. 在旁邊框中，可以選擇帶有鎖定表徵圖的頁籤，然後按兩下 **審核日誌……**
1. 將開啟一個新窗口，其中顯示當前頁面的審計記錄清單。
...
在 `/var/audit` 資料夾，根據資源保存審計記錄。 您可以向下鑽取，直到您看到各個記錄及其包含的資訊。
...
[效能優化](/help/sites-deploying/configuring-performance.md) 是一個在開發過程中接受焦點的交互過程。 部署後，通常在特定時間間隔或事件後對其進行審閱。
...
|  | 在伺服器上本地瀏覽的速度（相對而言）較快。 | 增加網路頻寬。 | 減少網頁的「重量」(例如減少影像、優化HTML)。 |
| Web伺服器 | Web伺服器上的CPU使用率很高。 | 將Web伺服器群集。 | 減少每頁的點擊量（訪問）。 |
|  |  | 使用硬體負載平衡器。 |  |
| 應用程式 | 伺服器CPU使用率高。 | 集AEM群實例。 | 搜索並消除CPU和記憶體掛起（使用代碼檢查、定時輸出等）。 |
|  | 記憶體消耗高。 |  | 改進所有級別的快取。 |
|  | 響應時間較短。 |  | 優化模板和元件（如結構、邏輯）。 |
| 存放庫 |  |  |  |
| 快取 |  |  |  |

效能問題可能源於與您的網站無關的許多原因，包括連接速度、CPU負載等方面的暫時減慢。

它還可能影響您的所有訪問者，或僅影響其中的一部分。

所有這些資訊都需要獲得、分類和分析，然後才能優化總體效能或解決特定問題。
...
* 遇到效能問題時：
   ...
   * 嘗試使用一個（或最好是多個）標準Web瀏覽器在您知道具有良好一般效能和/或在伺服器本身（如果可能）上的另一個客戶端上複製它
...
<table>
 <tbody>
  ...
  <tr>
   <td>桁架/層</td>
   <td>頁面載入</td>
   <td><p>用於跟蹤系統調用和信號的Unix/Linux命令。 將日誌級別提高到 <code>INFO</code>。</p> <p>分析每個請求的頁載入數，以及哪些頁等。</p> </td>
  </tr>
  <tr>
   <td>線程轉儲</td>
   <td>觀察JVM線程。 識別爭執、鎖和長跑者。</td>
   <td><p>取決於作業系統：<br /> - Unix/Linux: <code>kill -QUIT &lt;<em>pid</em>&gt;</code><br /> - Windows（控制台模式）:Ctrl-Break<br /> </p> <p>還提供分析工具，如 <a href="https://java.net/projects/tda/">TDA</a>。<br /> </p> </td>
  </tr>
  <tr>
   <td>堆轉儲</td>
   <td>記憶體不足，導致效能下降。</td>
   <td><p>添加：<br /> <code>-XX:+HeapDumpOnOutOfMemoryError</code><br /> 選項AEM。</p> <p>查看 <a href="https://java.sun.com/javase/6/webnotes/trouble/TSG-VM/html/clopts.html#gbzrr">Java SE 6與HotSpot VM的故障排除指南</a>。</p> </td>
  </tr>
  <tr>
   <td>系統調用</td>
   <td>確定時間問題。</td>
   <td><p>呼叫 <code>System.currentTimeMillis()</code> 或 <code>com.day.util</code>.Timing用於根據代碼或通過 <a href="#html-comments">HTML注釋</a>。</p> <p><strong>注：</strong> 應實施這些措施，以便根據需要激活/停用它們；當系統運行順利時，不需要收集統計資訊的開銷。</p> </td>
  </tr>
  <tr>
   <td>阿帕奇長座</td>
   <td>識別記憶體洩漏，有選擇地分析響應時間。</td>
   <td><p>基本用法為：</p> <p><code>ab -k -n &lt;<em>requests</em>&gt; -c &lt;<em>concurrency</em>&gt; &lt;<em>url</em>&gt;</code></p> <p>請參閱 <a href="#apache-bench">阿帕奇長座</a> 和 <a href="https://httpd.apache.org/docs/2.2/programs/ab.html">ab手冊頁</a> 的雙曲餘切值。</p> </td>
  </tr>
  <tr>
   <td>搜索分析</td>
   <td> </td>
   <td>離線執行搜索查詢，確定查詢的響應時間，test和確認結果集。<br /> </td>
  </tr>
  <tr>
   <td>JMeter</td>
   <td>載入和功能test。</td>
  ...
  <tr>
   <td>JConsole</td>
   <td>觀察JVM度量和線程。</td>
   <td><p>用法：jconsole</p> <p>請參閱 <a href="https://java.sun.com/developer/technicalArticles/J2SE/jconsole.html">jconsole</a> 和 <a href="#monitoring-performance-using-jconsole">使用JConsole監視效能</a>。</p> <p><strong>注：</strong> 在JDK 1.6中，JConsole可通過插件進行擴展；例如，Top或TDA（線程轉儲分析器）。</p> </td>
  </tr>
  <tr>
   <td>Java VisualVM</td>
   <td>觀察JVM度量、線程、記憶體和效能分析。</td>
   <td><p>用法：jvisualvm或visualvm<br /> </p> <p>請參閱 <a href="https://java.sun.com/javase/6/docs/technotes/tools/share/jvisualvm.html">jvisualvm</a>。 <a href="https://visualvm.dev.java.net/">可視化</a> 和 <a href="#monitoring-performance-using-j-visualvm">使用(J)VisualVM監視效能</a>。</p> <p><strong>注：</strong> 使用JDK 1.6,VisualVM可通過插件進行擴展。</p> </td>
  </tr>
  <tr>
   <td>桁架/層，l</td>
  ...
  <tr>
   ...
   <td>比如說， <a href="https://www.yourkit.com/">您的套件</a>。</td>
  </tr>
  <tr>
   <td><a href="#information-collection">資訊收集</a></td>
   <td>安裝的當前狀態。</td>
   <td>盡可能瞭解您的安裝情況，還有助於您瞭解哪些因素可能導致效能變化，以及這些更改是否合理。 需要定期收集這些指標，以便您能夠輕鬆地看到重大變化。</td>
  </tr>
 </tbody>
</table>
...
的 `request.log` 提供了一種內置的方法，可查看請求需要多長時間。 就發展而言， `tail -f` 這樣 `request.log` 注意反應緩慢。 要分析 `request.log` 我們推薦 [使用 `rlog.jar` 允許您對響應時間進行排序和篩選](#using-rlog-jar-to-find-requests-with-long-duration-times)。

建議將「慢」頁面與 `request.log`，然後單獨調整它們以獲得更好的效能。 通常通過包括每個元件的效能度量或使用效能分析工具(如 ` [yourkit](https://www.yourkit.com/)`。
...
通過在特定時段內（例如在不同的24小時時段內）總計所有GET條目，您可以就網站上的平均通信量發表聲明。
...
* 發出每個請求或響應的日期。
* 請求數（方括弧）。 此數字與請求和響應匹配。
* 一個箭頭，指示這是請求（指向右側的箭頭）還是響應（向左的箭頭）。
* 對於請求，行包含：
...
使用小指令碼，可以從日誌檔案中提取所需資訊並匯編所需的統計資訊。 從這些頁面中，您可以看到哪些頁面或類型的頁面速度較慢，以及整體效能是否令人滿意。
...
但是，一旦確定了響應時間，您可能需要分析為什麼請求佔用它所佔用的時間，以及可以採取哪些措施來改進響應。
...
* 監視在特定時間範圍內發出的請求數，例如1分鐘
* test特定數量的用戶在同一時間（盡可能接近）提出相同請求的影響；例如，30個用戶按一下 **保存** 同時。
...
其中一個， `rlog.jar`，可用於快速排序 `request.log` 以便按持續時間（從最長到最短）顯示請求。
...
例如，可以運行它，指定 `request.log` 檔案作為參數，並顯示持續時間最長的10個首次請求：
...
您可能需要將單個 `request.log` 檔案。

### 阿帕奇長座 {#apache-bench}

為盡量減少特殊情況（如垃圾收集等）的影響，建議使用工具，如 `apachebench` (請參見， [ab](https://httpd.apache.org/docs/2.2/programs/ab.html) 以幫助識別記憶體洩漏並有選擇地分析響應時間。
...
上述數字取自訪問geometrixx公司頁面的標準MAcBook Pro筆記型電腦（2010年年中），包括在預設安AEM裝中。 該頁非常簡單，但未針對效能進行優化。

`apachebench` 還顯示所有併發請求中每個請求的平均時間；見 `Time per request: 54.595 [ms]` （指所有併發請求）。 可以更改併發參數的值 `-c` （一次要執行的多個請求數）以查看任何效果。

### 請求計數器 {#request-counters}

有關請求通信量（特定時間段內的請求數）的資訊會指示實例上的負載。 可以從 [請求日誌](#interpreting-the-request-log)，但使用計數器將自動收集資料，以便您查看：

* 活動差異顯著（即「多請求」和「低活動」的區別）
* 實例時
* 任何重新啟動（計數器重置為0）

要自動收集資訊，您還可以安裝RequestFilter以在每個請求上增加計數器。 多個計數器可用於不同的時間段。
...
建議每個項目都包括 `html comments` 伺服器效能。 許多好的公眾例子，選擇一個頁面，開啟頁面源進行查看並滾動到底部，可以看到以下代碼：
...
1. 從 `Local` 應用程式，按兩下 `com.day.crx.quickstart.Main`;概述將顯示為預設值：
...
自JDK 1.6以來，工具命令 `jvisualvm` 的子菜單。 安裝JDK 1.6後，可以：

1. 啟動AEM實例。

   >[!NOTE]
   >
   >如果使用Java 5，則可以 `-Dcom.sun.management.jmxremote` 用於啟動JVM的java命令行的參數。 JMX在Java 6中按預設啟用。

1. 運行其中之一：

   * `jvisualvm`:在JDK 1.6 bin資料夾中（測試版）
   * `visualvm`:可從 [VisualVM](https://visualvm.dev.java.net/) （出血邊緣版本）
...
盡可能瞭解您的安裝，有助於您瞭解哪些因素可能導致效能變化，以及這些更改是否合理。 需要定期收集這些指標，以便您能夠輕鬆地看到重大變化。
...
查看自伺服器安裝以來使用儲存庫查詢的頁面激活總數；通過CRXDE — 工具 — 查詢：
...
要確定自安裝以來使用儲存庫查詢的總部署數；通過CRXDE — 工具 — 查詢：
...
>* [使用內置Profiler分析](https://helpx.adobe.com/experience-manager/kb/AnalyzeUsingBuiltInProfiler.html)
...
如果系統記憶體不足，可以通過各種方式來查看此問題，包括效能下降和錯誤消息，包括子文本：
...
如果系統的磁碟空間不足，或者您注意到磁碟出現問題，請參閱：

* 是否已禁用調試資訊的收集；可在不同位置配置，包括：

   * [Apache Sling JSP指令碼處理程式](/help/sites-deploying/osgi-configuration-settings.md#apacheslingjspscripthandler)
   * [Apache Sling Java指令碼處理程式](/help/sites-deploying/osgi-configuration-settings.md#apacheslingjavascripthandler)
...
* 知識庫：
   ...
   * [開啟的檔案過多](https://helpx.adobe.com/experience-manager/kb/TooManyOpenFiles.html)
...
如果您看到實例的效能在每次重新引導後（有時是一週或更晚）有所惡化，則可以檢查以下內容：
...
Java虛擬機(JVM)在優化方面已有顯著改進（尤其是自Java 7以來）。 因此，指定合理的固定JVM大小和使用預設值通常是合適的。

如果預設設定不合適，則在嘗試調整JVM之前，必須建立一種方法來監視和評估GC效能；這可能涉及監控因素，包括堆大小、算法等。
...
這將幫助您瞭解正在使用多少記憶體、使用了哪些GC算法、它們需要多長時間才能運行，以及這對應用程式效能有何影響。 如果沒有這一點，調音就只是「隨機擺弄的旋鈕」。
