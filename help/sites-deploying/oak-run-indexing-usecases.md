...
通過`--index-consistency-check`操作可以檢查損壞的索引：

```shell
java -jar oak-run*.jar index --fds-path=/path/to/datastore  /path/to/segmentstore/ --index-consistency-check
```

這會在`indexing-result/index-consistency-check-report.txt`中產生報表。 請參閱以下範例報表：
...
報告將生成於`indexing-result/index-info.txt`和`indexing-result/index-definitions.json`中
...
根據[藍本](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)，在某些情況下需要執行重新索引。 當前，通過通過CRXDE或通過「索引管理器」用戶介面將`reindex`標誌設定為索引定義節點中的`true` ，即可完成重新索引。 設定旗標後，會以非同步方式重新建立索引。

關於重新編製索引，有些地方需要注意：

* 在`DocumentNodeStore`設定中，與所有內容都是本機的`SegmentNodeStore`設定相比，重新建立索引的速度要慢得多；
...
* 對於`DocumentNodeStore`安裝，重新編製索引可能需要相當長的時間，如果到Mongo資料庫的連接在操作過程中失敗，則必須從頭開始編製索引；
...
* **非通道重新索引** -重新索引可在不影響索引編製操作的情況下進行。這表示非同步索引器可以繼續為其他索引建立索引；

* **簡化DocumentNodeStore安裝的重新索引** -對於 `DocumentNodeStore` 安裝，只需使用單一命令即可重新建立索引，以確保以最佳的方式重新建立索引；
...
遵循建立的方式，透過設定`reindex`標幟來重新建立索引。

#### Online Reindex - SegmentNodeStore - AEM Instance is Running {#onlinereindexsegmentnodestoretheaeminstanceisrunning}
...
1. 步驟文字
1. 將`oak-run`連線至AEM在唯讀模式中使用的相同儲存庫，並執行索引。 如何達成此目的的範例：

   ```shell
   java -jar oak-run-1.7.6.jar index --fds-path=/Users/dhasler/dev/cq/quickstart/target/crx-quickstart/repository/datastore/ --checkpoint 26b7da38-a699-45b2-82fb-73aa2f9af0e2 --reindex --index-paths=/oak:index/lucene /Users/dhasler/dev/cq/quickstart/target/crx-quickstart/repository/segmentstore/
   ```

1. 最後，透過`IndexerMBean#importIndex`操作，從oak-run在執行上述命令後儲存索引檔案的路徑匯入建立的索引檔案。
...
1. 透過JMX作業建立查核點。 您可以前往[JMX Console](/help/sites-administering/jmx-console.md)並搜尋`CheckpointManager`來執行此動作。 然後，按一下使用高值（以秒為單位）的&#x200B;**createCheckpoint(long p1)**&#x200B;操作，即過期時間（例如&#x200B;**2592000**）。
...
在此使用案例中，假定資料儲存可以在其它實例上訪問，如果`FileDataStore`放置在基於雲的儲存解決方案（如EBS）上，則可能無法訪問該實例。 這會排除`FileDataStore`也克隆的藍本。 如果索引定義不執行全文索引，則不需要訪問`DataStore`。

## 使用案例4 —— 更新索引定義{#usecase4updatingindexdefinitions}

目前，您可以通過[ACS Ensure Index](https://adobe-consulting-services.github.io/acs-aem-commons/features/ensure-oak-index/index.html)包發送索引定義更改。 這允許通過內容包發送索引定義，後續需要通過將`reindex`標誌設定為`true`來執行重新索引。
...
1. 開發人員會更新本機例項上的索引定義，然後透過`--index-definitions`選項產生索引定義JSON檔案
