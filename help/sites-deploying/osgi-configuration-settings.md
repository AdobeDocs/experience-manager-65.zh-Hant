...
* **重寫配置清單** 指定內容連結檢查程式設定的位置清單。 設定可以根據執行模式。 由於連結檢查程式的設定可能有所不同，因此，要區分製作環境和發佈環境，這個事實很重要。
...
* 安全地存取CDN中快取的資源。 在CDN中快取資源後，請求不會前往AEM，而所有可存取該資源的使用者都應從CDN提供。
