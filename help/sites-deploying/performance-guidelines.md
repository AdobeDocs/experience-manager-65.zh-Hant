...
此頁提供有關如何優化部署效能的一般指AEM南。 如果您是新入職者AEM，請在開始閱讀效能指南之前查看以下頁面：
...
* **首次部署**:在首次計畫部署AEM Sites或資產時，瞭解可用選項非常重要。 尤其是配置微內核、節點儲存和資料儲存時（與預設設定相比）。 例如，將TarMK的資料儲存的預設設定更改為檔案資料儲存。
* **升級到新版本**:升級到新版本時，瞭解與運行環境相比的效能差異非常重要。 例如，從AEM6.1升級到6.2，或AEM從6.0 CRX2升級到6.2 OAK。
* **響應時間慢**:當選定的Nodestore體系結構不滿足您的要求時，瞭解與其他拓撲選項相比的效能差異非常重要。 例如，部署TarMK而不是MongoMK，或使用檔案資料儲存而不是AmazonS3或Microsoft® Azure資料儲存。
* **添加更多作者**:當建議的TarMK拓撲不滿足效能要求且Author節點的升級已達到可用的最大容量時，瞭解效能差異。 比較一下，將MongoMK與三個或更多Author節點一起使用。 例如，部署MongoMK而不是TarMK。
* **添加更多內容**:當建議的資料儲存體系結構不滿足您的要求時，瞭解與其他資料儲存選項相比的效能差異非常重要。 示例：使用AmazonS3或Microsoft® Azure資料儲存而不是檔案資料儲存。
...
Micro Kernels充當中的持久性管理AEM器。 使用的微內核有三種類型AEM:TarMK、MongoDB和關係資料庫（受限制支援）。 根據實例的用途和您考慮的部署類型選擇適合您需要的部署類型。 有關微內核的其他資訊，請參見 [建議的部署](/help/sites-deploying/recommended-deploys.md) 的子菜單。
...
>[!CAUTION]
>
>關係資料庫微內核受到限制支援。 聯繫人 [Adobe客戶關懷](https://experienceleague.adobe.com/?support-solution=General&amp;support-tab=home#support) 之後再使用此類型的微內核。

![chlimage_1-3](assets/chlimage_1-3a.png)

### 資料儲存 {#data-store}

在處理大量二進位檔案時，建議使用外部資料儲存而不是預設節點儲存來最大化效能。 例如，如果您的項目需要許多媒體資產，則將它們儲存在檔案或Azure/S3資料儲存下比直接儲存在MongoDB中更快地訪問它們。

有關可用配置選項的詳細資訊，請參見 [配置節點和資料儲存](/help/sites-deploying/data-store-config.md)。

>[!NOTE]
>
>Adobe建議您選擇在Azure或AEMAmazon Web Services(AWS)上使用Adobe托管服務部署的選項。 客戶可從團隊中獲益，該團隊具備在這些雲計算環境中部署和AEM操作的經驗和技能。 請參閱 [有關Adobe Managed Services的其他文檔](https://business.adobe.com/products/experience-manager/managed-services.html?aemClk=t)。
>
>有關如何在Azure或AEMAWS上部署Adobe托管服務之外的建議，Adobe建議直接與雲提供程式協作。 或者，與Adobe的一個合作夥伴合作，支援在您AEM選擇的雲環境中部署。 選定的雲提供商或合作夥伴負責規模規格、設計和實施他們支援的體系結構，以滿足您的特定效能、負載、可擴充性和安全性要求。
>另請參閱 [技術要求](/help/sites-deploying/technical-requirements.md#supported-platforms) 的子菜單。
>
>
>
### 搜尋 {#search-features}
...
>[!NOTE]
對於大多數部署，Adobe建議使用Lucene索引。 僅將Solr用於專業化和複雜部署的可擴充性。

![chlimage_1-4](assets/chlimage_1-4a.png)

### 開發指導方針 {#development-guidelines}

為AEM目標 **效能和可擴充性**。 以下是您可以遵循的最佳做法：
...
* 不更改/libs，而是使用重疊
* 不要盡可能使用查詢
* 不要使用Sling Bindings在Java™代碼中獲取OSGi服務，而是使用：
...
>[!NOTE]
下面介紹的最低體系結構指南針對生產環境和高流量站點。 這些准則 **不** 這樣 [最小規格](/help/sites-deploying/technical-requirements.md#prerequisites) 跑AEM。
...
為獲得良好效能，您應遵循下面介紹的設定准則。 有關如何更改設定的說明， [查看此頁](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/configuring-performance.html?lang=zh-Hant)。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>要防止擴展查詢使系統過載，請在啟動指令碼中添AEM加這些JVM參數。</td>
  </tr>
  <tr>
   <td>Lucene索引配置</td>
...
選擇MongoMK持久性後端(TarMK)的主要原因是橫向縮放實例。 此能力意味著有兩個或兩個以上活動的作者實例始終運行並使用MongoDB作為持久性儲存系統。 需要運行多個作者實例通常是因為單個伺服器的CPU和記憶體容量（支援所有併發創作活動）不再可持續。
...
>[!NOTE]
在生產環境中， MongoDB始終用作具有主節點和兩個輔助節點的複製副本集。 讀取和寫入操作將轉至主資料庫，讀取操作可轉至輔助資料庫。 如果儲存不可用，可以將其中一個輔助節點替換為仲裁伺服器，但MongoDB複製副本集必須始終由奇數個實例組成。
...
在兩者之間選擇時，要考慮的基本規則是TarMK是為效能而設計的，而MongoMK是為可擴充性而設計的。 Adobe建議將TarMK作為所有部署方案中客戶使用的預設持久性技術，適用於AEM作者和發佈實例。

選擇MongoMK持久性後端(TarMK)的主要原因是橫向縮放實例。 此功能意味著有兩個或兩個以上活動的作者實例始終運行，並將MongoDB用作持久性儲存系統。 需要運行多個作者實例通常是因為單個伺服器的CPU和記憶體容量（支援所有併發創作活動）不再可持續。
...
* **TarMK和檔案資料儲存**  — 為大多數客戶推薦的體系結構：

   * 最小拓撲：一個作者實例、兩個發佈實例、兩個調度程式
   * 如果共用檔案資料儲存區，則啟用無二進位複製

* **MongoMK和檔案資料儲存**  — 推薦的體系結構，用於作者層的橫向可擴充性：

   * 最小拓撲：三個作者實例、三個MongoDB實例、兩個發佈實例、兩個調度程式
   * 如果共用檔案資料儲存區，則啟用無二進位複製

* **諾德斯托雷**  — 儲存在本地磁碟上，而不是網路連接儲存(NAS)
* 使用時 **AmazonS3**:
...
* **定制工作流可以顯著提高效能**  — 刪除「更新資產」工作流中的視頻步驟，禁用未使用的監聽程式等。
