...
在[就地升級後](/help/sites-deploying/in-place-upgrade.md)應執行下列活動以完成升級。 假設AEM已從6.5 jar啟動，且已部署升級的程式碼基底。
...
在使用目標版本jar啟動AEM期間和之後，應仔細檢閱error.log。 應審核任何警告或錯誤。 一般而言，最好在記錄的開頭尋找問題。 記錄中稍後發生的錯誤實際上可能是在檔案早期調用的根本原因的副作用。 如果出現重複的錯誤和警告，請參閱下面的[分析升級的問題。](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md#analyzing-issues-with-the-upgrade)
...
AEM中的數項功能需要進行升級後執行其他步驟。 您可以在[升級代碼和自訂](/help/sites-deploying/upgrading-code-and-customizations.md)頁面上找到這些功能的完整清單以及在AEM 6.5中移轉這些功能的步驟。
...
發佈環境完全升級和驗證後，請在製作環境上啟用復寫代理。 驗證代理程式是否能連線至個別的發佈執行個體。 有關事件順序的詳細資訊，請參閱U [升級過程](/help/sites-deploying/upgrade-procedure.md)。
...
建議您將最新AEM 6.5執行個體的套件組合清單與升級的執行個體進行比較，以偵測未升級的套件組合。 這將提供`error.log`中要搜尋內容的更詳細範圍。
...
### /etc {#missing-customizations-under-etc}下缺少自定義項
