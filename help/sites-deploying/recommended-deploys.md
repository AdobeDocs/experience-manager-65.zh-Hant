...
>[!NOTE]
>
>本頁面說明AEM的建議拓撲。 如需叢集功能及如何設定的詳細資訊，請參閱[Apache Sling Discovery API檔案](https://sling.apache.org/documentation/bundles/discovery-api-and-impl.html)。
...
>[!NOTE]
>
>如需如何使用TarMK冷備配置AEM的詳細資訊，請參閱[this](/help/sites-deploying/tarmk-cold-standby.md)文章。

>[!NOTE]
>
>此TarMK示例中的冷備用部署要求主實例和備用實例都分別獲得許可，因為對故障轉移伺服器的複製是持續的。 有關許可的詳細資訊，請參閱[Adobe一般許可條款](https://www.adobe.com/legal/terms/enterprise-licensing.html)。

### TarMK伺服器場{#tarmk-farm}
...
若為AEM Communities，則不會復寫使用者產生的內容(UGC)。 如需在TarMK伺服器陣列上支援UGC，請參閱AEM Communities](#considerations-for-aem-communities)的[考量事項。
...
### 具有MongoMK故障轉移的Oak群集，在單個資料中心{#oak-cluster-with-mongomk-failover-for-high-availability-in-a-single-datacenter}中實現高可用性
...
### 具有跨多個資料中心進行MongoMK故障轉移的Oak群集{#oak-cluster-with-mongomk-failover-across-multiple-datacenters}
...
>[!NOTE]
>
>在上圖中，假設資料中心2的AEM伺服器與資料中心1的MongoDB主節點之間的網路延遲高於[此處](/help/sites-deploying/aem-with-mongodb.md#checklists)記錄的要求，AEM Server 3和AEM Server 4會呈現非作用中狀態。 如果最大延遲與要求相容（例如通過使用可用區），則資料中心2中的AEM伺服器也可以處於活動狀態，從而跨多個資料中心建立活動 — 活動的AEM群集。

>[!NOTE]
>
>有關本節中描述的MongoDB體系結構概念的其他資訊，請參見[MongoDB Replication](https://docs.mongodb.org/manual/replication/)。

## 微內核：其中一個使用{#microkernels-which-one-to-use}
...
### 在製作執行個體{#exceptions-for-choosing-aem-mongomk-over-tarmk-on-author-instances}上選擇AEM MongoMK而非TarMK的例外
...
>[!NOTE]
>
>在部署的硬體配置中，可以使用艱苦的日子來評估客戶應用程式的效能。 [此處](/help/sites-developing/tough-day.md)提供有關此工具的詳細資訊。
...
如果上述條件在部署後的前18個月內未達成，建議您先使用TarMK部署AEM，然後在稍後適用上述條件時重新評估您的設定，最後決定是要保留在TarMK上，還是要移轉至MongoMK。

### 在發佈執行個體{#exceptions-for-choosing-aem-mongomk-over-tarmk-on-publish-instances}上選擇AEM MongoMK而不選擇TarMK的例外
...
若發佈層級上有多個發佈者，則不適用於在MongoMK叢集上使用AEM Communities。 若選擇JSRP（請參閱[社群內容儲存](/help/communities/working-with-srp.md)），則MongoMK叢集即適用，無論選擇什麼MK，例如MongoDB或RDB，任何發佈端叢集皆適用。

### 使用MongoMK {#prerequisites-and-recommendations-when-deploying-aem-with-mongomk}部署AEM時的必要條件和Recommendations
...
* 請參閱Adobe Experience Manager適用的MongoDB [article](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager);
* 查看MongoDB生產[檢查清單](https://docs.mongodb.org/manual/administration/production-checklist/);
* 參加線上[此處](https://university.mongodb.com/)提供的MongoDB認證課程。
...
對於計畫部署[AEM Communities](/help/communities/overview.md)的網站，建議選擇[最佳化部署](/help/communities/working-with-srp.md#characteristicsofstorageoptions)，以處理社群成員在發佈環境中張貼的UGC。

借由使用[公用存放區](/help/communities/working-with-srp.md),UGC不需要在製作執行個體與其他發佈執行個體之間複製，即可取得UGC的一致檢視。

以下是一組決策矩陣，可幫助您為部署選擇最佳類型的持久性：

#### 選擇製作實例{#choosing-the-deployment-type-for-author-instances}的部署類型
