---
title: 技術需求
description: Adobe Experience Manager支援的用戶端和伺服器平台清單。
topic-tags: platform
...
Adobe在平台上支援(AEM)Adobe Experience Manager，如本檔案的下列資訊所述。

如有任何與平台相關的問題，請聯絡平台廠商。
...
* 已安裝Java™平台、標準版JDK或其他支援的 [Java™虛擬機](#java-virtual-machines)
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>為確保Adobe項目成功，客戶在受限支援計畫中提供全面支援，這要求滿足特定條件。 R級支援需要正式的客戶請求和按Adobe確認。 如需詳細資訊，請聯絡Adobe客戶服務。</td>
  </tr>
 </tbody>
</table>
...
| 支援程度 | 說明 |
|---|---|
| **Z:不支援** | 不支援配置。 Adobe不會針對設定是否有效進行陳述，也不支援。 |

## 支援平台 {#supported-platforms}

### Java™虛擬機 {#java-virtual-machines}

應用程式需要Java™虛擬機才能運行，該虛擬機由Java™開發套件(JDK)分發提供。

Adobe Experience Manager使用下列版本的Java™虛擬機：

>[!CAUTION]
>
>追蹤來自Java™廠商的安全性佈告欄。 這樣可確保生產環境的安全。 此外，請務必安裝最新的Java™更新。
...
1. 「檔案系統」包括符合POSIX的塊儲存。 包括網路儲存技術。 請注意，檔案系統效能可能會有所不同，並影響整體效能。 使用網路/遠程檔案系統載入測試AEM。
...
>[!NOTE]
MongoDB是協力廠商軟體，不包含在AEM授權套件中。 如需詳細資訊，請參閱 [MongoDB許可策略](https://www.mongodb.com/community/licensing) 頁面。
...
如果您想要在MongoDB上同時執行製作和發佈，則必須購買兩個不同的授權。
Adobe客戶服務可協助確認與搭配AEM使用MongoDB相關的問題。
...
要使用支援的關係資料庫運行AEM 6.5，需要與資料庫供應商單獨簽訂支援合同。 Adobe客戶服務可協助您針對AEM 6.5使用關係資料庫的相關資格問題。
...
| IBM® WebSphere® Application Server Continuous Delivery(LibertyProfile)，帶Web配置檔案7.0和IBM® JRE 1.8 | R:對新合同的限制支援 `[2]` |
...
| JBoss® EAP 7.1.4與JBoss®應用程式伺服器 | R:對新合同的限制支援 `[1]` `[2]` |
| JBoss® EAP 7.0.x(含JBoss®應用程式伺服器) | Z:不支援 |

1. 建議用於部署AEM Forms。
1. 在應用程式伺服器上啟動AEM 6.5部署會移至「限制支援」。 現有客戶可升級至AEM 6.5，並繼續使用應用程式伺服器。 對於新客戶，它附帶了支援標準和支援計畫，如上面的R級描述所述。
...
1. Linux® Kernel 2.6、3。 x, 4。 x和5。 x包括Red Hat®發佈的衍生產品，包括Red Hat® Enterprise Linux®、CentOS、OracleLinux®和Amazon Linux®。 AEM Forms附加元件功能僅在CentOS 7、Red Hat® Enterprise Linux® 7、Red Hat® Enterprise Linux® 8和Red Hat® Enterprise Linux® 9上受支援。
1. AEM Forms在Ubuntu 20.04 LTS上受支援。
1. Adobe Managed Services支援的Linux®發佈。
1. Microsoft® Windows生產部署支援供升級至6.5的客戶使用，以及非生產用途。 AEM Sites和Assets的新部署可應要求提供。
1. AEM Forms在Microsoft® Window Server上不受支援級別R限制而受支援。

>[!NOTE]
如果要安裝AEM Forms 6.5，請確定您已安裝下列32位元Microsoft® Visual C++可重新分發。
...
支援在雲端運算環境的虛擬機中執行Adobe Experience Manager。 這些環境包括Microsoft® Azure和Amazon Web Services(AWS)，執行時符合本頁所列的技術需求，並符合Adobe的標準支援條款。
...
Adobe也提供Adobe Managed Services，以在Azure或AWS上部署AEM。 Adobe Managed Services為專家提供在這些雲端運算環境中部署和操作AEM的經驗和技能。 請參閱 [Adobe Managed Services的其他檔案](https://business.adobe.com/products/experience-manager/managed-services.html?aemClk=t).

在將AEM部署在Azure或AWS或任何其他雲端計算環境的所有其他情況下，虛擬計算環境均包含來自Adobe的支援。 該虛擬環境必須符合本頁所列的技術規範運行。 與在任何這些雲端環境中執行的AEM相關的任何回報問題，都必須可與雲端運算環境專用的任何雲端服務獨立重現。 也就是說，除非雲端服務受本頁面所列技術需求的支援，例如Azure Blob儲存或AWS S3。

如需有關如何在Azure或AWS上部署AEM的建議，請在Adobe Managed Services外部，建議您直接與雲端提供者合作。 或者，與Adobe合作夥伴合作，支援在您所選擇的雲端環境中部署AEM。 所選雲端提供商或合作夥伴負責架構的規模規格、設計和實作，以符合您的特定效能、負載、可擴充性及安全性需求。

### Dispatcher平台（Web伺服器） {#dispatcher-platforms-web-servers}

Dispatcher是快取和負載平衡元件。 [下載最新的Dispatcher版本](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/getting-started/release-notes.html?lang=en). Experience Manager6.5需使用Dispatcher 4.3.2版或更新版本。
...
1. 以Apache httpd原始碼為基礎的Web伺服器，其支援度與其所依據的httpd版本相同。 如果有疑問，請Adobe確認與相應伺服器產品相關的支援級別。 下列案例：
...
1. Firefox延伸支援版本 [進一步了解mozilla.org](https://www.mozilla.org/en-US/firefox/enterprise/)
1. 支援Apple iPad
...
將Microsoft® Windows 7+連線至未透過SSL保護的AEM執行個體時，必須在Windows中啟用不安全網路上的基本驗證。 它需要在WebClient的Windows註冊表中進行更改：
...
操作無縫，因為不需要特殊配置。 如有必要，可使用適合您網路類型的格式來指定IP地址。

必須指定IP位址時，您可以選取（視需要）下列項目：
...
* 預設大小寫為 `localhost` 會解釋為IPv4和IPv6網路安裝。 例如 `https://localhost:4502`
...
執行Dynamic Media - Scene7模式的Dynamic Media客戶(即 **dynamicmedia_scene7** 運行模式)，則沒有其他系統要求；只有與AEM相同的系統需求。 Dynamic Media - Scene7模式架構使用雲端型影像服務，而非AEM中內嵌的服務。

#### 硬體 {#hardware}

下列硬體要求適用於Linux®和Windows:

* 至少四核的英特爾至強®或AMD®皓龍CPU
* 至少16 GB的RAM

#### Linux® {#linux}

如果您在Linux®上使用Dynamic Media，則必須符合下列必要條件：

* Red Hat® Enterprise 7或CentOS 7及更新版本，以及最新的修補程式
* 64位作業系統
* 禁用交換（建議）
* SELinux已禁用（請參閱以下備注）

>[!NOTE]
如果設定了LC_CTYPE不等於 `en_US.UTF-8`，會使Dynamic Media無法運作。 要查看其值，請在命令提示符下鍵入&quot;locale&quot;。 如果未正確設定，請在運行AEM之前鍵入&quot;export LC_CTYPE=&quot;，將LC_CTYPE環境變數設定為空字串。
...
>[!NOTE]
**NUMA架構：** 具有AMD64和英特爾® EM64T處理器的系統通常配置為非統一記憶體架構(NUMA)平台。 也就是說，內核在啟動時構造多個記憶體節點，而不是構造單個記憶體節點。
...
若要在Windows上使用Dynamic Media，請安裝適用於x64和x86的Microsoft® Visual Studio 2010、2013和2015可重新分配。

對於Windows x64:

* 在以下位置獲得Microsoft® Visual Studio 2010可再發行 [https://www.microsoft.com/en-us/download/details.aspx?id=26999](https://www.microsoft.com/en-us/download/details.aspx?id=26999)
* 在以下位置獲得Microsoft® Visual Studio 2013可再發行 [https://www.microsoft.com/en-us/download/details.aspx?id=40784](https://www.microsoft.com/en-us/download/details.aspx?id=40784)
* 在以下位置獲得Microsoft® Visual Studio 2015可再發行 [https://www.microsoft.com/en-us/download/details.aspx?id=48145](https://www.microsoft.com/en-us/download/details.aspx?id=48145)

對於Windows x86:

* 在以下位置獲得Microsoft® Visual Studio 2010可再發行 [https://www.microsoft.com/en-us/download/details.aspx?id=26999](https://www.microsoft.com/en-us/download/details.aspx?id=26999)
* 在以下位置獲得Microsoft® Visual Studio 2013可再發行 [https://www.microsoft.com/en-in/download/details.aspx?id=40769](https://www.microsoft.com/en-in/download/details.aspx?id=40769)
* 在以下位置獲得Microsoft® Visual Studio 2015可再發行 [https://www.microsoft.com/en-us/download/details.aspx?id=52685](https://www.microsoft.com/en-us/download/details.aspx?id=52685)
...
* 只有Windows和Linux®支援OpenOffice的PDF生成器轉換。
* PDF生成器僅支援Microsoft® Office Professional Plus的32位零售版，以及在Windows作業系統上轉換所需的其他軟體。
* PDF產生器支援Linux®作業系統上的32位和64位版本的OpenOffice。
* PDF生成器不支援Microsoft® Office 365。
* OCRPDF、Optimize PDF和Export PDF功能僅在Windows上受支援。
* Acrobat版本與AEM Forms搭配，以啟用PDF產生器功能。 在AEM Forms授權期限內，僅以程式設計方式存取隨AEM Forms提供的套件版本，以便與AEM FormsPDF產生器搭配使用。 如需詳細資訊，請參閱AEM Forms產品說明，如需部署([內部部署](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-on-premise.html) 或 [Managed Services](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-managed-services.html))
* PDF生成器服務不支援Microsoft® Windows 10。
* PDF生成器無法使用Microsoft® Visio 2019轉換檔案。 您可以繼續使用Microsoft® Visio 2016進行轉換 `.VSD` 和 `.VSDX` 檔案。
* PDF產生器無法使用Microsoft® Project 2019轉換檔案。 您可以繼續使用Microsoft® Project 2016來轉換 `.VSD` 和 `.VSDX` 檔案。
>
...
* **作業系統：**
   ...
   * Linux®（64位系統上支援32位和32位應用程式）。 有關安裝32位客戶端庫的步驟，請參見 [如何在64位Red Hat® Linux®上啟用XMP提取和回寫](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html).
...
XMPFilesProcessor過程需要庫GLIBC_2.14才能工作。 使用包含GLIBC_2.14的Linux®內核，例如Linux®內核3.1.x版。它可改善處理包含大量中繼資料(例如PSD檔案)的資產的效能。 使用舊版GLIBC會導致以 `com.day.cq.dam.core.impl.handler.xmp.NCommXMPHandler Failed to read XMP`.
