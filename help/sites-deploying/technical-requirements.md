---
title: 技術要求
description: Adobe Experience Manager支援的客戶端和伺服器平台清單。
topic-tags: platform
...
Adobe支AEM持()Adobe Experience Manager平台，如本文檔下列資訊所詳述。
...
* 已安裝Java™平台、標準版JDK或其他受支援的 [Java™虛擬機](#java-virtual-machines)
...
| 支援程度 | 說明 |
|---|---|
| **Z:不支援** | 不支援配置。 Adobe不會聲明配置是否有效，也不支援它。 |

## 支援的平台 {#supported-platforms}

### Java™虛擬機 {#java-virtual-machines}

應用程式需要運行Java™虛擬機，該虛擬機由Java™開發工具包(JDK)分發提供。

Adobe Experience Manager使用以下版本的Java™虛擬機運行：

>[!CAUTION]
>
>從Java™供應商跟蹤安全公告。 這樣可確保生產環境的安全。 此外，始終安裝最新的Java™更新。
...
| IBM® J9 VM — 內部版本2.9,JRE 1.8.0 | 答：支援 `[2]` |
| IBM® J9 VM — 內部版本2.8,JRE 1.8.0 | 答：支援 `[2]` |
| Azul Zulu OpenJDK 11 - 64位 | 答：支援 `[3]` |  |
| Azul Zulu OpenJDK 8 - 64位 | 答：支援 `[3]` |  |

1. Oracle已經轉向了OracleJava™ SE產品的「長期支援」(LTS)模型。 Java™ 9、Java™ 10和Java™ 12是非LTS版本，按Oracle排列(請參見 [OracleJava™ SE支援路線圖](https://www.oracle.com/technetwork/java/eol-135779.html))。 要在生AEM產環境中部署，Adobe僅支援Java™的LTS版本。 支援和分發OracleJava™ SE JDK，包括所有在公共更新結束之後的LTS版本的維護更新，而所有使用OracleJava™ SE技術的客戶都可直AEM接進行Adobe。 查看 [針對Adobe Experience Manager的Java™支援策略](assets/Java_Policy_for_Adobe_Experience_Manager.pdf)。
   **重要提示：OracleJava™ 11至少支援到2026年9月。 正在準備對OracleJava™ 17的支援。**

1. IBM® JRE僅與WebSphere® Application Server一起受支援。

1. 從6.5 SP9版開始的內部部署支援Azul Zulu OpenJDK LTSAEM版本。 Adobe客戶必須直接從阿祖爾獲得對阿祖爾祖魯JDK LTS版本的支援和分發。
...
| AmazonS3 | 二進位檔案 | 答：支援 |
| Microsoft® AzureBlob儲存 | 二進位檔案 | 答：支援 |
...
| IBM® DB2® 10.5 | 資料庫和Forms資料庫 | R:限制支援 `[5]` |
| Oracle資料庫12c(12.1.x) | 資料庫和Forms資料庫 | R:限制支援 |
| Microsoft® SQL Server 2016 | Forms資料庫 | 答：支援 |
| **Apache Lucene（Quickstart內置）** | 搜索服務 | 答：支援 |
| 阿帕奇索爾 | 搜索服務 | 答：支援 |

1. 「檔案系統」包括符合POSIX的塊儲存。 包括網路儲存技術。 請注意，檔案系統效能可能會有所不同，並會影響整體效能。 載入AEM網路/遠程檔案系統的test。
1. MongoDB Enterprise版本4.2和4.4至少AEM要求6.5 SP9。
1. 中不支援MongoDB共用AEM。
...
>[!NOTE]
MongoDB是第三方軟體，不包括在許AEM可包中。 有關詳細資訊，請參見 [MongoDB許可策略](https://www.mongodb.com/community/licensing) 的子菜單。
...
如果要同時運行作者和在MongoDB上發佈，則必須購買兩個單獨的許可證。
Adobe客戶服務可幫助確定與使用MongoDB相關的問題AEM。
...
要使用AEM受支援的關係資料庫運行6.5，需要與資料庫供應商簽訂單獨的支援合同。 Adobe客戶關懷幫助確定與使用6.5關係資料庫AEM相關的問題。
...
| IBM® WebSphere®應用程式伺服器Continuous Delivery(LibertyProfile)，帶Web Profile 7.0和IBM® JRE 1.8 | R:新合同的限制支援 `[2]` |
| IBM® WebSphere® Application Server 9.0和IBM® JRE 1.8 | R:新合同的限制支援 `[1]` `[2]` |
| Apache Tomcat 8.5.x | R:新合同的限制支援 `[2]` |
| JBoss® EAP 7.2.x（帶JBoss®應用程式伺服器） | Z:不支援 |
| JBoss® EAP 7.1.4與JBoss®應用程式伺服器 | R:新合同的限制支援 `[1]` `[2]` |
| JBoss® EAP 7.0.x（帶JBoss®應用程式伺服器） | Z:不支援 |

1. 建議部署AEM Forms。
1. 在應用AEM程式伺服器上啟動6.5部署將移至「受限支援」。 現有客戶可以升級到AEM6.5並繼續使用應用程式伺服器。 對於新客戶，它附帶了支援標準和支援計畫，如上面R級說明中所述。
...
| **Platform** | **支援程度** |
|---|---|
| **Linux®，基於Red Hat®發行版** | 答：支援 `[1]` `[3]` |
| Linux®，基於Debian分發，包括 烏邦圖 | 答：支援 `[1]` `[2]` |
| Linux®，基於SUSE®分發 | 答：支援 `[1]` |
| Microsoft® Windows Server 2019 `[4]` | R:新合同的限制支援 `[5]` |
| Microsoft® Windows Server 2016 `[4]` | R:新合同的限制支援 `[5]` |
| Microsoft® Windows Server 2012 R2 | Z:不支援 |
| OracleSolaris™ 11 | Z:不支援 |
| IBM® AIX® 7.2 | Z:不支援 |

1. Linux®內核2.6、3。 x, 4。 x和5。 x包括Red Hat®發行版的衍生產品，包括Red Hat® Enterprise Linux®、CentOS、OracleLinux®和AmazonLinux®。 AEM Forms附加功能僅在CentOS 7、Red Hat® Enterprise Linux® 7、Red Hat® Enterprise Linux® 8和Red Hat® Enterprise Linux® 9上受支援。
1. AEM Forms在Ubuntu 20.04 LTS上受支援。
1. Adobe Managed Services支援的Linux®分發。
1. Microsoft® Windows生產部署支援升級至6.5和非生產用途的客戶。 新部署是應AEM Sites和資產部門的要求而部署的。
1. AEM Forms在Microsoft® Windows Server上不受支援級別R限制。

>[!NOTE]
如果要安裝AEM Forms6.5，請確保已安裝以下32位Microsoft® Visual C++可再發行版。
* Microsoft® Visual C++ 2008可再發行版
* Microsoft® Visual C++ 2010可再發行版
* Microsoft® Visual C++ 2012可再發行版
* Microsoft® Visual C++ 2013可再發行版（截至6.5）



### 虛擬和雲計算環境 {#virtual-cloud-computing-environments}

Adobe Experience Manager支援在雲計算環境中的虛擬機中運行。 這些環境包括作為Microsoft® Azure和Amazon Web Services(AWS)運行，符合本頁所列的技術要求，並符合Adobe的標準支援條款。
...
Adobe還提供Adobe托管服務，以在AEMAzure或AWS上部署。 Adobe Managed Services為專家提供了在這些雲計算環境中部署和運AEM營的經驗和技能。 請參閱 [有關Adobe Managed Services的其他文檔](https://business.adobe.com/products/experience-manager/managed-services.html?aemClk=t)。

在Azure或AWS或任AEM何其他雲計算環境上部署的所有其他情況下，都會將來自Adobe的支援包含到虛擬計算環境中。 該虛擬環境必須符合本頁列出的技術規範。 任何與在這些雲環境AEM中運行相關的報告問題必須獨立於特定於雲計算環境的任何雲服務可重複。 也就是說，除非雲服務作為此頁上列出的技術要求的一部分而受支援，例如Azure Blob儲存或AWSS3。

有關如何在Azure或AEMAWS上部署Adobe托管服務之外的建議，Adobe建議直接與雲提供程式協作。 或者，與Adobe合作夥伴合作，AEM支援在您選擇的雲環境中部署。 選定的雲提供商或合作夥伴負責架構的規模規格、設計和實施，以滿足您的特定效能、負載、可擴充性和安全性要求。

### Dispatcher平台（Web伺服器） {#dispatcher-platforms-web-servers}

Dispatcher是快取和負載平衡元件。 [下載最新的Dispatcher版本](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/getting-started/release-notes.html?lang=en)。 Experience Manager6.5要求Dispatcher版本4.3.2或更高版本。
...
1. 基於Apache httpd原始碼構建的Web伺服器所支援的內容與其所基於的httpd版本一樣多。 如果有疑問，請向Adobe詢問與相應伺服器產品相關的支援級別的確認。 以下案例：

   1. HTTP伺服器僅使用官方的Apache源分發，或
   1. HTTP伺服器作為運行該伺服器的作業系統的一部分被傳送。 示例：IBM® HTTP Server,OracleHTTP伺服器
...
用戶界AEM面已針對大螢幕（通常是筆記型電腦和台式電腦）和平板電腦外形規格(如AppleiPad或Microsoft® Surface)進行了優化。 不支援電話機外形。

>[!NOTE]
**支援具有快速發佈週期的瀏覽器：**
Mozilla Firefox、GoogleChrome和Microsoft® Edge版本每隔幾個月更新一次。 Adobe承諾為Adobe Experience Manager提供更新，以保持以下所列支援級別，即將推出的這些瀏覽器版本。
...
1. Firefox的擴展支援版本 [在mozilla.org上瞭解更多資訊](https://www.mozilla.org/en-US/firefox/enterprise/)
1. 支援AppleiPad
...
在與Microsoft® Windows 7+連接到AEM未使用SSL保護的實例時，必須在Windows中啟用基於不安全網路的基本身份驗證。 它需要更改WebClient的Windows註冊表：
...
操作是無縫的，因為不需要特殊配置。 如有必要，可使用適合您網路類型的格式指定IP地址。

當必須指定IP地址時，可以（根據需要）從以下選項中選擇：
...
* 預設大小寫 `localhost` 解釋為IPv4和IPv6網路安裝。 例如 `https://localhost:4502`
...
對於運行Dynamic Media-Scene7模式的Dynamic Media客戶(即， **動態媒體_場景7** 運行模式)，沒有其他系統要求；只與系統要求相同AEM。 Dynamic Media-Scene7模式體系結構使用基於雲的影像服務，而不是嵌入在中的服AEM務。

#### 硬體 {#hardware}

以下硬體要求適用於Linux®和Windows:

* 至少具有四核的英特爾至強®或AMD皓龍CPU
* 至少16 GB的RAM

#### Linux® {#linux}

如果您在Linux®上使用Dynamic Media，則必須滿足以下先決條件：

* Red Hat® Enterprise 7或CentOS 7及更高版本及最新修補程式
* 64位作業系統
* 已禁用交換（推薦）
* SELinux已禁用（請參閱以下注釋）

>[!NOTE]
如果區域設定為LC_CTYPE不等於 `en_US.UTF-8`它阻止了Dynamic Media工作。 要查看其值，請在命令提示符下鍵入&quot;locale&quot;。 如果未正確設定，則在運行前鍵入&quot;export LC_CTYPE=&quot;將LC_CTYPE環境變數設定為空字串AEM。
...
>[!NOTE]
**NUMA體系結構：** 帶有AMD64和英特爾® EM64T處理器的系統通常配置為非統一記憶體體系結構(NUMA)平台。 即，內核在啟動時構造多個記憶體節點，而不是構造單個記憶體節點。
...
要在Windows上使用Dynamic Media，請為x64和x86安裝Microsoft® Visual Studio 2010、2013和2015可再發行版。

對於Windows x64:

* 獲取Microsoft® Visual Studio 2010可再發行版，網址為 [https://www.microsoft.com/en-us/download/details.aspx?id=26999](https://www.microsoft.com/en-us/download/details.aspx?id=26999)
* 獲取Microsoft® Visual Studio 2013可再發行版，網址為 [https://www.microsoft.com/en-us/download/details.aspx?id=40784](https://www.microsoft.com/en-us/download/details.aspx?id=40784)
* 獲取Microsoft® Visual Studio 2015可再發行版，網址為 [https://www.microsoft.com/en-us/download/details.aspx?id=48145](https://www.microsoft.com/en-us/download/details.aspx?id=48145)

對於Windows x86:

* 獲取Microsoft® Visual Studio 2010可再發行版，網址為 [https://www.microsoft.com/en-us/download/details.aspx?id=26999](https://www.microsoft.com/en-us/download/details.aspx?id=26999)
* 獲取Microsoft® Visual Studio 2013可再發行版，網址為 [https://www.microsoft.com/en-in/download/details.aspx?id=40769](https://www.microsoft.com/en-in/download/details.aspx?id=40769)
* 獲取Microsoft® Visual Studio 2015可再發行版，網址為 [https://www.microsoft.com/en-us/download/details.aspx?id=52685](https://www.microsoft.com/en-us/download/details.aspx?id=52685)
...
* OpenOffice的PDF生成器轉換僅在Windows和Linux®上受支援。
* PDF生成器僅支援Microsoft® Office Professional Plus的32位零售版，以及在Windows作業系統上轉換所需的其他軟體。
* PDF生成器支援Linux®作業系統上的32位和64位版本的OpenOffice。
* PDF生成器不支援Microsoft® Office 365。
* OCRPDF、Optimize PDF和Export PDF功能僅在Windows上受支援。
* Acrobat版本與AEM Forms捆綁，以啟用PDF生成器功能。 在AEM Forms許可期間，只以寫程式方式訪問與AEM Forms捆綁的版本，以便與AEM FormsPDF生成器一起使用。 有關詳細資訊，請參閱按部署列出的AEM Forms產品說明([內部部署](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-on-premise.html) 或 [Managed Services](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-managed-services.html))
* PDF生成器服務不支援Microsoft® Windows 10。
* PDF生成器無法使用Microsoft® Visio 2019轉換檔案。 您可以繼續使用Microsoft® Visio 2016進行轉換 `.VSD` 和 `.VSDX` 的子菜單。
* PDF生成器無法使用Microsoft® Project 2019轉換檔案。 您可以繼續使用Microsoft® Project 2016進行轉換 `.VSD` 和 `.VSDX` 的子菜單。
>
...
* **作業系統：**
   ...
   * Linux®（在64位系統上支援32位和32位應用程式）。 有關安裝32位客戶端庫的步驟，請參見 [如何在XMP64位Red Hat® Linux®上啟用提取和回寫](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html)。
...
### AEM Assets在Linux®上處理元資料密集型資產的要求 {#assetsonlinux}

XMPFilesProcessor進程需要庫GLIBC_2.14才能工作。 使用包含GLIBC_2.14的Linux®內核，例如Linux®內核3.1.x版。它改進了處理包含大量元資料的資產(如PSD檔案)的效能。 使用GLIBC的早期版本會導致日誌中的錯誤 `com.day.cq.dam.core.impl.handler.xmp.NCommXMPHandler Failed to read XMP`。
