...
| ® DB2® 10.5 | 存放庫和Forms資料庫 | R：受限制的支援`[5]` |
| Oracle資料庫12c (12.1.x) | 存放庫和Forms資料庫 | R：限制支援 |
| Oracle資料庫19c | 存放庫和Forms資料庫 | R：限制支援 |
| ® SQL Server 2016 | Forms資料庫 | A：支援 |
| ® SQL Server 2019 （已棄用） | Forms資料庫 | A：支援 |
| ® SQL Server 2022 | Forms資料庫 | A：支援 |
...
| ® WebSphere® Application Server Continuous Delivery (LibertyProfile)，搭配Web Profile 7.0和IBM® JRE 1.8 | R：新合約`[2]`的限制支援 |
| ® WebSphere® Application Server 9.0和IBM® JRE 1.8 | R：新合約`[1]`的支援受限`[2]` |
| ® WebSphere®應用程式伺服器9.0.0.10 | R：新合約`[1]`的支援受限`[2]` |
...
3. 僅適用AEM Forms：
   ...
   * 自AEM Forms Service Pack 6.5.25.0起，新增對JBoss® EAP 7.4.23的支援。 您可以使用此[連結](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/jboss-eap-7.4.23-1.0.16.zip)，從軟體發佈入口網站下載JBoss® EAP 7.4.23。
...
| Linux®，根據Debian分佈，包括Ubuntu | A：支援的`[1]` `[2]` |
| Linux®，根據SUSE®分佈 | A：支援的`[1]` |
| ® Windows Server 2022 | R：限制支援 |
| ® Windows Server 2019 `[4]` （已棄用） | R：新合約`[5]`的限制支援 |
| ® Windows Server 2016 `[4]` | R：新合約`[5]`的限制支援 |
| ® Windows Server 2012 R2 | Z：不支援 |
| Oracle Solaris™ 11 | Z：不支援 |
| ® AIX® 7.2 | Z：不支援 |

1. Linux® Kernel 2.6、3。 x， 4. x， 5. x， 6. x和9。 x包含來自Red Hat® Distribution的衍生程式，包括Red Hat® Enterprise Linux®、Oracle Linux®和Amazon Linux®。 只有Red Hat® Enterprise Linux® 8和Red Hat® Enterprise Linux® 9支援AEM Forms附加元件功能。
...
4. ® Windows生產部署支援客戶升級至6.5版本及用於非生產用途。 AEM Sites和Assets會根據請求進行新部署。
5. ®視窗伺服器支援AEM Forms，但沒有支援層級R限制。
...
>* ® Visual C++ 2012可轉散發套件
>* ® Visual C++ 2013可轉散發套件
>* ® Visual C++ 2019 （VC14.28或更新版本）可轉散發套件

### 虛擬與雲端運算環境 {#virtual-cloud-computing-environments}

支援Adobe Experience Manager在雲端運算環境的虛擬機器器中執行。 這些環境包括®Azure和Amazon Web Services (AWS)，其執行符合本頁所列的技術要求，並符合Adobe的標準支援條款。
...
| ® IIS 10 (Internet Information Server) | A：支援 |
| ® IIS 8.5 (Internet Information Server) | Z：不支援 |

1. 以Apache httpd原始程式碼為基礎建立的Web伺服器，與其所根據的httpd版本同樣具備支援。 如有疑問，請向Adobe索取確認個別伺服器產品相關的支援等級。 下列情況：

   1. HTTP伺服器是僅使用官方Apache來源發佈所建置，或
   1. HTTP伺服器是作為執行伺服器之作業系統的一部分所提供。 範例： ® HTTP伺服器、Oracle HTTP伺服器
...
當使用® Windows 7+連線到不以SSL保護的AEM執行個體時，必須在Windows中啟用透過不保護網路的基本驗證。 它需要在WebClient的Windows登入中進行變更：
...
* Red Hat® Enterprise 8和更新版本，提供最新的修正修補程式
* 64位元作業系統
* 已停用交換（建議）
* SELinux已停用（請參閱以下說明）
...
若要在Windows上使用Dynamic Media，請安裝適用於x64和x86的® Visual Studio 2010、2013和2015可轉散發套件。
