...
1. 在Publish 2上執行[就地升級](/help/sites-deploying/in-place-upgrade.md)。
1. 視需要更新Dispatcher或Web模組&#x200B;**。
1. 排清Dispatcher快取。
1. QA會透過Dispatcher在防火牆後面驗證Publish 2。
1. 關閉Publish 2。
1. 複製Publish 2執行個體。
1. 啟動Publish 2.
...
1. 視需要更新Dispatcher或Web模組&#x200B;**。
1. 清除Publish 1的Dispatcher快取。
1. 啟動Publish 1.
1. QA會透過Dispatcher在防火牆後面驗證Publish 1。
