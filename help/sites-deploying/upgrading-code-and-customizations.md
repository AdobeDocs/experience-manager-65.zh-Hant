...
1. **模式偵測器**  — 如升級規劃中所述並在本頁中詳細說明，執行模式偵測器，以取得模式偵測器報表，其中包含除了AEM Target版本中無法使用的API/套件組合外，還需解決之區域的詳細資訊。 [](/help/sites-deploying/pattern-detector.md) 「模式偵測」報表應可指出程式碼中有任何不相容之處，如果沒有相容，表示您的部署已相容6.5，您仍可以選擇進行新開發，以利用6.5功能，但您不僅需要它來維護相容性。 如果報告了不相容，則可以選擇a)以相容模式運行，並推遲新的6.5功能或相容性的開發；b)升級後決定進行開發，然後移至步驟2。 如需詳細資訊，請參閱AEM 6.5](/help/sites-deploying/backward-compatibility.md)中的[向後相容性。

1. **開發6.5適用的程式碼基底** — 為Target版本的程式碼基底建立專用的分支或存放庫。 使用升級前相容性中的資訊來規劃要更新的代碼區域。
1. **使用6.5 Uber jar **編譯 — 更新代碼基POM以指向6.5 Uber jar並針對此編譯代碼。
1. **更新AEM自訂*** - *對AEM的任何自訂或擴充功能應更新/驗證，以在6.5中運作，並新增至6.5程式碼基底。包含UI搜尋Forms、資產自訂、任何使用/mnt/覆蓋的項目

1. **部署至6.5環境**  — 在開發/QA環境中，應備妥AEM 6.5（製作+發佈）的簡潔例項。應部署更新的程式碼基底和具有代表性的內容範例（來自目前的生產環境）。
1. **QA驗證和錯誤修正**  - QA應驗證6.5的製作和發佈例項上的應用程式。發現的任何錯誤都應修正並提交至6.5程式碼基底。視需要重複開發週期，直到所有錯誤都修正完畢。

繼續升級之前，您應該有穩定的應用程式程式碼基底，且已針對AEM的目標版本進行徹底測試。 根據測試中的觀察，可以有方法最佳化自訂程式碼。 這可能包括重構程式碼以避免遍歷儲存庫、自訂索引以最佳化搜尋，或在JCR中使用無序節點等。

除了升級程式碼基底和自訂以與新AEM版本搭配使用的選項外， 6.5也有助於使用「向後相容性」功能更有效率地管理您的自訂，如[本頁](/help/sites-deploying/backward-compatibility.md)所述。

如上所述並如下圖所示，在第一步中執行[模式偵測器](/help/sites-deploying/pattern-detector.md)將幫助您評估升級的整體複雜性，以及您是否要以相容模式執行或更新您的自訂項目以使用所有新AEM 6.5功能。 如需詳細資訊，請參閱AEM 6.5](/help/sites-deploying/backward-compatibility.md)中的[向後相容性頁面。
[ ![opt_scift](assets/opt_cropped.png)](assets/upgrade-code-base-highlevel.png)

## 升級代碼庫{#upgrade-code-base}

### 在版本控制項{#create-a-dedicated-branch-for-6.5-code-in-version-control}中為6.5代碼建立專用分支
...
若要為現有資產產生資產ID，請在您升級AEM執行個體以執行AEM 6.5時升級資產。這是啟用[Assets Insights功能](/help/assets/asset-insights.md)的必要項目。 如需詳細資訊，請參閱[新增內嵌程式碼](/help/assets/use-page-tracker.md#add-embed-code)。
...
ContextHub設定會受升級影響。 有關如何恢復現有ContextHub配置的說明，請在[此處](/help/sites-developing/ch-configuring.md#recovering-contexthub-configurations-after-upgrading)找到。

### 工作流自定義{#workflow-customizations}

通常會更新現成可用的修改工作流程，以新增或移除不需要的功能。 自訂的常見工作流程是[!UICONTROL  DAM更新資產]工作流程。 自訂實作所需的所有工作流程都應備份並儲存在版本控制項中，因為升級期間可能會加以覆寫。
...
AEM 6.2和6.3之間更改了可編輯模板的結構。如果您從6.2或更早版本升級，並且使用可編輯模板構建站點內容，則需要使用[響應式節點清理工具](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration)。 此工具的用途是在&#x200B;**升級後執行**，以清理內容。 它需要同時在製作和發佈層級上執行。

### CUG實作變更{#cug-implementation-changes}

封閉使用者群組的實作已大幅變更，以解決舊版AEM的效能和可擴充性限制。 舊版CUG已於6.3中淘汰，而新實作僅支援觸控式UI。 如果您從6.2或更舊版本升級，則可在[此處](/help/sites-administering/closed-user-groups.md#upgradetoaem63)找到要遷移到新CUG實作的說明。
