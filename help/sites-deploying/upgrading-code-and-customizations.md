...
1. **圖樣偵測器**  — 依照升級規劃中的說明執行模式偵測器，並詳述於 [此頁面](/help/sites-deploying/pattern-detector.md). 除了AEM的Target版本中無法使用的API/套件組合，您還會收到模式偵測器報告，其中包含必須解決之區域的詳細資訊。 「模式偵測」報表可指出程式碼中的任何不相容專案。 如果不存在任何專案，表示您的部署已經與6.5相容。 您仍然可以選擇使用6.5功能執行新的開發，但並非只是為了維護相容性。 如果回報不相容，您可以選擇在相容模式中執行，並延遲開發新的6.5功能或相容性。 或者，您可以在升級後決定進行開發，並移至步驟2。 另請參閱 [AEM 6.5的回溯相容性](/help/sites-deploying/backward-compatibility.md) 以取得更多詳細資料。

1. **開發6.5的程式碼基底** — 為Target版本的程式碼基底建立專屬的分支或存放庫。 使用升級前相容性的資訊，規劃要更新的程式碼區域。
1. **使用6.5 Uber jar編譯** — 更新程式碼基底POM以指向6.5 uber jar並編譯對它適用的程式碼。
1. **更新AEM自訂*** - *AEM的任何自訂專案或擴充功能應進行更新/驗證，以在6.5中運作，並新增至6.5程式碼基底。 包括UI搜尋Forms、資產自訂、使用/mnt/overlay的任何內容

1. **部署至6.5環境** - AEM 6.5的乾淨例項（製作+發佈）應在Dev/QA環境中站立。 應部署更新的程式碼庫和代表性內容範例（來自目前生產環境）。
1. **QA驗證和錯誤修正** - QA應在6.5的製作和發佈執行個體上驗證應用程式。找到的任何錯誤都應修正並認可至6.5程式碼基底。 視需要重複開發週期，直到修復所有錯誤為止。

在繼續升級之前，您應該要有已針對AEM目標版本完整測試的穩定應用程式程式碼基底。 根據測試中所做的觀察，可能有方法來最佳化自訂程式碼。 例如，其中可能包括重構程式碼以避免周遊存放庫、自訂索引以最佳化搜尋，或在JCR中使用無序節點等。

除了選擇性地升級您的程式碼基底和自訂以與新的AEM版本搭配使用外，6.5還可協助您使用回溯相容性功能（如所述）更有效率地管理自訂 [此頁面](/help/sites-deploying/backward-compatibility.md).

如上所述及下圖所示，執行 [圖樣偵測器](/help/sites-deploying/pattern-detector.md) 在第一個步驟中，可以協助您評估升級的整體複雜性。 它也可以協助您決定是否要以相容性模式執行，或更新自訂以使用所有新的AEM 6.5功能。 請參閱 [AEM 6.5的回溯相容性](/help/sites-deploying/backward-compatibility.md) 頁面以取得更多詳細資料。
...
AEM Uber jar包含所有AEM API，作為您Maven專案的 `pom.xml`. 納入Uber Jar作為單一相依性永遠是最佳實務，而不是納入個別AEM API相依性。 升級程式碼基底時，請變更Uber Jar的版本以指向目標AEM版本。 如果您的專案是在Uber Jar存在之前使用AEM版本開發的，請移除所有個別AEM API相依性。 以目標版本的AEM的Uber Jar的單一包含來取代它們。 針對新版本的Uber Jar重新編譯程式碼基底。 更新任何已棄用的API或方法，使其與AEM的目標版本相容。
...
透過使用管理工作階段 `SlingRepository.loginAdministrative()` 和 `ResourceResolverFactory.getAdministrativeResourceResolver()` 在AEM 6.0之前的程式碼基底中非常普遍。基於安全考量，這些方法已過時，因為它們提供太寬的存取層級。 [在未來Sling版本中，將移除這些方法](https://sling.apache.org/documentation/the-sling-engine/service-authentication.html#deprecation-of-administrative-authentication). 強烈建議您重構任何程式碼，以改用服務使用者。 有關服務使用者與的詳細資訊 [如何逐步淘汰管理工作階段可在此處找到](/help/sites-administering/security-service-users.md#how-to-phase-out=admin-sessions).
...
傳統UI編寫仍可在AEM 6.5中使用，但已過時。 可以找到更多資訊 [此處](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release). 如果您的應用程式在傳統UI作者環境中執行，建議升級至AEM 6.5並繼續使用傳統UI。 移轉至Touch UI可計畫為單獨的專案，以便透過數個開發週期完成。 若要在AEM 6.5中使用傳統UI，必須將數個OSGi設定認可至程式碼基底。 如需如何進行設定的詳細資訊，請參閱 [此處](/help/sites-administering/enable-classic-ui.md).
...
延伸AEM現成可用功能的常見作法是，在/apps底下以其他節點覆蓋/libs底下的節點和/或檔案。 您應在版本控制中追蹤這些覆蓋圖，並針對AEM的目標版本進行測試。 如果檔案（例如JS、JSP、HTL）重疊，Adobe建議您留下已增強哪些功能的註解，以便在AEM目標版本上更輕鬆地進行回歸測試。 一般覆蓋圖的更多資訊可以找到 [此處](/help/sites-developing/overlays.md). 您可以在下方找到特定AEM覆蓋圖的指示。
...
1. 移動的所有子節點 **content_backup** 在新建立的內容節點中，以滑鼠右鍵按一下總管窗格中的每個子項節點，然後選取 **移動**.

1. 刪除 **content_backup** 節點。

1. 下方更新的節點 `/apps/dam` 具有正確節點型別的 `sling:Folder` 理想情況下，應儲存至版本控制中，並以程式碼庫部署，或至少以內容封裝形式備份。

### 產生現有資產的資產ID {#generating-asset-ids-for-existing-assets}

若要產生現有資產的資產ID，請在升級AEM執行個體以執行AEM 6.5時升級資產。若要啟用 [Assets分析功能](/help/assets/asset-insights.md). 如需詳細資訊，請參閱 [新增內嵌程式碼](/help/assets/use-page-tracker.md#add-embed-code).
...
若為所有其他用途，請使用 `migrateAllAssets()` API。
...
ContextHub設定會受升級影響。 您可以找到有關如何復原現有ContextHub設定的說明 [此處](/help/sites-developing/ch-configuring.md#recovering-contexthub-configurations-after-upgrading).
...
可編輯範本的結構在AEM 6.2和6.3之間變更。如果您要從6.2或更舊版本升級，而且您的網站內容是使用可編輯的範本所建置，則必須使用 [回應式節點清理工具](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration). 此工具旨在執行 **晚於** 清理內容的升級。 在製作和發佈層級上執行。

### CUG實作變更 {#cug-implementation-changes}

封閉使用者群組的實作已大幅變更，以解決舊版AEM的效能和擴充性限制。 先前版本的CUG已在6.3中淘汰，新的實施僅支援觸控式UI。 如果您是從6.2版或更舊版本升級，您將會找到移轉至新CUG實作的指示 [此處](/help/sites-administering/closed-user-groups.md#upgradetoaem63).
...
這裡所述的升級程式應在開發和QA環境中進行測試，如您的自訂執行手冊中所述(請參閱 [規劃升級](/help/sites-deploying/upgrade-planning.md))。 升級程式應重複執行，直到所有步驟都記錄在升級執行手冊中且升級過程順利進行。
...
<table>
 <tbody>
  ...
  <tr>
   <td>驗證、安全性和許可權</td>
   <td>任何驗證機制（如LDAP/SAML）都應經過驗證。<br /> 應在Author和Publish上測試許可權和群組<br /> 階層。</td>
  </tr>
  <tr>
   <td>查詢</td>
