...
要保存jsontest.com站點的MD5服務中的資料，請使用以下屬性 [建立ContextHub儲存](/help/sites-developing/ch-configuring.md#creating-a-contexthub-store) :
...
使用「添 [](#adding-a-ui-module) 加UI模組」中的過程將UI模組添加到現有UI模式，例如Perona UI模式示例。 對於UI模組，請使用下列屬性值：
