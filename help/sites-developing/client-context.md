...
>[!NOTE]
>
>ContextHub已取代使用者端內容。 請參閱 [相關檔案](/help/sites-developing/contexthub.md) 以取得詳細資訊。
...
如需有關開啟Client Context、設定其顯示的資訊以及模擬使用者體驗的資訊，請參閱 [使用者端內容](/help/sites-administering/client-context.md).
...
Client Context架構提供 [JavaScript API](/help/sites-developing/ccjsapi.md) 可用來與工作階段存放區互動，以讀取和寫入使用者資料，以及接聽和回應存放區事件。 您也可以針對您用於內容目標定位或其他用途的使用者資料，建立工作階段存放區。
...
>[!NOTE]
>
>存放區持續性使用瀏覽器儲存空間或Cookie (以 `SessionPersistence` Cookie)。 瀏覽器儲存空間較為常見。
>
...
將Client Context元件加入網頁的主體區段，以啟用Client Context。 Client Context元件節點的路徑為 `/libs/cq/personalization/components/clientcontext`. 若要包含元件，請將下列程式碼新增至頁面元件的JSP檔案中， `body` 頁面元素：
...
>[!NOTE]
>
>如果您有（或建立） `JSONP` 可提供資料的服務，只要使用 `JSONP` 內容存放區元件，並將其對應至JSONP服務。 這會處理工作階段存放區。

### 建立工作階段存放區 {#creating-a-session-store}

針對您必須新增到Client Context以及從中擷取的資料建立工作階段存放區。 一般而言，您會使用以下程式來建立工作階段存放區：

1. 建立具有「 」的使用者端資料庫資料夾 `categories` 屬性值 `personalization.stores.kernel`. Client Context會自動載入此類別的使用者端資料庫。

1. 設定使用者端程式庫資料夾，使其依存於 `personalization.core.kernel` 使用者端資料庫資料夾。 此 `personalization.core.kernel` client library提供Client Context JavaScript API。
...
使用者端程式庫資料夾的JavaScript會建立並初始化工作階段存放區。 必須使用內容存放區管理員登入工作階段存放區。 以下範例會建立並註冊 [cq_Analytics.SessionStore](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore) 物件。
...
* 屬性值組：擴充 `GenericStoreProperties` 元件。 此元件會自動轉譯屬性值組的存放區。 提供了幾個互動點：
   ...
   * `prolog.jsp` 和 `epilog.jsp`：元件互動可讓您在元件轉譯之前或之後新增伺服器端邏輯。
...
>[!NOTE]
>
>所有使用者端內容元件都是一般存放區或一般存放區屬性元件的擴充功能。 中安裝了幾個範例 `/libs/cq/personalization/components/contextstores` 資料夾。
...
所有具有 `componentGroup` 屬性值 `Client Context` 預設會以Sidekick顯示。 如果您對 `componentGroup` 屬性，您必須使用設計模式手動將元件新增到Sidekick。
...
程式碼必須將檔案的mime型別設為 `text/javascript`，或不執行。
...
個人化Taglib提供 `personalization:storePropertyTag` 標籤，用來顯示工作階段存放區中屬性的值。 若要使用標籤，請在JSP檔案中加入下列程式碼行：
...
此 `propertyName` attribute是要顯示的存放區屬性的名稱。 此 `store` attribute是註冊存放區的名稱。 以下範例標籤顯示 `authorizableId` 的屬性 `profile` 商店：
...
此 `/libs/cq/personalization/components/contextstores/profiledata` 內容存放區元件使用此結構來顯示設定檔工作階段存放區的資料。 此 `cq-cc-thumbnail` class放置縮圖影像。 此 `cq-cc-store-property-level*x*` 類別會格式化英數字元資料：
...
下列範例會將監聽器新增至 `update` 的事件 `profile` 工作階段存放區。
...
目前頁面必須具有對應的行動頁面；只有在頁面有設定行動轉出設定( `rolloutconfig.path.toLowerCase` 包含 `mobile`)。
...
1. 用滑鼠右鍵按一下 `/apps` 資料夾，然後按一下「建立>建立資料夾」。 指定名稱 `myapp` 然後按一下「確定」。
1. 同樣地，在下方 `myapp`，建立名為的資料夾 `contextstores`.&quot;
1. 用滑鼠右鍵按一下 `/apps/myapp/contextstores` 資料夾，然後按一下「建立」>「建立元件」。 指定下列屬性值，然後按下一步：
...
1. 用滑鼠右鍵按一下 `/apps/myapp/contextstores/geoloc/dialog/items/items/tab1/items` 節點，然後按一下「建立>建立節點」。 指定下列屬性值，然後按一下「確定」：
...
1. 按一下「使用者端內容」頂端的編輯圖示，開啟「使用者端內容設計工具」。
   ...
   ![方形內以鉛筆表示的編輯圖示。](do-not-localize/chlimage_1.png)
