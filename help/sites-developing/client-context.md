...
每個工作階段存放區都是屬性值配對的集合。 工作階段存放區代表（任何型別的）資料集合，其概念意義可由設計人員、開發人員或兩者決定。 以下範例JavaScript程式碼會定義物件，該物件代表工作階段存放區可能包含的設定檔資料：
...
為您必須新增到Client Context並從中擷取的資料建立工作階段存放區。 通常，您會使用以下程式來建立工作階段存放區：
...
使用者端程式庫資料夾的JavaScript會建立並初始化工作階段存放區。 工作階段存放區必須使用內容存放區管理員註冊。 以下範例會建立並註冊 [cq_Analytics.SessionStore](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore) 物件。
...
* 複雜資料：擴充 `GenericStore` 元件。 您的工作階段存放區需要「轉譯器」方法，每次必須轉譯元件時都會呼叫此方法。 使用兩個引數呼叫轉譯器函式：
...
您的工作階段存放區需要「轉譯器」方法，每次必須轉譯元件時都會呼叫此方法。 使用兩個引數呼叫轉譯器函式：
...
防止的屬性 `PersistedSessionStore` 永久(亦即，將其從 `sessionpersistence` cookie)，將屬性新增至持續工作階段存放區的非持續屬性清單。
