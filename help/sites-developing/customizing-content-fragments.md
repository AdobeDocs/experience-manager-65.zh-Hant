...
* [使用內容片段建立和管理內容片段](/help/assets/content-fragments/content-fragments.md) , [以及使用內容片段製作頁面](/help/sites-authoring/content-fragments.md) ，以取得有關內容片段的詳細資訊。
...
![簡單內容片段（以範本為基礎）會對應至由主要資產和（選用）子資產組成的組合：](assets/chlimage_1-90.png)
...
#### 與標準資產一樣，內容片段的持有方式如下：{#asset-location}
...
功能整合 {#feature-integration}](/help/assets/content-fragments/content-fragments-delete.md)

#### 內容片段管理(CFM)功能建立在Assets核心上，但應盡可能獨立於它。{#feature-integration}
...
>現在 [建議使用內容片段核心元件[#$tu68] 。 如需詳 [細資訊，請參閱開發核心[#$tu69] 元件。
>
>



* 該元件允許您選擇要顯示的變化。`fragmentPath``fragmentPath`

* 此外，還可以選擇一系列段落來限制輸出； 例如，這可用於多欄輸出。
* 此元件可 [讓內容介於](/help/sites-developing/components-content-fragments.md#in-between-content):
* 此元件可讓您放置其他資產（影像等） 在引用片段的段落之間。[](/help/sites-developing/components-content-fragments.md#in-between-content)
...
#### 例如，內容片段的後端實作負責讓頁面上使用的片段執行個體可供搜尋，或管理混合媒體內容。 此實作需要知道哪些元件用於轉換片段，以及轉換的參數化方式。{#configuration-using-osgi-console}
...
>[!NOTE]AEM只會擷取可在段落上找到的第一個屬性。 因此，您應謹慎選擇屬性。
>
...
* 如果值是 `singleText` （且只設定了一個元素），則元素會呈現為文字，內含內容、版面支援等。 這是僅呈現單一元素之片段的預設值。`variationName`

* 否則，會使用更簡單的方法（可稱為「表單檢視」），其中不支援中間內容，且片段內容會以「原樣」呈現。`elementNames``displayMode`
...
* if `paragraphScope` == `range` then property `paragraphRange` defines the rendered of farges to be rendered
   ...
   * 與其他架構整合 {#integration-with-other-frameworks}`all``range`

   * 內容片段可與下列項目整合：`paragraphScope``range``paragraphRange`
...
* **內容片段的個別翻譯實際上是分開的片段； 例如：**
   ...
   它們位於不同的語言根系下：[](/help/sites-administering/tc-manage.md)
   ...
   * AEM翻譯工作流程可搭配 `/content`:
   ...
   >
   >範本會複製以建立片段，因此會隱含。`/content`
   >
   >    * **中繼資料結構**](/help/sites-developing/i18n-dev.md)
      >
      >    
...
* **CFM提供其專屬的特定架構：**
   ...
   * 如有需要，可加以擴充。
      ...
      `/libs/dam/content/schemaeditors/forms/contentfragment`各個模式表單與片段編輯器整合。
...
### **片段範本** ([FragmentTemplate[#$tu136])



* 

   內容片段模型或內容片段範本，以建立內容片段，`FragmentTemplate.createFragment()`
   ...
   * 取得特定元素的結構資訊
      ...
      * 存取片段變化的範本：
      * 清單變數範本`ElementTemplate`
   * 獲取給定變數的結構資訊

      * 存取變數範本(請參閱 `VariationTemplate`)
      * 取得初始相關內容
      * 代表重要資訊的介面：`VariationTemplate`
   * `ElementTemplate`

   取得基本資料（名稱、標題）

   * `ElementTemplate`取得初始元素內容

      * `VariationTemplate`
      * 取得基本資料（名稱、標題、說明）
   * **內容片段** ([ContentFragment[#$tu159])
...
* 
   ...
   * 
      ...
      * 依名稱取得變化
         ...
         * 移除變數(請參 [閱說明](#caveats))
         * 存取變數資料(請參 `ContentVariation`閱)](#caveats)
         * 解決變數的捷徑（如果指定的變數不適用於元素，則套用某些額外的實施特定備援邏輯）[](#caveats)
         * **內容變化** ([ContentVariation[#$tu191])
      * 
   * 
   ...
   元素的清單版本`ContentFragment``ContentElement``ContentVariation``Versionable`
...
* `Asset` -代表內 `Asset` 容片段的DAM抽象化； 請注意，直接更 `Asset` 新需要重建對 `ContentFragment` 像。
   ...
   * `ContentElement` 可適用於：`Resource``ContentFragment`

   * `ElementTemplate` -用於訪問元素的結構資訊。`Asset``Asset``ContentFragment`
...
* `FragmentTemplate`
   ...
   * 注意事項 {#caveats}
   * `FragmentTemplate`應當指出：

### 實作API以提供UI支援的功能。{#caveats}

整個API的設計不會 **自動** 保留變更（除非API JavaDoc另有說明）。 因此，您必須始終提交各個請求（或實際使用的解析程式）的資源解析程式。

* 可能需要額外努力的任務：
* 建立／移除新元素不會更新簡單片段的資料結構（以片段範本為基礎）。****
* 從中建立新變 `ContentElement` 數不會更新資料結構(但是從自願全域建 `ContentFragment` 立)。

   * 移除現有變數不會更新資料結構。
   * 內容片段管理API —— 用戶端 {#the-content-fragment-management-api-client-side}`ContentFragment`

   * [!CAUTION]

## 對於AEM 6.5，用戶端API是內部的。{#the-content-fragment-management-api-client-side}
...
### 編輯可跨多個檢視（= HTML頁面）的內容片段應是原子。{#requirements}
...
* 離開頁面
   ...
   * 如果編輯工作階段存在且變更尚未持續，則會顯示模式確認對話方塊，通知使用者可能遺失的內容，並允許使用者留在頁面上。[](#processes)

* Examples {#examples}

   * 範例： 存取現有內容片段 {#example-accessing-an-existing-content-fragment}

## 若要達成此目的，您可以將代表API的資源調整為：{#examples}
...
* 如果要設定5分鐘的自動儲存間隔，您需要在節點上定義屬性； 例如：`Long`

* 節點： `/conf/global/settings/dam/cfm/jcr:content``/libs/settings/dam/cfm/jcr:content`
...
如需詳細資訊，請參閱[](/help/sites-developing/content-fragment-templates.md)

## [核心元件——內容片段元件[#$tu291] （建議）
