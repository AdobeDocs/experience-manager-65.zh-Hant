...
>[!NOTE]
>
>教學課程 [建立自己的報表 — 範例](#creating-your-own-report-an-example) 也說明可使用下列原則的數量。
>
...
>
>* 每一行會定義一個節點或屬性，其中：
>  `N:<name> [<nodeType>]` ：說明名稱為 `<*name*>` 和節點型別 `<*nodeType*>`*.*
>  `P:<name> [<propertyType]` ：說明名稱為「 」的屬性 `<*name*>` 和屬性型別 `<*propertyType*>`.
...
* 通常包含：
   ...
   * 根路徑。
     ...
     為了協助將效能影響降至最低，建議（嘗試）將查詢限制在存放庫的特定子樹狀結構。 根路徑可在 [報告範本](#report-template) 或由使用者在 [組態（編輯）對話方塊](#configuration-dialog).

   * [一或多個條件](#query-definition).

     這些條件會強制執行，以產生（初始）結果集；例如，它們包括對節點型別的限制或屬性限制。

**這裡的關鍵點是，查詢結果集中傳回的每個單一節點都會用來在報表上產生單一列（因此是1:1關係）。**

開發人員必須確定為報表定義的查詢會傳回適合該報表的節點集。 但是，節點本身不需要保留所有必要的資訊，這也可以從父節點和/或子節點衍生。 例如，查詢用於 [使用者報告](/help/sites-administering/reporting.md#user-report) 根據節點型別選取節點(在此例中為 `rep:user`)。 不過，此報表上的大部分欄並非直接從這些節點取得資料，而是從子節點取得 `profile`.

### 處理佇列 {#processing-queue}

此 [查詢](#the-query-and-data-retrieval) 傳回資料結果集，在報表中顯示為列。 結果集中的每一列都會進行處理（伺服器端），位於 [幾個階段](#phases-of-the-processing-queue)，然後才傳輸給使用者端以在報表上顯示。
...
1. 轉換傳回的結果 [初始查詢(reportbase)](#query-definition) 使用值擷取器將結果集轉換為基本結果集。

   取決於 [欄型別](#column-specific-definitions). 它們用於從基礎JCR查詢讀取值，並從這些值建立結果集；之後，可以應用進一步處理。 例如，對於 `diff` 型別，值擷取器會讀取兩個屬性，並計算單一值，然後將其加入結果集。 無法設定值擷取器。
...
每種報表型別都需要衍生自的容器元件 `/libs/cq/reporting/components/reportbase`.
...
* `propertyConstraints`
  ...
  會傳回所有 `textimage` 上次由修改的元件 `admin` 使用者。
...
* `settings`
   ...
   * `active`
     ...
     由於可以定義多個設定，因此您可以使用此項來定義哪些設定目前為使用中。 這些是由節點陣列定義(這些節點沒有強制性的命名慣例，但標準報告通常會使用 `0`， `1`.. `x`)，則每個檔案都有以下屬性：
...
>[!NOTE]
>
>所參照的元件必須透過以下方式納入： `.infinity.json` 尾碼（請參閱上述範例）。
...
* **`rootPath`**
  ...
  這會將報表限制在存放庫的特定區段（樹狀結構或子樹狀結構），這是效能最佳化的建議。 根路徑是由 `rootPath` 的屬性 `report` 每個報告頁面的節點（建立頁面時從範本中獲取）。
...
這會定義特定的資料擷取(從 [報表資料結果集](#the-query-and-data-retrieval))以便在個別欄中使用。
...
此 [處理佇列](#processing-queue) 會定義各種解析器並設定預先處理：
...
* `resolver`
   ...
   * `path`
     ...
     解析路徑值，方法為選擇性附加子路徑，並從節點的屬性中取得實際值（如以下所定義） `resolverConfig`)中。 例如， `path` 之 `/content/.../page/jcr:content` 可解析為的內容 `jcr:title` 屬性，這表示頁面路徑會解析為頁面標題。

   * `pathextension`

     藉由在路徑前面加上並從已解析路徑之節點的屬性中取得實際值來解析值。 例如，值 `de` 前面可能會附加一個路徑，例如 `/libs/wcm/core/resources/languages`，從屬性取得值 `language`，解析國家/地區代碼 `de` 至語言說明 `German`.
...
* `preprocessing`
  ...
  前置處理是選擇性的，可以繫結至處理階段（單獨進行） *套用* 或 *applyAfter*：
...
針對預先處理，您可以指定 `pattern` (定義為 [規則運算式](https://en.wikipedia.org/wiki/Regular_expression) 或regex)，然後由 `replace` 模式：
...
「編輯組態」會定義監聽器偵測的必要事件，以及這些事件發生後要套用的動作。 請參閱 [元件開發簡介](/help/sites-developing/components.md) 以取得背景資訊。
...
* 使用標準AEM方法來定義（其他）欄屬性。
   ...
   * `definitions/data/resolver` 和 `definitions/data/resolverConfig` (但非 `definitions/data/preprocessing` 或 `.../clientFilter`) — 解析程式和設定
...
必要的設計屬性登入於 `jcr:content/reportpage/report/columns` (例如， `/etc/designs/reports/<reportName>/jcr:content/reportpage/report/columns`)：
...
每種報表型別都必須提供範本。 這些是標準 [CQ範本](/help/sites-developing/templates.md) 且可依此設定。
...
1. 定義您的第一欄（欄基底）元件。 例如 `bundlecol[cq:Component]` 在 `/apps/cq/reporting/components/osgireport`.
   ...
   * 指定事件的重新整理
   ...
   >[!NOTE]
   >
   >在此範例中，沒有定義 `N:data` 和 `P:clientFilter`. 這是因為從伺服器收到的值是以1:1為基準傳回，這是預設行為。
   >
   >這與定義相同：
   >
   >```
...
1. 定義您的報表範本。 例如 `osgireport[cq:Template]` 在 `/apps/cq/reporting/templates`.
   ...
   * 定義 `allowedPaths` 用於產生的報表 — 在上述案例中，位於下的任何位置 `/etc/reports`
   * 提供範本的標題和說明
...
* **時區** 會定義建立歷史資料的時區。 這是為了確保歷史圖表可顯示全球每位使用者的相同資料。
* **地區設定** 定義要搭配使用的地區設定 **時區** 以取得歷史資料。 地區設定是用來決定某些地區設定特定的行事曆設定（例如，一週的第一天是星期日還是星期一）。
...
>[!NOTE]
>
>每個使用者和語言的報表資料可能不同。 因此，會根據報表、使用者和語言，快取報表資料。 這表示 **專案上限** 值 `2` 實際上快取資料中的任一個：
>
