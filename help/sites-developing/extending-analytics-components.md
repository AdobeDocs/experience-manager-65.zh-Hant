...
頁面範本元件（例如`head.jsp, body.jsp`）需要JSP include才能載入ContextHub和Adobe Analytics整合(這是Cloud Service的一部分)。 所有功能都包含載入JavaScript檔案。

ContextHub專案應直接包含在`<head>`標籤下方，而Cloud Service應包含在`<head>`中及`</body>`區段之前；例如：
...
您在`<head>`和`<body>`區段中新增的`cloudservices`指令碼會套用至新增至頁面的雲端服務設定。 (如果頁面使用多個Cloud Service組態，您只能包含一次ContextHub jsp和Cloud Service jsp)。
...
Adobe Analytics整合模組使用AEM元件產生的`product`值來建構`s.products`變數。 AEM元件在JavaScript中產生的`product`值是具有以下結構的值陣列：
