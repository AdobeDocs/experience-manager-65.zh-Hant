...
Adobe Analytics中有數個預先定義的變數，可在架構上設定。 此 **charset**， **cookieLifetime**， **currencyCode** 和 **trackInlineStats** 變數包含在 **一般Analytics設定** 預設為清單。
...
下列程式會示範如何使用JavaScript方塊來自訂Adobe Analytics追蹤。 如果您的JavaScript需要使用Adobe Analytics外掛程式， [整合它們](/help/sites-administering/adobeanalytics.md) 進入AEM。
...
1. 選取您建立的使用者端程式庫資料夾，並使用右下方的屬性列來新增以下屬性：
...
1. 按一下您要新增外掛程式的連結，然後在頁面開啟時，找到外掛程式的JavaScript原始碼，然後選取程式碼並加以複製。
