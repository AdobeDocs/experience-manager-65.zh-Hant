...
description: 項AEM目可以採用無頭和無頭的模式，但選擇不是二進位的。 AEM 提供了在一個專案中利用兩種模型優勢的靈活性。
...
Adobe Experience Manager項目可以採用頭腦式和頭腦式兩種模式，但選擇不是二元的。 AEM 提供了在一個專案中利用兩種模型優勢的靈活性。本文檔概述了不同的模型並描述了整合級SPA別。
