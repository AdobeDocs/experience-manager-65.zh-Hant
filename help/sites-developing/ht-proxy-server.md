...
這是目標CRX執行個體的主機連線埠。 例如，新安裝的AEM安裝的預設為 **`4502`** 而且新安裝的AEM編寫執行個體的預設為 `4502`.
...
現在可以透過以下位置的Proxy存取CQ/CRX執行個體： `localhost:4444` 所有透過此連線埠的通訊都會記錄到 `test.log`.
