...
### 與儲存庫{#synchronizing-with-the-repository}同步
...
如果指定`--show-update`，則會根據遠程版本檢查每個檔案。 然後，第二字母指定更新操作將執行哪些操作。
...
>[!NOTE]
>
>在配置`sync-once`配置之前，`register`命令不會觸發同步。
