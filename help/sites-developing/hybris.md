...
>[!NOTE]
>
>電子商務架構可與任何電子商務解決方案搭配使用。 此處介紹的某些細節和示例將參考 [SAP Commerce cloud解決方案](https://www.hybris.com/) 。
...
* 該 `cq:commerceProvider` 屬性也用於參考相應的商務工廠定義。
   ...
   * 您可在這裡設定其他屬性， [**例如Catalog版本&#x200B;**(若適用](#configuringthecatalogversion)，則可使用)。
...
* 認證AEM被認為是唯一 *的* Web前端，因此會執行 *所有* 驗證。

* Slave AccountsAEM會針對每位購物者以Hybris建立Slave Accounts。 從屬帳戶的使用者名稱與AEM使用者名稱相同。 密碼隨機密碼會自動產生並儲存（加密）至AEM。
