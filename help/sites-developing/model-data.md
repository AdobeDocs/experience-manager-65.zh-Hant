...
# 資料建模- David Nuescheler的Model{#data-modeling-david-nuescheler-s-model}

## 來源 {#source}

以下是David Nuescheler所表達的想法和評論。

David是Day Software AG的共同創辦人暨技術長，Day Software AG是全球內容管理和內容基礎架構軟體的領先供應商，Adobe於2010年收購此軟體。 他目前是Adobe企業技術部的資深副總裁，也負責JSR-170(Java Content Repository(JCR)應用程式設計介面(API)，內容管理的技術標準)的開發。

在[https://wiki.apache.org/jackrabbit/DavidsModel](https://wiki.apache.org/jackrabbit/DavidsModel)上也可看到更新。
...
上述在例如&quot;blog post&quot;節點上使用`lastModified` Date屬性的範例並不表示需要特殊nodetype。 至少一開始，我一定會將`nt:unstructured`用於我的部落格貼文節點。 因為在我的部落格應用程式中，我只要顯示最後一個修改日期（可能是「依順序」），我幾乎不在乎它是否為日期。 由於我隱含地相信我的部落格撰寫應用程式會將「日期」放在那裡，所以您不需要以nodetype的形式宣告`lastModified`日期的存在。
...
>
>請參閱[CRX可處理多少資料？](https://helpx.adobe.com/experience-manager/kb/CrxLimitation.html) 的上界。
...
如果您未在應用程式中使用`clone()`、`merge()`或`update()`方法，則可能需要使用單一工作區。
...
雖然同名同級(SNS)已引入規範中，允許與為XML設計並通過JCR表示的資料結構相容，因此對JCR非常有價值，但SNS對儲存庫來說具有巨大的開銷和複雜性。
...
參照暗示參照完整性。 我發現，必須瞭解的是，引用不僅會為管理引用完整性的儲存庫增加額外的成本，而且從內容靈活性的角度來說，它們也會帶來高昂的成本。
...
如果內容模型公開的東西甚至遠程&#x200B;*聞起*，就像我嘗試使用（或從中延伸）`nt:file`、`nt:folder`和`nt:resource`的檔案或資料夾。
...
如果您需要為資源添加元資訊，例如「author」或「description」屬性，請擴展`nt:resource`而不是`nt:file`。 我很少延伸nt:file，而常常延伸`nt:resource`。
...
確實，某些節點在其整個生命週期中都需要一個穩定的標識。 但比你想像的要少很多。 mix:referenceable提供了內置在儲存庫中的這種機制，因此，確實不需要另外提供以穩定方式標識節點的方法。
