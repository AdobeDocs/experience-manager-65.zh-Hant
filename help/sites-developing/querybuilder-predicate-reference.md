...
符合JCR布林值屬性。 僅接受值&quot; `true`「和」 `false`「。 若為「 `false`「」，如果屬性的值為「」，則會相符 `false`」或如果它根本不存在。 這對於檢查只在啟用時設定的布林值標幟很有用。
...
* **操作**
  ...
  &quot; `equals`&quot;若為完全相符的專案， &quot; `!=`「若為不相等比較， 」 `greater`「屬性1大於屬性2， 」 `>=`&quot; （屬性1大於或等於property2）。 預設值為 &quot; `equals`&quot;.

### 日期範圍 {#daterange}

比對JCR DATE屬性與日期/時間間隔。 這會使用ISO8601格式處理日期和時間( `YYYY-MM-DDTHH:mm:ss.SSSZ`)並允許部分表示，例如 `YYYY-MM-DD`. 或者，時間戳記也可以以UTC時區(UNIX®時間格式)提供自1970年以來的毫秒數。
...
允許建立巢狀條件。 群組可包含巢狀群組。 查詢產生器查詢中的所有內容都隱含在根群組中，該根群組可能具有 `p.or` 和 `p.not` 引數。
...
將結果限製為目前工作階段具有指定值的專案 [JCR許可權。](https://developer.adobe.com/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html#16.2.3%20Standard%20Privileges)
...
* **hasPermission**
  ...
  目前使用者工作階段必須「全部」具有有問題的節點的逗號分隔JCR許可權。 例如， `jcr:write`， `jcr:modifyAccessControl`.
...
允許將結果排序。 如果需要依多個屬性排序，則必須使用數字首碼多次新增此述詞，例如 `1_orderby=first`， `2_oderby=second`.

#### 屬性 {#properties-13}

* **orderby**

  以@開頭表示的JCR屬性名稱，例如 `@jcr:lastModified` 或 `@jcr:content/jcr:title`，或是查詢中的其他述詞，例如 `2_property`，依此排序。

* **排序**

  排序方向，可以是&quot; `desc`&quot;表示降序或&quot; `asc`&quot;表示遞增（預設）。

* **案例**

  若設為 `ignore`，這會讓排序不區分大小寫，表示「a」在「B」之前；如果為空白或遺漏，排序會區分大小寫，表示「B」在「a」之前
...
* **確切**
  ...
  若 `exact` 為true/on，則確切路徑必須相符，但可包含簡單萬用字元( `*`)，這些名稱與名稱相符，但不符合「 `/`&quot;；如果為false （預設），則包含所有下級（選用）。

* **平坦**

  僅搜尋直接子項(如附加&quot; `/*`「在xpath中) (僅用於「 `exact`&#39; is not true， optional)。

* **self**

  搜尋子樹狀結構，但包含指定為路徑的基本節點（無萬用字元）。
...
* **N_value**
  ...
  使用 `1_value`， `2_value`， ...以檢查多個值(結合 `OR` 依預設，使用 `AND` if和=true) （從5.3開始）。

* **和**

  設定為true可組合多個值( `N_value`)與AND （自5.3開始）。

* **操作**

  &quot;`equals`「以取得完全相符的結果（預設）， 」 `unequals`「若為不相等比較， 」 `like`&quot;，以使用 `jcr:like` xpath函式（選用）， &quot; `not`」表示無相符項(例如，「`not(@prop)`「在xpath中，值引數會被忽略)或」 `exists`存在性檢查(值可以是true — 屬性必須存在，預設 — 或false — 與&quot; `not`「)。
...
您可以定義下限與上限，或只能定義其中一個。 也可以分別針對下限和上限指定操作（例如「小於」或「小於或等於」）。
...
* **p.hits**
   ...
   * **選擇性**:
     ...
     只有指定的屬性 `p.properties`，以空格分隔（在URL中使用「+」）的相對路徑清單；如果相對路徑的深度大於1，則這些物件會表示為子物件；特殊的jcr：path屬性包含點選的路徑

### savedquery {#savedquery}

將持續查詢產生器查詢的所有述詞納入目前查詢，作為子群組述詞。

這不會執行額外的查詢，但會擴充目前的查詢。

可使用以下程式設計方式儲存查詢： `QueryBuilder#storeQuery()`. 格式可以是多行字串屬性或 `nt:file` 以Java™屬性格式將查詢作為文字檔案包含的節點。
