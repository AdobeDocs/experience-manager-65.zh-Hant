...
* **[覆蓋](/help/sites-developing/overlays.md)** 資源的 [設定的搜尋路徑](/help/sites-developing/overlays.md#configuring-the-search-paths).
...
>[!CAUTION]
>
>Sling Resource Merger和相關方法只能用於 [Granite](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/index.html). 這也表示這僅適用於標準的觸控式UI，特別是以這種方式定義的覆寫僅適用於元件的觸控式對話方塊。
>
...
* 減少復寫來源的結構 `/libs`.
  ...
  在使用Sling Resource Merger時，不建議從複製整個結構 `/libs` 因為這會造成自訂中包含太多資訊(通常是 `/apps`)。 當系統以任何方式升級時，不必要地複製資訊會增加發生問題的機會。

>[!NOTE]
>
>覆寫不依存於搜尋路徑，而是使用屬性 `sling:resourceSuperType` 以建立連線。
>
...
這些屬性會影響對應/原始資源/屬性(來自 `/libs`)由覆蓋/覆寫使用(通常位於 `/apps`)。

### 建立結構 {#creating-the-structure}

若要建立覆蓋或覆寫，您需要在目的地下以同等結構重新建立原始節點(通常是 `/apps`)。 例如：
...
若要建立其中一個，您只需要重新建立骨架結構。 若要簡化結構的重新建立，所有中間節點都可以是型別 `nt:unstructured` (它們不需要反映原始節點型別；例如，在 `/libs`)。
...
* **重新定義自動建立的屬性**
  ...
  根據預設，自動建立的屬性(例如 `jcr:primaryType`)不受覆蓋/覆寫約束，以確保目前位於下的節點型別 `/libs` 已遵守。 若要強制覆蓋/覆寫，您必須在中重新建立節點 `/apps`，明確隱藏屬性並重新定義：

   1. 在下建立對應的節點 `/apps` 搭配所需的 `jcr:primaryType`
   1. 建立屬性 `sling:hideProperties` 在該節點上，且值設定為自動建立屬性的值；例如， `jcr:primaryType`

      此屬性，在下定義 `/apps`，的優先順序現在會高於下定義的優先順序 `/libs`
...
* **隱藏屬性**
  ...
  屬性定義於 `/libs`，但不需要 `/apps` 覆蓋/覆寫。
   ...
   1. 在中建立對應的節點 `/apps`
   1. 建立屬性 `sling:hideProperties` 型別 `String` 或 `String[]`. 使用此項可指定要隱藏/忽略的屬性。 也可以使用萬用字元。 例如：
...
* **隱藏節點的子系（同時保留節點的屬性）**
  ...
  節點、其屬性及其子系定義於 `/libs`. 節點及其屬性需要在 `/apps` 覆蓋/覆寫，但部分或全部子節點不需要在 `/apps` 覆蓋/覆寫。
...
* **重新排序節點**
  ...
  節點及其同層級定義於 `/libs`. 需要新位置，才能在中重新建立節點 `/apps` 覆蓋/覆寫，其中新位置是參考中適當的同層級節點所定義 `/libs`.
