...
>[!CAUTION]
>
>本檔案使用 [WKND Spa Project應用程式](https://github.com/adobe/aem-guides-wknd-spa) 僅供示範之用。 它不應用於任何專案。
>
...
1. 選取標題元件，並注意工具列的顯示方式與任何其他元件相同。 選取&#x200B;**編輯**。
...
1. 在新索引標籤中載入 `react.model.json`。
   ...
   每個元件都會選擇公開的項目及呈現方式（在伺服器端使用HTL，或在用戶端使用React）。 本文章重點介紹使用 React 進行用戶端呈現。
