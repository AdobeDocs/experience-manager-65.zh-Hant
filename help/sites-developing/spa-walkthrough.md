---
title: SPA簡介和逐步說明
seo-title: SPA簡介和逐步說明
description: 本文會介紹SPA的概念，並逐步說明如何使用基本的SPA應用程式進行製作，說明它與基礎AEM SPA編輯器的關聯。
seo-description: 本文會介紹SPA的概念，並逐步說明如何使用基本的SPA應用程式進行製作，說明它與基礎AEM SPA編輯器的關聯。
...
>[!NOTE]
>
>如需SPA在AEM中如何運作的技術詳細資訊，請參閱AEM](/help/sites-developing/spa-getting-started-react.md)中SPA快速入門一文。[
>
>如需深入了解SPA編輯器的設計、架構和技術工作流程，請參閱文章[SPA編輯器概述](/help/sites-developing/spa-overview.md)。

## 使用SPA {#content-editing-experience-with-spa}編輯內容體驗
...
1. 選取標題元件，並注意工具列的顯示方式與任何其他元件相同。 選擇&#x200B;**Edit**。
...
1. 在AEM中以正常方式編輯內容，請注意變更會持續存在。
   ...
   >[!NOTE]
如需現 [成文](spa-overview.md#requirements-limitations) 字編輯器和SPA的詳細資訊，請參閱SPA編輯器概述。
...
若要查看其運作方式，請繼續閱讀本文的下一節[SPA應用程式和AEM SPA編輯器](/help/sites-developing/spa-walkthrough.md#spa-apps-and-the-aem-spa-editor)。
...
1. 在發佈伺服器上或使用頁面編輯器中&#x200B;**頁面資訊**&#x200B;功能表的&#x200B;**以Published**&#x200B;檢視來載入We.Retail Journal應用程式。
...
1. 如果尚未載入，請在發佈伺服器上或使用頁面編輯器中&#x200B;**頁面資訊**&#x200B;選單的&#x200B;**以已發佈的形式查看**&#x200B;來載入We.Retail Journal應用程式。
...
1. 請注意，來源的內容極為有限。
   ...
   頁面內文中沒有任何內容。 它主要由樣式表和對React指令碼的調用`we-retail-journal-react.js`組成。
...
1. 在新索引標籤中載入`react.model.json`。
   ...
   AEM SPA編輯器會運用[AEM內容服務](/help/assets/content-fragments/content-fragments.md)，以JSON模型形式提供頁面的整個內容。
...
1. 模型也可以將頁面分組，以便同步載入，減少所需的頁面重新載入次數。
   ...
   在We.Retail Journal的範例中，會同步載入`home`、`blog`和`aboutus`頁面，因為訪客通常會瀏覽這些頁面。 但是，`weather`頁面會以非同步方式載入，因為訪客不太可能造訪它。
...
1. 要查看行為中的這種差異，請重新載入頁面並清除檢查器的網路活動。 導覽至部落格，並在頁面功能表中導覽關於我們的頁面，查看未回報任何網路活動。
   ...
   導覽至天氣頁面，查看`weather.model.json`是非同步呼叫。

   ![screen_shot_2018-06-07at155738](assets/screen_shot_2018-06-07at155738.png)

### 與SPA編輯器{#interaction-with-the-spa-editor}互動
...
1. 使用瀏覽器的內建開發人員工具來檢查頁面內容。 使用選取工具，在頁面上選取可編輯的元件並檢視元素詳細資料。
   ...
   >[!NOTE]
   這是AEM中伺服器端轉譯頁面的行為變更，每個可編輯元件都會插入`cq`元素。
...
* [AEM中的SPA快速](/help/sites-developing/spa-getting-started-react.md) 入門顯示如何建置基本SPA以搭配AEM中的SPA編輯器運作
* [SPA編輯](/help/sites-developing/spa-overview.md) 器概述深入探討AEM與SPA之間的通訊模型。
* [開發SPA for ](/help/sites-developing/spa-architecture.md) AEM說明如何與前端開發人員互動，以開發SPA for AEM以及SPA如何與AEM架構互動。
