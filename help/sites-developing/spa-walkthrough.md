...
* [GitHub 提供的範例 WKND SPA 專案應用程式](https://github.com/adobe/aem-guides-wknd-spa)
   * 下載[最新版本的React應用。](https://github.com/adobe/aem-guides-wknd-spa/releases)將命名為與`wknd-spa-react.all.classic-X.Y.Z-SNAPSHOT.zip`類似。
   * 下載應用的[最新示例映像](https://github.com/adobe/aem-guides-wknd-spa/releases)。 將命名為與`wknd-spa-sample-images-X.Y.Z.zip`類似。
   * [使用包管理器](/help/sites-administering/package-manager.md)將包安裝為與中的任何其他包一樣AEM。
   * 出於本逐步解說的目的，應用程式不需要使用 Maven 安裝。

>[!CAUTION]
>
>此文檔僅將[WKND Spa項目應用](https://github.com/adobe/aem-guides-wknd-spa)用於演示目的。 不要用於任何項目工作。
>
>任AEM何項目都應使用[AEM Project Archetype,](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=zh-Hant)，它支SPA持使用React或Angular的項目，並使用SPASDK。
...
伺服器背後的主SPA要思想是減少對伺服器的調用和依賴，以最小化由伺服器調用引起的延遲，SPA從而接近本機應用程式的響應。
...
通過在客戶端進行渲染，頁面元素會更快地反應，並且訪問者與頁面的交互是即時的。 可能需要的任何附加資料都將被非同步調用，以最大化頁面速度。

>[!NOTE]
>
>如需SPA在AEM中如何運作的技術詳細資訊，請參閱文章[在AEM中開始使用SPA](/help/sites-developing/spa-getting-started-react.md)。
>
>有關編輯器的設計、體系結構和技術工作流的詳細SPA瞭解，請參閱文章[SPA編輯器概述](/help/sites-developing/spa-overview.md)。

## SPA 的內容編輯體驗 {#content-editing-experience-with-spa}

構建SPA以使用編輯AEM器時，SPA內容作者在編輯和建立內容時沒有發現任何差異。 可以使用常用的 AEM 功能，作者的工作流程無需變更。
...
1. 按正常方式編輯內AEM容。 更改將保留。

   ![步驟3](assets/spa-walkthrough-step-3.png)

   >[!NOTE]
   >
   >有關就地文本編SPA輯器和的詳細資訊，請參閱[編輯器概述](spa-overview.md#requirements-limitations)SPA。
...
與任何非應用程式一樣，支援其他創作工具，如在頁面上拖放附加元件、重新排列元件和修改佈局SPA。
...
下一節[載入應用程SPA序，](#loading-an-spa-application)深入瞭解載入的機制以及如何同步和異SPA步載入內容。
...
1. 切換到開發人員工具的&#x200B;**網路**&#x200B;頁籤並重新載入頁面。

   忽略映像請求時，為頁面載入的主要資源為頁面本身、CSS、React JavaScript、其依賴項和頁面的JSON資料。
...
使用示例WKND SPA Project應用程式，可以清楚地看到應用在發佈時的行為和載入方式，使用內容服務進行JSON內容傳遞和非同步載入資源。
...
1. 使用瀏覽器的內建開發人員工具，檢查頁面的內容。使用選取工具，在頁面上選取一個可編輯的元件並查看元素詳細資料。
   ...
   這是編輯器將其識別為 SPA 中的可編輯元件所需的唯一標記屬性。根據此屬性，編SPA輯器將確定與元件關聯的可編輯配置，以便載入正確的框架、工具欄等。

   也會新增一些特定類別名稱，用於標記預留位置和資產拖放功能。

   >[!NOTE]
   >
   >這是從中的伺服器端呈現頁面的行為更改AEM，其中為每個可編輯元件插入了`cq`元素。
   >
   >
...
* [在{SPAAEM1}中開始的操作顯示了如何生成SPA與中的編輯器SPA一起使用的基AEM本](/help/sites-developing/spa-getting-started-react.md)
