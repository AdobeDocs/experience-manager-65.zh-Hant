...
* 有關連接Adobe Target的資訊，請參見[與Adobe Target整合](/help/sites-administering/target.md)。
* 如需有關製作目標內容的詳細資訊，請參閱[使用目標模式製作目標內容](/help/sites-authoring/content-targeting-touch.md)。

>[!NOTE]
>
>當您在作者中定位元件AEM時，元件會對Adobe Target進行一連串的伺服器端呼叫，以註冊促銷活動、設定選件及擷取Adobe Target區段（若已設定）。 不會從發佈到Adobe Target進行AEM伺服器端呼叫。
...
例如，如果您想使用[Marketing CloudID服務](https://docs.adobe.com/content/help/en/id-service/using/home.html)，您需要下載mbox.js，以便它包含以您的租用戶為基礎之`imsOrgID`變數的正確值。 此變數是與Marketing CloudID服務整合的必要項。 如需詳細資訊，請參閱[Adobe Analytics作為Adobe Target](https://docs.adobe.com/content/help/en/target/using/integrate/a4t/a4t.html)和[實施前的報告來源。](https://docs.adobe.com/content/help/en/target/using/integrate/a4t/before-implement.html)

>[!NOTE]
>
>如果自訂mbox已定義在Target設定中，則每個人都必須擁有發佈伺服器上&#x200B;**/etc/cloudservices**&#x200B;的讀取存取權。 若沒有此存取權，在發佈網站上載入mbox.js檔案會產生404錯誤。

1. 轉至「CQ **工具**」頁，然後選擇&#x200B;**Cloud Services**。 ([https://localhost:4502/libs/cq/core/content/tools/cloudservices.html](https://localhost:4502/libs/cq/core/content/tools/cloudservices.html))
1. 在樹中，選擇「Adobe Target」，並在配置清單中按兩下您的Target配置。
1. 在設定頁面上，按一下編輯。
1. 對於自訂mbox.js屬性，按一下「瀏覽」並選取檔案。
1. 若要套用變更，請輸入您Adobe Target帳戶的密碼，按一下重新連線至目標，然後在連線成功時按一下確定。 然後，在「編輯元件」(Edit Component)對話方塊中按一下「確定」(OK)。

您的Target設定包含自訂mbox.js檔案，頁面標題區段](/help/sites-developing/target.md#p-the-head-section-p)中所需的程式碼會將檔案新增至用戶端程式庫架構，而非對testandtarget.js程式庫的參考。[
...
例如，若要停用「Geometrixx示範網站」頁面標題元件的定位，請將屬性新增至/apps/geometrixx/components/title/cq:editConfig節點。

![chlimage_1-22](assets/chlimage_1-22.png)

## 發送訂單確認資訊至Adobe Target{#sending-order-confirmation-information-to-adobe-target}

>[!NOTE]
>
>如果您不使用DTM，請傳送訂單確認給Adobe Target。

若要追蹤您網站的效能，請從您的訂購確認頁面傳送購買資訊至Adobe Target。 (請參閱Adobe Target檔案中的[建立orderConfirmPage Mbox](https://docs.adobe.com/content/help/en/dtm/implementing/target/configure-target/mboxes/order-confirmation-mbox.html)。) 當您的MBox名稱為`orderConfirmPage`且使用下列特定參數名稱時，Adobe Target會將mbox資料辨識為訂購確認資料：
...
每個順序的每個參數值都不同。 因此，您需要一個元件，根據購買的屬性產生程式碼。 CQ [電子商務整合架構](/help/commerce/cif-classic/administering/ecommerce.md)可讓您與產品目錄整合，並建置購物車和結帳頁面。
...
* 根據Adobe TargetAPI的要求，新增`div`元素及`mboxDefault`類別。
