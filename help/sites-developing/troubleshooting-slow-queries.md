...
前兩種查詢分類（無索引和限制性差）速度較慢。 它們速度慢，因為它們迫使Oak查詢引擎檢查 **潛** 結果（內容節點或索引條目），用於標識屬於 **實際** 結果集。
...
解釋 **全部** 查詢，並確保其查詢計畫不包含 **/&amp;ast;橫** 解釋。 遍歷查詢計畫示例：
...
此查詢解析為 `cqPageLucene` 索引，但是因為沒有屬性索引規則 `jcr:content` 或 `cq:tags`，在評估此限制時，每個記錄 `cqPageLucene` 檢查索引以確定匹配項。 因此，如果指數包含100萬 `cq:Page` 然後檢查100萬條記錄以確定結果集。
...
更多的查詢限制會減少符合條件的結果集並進一步優化查詢優化。

同樣，沒有為 `cq:tags` 屬性，甚至具有限制的全文查詢 `cq:tags` 效果會很差，因為索引的結果將返回所有全文匹配項。 在cq:tags後將過濾該限制。

索引後篩選的另一個原因是訪問控制清單，在開發過程中經常會錯過該清單。 嘗試確保查詢不返回用戶可能無法訪問的路徑。 通過更好的內容結構以及對查詢提供相關的路徑限制可以做到這一點。

確定Lucene索引是否返回許多結果以返回小子集作為查詢結果的有用方法是為 `org.apache.jackrabbit.oak.plugins.index.lucene.LucenePropertyIndex`。 這樣，您就可以查看正在從索引中載入多少文檔。 最終結果的數量與載入檔案的數量之間不應不成比例。 有關詳細資訊，請參見 [記錄](/help/sites-deploying/configure-logging.md)。
...
為oak.queryLimitInMemory（例如10000）和oak.queryLimitReads（例如5000）設定低閾值，並在遇到UnsupportedOperationException時優化昂貴的查詢，該查詢說「查詢讀取的節點多於x個……」

設定低閾值有助於避免資源密集型查詢（即，不由任何索引支援或由覆蓋範圍較小的索引支援）。 例如，一個讀取100萬個節點的查詢會導致大量IO，並對整個應用程式效能產生負面影響。 因此，任何由於超出限制而失敗的查詢都應進行分析和優化。
...
在AEM6.3中，上述兩個參數預設預配置，並可通過OSGi QueryEngineSettings進行修改。
...
* **優化查詢**
   ...
   `nt:hierarchyNode` 是的父節點類型 `cq:Page`。 假設 `jcr:content/contentType=article-page` 僅應用於 `cq:Page` 節點通過Adobe的自定義應用程式，此查詢僅返回 `cq:Page` 節點 `jcr:content/contentType=article-page`。 但此流是次優限制，因為：
...
* **優化查詢**
   ...
   將屬性限制更改為 `jcr:content/contentType` （自定義值）到已知屬性 `sling:resourceType` 允許查詢解析到屬性索引 `slingResourceType` 將所有內容索引為 `sling:resourceType`。
...
* **優化查詢**
   ...
   將路徑限制範圍從 `path=/content`至 `path=/content/my-site/us/en` 允許索引減少必須檢查的索引項數。 當查詢可以很好地限制路徑時， `/content` 或 `/content/dam`，確保索引 `evaluatePathRestrictions=true`。

   注釋使用 `evaluatePathRestrictions` 增加索引大小。

1. 盡可能避免查詢函式和查詢操作，如： `LIKE` 和 `fn:XXXX` 隨著基於限制的結果的數量增加，成本會隨之增加。
...
* **優化查詢**
   ...
   LIKE條件計算速度慢，因為如果文本以通配符(「%。..」)開頭，則不能使用索引。 jcr:contains條件允許使用全文索引，因此是首選。 它要求解析的Lucene屬性索引具有的indexRule `jcr:content/contentType` 與 `analayzed=true`。
...
1. ***此調整特定於查詢生成器，不適用於JCR-SQL2或XPath。***
   ...
   * **優化查詢**
   ...
   `p.guessTotal=100` 指示查詢生成器僅收集前100個結果。 並且，設定一個布爾標誌，指示是否存在至少一個結果（但不存在多少個結果，因為計算此數字會導致速度減慢）。 此優化在分頁或無限載入使用情形中表現出色，在這些情況下只增量顯示結果子集。
...
1. 將XPath（或JCR-SQL2）提供到Oak索引定義生成器 `https://oakutils.appspot.com/generate/index` 以便生成優化的Lucene屬性索引定義。 <!-- The above URL is 404 as of April 24, 2023 -->
...
1. 以添加方式將生成的定義手動合併到現有Lucene屬性索引中。 請注意，不要刪除現有配置，因為它們可能用於滿足其他查詢。
   ...
   1. 根據AEM重新索引最佳做法，根據現有內容是否可能受此索引配置更改的影響，刷新或重新索引的順序是正確的。
...
1. 部署生成的Lucene屬性索引定義。
   ...
   在初始部署此索引時，AEM將使用必需的資料填充索引。

## 無索引和遍歷查詢何時可以？ {#when-index-less-and-traversal-queries-are-ok}

由於靈AEM活的內容體系結構，很難預測和確保內容結構的遍歷不會隨著時間而演化為不可接受的大。
...
* **查詢生成器調試器**
   ...
   * 開AEM於 [/libs/cq/search/content/querydebug.html](http://localhost:4502/libs/cq/search/content/querydebug.html)

* **CRXDE Lite — 查詢工具**

   * 用於執行XPath和JCR-SQL2查詢的WebUI。
   * 開AEM於 [/crx/de/index.jsp](http://localhost:4502/crx/de/index.jsp) >工具>查詢……
...
* **[索引管理員](/help/sites-administering/operations-dashboard.md#the-index-manager)**
   ...
   * 一AEM個操作WebUI，顯示實例上的AEM索引；有助於理解存在哪些索引；可以定向或增強。
...
* **Apache Jackrabbit查詢引擎設定OSGi配置**
   ...
   * 開AEM於 [/system/console/configMgr#org.apache.jackrabbit.oak.query.QueryEngineSettingsService](http://localhost:4502/system/console/configMgr#org.apache.jackrabbit.oak.query.QueryEngineSettingsService)

* **NodeCounter JMX Mbean**

   * JMX MBean用於估計中內容樹中的節點數AEM。
   * 開AEM於 [/system/console/jmx/org.apache.jackrabbit.oak%3名稱%3DnodeCounter%2Ctype%3DNodeCounter](http://localhost:4502/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3DnodeCounter%2Ctype%3DNodeCounter)

### 支援的社區 {#community-supported}

* **Oak索引定義生成器`https://oakutils.appspot.com/generate/index`** <!-- The above URL is 404 as of April 24, 2023 -->
