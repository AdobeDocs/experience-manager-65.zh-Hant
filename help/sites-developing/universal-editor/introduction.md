...
1. 調整作者實例的vhost檔案以包括反向代理。
   ...
   >[!NOTE]
   >
   >8080是預設埠。 如果您使用`UES_PORT`[檔案中的`.env`參數更改了此項，](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/implementing/developing/universal-editor/local-dev#setting-up-service)則必須相應地調整此處的埠值。
...
不過，您的應用程式必須檢測為使用通用編輯器。 這涉及包括中繼標籤，以指示編輯器如何以及在何處保留內容。 此工具的詳細資訊可在[通用編輯器文檔中找到，用於將AEM作為雲服務。](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/implementing/developing/universal-editor/getting-started#instrument-page)
