...
* [使用變數](/help/sites-developing/using-variables-in-aem-workflows.md#use-a-variable) 在「或」(OR)「分割」(Split)和「轉至AEM」(Goto )工作流步驟中，定義用於進行路由決策的表達式。 您也可以在所有AEM Forms工作流程步驟中使用變數。
...
如需AEM Forms工作流程中可用的其他複雜資料類型，請參閱 [AEM Forms工作流程中的變數](/help/forms/using/variable-in-aem-workflows.md).  使用ArrayList資料類型建立變數集合。 您可以為所有基元和複雜資料類型建立ArrayList變數。 例如，建立ArrayList變數，並選取String作為子類型，以使用變數儲存多個字串值。
...
1. 拖放 **設定變數** 步驟至工作流程編輯器，點選步驟並選取 ![](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/configure_icon.png) （配置）。
