...
如需AEM Forms工作流程中可用的其他複雜資料類型，請參閱「AEM Forms工作流程中的變數」[。  ](/help/forms/using/variable-in-aem-workflows.md)使用ArrayList資料類型來建立變數集合。 您可以為所有基本和複雜資料類型建立ArrayList變數。 例如，建立ArrayList變數，並選擇「字串」作為子類型，以使用變數儲存多個字串值。
