...
有關更多詳細資訊，請參閱標記的說明 [ &lt;ui:includeClientLib> ](/help/sites-developing/taglib.md#lt-ui-includeclientlib) 。&lt;/ui:includeClientLib>
...
* 顯示三個標籤(節點型別= `cq:Panel`)。
* 每個標籤都有兩個文字欄位（節點類型 = ， xtype = `cq:Widget` ` [textfield](/help/sites-developing/xtypes.md#textfield)` ）。
* 由以下節點定義：
...
此 **切換欄位** 對話方塊會顯示一個視窗，其中包含一個索引標籤。 標籤有一個核取方塊：選中時，將顯示包含兩個文字欄位的欄位集。
...
自訂Widget和外掛程式包含在元件中，稱為 **3. 使用 ExtJS 介面工具集套裝軟體的** 自訂介面工具集。 ****&#x200B;要將此元件包含在示例頁面中，請執行以下操作：

1. 添加 **3. 自訂介面工具集** 元件為範例頁面，來自 Sidekick **中的** 「 **使用 ExtJS 介面工具集** 」標籤。
1. 該元件顯示標題、一些文本，並按一下 **「屬性** 」連結時，將顯示存儲在存放庫中的段落的屬性。 再按一下會隱藏屬性。
元件顯示如下：
...
#### 範例 2： 自訂 `Treebrowse` Widget {#example-custom-treebrowse-widget}
...
**基於富文字編輯器 （RTE）** 外掛程式的對話方塊是基於富文字編輯器的對話方塊，具有在方括弧內插入一些自訂文本的自訂按鈕。自訂文本可以通過一些伺服器端邏輯（此示例中未實現）進行分析，例如，添加在給定路徑定義的一些文本：
...
現成可用 ` [CQ.Ext.tree.TreePanel](https://developer.adobe.com/experience-manager/reference-materials/6-5/widgets-api/index.html?class=CQ.Ext.tree.TreePanel)` 物件提供樹狀結構資料的樹狀結構UI表示法。 使用 ExtJS Widgets **包中包含的** 樹概述元件顯示了如何使用該 `TreePanel` 物件在給定路徑下顯示 JCR 樹。視窗本身可以停靠/取消停靠。 在此示例中，視窗邏輯嵌入在元件 jsp 中的標記之間 &lt;script> 。
