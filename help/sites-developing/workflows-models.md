...
您建立 [工作流程模型](/help/sites-developing/workflows.md#model) 定義使用者啟動工作流程時所執行的一系列步驟。 您也可以定義模型屬性，例如工作流程是暫時的或使用多個資源。

當使用者啟動工作流程時，會啟動執行個體；這是當您建立 [同步](#sync-your-workflow-generate-a-runtime-model) 您的變更。
...
1. 選取 **建立**，然後 **建立模型**.
1. 此 **新增工作流程模型** 對話方塊隨即顯示。 輸入 **標題** 和 **名稱** （選擇性），再選取 **完成**.
1. 新模型會列於 **工作流程模型** 主控台。
...
>
>然後，此資料夾可用於 [管理該資料夾中模型的存取權](/help/sites-administering/workflows-managing.md#create-a-subfolder-in-var-workflow-models-and-apply-the-acl-to-that).
...
編輯 [**預設及/或舊版** （現成可用的）工作流程](#editing-a-default-or-legacy-workflow-for-the-first-time) 有額外步驟，確保 [安全複製](/help/sites-developing/workflows-best-practices.md#locations-workflow-models) 會在您進行變更前進行。
...
當您開啟 [預設及/或舊版模型](/help/sites-developing/workflows.md#workflow-types) 進行編輯：
...
>[!NOTE]
>
>如需有關隨AEM安裝的主要步驟元件的資訊，請參閱 [工作流程步驟參考資訊](/help/sites-developing/workflows-step-ref.md).
...
1. 將適當的步驟拖曳到模型中的所需位置。
   ...
   一旦新增至流量，您就可以 [設定步驟](#configuring-a-workflow-step).
...
1. 設定 **步驟屬性** 視需要而定；可用的屬性取決於步驟型別，可能也有幾個可用的標籤。 例如，預設值 **參與者步驟**，在新工作流程中顯示為 `Step 1`：
...
1. 透過確認變更 **儲存並關閉**；後面接著 **同步** （編輯器工具列）以產生執行階段模型。
...
如果工作流程模型存在於傳統UI中，但在的選取彈出式選單中遺失 **[!UICONTROL 時間表]** 觸控式UI的邊欄，然後依照設定使其可用。 下列步驟說明如何使用名為的工作流程模型 **[!UICONTROL 請求啟用]**.

1. 確認模型不適用於觸控式UI。 使用存取資產 `/assets.html/content/dam` 路徑。 選取資產。 開啟 **[!UICONTROL 時間表]** 在左側邊欄中。 按一下 **[!UICONTROL 開始工作流程]** 並確認 **[!UICONTROL 請求啟用]** 模型不存在於快顯清單中。
...
1. 瀏覽至 **[!UICONTROL 「工具>工作流程>模型」]**. 選取 **[!UICONTROL 請求啟用]**，然後選取 **[!UICONTROL 編輯]**.

1. 選取 **[!UICONTROL 編輯]**，開啟 **[!UICONTROL 頁面資訊]** 功能表，然後從中選擇 **[!UICONTROL 開啟屬性]** 並前往 **[!UICONTROL 基本]** 標籤（如果尚未開啟）。

1. 新增 `Workflow : DAM` 至 **[!UICONTROL 標籤]** 欄位。 使用核取方塊（勾號）確認選取。

1. 確認使用新增標籤 **[!UICONTROL 儲存並關閉]**.
...
1. 開啟要編輯的工作流程模型。
1. 選取 **工作流程模型屬性** 工具列中的。 然後開啟 **階段** 標籤。
...
您也可以設定AEM以顯示 **偵錯** 日誌檔中的訊息。 另請參閱 [記錄](/help/sites-deploying/configure-logging.md) 如需詳細資訊以及開發完成時，請設定 **記錄層級** 返回 **資訊**.
...
1. 從 **工作流程** 選取步驟瀏覽器，拖曳 **參與者步驟** 移至工作流程上，並將其置於 **流程開始** 和 **流程結束**.
...
1. 拖曳 **Or分割** 移至工作流程上，並將其置於 `Validate Content` 和 **流程結束**.
1. 開啟 **Or分割** 進行設定。
1. 設定:
...
1. 將此工作流程套用至您的頁面，以便在使用者移至 **完成** 此 **驗證內容** 步驟，使用者可以選擇是否要 **依要求發佈頁面**，或 **拒絕發佈請求**.
...
例如， **請求啟用**. 此工作流程用於發佈頁面於 **網站** 當內容作者沒有適當的復寫許可權時，就會自動觸發和。 另請參閱 [自訂頁面編寫 — 自訂啟動請求工作流程](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow) 以取得更多詳細資料。
