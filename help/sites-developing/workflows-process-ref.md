...
程式步驟由Java™類別或ECMAScript定義。

* 對於Java™類別程式，會提供完整類別名稱。
...
在最常見的情況下，有效負載是存放庫中的JCR節點(例如AEM頁面或資產)。 JCR節點承載會以字串形式傳遞，該字串為JCR路徑或JCR識別碼(UUID)。 有時有效負載可能是JCR屬性（作為JCR路徑傳遞）、URL、二進位物件或通用Java™物件。 對裝載採取行動的個別程式步驟通常會預期某種型別的裝載，或根據裝載型別採取不同的行動。 對於下述的每個程式，都會說明預期的裝載型別（若有）。
...
>* 啟用 **工作流程處理舊模式**
>
>這回覆成向提供管理員工作階段的舊行為 `WorkflowProcess` 實作並提供對整個存放庫不受限制的存取。
...
* 從資產中擷取中繼資料。
* 建立三種指定大小的縮圖。
* 從資產建立JPEG影像，假設資產原本不是GIF或PNG (在此情況下不會建立JPEG)。
* 設定資產的上次修改日期。
